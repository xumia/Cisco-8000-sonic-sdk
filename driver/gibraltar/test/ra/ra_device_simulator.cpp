...
write_message(const char* message)
{
    ...
    // Message for command file and socket (if any)
    fprintf(s_ra_log_file, "%s\n", message);
}
                                    ...
                                    const void* in_val)
{
    ...
    lld_register_scptr reg = m_gibraltar_tree->get_register(block_id, reg_address);
    if (reg) {
...
ra_device_simulator::write_memory(la_block_id_t block_id,
                                  ...
                                  const void* in_val)
{
    lld_memory_scptr mem = m_gibraltar_tree->get_memory(block_id, mem_address);
    if (mem) {
                                   ...
                                   void* out_val)
{
    ...
    if (!ret) {
        if (s_ra_log_file) {
            fprintf(s_ra_log_file, "# -> no socket connected or address not found\n");
        }
...
ra_device_simulator::send_read_rtl_command(const char* cmd, size_t block_id, size_t addr, uint8_t* out_val, size_t val_size)
{
    ...
    if (current_sim_options.use_socket && should_use_socket) {
        ...
        if (s_ra_log_file) {
            fprintf(s_ra_log_file, "# -> received read %016zx %zd %s\n", addr, nbytes - 1, str.c_str());
        }
        return true;
    }
    return false;
                                              ...
                                              void* out_val)
{
    ...
    std::map<lld_register_scptr, size_t, lld_register_scptr_ops> reg_val_map{
        // Hard Reset
        {m_gibraltar_tree->rx_pdr->status_register, 0x1},
        {m_gibraltar_tree->slice_pair[0]->rx_pdr->status_register, 0x1},
        {m_gibraltar_tree->slice_pair[0]->idb->macdb->init_done_status_register, 0x1},
        {m_gibraltar_tree->slice_pair[0]->idb->encdb->init_done_status_register, 0x1},
        // Soft Reset
        {m_gibraltar_tree->slice[0]->ifg[0]->sch->oqse_shaper_init, 0x1},
        {m_gibraltar_tree->slice_pair[0]->idb->res->init_done_status_register, 0x1},
        // CDB ARC status
        {(*m_gibraltar_tree->cdb->top->access_reg)[38], 0x0}, // CEM_ARC_CPU_REGISTER_START_ADDR = 38
        {m_gibraltar_tree->cdb->top->valid_reg, 0x0},
        {m_gibraltar_tree->cdb->top->arc_mem_start, 0x0},

        // Counters
        {m_gibraltar_tree->counters->top->cpu_read, 0x800000}};
...
ra_device_simulator::check_address(size_t address, const std::string& val, bool is_mem) const
{
    ...
    bit_vector sim_val;
    if (is_mem) {
        lld_memory_scptr mem = m_gibraltar_tree->get_memory(block_id, entry_addr);
        if (!mem) {
        ...
        lld_register_scptr reg = m_gibraltar_tree->get_register(block_id, entry_addr);
        if (!reg) {
