...
class unit_test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        lsp_labels = []
        self.prefix = T.prefix_object(self, self.device, PREFIX_GID, self.nh.hld_obj)
        lsp_counter = self.device.create_counter(1)
        self.prefix.hld_obj.set_nh_lsp_properties(self.nh.hld_obj, lsp_labels, lsp_counter,
                                                  sdk.la_prefix_object.lsp_counter_mode_e_LABEL)

    def tearDown(self):
        self.device.tearDown()

    def test_prefix_object_spa_counters(self):
        (labels, counter, _) = self.prefix.hld_obj.get_nh_lsp_properties(self.nh.hld_obj)
        counter_impl = counter.imp()

        # check a single system port
        pair_idx0 = SLICE0 // 2
        for pi in range(T.NUM_SLICE_PAIRS_PER_DEVICE):
            # counter should have allocations only on slice-pairs that the prefix's next-hop has access to
            if pi != pair_idx0:
        ...
        self.prefix.hld_obj.set_nh_lsp_properties(self.nh.hld_obj, labels, counter1, sdk.la_prefix_object.lsp_counter_mode_e_LABEL)
