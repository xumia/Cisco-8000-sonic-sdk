...
#
# END_LEGAL
lld-device-name := gibraltar

lld-design-block-uid := $(DEVICES_DIR)/gibraltar/defines.pd_ver_2.0/gibraltar_auto_unit_id_defines.v
lld-verilog-default-path := 
lld-base-address := '0x800000'

lld-src-lbr-rev2-path := $(DEVICES_DIR)/gibraltar/lbr.pd_ver_2.0
lld-src-lbrs-rev2 := \
		counters/data/counters_bank_group_6k_db.lbr \
		ifg/ifgb/data/ifgb_24p.lbr \
		ifg/serdes_pool/data/serdes_pool16.lbr \
		ifg/serdes_pool/data/serdes_pool24.lbr \
		npu/flow_cache/data/flc_db.lbr \
		npu/flow_cache/data/flc_queues.lbr \
		npu/idb/data/idb_encdb.lbr \
		npu/idb/data/idb_macdb.lbr \
		npu/idb/data/res.lbr \
		top/regfile/data/top_db.lbr \
		voq_cgm/data/dram_cgm_registers.lbr \
...
lld-src-lbr-rev1-path :=
lld-src-lbrs-rev1 :=
lld-src-block-info-path := $(DEVICES_DIR)/gibraltar/block_info
