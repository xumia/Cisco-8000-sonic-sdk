...
#include "npu_host_event_queue_pacific.h"
...
npu_host_event_queue_pacific::npu_host_event_queue_pacific(const la_device_impl_wptr& device) : npu_host_event_queue_base(device)
{
}

npu_host_event_queue_pacific::~npu_host_event_queue_pacific()
{
}

std::vector<bit_vector>
npu_host_event_queue_pacific::collect_npu_host_events()
{
    npu_host_cpu_q_config_write_adress_register write_address_reg{{0}};
    npu_host_cpu_q_config_read_adress_register read_address_reg{{0}};

    auto& lld = m_device->m_ll_device;
    auto& npuh = m_device->m_pacific_tree->npuh;
    ...
    if (((write_address_reg.fields.write_adress & (1 << 10)) != (read_address_reg.fields.read_address & (1 << 10)))
        ...
        npu_host_evq_counters_register evq_counter{{0}};
