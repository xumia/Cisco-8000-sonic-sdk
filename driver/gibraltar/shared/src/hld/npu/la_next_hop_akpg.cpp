...
la_next_hop_akpg::la_next_hop_akpg(la_device_impl_wptr device) : la_next_hop_base(device)
{
}

la_next_hop_akpg::~la_next_hop_akpg() = default;

la_status
la_next_hop_akpg::populate_nh_payload_l2_info(npl_nh_payload_t& out_nh_payload,
                                              const la_l3_port_wptr& l3_port,
                                              la_slice_id_t slice) const
{
    ...
    if (status == LA_STATUS_SUCCESS) {
        ...
        if (l2_port->get_egress_feature_mode() == la_l2_service_port::egress_feature_mode_e::L2) {
            status = l2_port->populate_nh_l2_payload(out_nh_payload, slice / 2);
...
la_status
la_next_hop_akpg::populate_nh_payload(npl_nh_payload_t& out_nh_payload, const la_l3_port_wptr& l3_port, la_slice_id_t slice) const
{
    ...
    case la_object::object_type_e::SVI_PORT: {
        ...
        case nh_type_e::NORMAL: {
            status = populate_nh_payload_l2_info(out_nh_payload, l3_port, slice);
            return_on_error(status);
            break;
        }
...
la_status
la_next_hop_akpg::populate_nh_and_svi_payload(npl_nh_and_svi_payload_t& out_nh_and_svi_payload, la_slice_id_t slice) const
{
    ...
    status = populate_nh_payload(out_nh_and_svi_payload.nh_payload, port, slice);
    return_on_error(status);
...
la_status
la_next_hop_akpg::configure_global_tx_tables()
{
    for (la_slice_id_t slice : m_device->get_used_slices()) {
        la_status status = do_configure_global_tx_tables(slice);
        return_on_error(status);
    }
    return LA_STATUS_SUCCESS;
}
...
la_status
la_next_hop_akpg::do_configure_global_tx_tables(la_slice_id_t slice)
{
    const auto& table(m_device->m_tables.egress_nh_and_svi_direct_table[slice]);
    npl_egress_nh_and_svi_direct_table_key_t key;
    npl_egress_nh_and_svi_direct_table_value_t value;
    npl_egress_nh_and_svi_direct_table_entry_wptr_t entry;

    key.nh_ptr = m_gid;
    la_status status = populate_nh_and_svi_payload(value.payloads.nh_and_svi_payload, slice);
    return_on_error(status);

    status = table->insert(key, value, entry);
    return_on_error(status);

    m_nh_direct_entry[slice] = entry;
    return LA_STATUS_SUCCESS;
}

la_status
la_next_hop_akpg::update_global_tx_tables()
{
    for (la_slice_id_t slice : m_device->get_used_slices()) {
        la_status status = do_update_global_tx_tables(slice);
        return_on_error(status);
    }
    return LA_STATUS_SUCCESS;
}
...
la_status
la_next_hop_akpg::do_update_global_tx_tables(la_slice_id_t slice)
{
    npl_egress_nh_and_svi_direct_table_value_t value;
    ...
    status = m_nh_direct_entry[slice]->update(value);
    return_on_error(status);
...
la_status
la_next_hop_akpg::teardown_global_tx_tables()
{
    for (la_slice_id_t slice : m_device->get_used_slices()) {
        do_teardown_global_tx_tables(slice);
    }
    return LA_STATUS_SUCCESS;
}

la_status
la_next_hop_akpg::do_teardown_global_tx_tables(la_slice_id_t slice)
{
    const auto& table(m_device->m_tables.egress_nh_and_svi_direct_table[slice]);
    la_status status = table->erase(m_nh_direct_entry[slice]->key());
    return_on_error(status);
    m_nh_direct_entry[slice] = nullptr;

    return LA_STATUS_SUCCESS;
}

la_status
la_next_hop_akpg::initialize(la_object_id_t oid,
...
la_status
la_next_hop_akpg::notify_change(dependency_management_op op)
{
    la_status status = LA_STATUS_SUCCESS;
    switch (op.type_e) {
...
la_status
la_next_hop_akpg::get_resolution_cfg_handle(const resolution_cfg_handle_t*& out_cfg_handle) const
{
...
la_status
la_next_hop_akpg::set_nh_type(nh_type_e nh_type)
{
...
la_status
la_next_hop_akpg::get_fec_table_value(npl_fec_table_value_t& value, npl_destination_t& rpf_fec_table_dest) const
{
...
la_status
la_next_hop_akpg::instantiate(resolution_step_e prev_step)
{
...
la_status
la_next_hop_akpg::configure_resolution_step(resolution_step_e res_step)
{
    switch (res_step) {
...
la_status
la_next_hop_akpg::configure_resolution_step_fec()
{
...
la_status
la_next_hop_akpg::configure_resolution_step_stage2(nh_type_e nh_type)
{
...
la_status
la_next_hop_akpg::uninstantiate(resolution_step_e prev_step)
{
    resolution_step_e cur_step = get_next_resolution_step(prev_step);
...
la_status
la_next_hop_akpg::teardown_resolution_step(resolution_step_e res_step)
{
    switch (res_step) {
    case RESOLUTION_STEP_FEC: {
...
la_status
la_next_hop_akpg::teardown_resolution_step_fec()
{
...
la_status
la_next_hop_akpg::teardown_resolution_step_stage2()
{
    return m_device->m_resolution_configurators[2].unconfigure_entry(m_resolution_data.cfg_handle);
}

resolution_step_e
la_next_hop_akpg::get_next_resolution_step(resolution_step_e prev_step) const
{
...
la_next_hop_akpg::get_id(resolution_step_e prev_step) const
{
    resolution_step_e cur_step = get_next_resolution_step(prev_step);
...
lpm_destination_id
la_next_hop_akpg::get_lpm_destination_id(resolution_step_e prev_step) const
{
    resolution_step_e cur_step = get_next_resolution_step(prev_step);
...
destination_id
la_next_hop_akpg::get_destination_id(resolution_step_e prev_step) const
{
    resolution_step_e cur_step = get_next_resolution_step(prev_step);
...
la_next_hop_akpg::resolution_data::resolution_data()
{
...
la_status
la_next_hop_akpg::modify_mac_move_dsp_or_dspa()
{
...
la_status
la_next_hop_akpg::get_lb_resolution(const la_lb_pak_fields_vec& lb_vector, size_t& member, const la_object*& out_object) const
{
