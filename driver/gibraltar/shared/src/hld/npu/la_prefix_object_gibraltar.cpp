...
#include "la_prefix_object_gibraltar.h"
...
la_prefix_object_gibraltar::la_prefix_object_gibraltar(const la_device_impl_wptr& device) : la_prefix_object_base(device)
{
}

la_prefix_object_gibraltar::~la_prefix_object_gibraltar()
{
}

resolution_step_e
la_prefix_object_gibraltar::get_next_resolution_step(resolution_step_e prev_step) const
{
...
destination_id
la_prefix_object_gibraltar::get_destination_id(resolution_step_e prev_step) const
{
    resolution_step_e cur_step = get_next_resolution_step(prev_step);

    switch (cur_step) {
    case silicon_one::RESOLUTION_STEP_STAGE0_CE_PTR: {
        return destination_id(NPL_DESTINATION_MASK_CE_PTR | m_prefix_gid);
    }
...
la_status
la_prefix_object_gibraltar::get_resolution_cfg_handle(const resolution_cfg_handle_t*& out_cfg_handle) const
{
...
la_status
la_prefix_object_gibraltar::destroy()
{
...
la_status
la_prefix_object_gibraltar::get_fec_table_value(npl_fec_table_value_t& value)
{
...
la_status
la_prefix_object_gibraltar::set_destination(const la_l3_destination* destination)
{
...
la_status
la_prefix_object_gibraltar::update_destination(const la_l3_destination_wcptr& destination, bool is_global, bool is_init)
{
    ...
    status = instantiate_resolution_object(destination, RESOLUTION_STEP_STAGE0_CE_PTR, m_device->get_sptr(this));
    return_on_error(status);
...
void
la_prefix_object_gibraltar::register_attribute_dependency(const la_l3_destination_wcptr& destination)
{
...
void
la_prefix_object_gibraltar::deregister_attribute_dependency(const la_l3_destination_wcptr& destination)
{
    bit_vector registered_attributes((la_uint64_t)attribute_management_op::TE_TUNNEL_DESTINATION_CHANGED);
    if (destination->type() == object_type_e::TE_TUNNEL) {
...
la_status
la_prefix_object_gibraltar::update_dependent_attributes(dependency_management_op op)
{
...
la_status
la_prefix_object_gibraltar::do_clear_nh_lsp_properties(const la_next_hop_wcptr& nh)
{
    transaction txn;
...
la_status
la_prefix_object_gibraltar::do_set_nh_lsp_properties(const la_next_hop_wcptr& nh,
...
la_status
la_prefix_object_gibraltar::do_clear_vrf_properties(const la_vrf_wcptr& vrf, la_ip_version_e ip_version)
{
    if (vrf == nullptr) {
        return LA_STATUS_EINVAL;
    }
...
la_status
la_prefix_object_gibraltar::do_set_vrf_properties(const la_vrf_wcptr& vrf,
...
la_status
la_prefix_object_gibraltar::configure_stage0_prefix_table()
{
    object_type_e type = m_destination->type();

    if ((type == object_type_e::NEXT_HOP) || (type == object_type_e::L3_PROTECTION_GROUP)) {
        la_status status = configure_stage0_ce_ptr_to_nh_or_protected_nh_value();
        return status;
    }
    ...
    if (type == object_type_e::TE_TUNNEL) {
        la_status status = configure_stage0_ce_ptr_to_tenh_value();
        return status;
    }
...
la_status
la_prefix_object_gibraltar::configure_stage0_ce_ptr_to_nh_or_protected_nh_value()
{
    npl_resolution_stage_assoc_data_narrow_entry_t entry_pfx{{0}};
    ...
    la_uint8_t vpn_inter_as = m_vpn_enabled << CE_PTR_VPN_OFFSET;
    object_type_e dest_type = m_destination->type();

    if (dest_type == object_type_e::NEXT_HOP) {
        auto& entry(entry_pfx.stage0_ce_ptr_nh_no_tunnel);

        entry.l3_nh = dest_id.val;
        entry.vpn_inter_as = vpn_inter_as;
        entry.ce_ptr = m_prefix_gid;
        entry.type = NPL_ENTRY_TYPE_STAGE0_CE_PTR_L3_NH_VPN_INTER_AS_CE_PTR;
    } else {
        auto& entry(entry_pfx.stage0_ce_ptr_p_nh_no_tunnel);

        entry.p_l3_nh = dest_id.val;
        entry.vpn_inter_as = vpn_inter_as;
        entry.ce_ptr = m_prefix_gid;
        entry.type = NPL_ENTRY_TYPE_STAGE0_CE_PTR_P_L3_NH_VPN_INTER_AS_CE_PTR;
    }
...
la_status
la_prefix_object_gibraltar::configure_stage0_ce_ptr_to_tenh_value()
{
    npl_resolution_stage_assoc_data_narrow_entry_t entry_pfx{{0}};
    destination_id dest_id = silicon_one::get_destination_id(m_destination, RESOLUTION_STEP_STAGE0_CE_PTR);
    destination_id key = get_destination_id(RESOLUTION_STEP_FORWARD_MPLS);
    npl_em_common_data_t em_common_data{};

    la_uint8_t vpn_inter_as = m_vpn_enabled << CE_PTR_VPN_OFFSET;

    const auto& te_tunnel_impl = m_destination.weak_ptr_static_cast<const la_te_tunnel_impl>();
    la_te_tunnel::tunnel_type_e tunnel_type;
    la_status status = te_tunnel_impl->get_tunnel_type(tunnel_type);
    return_on_error(status);

    object_type_e dest_type = te_tunnel_impl->get_destination()->type();

    if (dest_type == object_type_e::NEXT_HOP && tunnel_type == la_te_tunnel::tunnel_type_e::LDP_ENABLED) {
        auto& entry = entry_pfx.stage0_ce_ptr_nh_ldp_over_te;

        entry.l3_nh = dest_id.val;
        entry.vpn_inter_as = vpn_inter_as;
        entry.ce_ptr = m_prefix_gid;
        entry.type = NPL_ENTRY_TYPE_STAGE0_L3_NH_VPN_INTER_AS_CE_PTR_WITH_COMMON_DATA1;
    } else if (dest_type == object_type_e::NEXT_HOP && tunnel_type == la_te_tunnel::tunnel_type_e::NORMAL) {
        auto& entry = entry_pfx.stage0_ce_ptr_nh_ldp_over_te;

        entry.l3_nh = dest_id.val;
        entry.vpn_inter_as = vpn_inter_as;
        entry.ce_ptr = m_prefix_gid;
        entry.type = NPL_ENTRY_TYPE_STAGE0_CE_PTR_L3_NH_VPN_INTER_AS_CE_PTR_WITH_COMMON_DATA;
    } else if (dest_type == object_type_e::L3_PROTECTION_GROUP && tunnel_type == la_te_tunnel::tunnel_type_e::LDP_ENABLED) {
        auto& entry = entry_pfx.stage0_ce_ptr_p_nh_ldp_over_te;

        entry.p_l3_nh = dest_id.val;
        entry.vpn_inter_as = vpn_inter_as;
        entry.ce_ptr = m_prefix_gid;
        entry.type = NPL_ENTRY_TYPE_STAGE0_P_L3_NH_VPN_INTER_AS_CE_PTR_WITH_COMMON_DATA1;
    } else {
        auto& entry = entry_pfx.stage0_ce_ptr_p_nh_te_he;

        entry.p_l3_nh = dest_id.val;
        entry.vpn_inter_as = vpn_inter_as;
        entry.ce_ptr = m_prefix_gid;
        entry.type = NPL_ENTRY_TYPE_STAGE0_CE_PTR_P_L3_NH_VPN_INTER_AS_CE_PTR_WITH_COMMON_DATA;
    }

    em_common_data.common_data_prefix.te_tunnel16b = te_tunnel_impl->get_gid();
    status = m_device->m_resolution_configurators[0].configure_dest_map_entry(key, entry_pfx, m_res_cfg_handle, em_common_data);
    return status;
}

la_status
la_prefix_object_gibraltar::configure_stage0_ce_ptr_to_ecmp_group_value()
{
    ...
    entry.vpn_inter_as = m_vpn_enabled << CE_PTR_VPN_OFFSET;
    entry.ce_ptr = m_prefix_gid;
...
la_status
la_prefix_object_gibraltar::teardown_stage0_prefix_table()
{
...
la_status
la_prefix_object_gibraltar::configure_small_encap_mpls_he_asbr_table(la_slice_pair_id_t pair_idx,
