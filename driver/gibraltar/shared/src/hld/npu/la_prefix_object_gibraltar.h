...
#ifndef __LA_PREFIX_OBJECT_GIBRALTAR_H__
#define __LA_PREFIX_OBJECT_GIBRALTAR_H__
...
class la_prefix_object_gibraltar : public la_prefix_object_base
{

    CEREAL_SUPPORT_PRIVATE_MEMBERS

public:
    explicit la_prefix_object_gibraltar(const la_device_impl_wptr& device);
    ~la_prefix_object_gibraltar() override;
...
private:
    ...
    la_status configure_stage0_ce_ptr_to_nh_or_protected_nh_value();
    la_status configure_stage0_ce_ptr_to_tenh_value();
    la_status configure_stage0_ce_ptr_to_ecmp_group_value();
    la_status teardown_stage0_prefix_table();

    la_prefix_object_gibraltar() = default; // For serialization purposes only.
};

} // namespace silicon_one

#endif // __LA_PREFIX_OBJECT_GIBRALTAR_H__
