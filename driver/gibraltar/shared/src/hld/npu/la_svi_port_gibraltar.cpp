...
#include "la_svi_port_gibraltar.h"
...
la_svi_port_gibraltar::la_svi_port_gibraltar(const la_device_impl_wptr& device) : la_svi_port_base(device)
{
}

la_svi_port_gibraltar::~la_svi_port_gibraltar()
{
}

la_status
la_svi_port_gibraltar::add_ipv4_host(la_ipv4_addr_t ip_addr, la_mac_addr_t mac_addr, la_class_id_t class_id)
{
...
la_status
la_svi_port_gibraltar::modify_ipv4_host(la_ipv4_addr_t ip_addr, la_mac_addr_t mac_addr, la_class_id_t class_id)
{
    start_api_call("ip_addr=", ip_addr, "mac_addr=", mac_addr, "class_id=", class_id);
...
la_status
la_svi_port_gibraltar::add_ipv6_host(la_ipv6_addr_t ip_addr, la_mac_addr_t mac_addr, la_class_id_t class_id)
{
    start_api_call("ip_addr=", ip_addr, "mac_addr=", mac_addr, "class_id=", class_id);
...
la_status
la_svi_port_gibraltar::modify_ipv6_host(la_ipv6_addr_t ip_addr, la_mac_addr_t mac_addr, la_class_id_t class_id)
{
    start_api_call("ip_addr=", ip_addr, "mac_addr=", mac_addr, "class_id=", class_id);
...
la_status
la_svi_port_gibraltar::populate_recycled_inject_up_info_table(const la_l2_service_port_base_wptr& inject_up_port)
{
...
la_status
la_svi_port_gibraltar::clear_recycled_inject_up_info_table()
{
    npl_recycled_inject_up_info_table_key_t key;
    key.tx_to_rx_rcy_data = NPL_TX2RX_RCY_DATA_OBM_TO_INJECT_UP & 0x3f;
    for (la_slice_id_t slice : m_device->get_used_slices()) {
...
la_status
la_svi_port_gibraltar::update_additional_l3_lp_attributes(const npl_l3_lp_additional_attributes_t& additional_attribs)
{
...
void
la_svi_port_gibraltar::fill_npl_mac_termination_em_table_key(la_switch_gid_t sw_gid,
