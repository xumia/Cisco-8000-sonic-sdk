...
class bfd_ipv6_remote_rx(bfd_init_flags, bfd_ipv6_base):
    ...
    def test_bfd_remote_rx(self):
        ...
        # This tests the path of a packet coming from the fabric to the NPU host
        # and then to the CPU punt path.
        run_and_compare(
            self,
            self.device,
            self.IPV6_MH_OVER_FABRIC,
            EGRESS_RX_SLICE,
            EGRESS_RX_IFG,
            EGRESS_RX_SERDES_FIRST,
            self.PUNT_PACKET_IPV6_REMOTE_MH_FLAG_CHANGE,
            INJECT_SLICE,
            T.PI_IFG,
