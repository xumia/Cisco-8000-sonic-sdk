...
class egress_lc_multicast(lc_multicast_v6_base):
...
#        out_rx_fabric_port.hld_obj.activate(sdk.la_fabric_port.link_protocol_e_LINK_KEEPALIVE)
        ...
        # Create mac system port above the mac port
        mac_sys_port1 = T.system_port(self, self.device, EGRESS_SYS_PORT_GID, mac_port1)

        self.spa_port.add(mac_sys_port1)

        # Create mac port 1
        mac_port2 = T.mac_port(
            self,
            self.device,
            EGRESS_TX_SLICE,
            EGRESS_TX_IFG,
        ...
        # Create mac system port above the mac port
        mac_sys_port2 = T.system_port(self, self.device, EGRESS_SYS_PORT_GID + 1, mac_port2)

        self.spa_port.add(mac_sys_port2)

        out_tx_eth_port = T.sa_ethernet_port(self, self.device, self.spa_port)
        ...
        out_l3_ac = T.l3_ac_port(self, self.device,
                                 ...
                                 vrf,
                                 out_l3_port_mac)
        out_l3_ac.hld_obj.set_protocol_enabled(sdk.la_l3_protocol_e_IPV6_MC, True)
        ...
        self.mc_group.add(out_l3_ac.hld_obj, None, mac_sys_port1.hld_obj)
