...
class inject_down_ptp(punt_inject_port_base):

    @unittest.skipIf(decor.is_asic4(), "Test is not yet enabled on PL")
    @unittest.skipIf(decor.is_asic5(), "Test is not yet enabled on AR")
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_inject_down_ptp(self):
        ...
        inject_packet_base = \
            ...
            InjectDown(dest=dest_id, encap=sdk.la_packet_types.LA_PACKET_INJECT_DOWN_ENCAP_NONE, ts_opcode=1, ts_offset=54) / \
