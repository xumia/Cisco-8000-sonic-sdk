...
class ipv6_svi_erspan_base(erspan_base):
    ...
    def create_packets(self):
        self.in_packet, pad_len = \
            U.enlarge_packet_to_min_length(S.Ether(dst=T.RX_SVI_MAC.addr_str, src=SA.addr_str, type=U.Ethertype.Dot1Q.value) /
                                           S.Dot1Q(vlan=T.RX_L2_AC_PORT_VID1) /
                                           S.IPv6(src=IPV6_SIP.addr_str, dst=IPV6_DIP.addr_str, plen=40, hlim=TTL))

        self.out_packet = \
            U.add_payload(S.Ether(dst=T.NH_SVI_DEF_MAC.addr_str, src=T.TX_SVI_MAC.addr_str) /
        ...
        if (punt_egr_packets is False):
            span_packet_base = \
                ...
                S.IP(src=TUNNEL_SOURCE.addr_str,
                     ...
                     ttl=TUNNEL_TTL, tos=TUNNEL_DSCP << 2, flags="DF", id=0,
                ...
                S.Ether(dst=T.RX_SVI_MAC.addr_str, src=SA.addr_str,
            ...
            l3_span_packet_base = \
                ...
                U.sflow_tunnel_metadata(source_sp=T.RX_SYS_PORT_GID,
                                        ...
                                        destination_sp=T.TX_SVI_SYS_PORT_DEF_GID) / \
                S.Ether(dst=T.RX_SVI_MAC.addr_str, src=SA.addr_str,
                        type=U.Ethertype.Dot1Q.value) / \
                S.Dot1Q(vlan=T.RX_L2_AC_PORT_VID1) / \
                S.IPv6(src=IPV6_SIP.addr_str, dst=IPV6_DIP.addr_str, plen=40, hlim=TTL)
            ...
            span_packet_base = \
                ...
                S.IP(src=TUNNEL_SOURCE.addr_str,
                     dst=TUNNEL_DEST.addr_str,
                     ttl=TUNNEL_TTL,
                     tos=TUNNEL_DSCP << 2, flags="DF", id=0,
                     proto=sdk.la_l4_protocol_e_GRE) / \
                S.GRE(proto=nplapicli.NPL_ETHER_TYPE_ERSPAN_II,
                      seqnum_present=1, seqence_number=0) / \
                U.ERSPAN(session_id=SESSION_ID, en=3) / \
