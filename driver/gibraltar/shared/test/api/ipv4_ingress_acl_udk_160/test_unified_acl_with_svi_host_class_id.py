...
class unified_acl(ipv4_ingress_acl_class_id_160_base):
    ...
    def test_unified_acl_with_svi_host_class_id(self):
        ...
        expected_output_packet = EXPECTED_OUTPUT_HOST1_PACKET.copy()
        expected_output_packet[IP].tos = QOS_MARK_DSCP << 2
        run_and_compare_inner_fields(self, self.device,
                                     INPUT_PACKET_HOST1, T.RX_SLICE, T.RX_IFG, T.FIRST_SERDES,
                                     expected_output_packet, T.TX_SLICE_EXT, T.TX_IFG_EXT, T.FIRST_SERDES_SVI,
                                     control_expected)

        # Verify Q counter
        packet_count, byte_count = q_counter.read(SIMPLE_QOS_COUNTER_OFFSET, True, True)
        self.assertEqual(packet_count, 1)
        assertPacketLengthIngress(self, INPUT_PACKET_HOST1, T.RX_SLICE, byte_count)
        ...
        # Pass packet with ACL applied, ensure DSCP and PHB changes to non-default.
        expected_output_packet = EXPECTED_OUTPUT_HOST2_PACKET.copy()
        expected_output_packet[IP].tos = QOS_MARK_DSCP << 2
        run_and_compare_inner_fields(self, self.device,
                                     INPUT_PACKET_HOST2, T.RX_SLICE, T.RX_IFG, T.FIRST_SERDES,
                                     expected_output_packet, T.TX_SLICE_REG, T.TX_IFG_REG, T.FIRST_SERDES_SVI,
                                     control_expected)

        # Verify Q counter
        packet_count, byte_count = q_counter.read(SIMPLE_QOS_COUNTER_OFFSET, True, True)
        self.assertEqual(packet_count, 1)
        assertPacketLengthIngress(self, INPUT_PACKET_HOST2, T.RX_SLICE, byte_count)
        ...
        # Update the L2 table MAC entry - this should not affect L3 class ID ACL
