...
class ipv6_g_mc(sdk_multi_test_case_base):
    MC_GROUP_ADDR = T.ipv6_addr('ff01:0:0:0:0:1:ffe8:658f')
    MC_GROUP_ADDR_MISS = T.ipv6_addr('ff01:0:0:0:0:1:ffe8:658e')  # This group address will be used for (*,g) MISS testcases
    ...
    def do_test_route(self, extra_packet=None, punt_and_forward=False):
        ...
        self.do_test_mc_route(self.mc_group, extra_packet, punt_and_forward)

    def do_test_route_ir(self, extra_packet=None, punt_and_forward=False):
        # Test ingress replication
        self.setup_for_ingress_rep()
        self.do_test_mc_route(self.ingress_group, extra_packet, punt_and_forward)

    def do_test_mc_route(self, mc_group, extra_packet, punt_and_forward):

        self.topology.vrf.hld_obj.add_ipv6_multicast_route(
            sdk.LA_IPV6_ANY_IP,
            ipv6_g_mc.MC_GROUP_ADDR.hld_obj,
            mc_group,
            self.rpf_intf,
            False,
            punt_and_forward,
        ...
        if(self.svi):
            self.topology.rx_switch.hld_obj.add_ipv6_multicast_route(ipv6_g_mc.MC_GROUP_ADDR.hld_obj, self.rxsw_snoop)
        ...
        for c in self.port_counters:
            c.read(sdk.la_l3_protocol_e_IPV6_MC, True, True)  # clear port MC counters
...
#                                                     True)  # clear_on_read
        ...
        port_counts_mc = [c.read(sdk.la_l3_protocol_e_IPV6_MC, True, True) for c in self.port_counters]
...
#
        ...
        # Cleanup
        self.topology.vrf.hld_obj.delete_ipv6_multicast_route(sdk.LA_IPV6_ANY_IP, ipv6_g_mc.MC_GROUP_ADDR.hld_obj)
        if(self.svi):
            self.topology.rx_switch.hld_obj.delete_ipv6_multicast_route(ipv6_g_mc.MC_GROUP_ADDR.hld_obj)
...
#
    ...
    def do_test_route_mtu(self, punt_and_forward=False):
