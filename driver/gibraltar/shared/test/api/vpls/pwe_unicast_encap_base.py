...
class pwe_encap_base(sdk_test_case_base):
    ...
    EXPECTED_OUTPUT_PACKET_LDP_TENH_IMPLICIT_NULL_BASE = \
        Ether(dst=T.NH_L3_AC_REG_MAC.addr_str, src=T.TX_L3_AC_REG_MAC.addr_str, type=U.Ethertype.MPLS.value) / \
        MPLS(label=PWE_REMOTE_LABEL.label, ttl=PWE_TTL, s=1) / \
        Ether(dst=DA.addr_str, src=SA.addr_str, type=Ethertype.Dot1Q.value) / \
        Dot1Q(vlan=VLAN, type=U.Ethertype.IPv4.value) / \
    ...
    EXPECTED_OUTPUT_PACKET_LDP_TENH_IMPLICIT_NULL = U.add_payload(
        EXPECTED_OUTPUT_PACKET_LDP_TENH_IMPLICIT_NULL_BASE, BASE_INPUT_PACKET_PAYLOAD_SIZE)
    ...
    def create_pwe_port(self, l3_destination):
        ...
        status = self.sw1.hld_obj.set_mac_entry(self.DA.hld_obj, self.pwe_port.hld_obj, sdk.LA_MAC_AGING_TIME_NEVER)
    ...
    def create_ldp_mpls_nh_1(self):
        self.pfx_obj_vpls1 = T.prefix_object(self, self.device, self.PREFIX1_GID, self.l3_port_impl.reg_nh.hld_obj)
        self.assertNotEqual(self.pfx_obj_vpls1.hld_obj, None)

        lsp_labels = []
        lsp_labels.append(self.LDP_LABEL1)

        self.pfx_obj_vpls1.hld_obj.set_nh_lsp_properties(
    ...
    def create_ldp_mpls_nh_2(self):
        ...
        self.pfx_obj_vpls2.hld_obj.set_nh_lsp_properties(
    ...
    def create_ldp_tenh_to_mpls(self, implicit_null=False):
        ...
        self.pfx_obj_vpls1 = T.prefix_object(self, self.device, self.PREFIX1_GID, self.te_tunnel.hld_obj)
        ...
        self.pfx_obj_vpls1.hld_obj.set_te_tunnel_lsp_properties(self.te_tunnel.hld_obj, lsp_labels, lsp_counter)
    ...
    def setUp(self):
        ...
        status = self.ac_ports[0].hld_obj.attach_to_switch(self.sw1.hld_obj)
    ...
    def _test_pwe_encap_attach(self):
        self.create_ldp_mpls_nh_1()
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
        self.traffic_run_and_compare(self.INPUT_PACKET, self.EXPECTED_OUTPUT_PACKET)

    def _test_pwe_encap_change_l3_dest(self):
        self.create_ldp_mpls_nh_1()
        self.create_ldp_mpls_nh_2()
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
        self.traffic_run_and_compare(self.INPUT_PACKET, self.EXPECTED_OUTPUT_PACKET)

        self.pwe_port.hld_obj.set_l3_destination(self.pfx_obj_vpls2.hld_obj)
        U.run_and_compare(self, self.device,
                          self.INPUT_PACKET, self.ports[0].slice, self.ports[0].ifg, self.ports[0].first_serdes,
                          self.EXPECTED_OUTPUT_PACKET_PATH2, T.TX_SLICE_DEF, T.TX_IFG_DEF, T.FIRST_SERDES_L3)

    def _test_pwe_encap_detach(self):
        self.create_ldp_mpls_nh_1()
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
        self.traffic_run_and_compare(self.INPUT_PACKET, self.EXPECTED_OUTPUT_PACKET)
    ...
    def _test_pwe_encap_flow_label(self):
        self.create_ldp_mpls_nh_1()
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
    ...
    def _test_pwe_encap_cw_flow_label(self):
        self.create_ldp_mpls_nh_1()
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
        self.pwe_port.hld_obj.set_flow_label_enabled(True)
        self.pwe_port.hld_obj.set_control_word_enabled(True)
    ...
    def _test_pwe_encap_bgp_lu(self):
    ...
    def _test_pwe_encap_ldp_tenh(self):
        self.create_ldp_tenh_to_mpls()
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
        self.traffic_run_and_compare(self.INPUT_PACKET, self.EXPECTED_OUTPUT_PACKET_LDP_TENH)

    def _test_pwe_encap_ldp_tenh_implicit_null(self):
        self.create_ldp_tenh_to_mpls(True)
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
        self.traffic_run_and_compare(self.INPUT_PACKET, self.EXPECTED_OUTPUT_PACKET_LDP_TENH_IMPLICIT_NULL)

    def _test_pwe_encap_ac_vlan_pop_1(self):
        self.create_ldp_mpls_nh_1()
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
    ...
    def _test_pwe_encap_ac_vlan_push_1(self):
        self.create_ldp_mpls_nh_1()
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
        eve = sdk.la_vlan_edit_command()
    ...
    def _test_pwe_encap_ac_translate_1_1(self):
        self.create_ldp_mpls_nh_1()
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
        eve = sdk.la_vlan_edit_command()
        eve.num_tags_to_push = 1
    ...
    def _test_pwe_encap_shg(self):
        self.create_ldp_mpls_nh_1()
        self.create_pwe_port(self.pfx_obj_vpls1.hld_obj)
        # Deny traffic from grp1 to grp2
        self.grp1.hld_obj.set_filtering_mode(self.grp2.hld_obj, True)
        self.ac_ports[0].hld_obj.set_filter_group(self.grp1.hld_obj)
        self.pwe_port.hld_obj.set_filter_group(self.grp2.hld_obj)

        U.run_and_drop(self, self.device,
                       self.INPUT_PACKET, self.ports[0].slice, self.ports[0].ifg, self.ports[0].first_serdes)

        # Permit traffic from grp1 to grp2
