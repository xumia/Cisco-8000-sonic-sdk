...
class test_gre_qos_mpls_ipv4(test_gre_qos_mpls_base):
    ...
    GRE_MPLS_DECAP_TERM_INPUT = S.IP(dst=OVL_IP_PACKET_SIP,
                                     ...
                                     ttl=test_gre_qos_mpls_base.OVL_IP_TTL)
    ...
    GRE_MPLS_DECAP_TERM_OUT_PIPE = S.IP(dst=OVL_IP_PACKET_SIP,
                                        src=OVL_IP_PACKET_DIP,
                                        ttl=test_gre_qos_mpls_base.OVL_IP_TTL - 1)

    GRE_MPLS_DECAP_TERM_OUT_UNIFORM = S.IP(dst=OVL_IP_PACKET_SIP,
                                           src=OVL_IP_PACKET_DIP,
                                           ttl=test_gre_qos_mpls_base.IP_TTL - 1)

    GRE_MPLS_DECAP_PHP_PIPE_ZERO_TC = S.IP(dst=OVL_IP_PACKET_SIP,
                                           src=OVL_IP_PACKET_DIP,
                                           ttl=test_gre_qos_mpls_base.MPLS_TTL - 1,
                                           tos=0)

    GRE_MPLS_DECAP_PHP_PIPE_DSCP_TC = S.IP(dst=OVL_IP_PACKET_SIP,
                                           src=OVL_IP_PACKET_DIP,
                                           ttl=test_gre_qos_mpls_base.MPLS_TTL - 1,
                                           tos=test_gre_qos_mpls_base.OUT_TOS.flat)

    GRE_MPLS_DECAP_PHP_PIPE_PIPE_ZERO_TC = S.IP(dst=OVL_IP_PACKET_SIP,
                                                src=OVL_IP_PACKET_DIP,
                                                ttl=test_gre_qos_mpls_base.OVL_IP_TTL,
                                                tos=0)

    GRE_MPLS_DECAP_PHP_PIPE_PIPE_DSCP_TC = S.IP(dst=OVL_IP_PACKET_SIP,
                                                src=OVL_IP_PACKET_DIP,
                                                ttl=test_gre_qos_mpls_base.OVL_IP_TTL,
                                                tos=test_gre_qos_mpls_base.OUT_TOS.flat)

    GRE_MPLS_DECAP_PHP_UNIFORM_ZERO_TC = S.IP(dst=OVL_IP_PACKET_SIP,
                                              src=OVL_IP_PACKET_DIP,
                                              ttl=test_gre_qos_mpls_base.IP_TTL - 1,
                                              tos=0)

    GRE_MPLS_DECAP_PHP_UNIFORM_DSCP_TC = S.IP(dst=OVL_IP_PACKET_SIP,
                                              src=OVL_IP_PACKET_DIP,
                                              ttl=test_gre_qos_mpls_base.IP_TTL - 1,
                                              tos=test_gre_qos_mpls_base.OUT_TOS.flat)

    EXPLICIT_NULL_LABEL = sdk.la_mpls_label()
    EXPLICIT_NULL_LABEL.label = sdk.LA_MPLS_LABEL_EXPLICIT_NULL_IPV4
    ...
    def test_gre_decap_mpls_pop_double_label_qos_acl(self):
        ...
        # default TTL mode for device is UNIFORM
        self.device.set_ttl_inheritance_mode(sdk.la_mpls_ttl_inheritance_mode_e_PIPE)
