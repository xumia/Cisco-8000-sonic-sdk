...
from ip_routing_svi_eve_base_vlan_vlan import *
import unittest
...
class test_ipv4_svi_eve(ip_routing_svi_eve_base_vlan_vlan):
    ...
    base = ip_routing_svi_eve_base_vlan_vlan
    ...
    rx_svi1_trunk_in_packet = \
        S.Ether(dst=T.RX_SVI_MAC1.addr_str, src=base.SVI1_TRUNK_HOST_MAC.addr_str, type=U.Ethertype.SVLAN.value) / \
        S.Dot1Q(vlan=base.SVI1_VLAN_1, type=U.Ethertype.Dot1Q.value) / \
        S.Dot1Q(vlan=base.SVI1_VLAN_2) / \
    ...
    rx_svi1_trunk_out_packet = \
        S.Ether(dst=base.SVI1_TRUNK_HOST_MAC.addr_str, src=T.RX_SVI_MAC1.addr_str, type=U.Ethertype.SVLAN.value) / \
        S.Dot1Q(vlan=base.SVI1_VLAN_1, type=U.Ethertype.Dot1Q.value) / \
        S.Dot1Q(vlan=base.SVI1_VLAN_2) / \
    ...
    rx_svi1_trunk_out_packet2 = \
        S.Ether(dst=base.SVI1_HOST_MAC.addr_str, src=T.RX_SVI_MAC1.addr_str, type=U.Ethertype.SVLAN.value) / \
        S.Dot1Q(vlan=base.SVI1_VLAN_1, type=U.Ethertype.Dot1Q.value) / \
        S.Dot1Q(vlan=base.SVI1_VLAN_2) / \
        S.IP(dst=SVI1_TRUNK_HOST_IP_ADDR.addr_str, src=SVI_HOST_IP_ADDR.addr_str, ttl=63) / \
        S.TCP()
    ...
    rx_svi_trunk_out_packet = \
        S.Ether(dst=base.SVI_TRUNK_HOST_MAC.addr_str, src=T.RX_SVI_MAC.addr_str, type=U.Ethertype.SVLAN.value) / \
        S.Dot1Q(vlan=base.SVI_VLAN_1, type=U.Ethertype.Dot1Q.value) / \
        S.Dot1Q(vlan=base.SVI_VLAN_2) / \
        S.IP(dst=SVI_TRUNK_HOST_IP_ADDR.addr_str, src=SVI1_TRUNK_HOST_IP_ADDR.addr_str, ttl=63) / \
        S.TCP()

    rx_svi_trunk_out_packet2 = \
        S.Ether(dst=base.SVI_HOST_MAC.addr_str, src=T.RX_SVI_MAC.addr_str, type=U.Ethertype.SVLAN.value) / \
        S.Dot1Q(vlan=base.SVI_VLAN_1, type=U.Ethertype.Dot1Q.value) / \
        S.Dot1Q(vlan=base.SVI_VLAN_2) / \
        S.IP(dst=SVI_TRUNK_HOST_IP_ADDR.addr_str, src=SVI1_TRUNK_HOST_IP_ADDR.addr_str, ttl=63) / \
        S.TCP()
    ...
    rx_svi2_trunk_out_packet = \
        S.Ether(dst=base.SVI2_TRUNK_HOST_MAC.addr_str, src=base.RX_SVI_MAC2.addr_str, type=U.Ethertype.SVLAN.value) / \
        S.Dot1Q(vlan=base.SVI2_VLAN_1, type=U.Ethertype.Dot1Q.value) / \
        S.Dot1Q(vlan=base.SVI2_VLAN_2) / \
        S.IP(dst=SVI2_TRUNK_HOST_IP_ADDR.addr_str, src=SVI1_TRUNK_HOST_IP_ADDR.addr_str, ttl=63) / \
        S.TCP()

    def setUp(self):
        super().setUp()
        ip_routing_svi_eve_base_vlan_vlan.setup_ip_routing_svi_eve(self)
    ...
    def test_access_to_access(self):
        ip_routing_svi_eve_base_vlan_vlan.add_hosts(self)
    ...
    @unittest.skipIf(True, "Double Tag is not supported on non_native trunk port")
    ...
    def test_access_to_access_nh(self):
        ip_routing_svi_eve_base_vlan_vlan.create_next_hop(self)
        self.do_test_access_to_access(self.rx_svi_access_in_packet, self.rx_svi1_access_out_packet)

    @unittest.skipIf(decor.is_pacific(), "Test is not support on Pacific. No double tag NH space left")
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_access_to_trunk_nh(self):
        ip_routing_svi_eve_base_vlan_vlan.create_next_hop(self)
        ip_routing_svi_eve_base_vlan_vlan.move_nh_mac_from_access_to_trunk(self,
    ...
    def test_trunk_to_access_nh(self):
        ip_routing_svi_eve_base_vlan_vlan.create_next_hop1(self)
        self.do_test_trunk_to_access(self.rx_svi1_trunk_in_packet, self.rx_svi_access_out_packet)
    ...
    def test_trunk_to_trunk_nh(self):
        ip_routing_svi_eve_base_vlan_vlan.create_next_hop1(self)
        ip_routing_svi_eve_base_vlan_vlan.move_nh1_mac_from_access_to_trunk(self,
    ...
    def test_trunk_to_trunk_non_native_nh(self):
        ip_routing_svi_eve_base_vlan_vlan.create_next_hop2(self)
        self.do_test_trunk_to_trunk_non_native(self.rx_svi1_trunk_in_packet3, self.rx_svi2_trunk_out_packet)
