...
from ipv4_ingress_acl_class_id_320_base import *
import sim_utils
import topology as T


@unittest.skipUnless(decor.is_gibraltar(), "Class ID tests supported only on GB")
class unified_acl(ipv4_ingress_acl_class_id_320_base):

    @unittest.skipUnless(decor.is_gibraltar(), "Class ID tests supported only on GB")
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_unified_acl_with_lpm_class_id(self):
        ...
        run_and_compare_inner_fields(self, self.device,
                                     INPUT_DEFAULT_PACKET, T.RX_SLICE, T.RX_IFG, T.FIRST_SERDES,
                                     EXPECTED_DEFAULT_OUTPUT_PACKET, T.TX_SLICE_DEF, T.TX_IFG_DEF, T.FIRST_SERDES_L3,
                                     control_expected)
        ...
        pref_dests = [(sdk.la_route_entry_action_e_MODIFY, prefix, fec_l3_ac_reg1, self.CLASS_ID, self.PRIVATE_DATA, False)]
        ...
        expected_output_packet = EXPECTED_OUTPUT_PACKET.copy()
