...
class spa_port_base(sdk_test_case_base):
    ...
    def inject_and_verify_packet(self, in_packet, out_packet, tx_port_num):

        # The expected values are the slice, ifg, first_serdes of the first system_port in the spa.
        run_and_compare(
            self,
            self.device,
            in_packet,
            ...
            out_packet,
            self.s_tx[tx_port_num].slice,
            self.s_tx[tx_port_num].ifg,
            self.s_tx[tx_port_num].first_serdes)

    def inject_and_verify_packet_ingress(self, in_packet, out_packet, member):
        run_and_compare(
            self,
            self.device,
            in_packet,
            self.s_tx[member].slice,
            self.s_tx[member].ifg,
            self.s_tx[member].first_serdes,
            out_packet,
            self.IN_SLICE,
            self.IN_IFG,
            self.IN_SERDES_FIRST)
