...
class php_to_ip_base(mpls_forwarding_qos_remark_base, qos_pipe_mode_test):

    def _test_php_to_ip(self):
        ...
        U.run_and_compare(self, self.device,
                          ...
                          self.EXPECTED_OUTPUT_PHP_TO_IP_PACKET, T.TX_SLICE_REG, T.TX_IFG_REG, self.l3_port_impl.serdes_reg)
        if self.qos_inheritance_mode == sdk.la_mpls_qos_inheritance_mode_e_UNIFORM:
            U.run_and_compare(
                self,
                self.device,
                self.INPUT_PACKET_SMALLER_INNER_TTL,
                T.RX_SLICE,
                T.RX_IFG,
                T.FIRST_SERDES,
                self.EXPECTED_OUTPUT_PHP_TO_IP_PACKET_SMALLER_TTL,
                T.TX_SLICE_REG,
                T.TX_IFG_REG,
