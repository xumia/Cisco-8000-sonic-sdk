...
class voq_profile_setting_sms_age_quantization(voq_cgm_profile_base):

    @unittest.skipIf(decor.is_asic4(), "Test is not yet enabled on PL")
    @unittest.skipIf(decor.is_asic5(), "Test is not yet enabled on AR")
    @unittest.skipIf(decor.is_gibraltar(), "Test is not yet enabled on GB")
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_voq_profile_setting_sms_age_quantization(self):
        try:
            res_lst = self.voq_cgm_profile.get_sms_age_quantization()
        except sdk.BaseException as STATUS:
            self.assertEqual(STATUS.args[0], sdk.la_status_e_E_NOTFOUND)

        lst = []
        for i in range(0, sdk.la_voq_cgm_profile.SMS_NUM_AGE_QUANTIZATION_CONFIGURABLE_THRESHOLDS):
            lst.append(i * 1000)
        thresholds = sdk.sms_age_quantization_thresholds()
        thresholds.thresholds = lst

        try:
            self.voq_cgm_profile.set_sms_age_quantization(thresholds)
        except sdk.BaseException as STATUS:
            self.assertEqual(STATUS.args[0], sdk.la_status_e_E_NOTINITIALIZED)

        # Set/get check
        self.device.set_cgm_sms_voqs_age_time_granularity(1000)
        self.voq_cgm_profile.set_sms_age_quantization(thresholds)
        res_thresholds = self.voq_cgm_profile.get_sms_age_quantization()
        ...
        thresholds = self.voq_cgm_profile.get_sms_age_quantization()
        res_lst = res_thresholds.thresholds
        self.assertEqual(lst, res_lst)
