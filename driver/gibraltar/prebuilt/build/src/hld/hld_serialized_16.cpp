...
template<>
class serializer_class<silicon_one::la_npu_host_port_gibraltar> {
public:
    template <class Archive>
    static void
    do_save(Archive& archive, const silicon_one::la_npu_host_port_gibraltar& m) {
    }
    template <class Archive>
    static void
    do_load(Archive& archive, silicon_one::la_npu_host_port_gibraltar& m) {
    }
};
template <class Archive>
void
save(Archive& archive, const silicon_one::la_npu_host_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_npu_host_port_base>(&m));
    serializer_class<silicon_one::la_npu_host_port_gibraltar>::do_save(archive, m);
}
template void save<cereal_output_archive_class>(cereal_output_archive_class&, const silicon_one::la_npu_host_port_gibraltar&);

template <class Archive>
void
load(Archive& archive, silicon_one::la_npu_host_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_npu_host_port_base>(&m));
    serializer_class<silicon_one::la_npu_host_port_gibraltar>::do_load(archive, m);
}
template void load<cereal_input_archive_class>(cereal_input_archive_class&, silicon_one::la_npu_host_port_gibraltar&);
...
template<>
class serializer_class<silicon_one::la_pci_port_gibraltar> {
public:
    template <class Archive>
    static void
    do_save(Archive& archive, const silicon_one::la_pci_port_gibraltar& m) {
    }
    template <class Archive>
    static void
    do_load(Archive& archive, silicon_one::la_pci_port_gibraltar& m) {
    }
};
template <class Archive>
void
save(Archive& archive, const silicon_one::la_pci_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_pci_port_base>(&m));
    serializer_class<silicon_one::la_pci_port_gibraltar>::do_save(archive, m);
}
template void save<cereal_output_archive_class>(cereal_output_archive_class&, const silicon_one::la_pci_port_gibraltar&);

template <class Archive>
void
load(Archive& archive, silicon_one::la_pci_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_pci_port_base>(&m));
    serializer_class<silicon_one::la_pci_port_gibraltar>::do_load(archive, m);
}
template void load<cereal_input_archive_class>(cereal_input_archive_class&, silicon_one::la_pci_port_gibraltar&);
...
template<>
class serializer_class<silicon_one::la_ptp_handler_gibraltar> {
public:
    template <class Archive>
    static void
    do_save(Archive& archive, const silicon_one::la_ptp_handler_gibraltar& m) {
            archive(::cereal::make_nvp("m_gb_tree", m.m_gb_tree));
    }
    template <class Archive>
    static void
    do_load(Archive& archive, silicon_one::la_ptp_handler_gibraltar& m) {
            archive(::cereal::make_nvp("m_gb_tree", m.m_gb_tree));
    }
};
template <class Archive>
void
save(Archive& archive, const silicon_one::la_ptp_handler_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_ptp_handler_base>(&m));
    serializer_class<silicon_one::la_ptp_handler_gibraltar>::do_save(archive, m);
}
template void save<cereal_output_archive_class>(cereal_output_archive_class&, const silicon_one::la_ptp_handler_gibraltar&);

template <class Archive>
void
load(Archive& archive, silicon_one::la_ptp_handler_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_ptp_handler_base>(&m));
    serializer_class<silicon_one::la_ptp_handler_gibraltar>::do_load(archive, m);
}
template void load<cereal_input_archive_class>(cereal_input_archive_class&, silicon_one::la_ptp_handler_gibraltar&);
...
template<>
class serializer_class<silicon_one::la_punt_inject_port_gibraltar> {
public:
    template <class Archive>
    static void
    do_save(Archive& archive, const silicon_one::la_punt_inject_port_gibraltar& m) {
    }
    template <class Archive>
    static void
    do_load(Archive& archive, silicon_one::la_punt_inject_port_gibraltar& m) {
    }
};
template <class Archive>
void
save(Archive& archive, const silicon_one::la_punt_inject_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_punt_inject_port_pacgb>(&m));
    serializer_class<silicon_one::la_punt_inject_port_gibraltar>::do_save(archive, m);
}
template void save<cereal_output_archive_class>(cereal_output_archive_class&, const silicon_one::la_punt_inject_port_gibraltar&);

template <class Archive>
void
load(Archive& archive, silicon_one::la_punt_inject_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_punt_inject_port_pacgb>(&m));
    serializer_class<silicon_one::la_punt_inject_port_gibraltar>::do_load(archive, m);
}
template void load<cereal_input_archive_class>(cereal_input_archive_class&, silicon_one::la_punt_inject_port_gibraltar&);
...
template<>
class serializer_class<silicon_one::la_recycle_port_gibraltar> {
public:
    template <class Archive>
    static void
    do_save(Archive& archive, const silicon_one::la_recycle_port_gibraltar& m) {
    }
    template <class Archive>
    static void
    do_load(Archive& archive, silicon_one::la_recycle_port_gibraltar& m) {
    }
};
template <class Archive>
void
save(Archive& archive, const silicon_one::la_recycle_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_recycle_port_base>(&m));
    serializer_class<silicon_one::la_recycle_port_gibraltar>::do_save(archive, m);
}
template void save<cereal_output_archive_class>(cereal_output_archive_class&, const silicon_one::la_recycle_port_gibraltar&);

template <class Archive>
void
load(Archive& archive, silicon_one::la_recycle_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_recycle_port_base>(&m));
    serializer_class<silicon_one::la_recycle_port_gibraltar>::do_load(archive, m);
}
template void load<cereal_input_archive_class>(cereal_input_archive_class&, silicon_one::la_recycle_port_gibraltar&);
...
template<>
class serializer_class<silicon_one::la_spa_port_gibraltar> {
public:
    template <class Archive>
    static void
    do_save(Archive& archive, const silicon_one::la_spa_port_gibraltar& m) {
            archive(::cereal::make_nvp("m_resolution_cfg_handles", m.m_resolution_cfg_handles));
    }
    template <class Archive>
    static void
    do_load(Archive& archive, silicon_one::la_spa_port_gibraltar& m) {
            archive(::cereal::make_nvp("m_resolution_cfg_handles", m.m_resolution_cfg_handles));
    }
};
template <class Archive>
void
save(Archive& archive, const silicon_one::la_spa_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_spa_port_pacgb>(&m));
    serializer_class<silicon_one::la_spa_port_gibraltar>::do_save(archive, m);
}
template void save<cereal_output_archive_class>(cereal_output_archive_class&, const silicon_one::la_spa_port_gibraltar&);

template <class Archive>
void
load(Archive& archive, silicon_one::la_spa_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_spa_port_pacgb>(&m));
    serializer_class<silicon_one::la_spa_port_gibraltar>::do_load(archive, m);
}
template void load<cereal_input_archive_class>(cereal_input_archive_class&, silicon_one::la_spa_port_gibraltar&);
...
template<>
class serializer_class<silicon_one::la_system_port_gibraltar> {
public:
    template <class Archive>
    static void
    do_save(Archive& archive, const silicon_one::la_system_port_gibraltar& m) {
    }
    template <class Archive>
    static void
    do_load(Archive& archive, silicon_one::la_system_port_gibraltar& m) {
    }
};
template <class Archive>
void
save(Archive& archive, const silicon_one::la_system_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_system_port_pacgb>(&m));
    serializer_class<silicon_one::la_system_port_gibraltar>::do_save(archive, m);
}
template void save<cereal_output_archive_class>(cereal_output_archive_class&, const silicon_one::la_system_port_gibraltar&);

template <class Archive>
void
load(Archive& archive, silicon_one::la_system_port_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::la_system_port_pacgb>(&m));
    serializer_class<silicon_one::la_system_port_gibraltar>::do_load(archive, m);
}
template void load<cereal_input_archive_class>(cereal_input_archive_class&, silicon_one::la_system_port_gibraltar&);
...
template<>
class serializer_class<silicon_one::npu_host_event_queue_gibraltar> {
public:
    template <class Archive>
    static void
    do_save(Archive& archive, const silicon_one::npu_host_event_queue_gibraltar& m) {
    }
    template <class Archive>
    static void
    do_load(Archive& archive, silicon_one::npu_host_event_queue_gibraltar& m) {
    }
};
template <class Archive>
void
save(Archive& archive, const silicon_one::npu_host_event_queue_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::npu_host_event_queue_base>(&m));
    serializer_class<silicon_one::npu_host_event_queue_gibraltar>::do_save(archive, m);
}
template void save<cereal_output_archive_class>(cereal_output_archive_class&, const silicon_one::npu_host_event_queue_gibraltar&);

template <class Archive>
void
load(Archive& archive, silicon_one::npu_host_event_queue_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::npu_host_event_queue_base>(&m));
    serializer_class<silicon_one::npu_host_event_queue_gibraltar>::do_load(archive, m);
}
template void load<cereal_input_archive_class>(cereal_input_archive_class&, silicon_one::npu_host_event_queue_gibraltar&);
...
template<>
class serializer_class<silicon_one::slice_id_manager_gibraltar> {
public:
    template <class Archive>
    static void
    do_save(Archive& archive, const silicon_one::slice_id_manager_gibraltar& m) {
            archive(::cereal::make_nvp("m_use_mapping", m.m_use_mapping));
    }
    template <class Archive>
    static void
    do_load(Archive& archive, silicon_one::slice_id_manager_gibraltar& m) {
            archive(::cereal::make_nvp("m_use_mapping", m.m_use_mapping));
    }
};
template <class Archive>
void
save(Archive& archive, const silicon_one::slice_id_manager_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::slice_id_manager_base>(&m));
    serializer_class<silicon_one::slice_id_manager_gibraltar>::do_save(archive, m);
}
template void save<cereal_output_archive_class>(cereal_output_archive_class&, const silicon_one::slice_id_manager_gibraltar&);

template <class Archive>
void
load(Archive& archive, silicon_one::slice_id_manager_gibraltar& m)
{
    archive(cereal::base_class<silicon_one::slice_id_manager_base>(&m));
    serializer_class<silicon_one::slice_id_manager_gibraltar>::do_load(archive, m);
}
template void load<cereal_input_archive_class>(cereal_input_archive_class&, silicon_one::slice_id_manager_gibraltar&);
...
{
    silicon_one::la_npu_host_port_gibraltar var0;
    ar(var0);
    silicon_one::la_pci_port_gibraltar var1;
    ar(var1);
    silicon_one::la_ptp_handler_base var2;
    ar(var2);
    silicon_one::la_ptp_handler_gibraltar var3;
    ar(var3);
    silicon_one::la_punt_inject_port_gibraltar var4;
    ar(var4);
    silicon_one::la_recycle_port_gibraltar var5;
    ar(var5);
    silicon_one::la_remote_device_base var6;
    ar(var6);
    silicon_one::la_spa_port_gibraltar var7;
    ar(var7);
    silicon_one::la_system_port_gibraltar var8;
    ar(var8);
    silicon_one::npu_host_event_queue_gibraltar var9;
    ar(var9);
    silicon_one::slice_id_manager_base var10;
    ar(var10);
    silicon_one::slice_id_manager_gibraltar var11;
    ar(var11);
    silicon_one::slice_manager_smart_ptr var12;
    ar(var12);
    silicon_one::slice_manager_smart_ptr_owner var13;
    ar(var13);
}
