...
public:
    la_device_impl(ll_device_sptr ldevice);

    // Disallow default and copy c'tors
    la_device_impl(la_device_impl&) = delete;
    ...
    la_status warm_boot_save_and_destroy(const std::string& warm_boot_file, bool free_objects, la_uint32_t target_wb_revision);
    la_status warm_boot_restore(const std::string& device_path, const silicon_one::la_platform_cbs& platform_cbs);
    ...
    std::shared_ptr<T> get_sptr(const la_object* o) const
    {
        ...
        if (!of_same_device(o, this)) {
            dassert_ncrit(false, "%s belongs to a device ID %d", o->to_string().c_str(), o->get_device()->get_id());
            return nullptr;
        }
    ...
    la_status set_minimum_fabric_links_per_lc(la_device_id_t device_id, size_t links_num) override;
    la_status get_minimum_fabric_links_per_lc(la_device_id_t device_id, size_t& out_links_num) const override;
    ...
    la_status do_create_ip_multicast_group(la_multicast_group_gid_t multicast_gid,
                                           la_replication_paradigm_e rep_paradigm,
                                           la_ip_multicast_group_gibraltar_wptr& out_group);
    ...
    la_status configure_sda_mode(bool mode);
    ...
    la_status validate_acl_create(const la_acl_key_profile* acl_key_profile, const la_acl_command_profile* acl_command_profile);
    ...
    la_status add_to_mc_copy_id_table(const la_l3_ac_port_impl_wcptr& ac_port,
    ...
    la_status remove_from_mc_copy_id_table(const la_l3_ac_port_impl_wcptr& ac_port,
                                           const la_system_port_wcptr& dsp,
                                           uint64_t mc_copy_id);
...
public:
    ...
    enum {
        ...
        MAX_L2_SERVICE_PORT_GID = 128 * 1024 /* 128K */,
        MAX_L2_SERVICE_PORT_PROTECTED_GIDS = (1 << 14) /* 16K */,
        MAX_PWE_SERVICE_PORT_GID = (1 << 14),
        MAX_L3_PROTECTED_GIDS = (1 << 12) /* 4K */,
        L3_PORT_GID_NAMESPACE_WIDTH = 12,
        L3_PORT_GID_NAMESPACE_EXTENSION_WIDTH = 2,
        ...
        MAX_SYSTEM_PORT_GID_WITH_ECN_ENABLED = (1 << 12),
        ...
        NUM_VOQ_CGM_EVICTED_PROFILES_PER_DEVICE = (1 << 2),
        VOQ_CGM_DEFAULT_EVICTED_PROFILE = 0,
        NUM_VOQ_CGM_PROFILES_PER_DEVICE = (1 << 5),
        VOQ_CGM_DROP_PROFILE = 0,
        MC_VOQ_CGM_PROFILE = 1,

        MAX_DEVICES = 288, ///< Maximum number of devices in a system. NOTE - similar const is declared in reconnect_metadata.h. If
                           /// the value here is changed then need to change also there
        GB_MAX_DEVICES = 512,                   ///< Maximum number of devices in a system for GB device.
        MAX_REMOTE_SLICE = 4,                   ///< Maximum number of remote slices.
        CSMS_ALL_DEV_SUPPORT_LAST_SLICE = 2,    ///< The last network slice that can reach all remote devices
        CSMS_SUBSET_DEV_SUPPORT_SLICE = 3,      ///< The network slice that can reach a subset of remote devices
        CSMS_LAST_SUPPORTED_SUBSET_DEVICE = 71, ///< The last supported device on the special slice
        ...
        MAX_SLPS_PER_SLICE = (1 << 16) - MAX_PWE_PER_SLICE,

        // Standalone device max number of VOQs and VSCs
        // In GB, only 56K voqs are available out of 64K because of a HW bug.
        // This affects the VOQs only - not VSCs or invalid VOQ...
        MAX_AVAILABLE_VOQS_PER_SLICE = 56 * 1024,
        MAX_VOQS_PER_SLICE = 64 * 1024,
        MAX_VOQS_PER_FABRIC_SLICE
        = MAX_VOQS_PER_SLICE, // TODO GB - there is no difference in GB. Consider to delete this constant, and its subsidiaries.
        ...
        MAX_VSCS_PER_IFG_IN_SLICE = MAX_VOQS_PER_SLICE / 2,
        MAX_VSCS_PER_IFG_IN_NETWORK_SLICE
        = MAX_VOQS_PER_NETWORK_SLICE
          / 2, // TODO GB - there is no difference in GB. Consider to delete this constant, and its subsidiaries.
        MAX_VSCS_PER_IFG_IN_FABRIC_SLICE
        = MAX_VOQS_PER_FABRIC_SLICE
          / 2, // TODO GB - there is no difference in GB. Consider to delete this constant, and its subsidiaries.

        IPV4_MC_VRF_GID_RANGE_LIMIT = (MAX_VRF_GID),
        IPV6_VRF_GID_RANGE_LIMIT = (MAX_VRF_GID),
        EGRESS_DIRECT0_TABLE_SIZE = 6 * 1024,
        ...
        MAX_CLASS_IDENTIFIER = 255, // Maximum allowed class ID for a route

        MAX_CLASS_IDENTIFIER_FOR_HOSTS = 15, // Maximum allowed class ID for a host

        MAX_HOSTS_WITH_CLASS_IDENTIFIER = 511, // Maximum allowed Hosts with an associated class ID

        MAX_L2_HOSTS_WITH_CLASS_IDENTIFIER = 1023, // Maximum allowed L2 Hosts with an associated class ID
    ...
    /// @brief Limits of resolution tables
    enum {
        RESOLUTION_STAGE0_ASSOC_DATA_TABLE_SIZE = (48 * 1024),
        RESOLUTION_STAGE1_ASSOC_DATA_TABLE_SIZE = (8 * 1024),
        RESOLUTION_STAGE2_ASSOC_DATA_TABLE_SIZE = (8 * 1024),
        RESOLUTION_STAGE3_ASSOC_DATA_TABLE_SIZE = (3 * 1024),

        RESOLUTION_STAGE0_LB_GROUP_SIZE_TABLE_SIZE = (16 * 1024), // Stage0 can be configured to 32K
        RESOLUTION_STAGE1_LB_GROUP_SIZE_TABLE_SIZE = (16 * 1024),
        RESOLUTION_STAGE2_LB_GROUP_SIZE_TABLE_SIZE = (16 * 1024),
        RESOLUTION_STAGE3_LB_GROUP_SIZE_TABLE_SIZE = (16 * 1024),
    };

    /// @brief Limits of object IDs in resolution tables
    enum {
        MAX_ECMP_GROUP_STAGE0 = (1 << 14),
        MAX_ECMP_GROUP_STAGE1 = (1 << 14),
        MAX_SPA_PORT_GID = (1 << 13),
        MAX_MIRROR_GID = (1 << 6),
        MAX_MIRROR_EGRESS_GID = (1 << 5) - 1,
    ...
    enum {
        ...
        SVL_REMOTE_RESOLUTION_GID = NPL_SVL_REMOTE_RESOLUTION_GID,
        MIN_SYSTEM_PORT_GID = SVL_REMOTE_RESOLUTION_GID + 1, //=4
        ...
        RX_SILENT_DROP_DESTINATION = 0xfffff, ///< Special destination encoding that results with packet drop in the Rx.
        ...
        INVALID_FABRIC_PORT_NUM = (1 << gibraltar::pier_oob_fabric_link_to_src_link_map_table_memory::SIZE_IN_BITS) - 1,
        MAX_MIN_LINKS_THRESHOLD = (1 << gibraltar::frm_min_thr_per_device_id_cfg_table_memory::SIZE_IN_BITS) - 1,
        DEFAULT_MIN_LINKS_THRESHOLD = 1, // Default minimum fabric links global threshold for connectivity.
        MAX_COPC_ETHERNET_PROFILES = ((1 << 8) - 1),
        MAX_COPC_SWITCH_PROFILES = ((1 << 6) - 1),
        MAX_COPC_L2_SERVICE_PORT_PROFILES = ((1 << 2) - 1),
        DEFAULT_SGACL_MAX_CELL_COUNTERS = (0), ///< Default sgacl cell counters (0)
    };
    ...
    enum {
        ...
        FIRST_ALLOCATABLE_VOQ_CGM_PROFILE_ID = 2,
    ...
    enum class lp_map_prefix_type_e {
        L2_DLP_0 = 0x0,
        L2_DLP_1 = 0x1,
        DSP = 0x2,
        DSPA = 0x3,
        L2_DLP_WITH_CLASS_0 = 0x4,
        L2_DLP_WITH_CLASS_1 = 0x5,
        DSP_WITH_CLASS = 0x6,
        DSPA_WITH_CLASS = 0x7,
    };
    ...
    /// @brief Device ID location in the GID
    enum { SVL_DEVICE_ID_MARKING_BIT_IN_SYS_PORT_GID = 10 };
    ...
    static constexpr size_t INVALID_BUNDLE
        = (1 << gibraltar::rx_pdr_2_slices_fb_link_to_link_bundle_table_memory::fields::TABLE_BUNDLE_NUM_WIDTH) - 1;

    static constexpr size_t INVALID_LINK
        = (1 << gibraltar::rx_pdr_2_slices_fe_uc_link_bundle_desc_table_memory::fields::SLICE_BUNDLE_LINK0_WIDTH) - 1;
    ...
    static bool is_vsc_id_in_range(la_slice_id_t slice_id, la_vsc_gid_t vsc_id);
    ...
    ///
    /// @return     True in case the given VSC ID is in supported range of the given slice ID, false otherwise.
    bool is_dsp_in_range(la_system_port_gid_t system_port_gid) const;
    ...
    /// @brief Return the duration until age out of counter bank shadow.
    ///
    /// @return The duration until age out of counter bank shadow.
    std::chrono::milliseconds get_counter_shadow_duration_until_age_out() const;

    /// @brief Return a list of all enabled IFGs.
    const slice_ifg_vec_t& get_used_ifgs() const;

    /// @brief Return a list of all slice-pairs.
    std::vector<la_slice_pair_id_t> get_all_slice_pairs() const;
...
public:
    ...
    const std::string m_sdk_version = STRINGIFY(VERSION);
    ...
    struct profile_allocators {
        ...
        using voq_probability_profile_profile_allocator = profile_allocator<double, std::less<double> >;
        std::shared_ptr<voq_probability_profile_profile_allocator> voq_probability_profile;
    ...
    // PTP handler
    std::shared_ptr<la_ptp_handler_gibraltar> m_ptp_handler;

    // Process-Voltage-Temperature (PVT) handler
    std::unique_ptr<pvt_handler> m_pvt_handler;

    // CPU2JTAG handler
    cpu2jtag_sptr m_cpu2jtag_handler;

    // APB handlers
    std::map<apb_interface_type_e, apb_sptr> m_apb_handlers;
    ...
    using ifg_serdes_bitset = std::bitset<MAX_NUM_SERDES_PER_IFG>;
    ...
    std::vector<la_voq_cgm_evicted_profile_wptr> m_voq_cgm_evicted_profiles;
    ...
    struct security_group_cell_t {
        la_sgt_t sgt;
        la_dgt_t dgt;
        la_ip_version_e ip_version;
    };

    struct security_group_cell_t_lt {
        bool operator()(const security_group_cell_t& cell1, const security_group_cell_t& cell2) const
        {
            return (std::tie(cell1.sgt, cell1.dgt, cell1.ip_version) < std::tie(cell2.sgt, cell2.dgt, cell2.ip_version));
        }
    };

    std::map<security_group_cell_t, la_security_group_cell_wptr, security_group_cell_t_lt> m_security_group_cell_map;
    ...
    struct _index_generators {
        ...
        ranged_index_generator voq_cgm_evicted_profiles;
        ...
        ranged_index_generator pbts_map_profiles[NUM_PBTS_LEVELS];
        ...
        // Global SGACL ID generators
        ranged_index_generator sgacl_ids;
    } m_index_generators;

    // Resolution entry configurators
    resolution_configurator m_resolution_configurators[NUM_RESOLUTION_STAGES]; // array index is stage ID
    ...
    counter_manager_sptr m_counter_bank_manager;

    /// CUD range manager
    std::array<std::unique_ptr<cud_range_manager>, ASIC_MAX_SLICES_PER_DEVICE_NUM> m_cud_range_manager;

    /// MC-CUD table manager
    std::array<std::unique_ptr<mc_copy_id_manager>, ASIC_MAX_SLICES_PER_DEVICE_NUM> m_mc_copy_id_manager;
    ...
    /// Flow cache handler
    std::shared_ptr<la_flow_cache_handler_impl> m_flow_cache_handler;
    la_status create_flow_cache_handler();
    ...
    // encap_ptr to SMAC map, used to manage sa_mac rewrite indices
    std::map<uint64_t, la_mac_addr_t> m_encap_ptr_smac_map;
    ...
    const hld_notification_base_wptr get_notificator();
    ...
    cpu2jtag_sptr get_cpu2jtag_handler_sptr();
    la_status get_apb_handler_sptr(apb_interface_type_e interface_type, apb_sptr& out_apb_sptr);
    ...
    // Security Group ACL ID Functions
    la_uint64_t allocate_security_group_acl_id();
    la_status release_security_group_acl_id(la_uint64_t sgacl_id);
...
private:
    ...
    enum {
        ...
        RESOLUTION_ENCODING_PREFIX_MAX_LEN = 6,
    ...
    enum {
        ...
        /// TM header sizes
        TM_HEADER_TYPE_UNICAST_OR_MUU_PLB_SIZE_PACIFIC_MODE = 4,
        TM_HEADER_TYPE_UNICAST_OR_MUU_PLB_SIZE_GB_MODE = 5,
    ...
    enum {
        ...
        MC_OQ_ID_LO_PRIORITY = 416,
        MC_OQ_ID_HI_PRIORITY = 417,
        ...
        NUMBER_OF_FUSE_REGISTERS = 6, ///< number of fuse user bits registers
    ...
    void initialize_resolution_configurators();
    ...
    la_status initialize_l3_lp_profile_filter_default_values();
    ...
    la_status initialize_sgacl_allocation_cache();
    la_status release_sgacl_allocation_cache();

    /// @brief Functions called after la_device_impl serialize and deserialize
    ///        to serialize manual fields
    la_status warm_boot_pre_save();
    la_status warm_boot_post_restore();
    la_status warm_boot_apply_upgrade_patches(la_uint32_t base_wb_revision);
    la_status warm_boot_apply_rollback_patches(la_uint32_t target_wb_revision);
    ...
    la_status destroy_ethernet_port(const la_ethernet_port_gibraltar_wptr& ethernet_port);
    la_status destroy_l2_service_port(const la_l2_service_port_gibraltar_wptr& port);
    ...
    la_status destroy_voq_set(const la_voq_set_impl_wptr& voq_set);
    la_status destroy_voq_cgm_evicted_profile(const la_voq_cgm_evicted_profile_impl_wptr& profile);
    ...
    la_status destroy_vxlan_next_hop(const la_vxlan_next_hop_gibraltar_wptr& vxlan_next_hop);
    ...
    npl_resolution_dest_type_decoding_result_t get_resolution_destination_decoding_value(uint64_t destination_encoding);
    npl_resolution_dest_type_e get_resolution_destination_type(uint64_t destination_encoding);
    ...
    la_status configure_mpls_fwd_lookup_selector_table();
    ...
    la_status configure_dsp_dest_msbs_for_ecn_table();
    la_status configure_pdoq_oq_ifc_mapping();
    la_status configure_pdoq_oq_ifc_mapping_network(la_slice_id_t sid, la_ifg_id_t ifg);
    la_status configure_pdoq_oq_ifc_mapping_network_entry(la_slice_id_t sid, la_ifg_id_t ifg, la_uint_t serdes);
    ...
    la_status configure_fabric_tables();
    ...
    la_status configure_standalone_static_mc_voqs();
    la_status configure_linecard_static_mc_voqs();
    ...
    la_status init_fabric_minimum_links();
    la_status configure_svl_mode_table(bool svl_mode);
    la_status configure_svl_remote_destination_resolution_table();
    la_status configure_voq_cgm_default_evicted_profile();

    la_status configure_learn_manager();

    la_status initialize_acl_select_tables(la_slice_pair_id_t slice_pair);
    ...
    using slice_pair_sgacl_counter_allocation = std::vector<counter_allocation>;
    std::vector<slice_pair_sgacl_counter_allocation> m_sgacl_allocation_cache;
    ...
    using vsc_ifg_usage_t = std::bitset<MAX_VOQS_PER_SLICE>;
    //    = std::bitset<constexpr_max((int)MAX_VOQS_PER_SLICE_IN_STANDALONE_DEVICE, (int)MAX_VSCS_PER_IFG_IN_LINECARD_DEVICE)>;
    //    // TODO GB - in GB there is no difference between number of VOQs
    ...
    la_status init_config_memory(lld_memory_scptr mem, bool should_skip);
    ...
    */
    la_status init_tm_pdoq_fdoq();
    la_status init_counters_bank_interrupt_config();
    ...
    la_status turn_off_registers_for_matilda();
    la_status turn_off_idb_res_and_encdb_blocks();
    la_status turn_off_hbm_and_mmu_blocks();
    la_status rxpp_use_single_npe_on_fabric_slices();
    la_status disable_npe_powered_down_blocks();
    la_status disable_idb_res_and_encdb_powered_down_blocks();
    la_status disable_hbm_powered_down_blocks();

    /* Matilda: To disable interrupts of msi_rxpp_term_interrupt_summary for disabled slices. */
    uint64_t get_rxpp_term_interrupt_summary_mask() const;

    la_status set_default_cif_masks();
    ...
    void override_masks_tcam_parity(lld_register_value_list_t& reg_val_list);
    ...
    la_status apply_gibraltar_a1_workarounds();

    la_status init_dynamic_memories();
    la_status apply_topology_pre_2nd_soft_reset_workaround();
    la_status apply_topology_pre_2nd_soft_reset_workaround_ics();
    la_status apply_topology_pre_2nd_soft_reset_workaround_pdvoq();
    la_status apply_topology_pre_2nd_soft_reset_workaround_reorder();
    la_status apply_topology_post_soft_reset_workaround();
    la_status apply_topology_post_soft_reset_workaround_ics();
    la_status apply_topology_post_soft_reset_workaround_ics_memory_access_timeout(lld_register_value_list_t& reg_val_list);
    la_status apply_topology_post_soft_reset_workaround_pdvoq();
    ...
    la_status apply_topology_soft_reset_workaround_ics_dram_pool_fbm_conf(lld_register_value_list_t& reg_val_list,
                                                                          uint64_t dram_pool_fbm_rstn_cfg);
    la_status apply_topology_soft_reset_workaround_pdvoq_fbm_rstn_cfg(lld_register_value_list_t& reg_val_list,
                                                                      uint64_t fbm_rstn_cfg);
    la_status apply_topology_soft_reset_workaround_nw_reorder_em_fbm_cfg(lld_register_value_list_t& reg_val_list,
                                                                         uint64_t em_fbm_cfg);
    la_status apply_topology_soft_reset_workaround_pp_reorder_em_fbm_cfg(lld_register_value_list_t& reg_val_list,
                                                                         uint64_t em_fbm_cfg);
    ...
    // Notifications
    hld_notification_base_sptr m_notification;

    // fuse registers values
    bit_vector m_fuse_userbits;

    matilda_model_e m_matilda_eFuse_type;
    ...
    la_status set_fabric_protocols_version();
    ...
    std::vector<la_l2_multicast_group_gibraltar_wptr> m_l2_multicast_groups;
    std::vector<la_ip_multicast_group_gibraltar_wptr> m_ip_multicast_groups;
    ...
    la_status set_minimum_fabric_links_for_all_devices(size_t global_min_links);
    ...
    size_t get_max_devices_based_on_mode() const;
    ...
    // Periodic reset of age filter entries (used since GB A1)
    void periodic_workaround_reset_age_filter_entries();

    la_status gb_rev_a2_apply_fixes(bool fixes_enabled);
    ...
    la_status do_get_temperature(la_temperature_sensor_e sensor, la_temperature_t& out_temperature);
    ...
    la_status read_core_hard_rstn(bool& core_hard_rstn);

    la_status init_resolution_set_next_macro_table();

    // This field indicates if GB FE runs with FE mode (i.e. advertising Pacific format reachability)
    // TODO: Currently it is set to true by default, we should introduce an API to control it.
    bool m_is_in_pacific_mode;
    fabric_mac_ports_mode_e m_fabric_mac_ports_mode;
    ...
    std::unique_ptr<device_configurator_base> m_device_configurator;
    ...
    la_status register_object(const la_object_sptr& new_object, la_object_id_t& oid);
    ...
    std::vector<la_recycle_port_gibraltar_wptr> m_recycle_ports;
    std::vector<la_system_port_gibraltar_wptr> m_system_ports;
    std::vector<la_system_port_gibraltar_wptr> m_rcy_system_ports;
    std::vector<la_spa_port_gibraltar_wptr> m_spa_ports;
    ...
    la_status do_create_voq_cgm_evicted_profile(uint64_t profile_id, la_voq_cgm_evicted_profile_wptr& out_evicted_profile);
    la_status do_create_cgm_profile(uint64_t profile_id, la_voq_cgm_profile_wptr& out_voq_cgm_profile);
    ...
    template <class _PrefixType>
    la_status configure_ip_inactivity_entry(la_vrf_gid_t vrf_gid, _PrefixType prefix, bool add);

    std::vector<bool> m_is_builtin_objects;

    // Egress multicast slice replication VOQ mapping
    ...
    la_voq_set_sptr m_egress_multicast_fabric_replication_voq_set;
    std::unique_ptr<npu_host_event_queue_base> m_npu_host_eventq;

    // SDA mode for Security Group Policy (SGACL), by default set to false.
    bool m_sda_mode;
