...
static const std::vector<counter_user_group_vec> counter_user_groups[COUNTER_DIRECTION_NUM] = {
    {
        ...
        {COUNTER_USER_TYPE_DROP,
         ...
         COUNTER_USER_TYPE_SECURITY_GROUP_CELL},          // GROUP_C
...
public:
    enum {
        ...
        NUM_MCG_BANK_PROFILES_PER_SLICE = 2, // Max number of banks which can be allocated for MCG counters per slice
    };
    ...
    // In GB, NPU has only 6 bits for bank_id encoding, thus the for a mapping between {cb_id}->{npu_cb_id}
    // and that must be done in accordance with bank id mapping tables
    static size_t counter_bank_id_to_npu_bank_id(size_t counter_bank_id);
    static size_t npu_bank_id_to_counter_bank_id(size_t npu_bank_id, la_slice_id_t slice, la_ifg_id_t ifg);
...
private:
    ...
    // Mapping between 1 bit MCG counter bank profile to 7 bits counter bank per slice
...
private:
    ...
    // A helper function to initialze the counters bank IDs mapping {7bit NPU_BANK_ID}->{6bit COUNTER_BANK_ID}
    la_status initialize_counters_bank_id_map_config();

    // A helper function to initialze the meters bank IDs mapping {7bit NPU_BANK_ID}->{6bit COUNTER_BANK_ID}
    la_status initialize_meter_offset_bank_map();

    // Adds/removes the MCG counter bank_index to/from m_mcg_bank_profiles
    la_status add_bank_to_mcg_bank_profiles(size_t bank_index, la_slice_id_t slice);
