...
la_meter_set_impl::validate_meter_profile(size_t meter_index, const la_meter_profile_impl_wcptr& meter_profile)
{
    ...
    if ((meter_profile_type == la_meter_profile::type_e::GLOBAL) && (m_meter_type == type_e::PER_IFG_EXACT)) {
        log_err(HLD, "GLOBAL meter profile cannot be set to per-ifg meter");
        return LA_STATUS_EINVAL;
    }
...
la_meter_set_impl::user_rate_to_meter_rate(size_t meter_index, la_rate_t rate) const
{
    ...
    } else if (meter_measure_mode == la_meter_profile::meter_measure_mode_e::PACKETS) {
        if (get_type() == la_meter_set::type_e::STATISTICAL) {
            rate = (la_rate_t)(rate * STAT_PKT_FRACTION_COMPENSATION);
        }
    }
...
la_meter_set_impl::set_cir(size_t meter_index, la_rate_t cir)
{
    ...
    status = populate_weight_from_cir_or_eir(meter_index, cir, true /*is_cir*/, out_weight);
    return_on_error(status);
                                                   ...
                                                   la_rate_t& out_rate) const
{
    ...
    if (weight.weight == 0 && weight.weight_factor == 0) {
        out_rate = 0;
    } else {
        float shaper_max_rate = get_shaper_max_rate(meter_index, is_cir);
                                                   ...
                                                   size_t line_index)
{
    ...
    */
    la_meter_profile::meter_measure_mode_e meter_measure_mode;
    status = meter_profile->get_meter_measure_mode(meter_measure_mode);
    return_on_error(status);

    if ((meter_measure_mode == la_meter_profile::meter_measure_mode_e::PACKETS) && (m_meter_type == type_e::STATISTICAL)) {
        cbs *= STAT_PKT_FRACTION_COMPENSATION;
        ebs *= STAT_PKT_FRACTION_COMPENSATION;
    }
    ...
    if ((meter_index % 2) == 0) {
        entry.fields.table_entry0_commited_meter = cbs;
        entry.fields.table_entry0_excess_meter = ebs;
    } else {
        entry.fields.table_entry1_commited_meter = cbs;
        entry.fields.table_entry1_excess_meter = ebs;
    }
...
template la_status la_meter_set_impl::populate_meter_state_entry(la_slice_ifg,
                                                                 size_t,
                                                                 gibraltar::rx_meter_block_meters_state_table_memory&) const;
template la_status la_meter_set_impl::populate_meter_state_entry(la_slice_ifg,
                                                                 size_t,
                                                                 gibraltar::rx_meter_meters_state_table_memory&) const;
...
template la_status la_meter_set_impl::do_configure_meters_table_entry<gibraltar::rx_meter_block_meters_table_memory>(
    la_slice_ifg,
    size_t,
    lld_memory_scptr,
    size_t);
template la_status la_meter_set_impl::do_configure_meters_table_entry<gibraltar::rx_meter_meters_table_memory>(la_slice_ifg,
                                                                                                               size_t,
                                                                                                               lld_memory_scptr,
                                                                                                               size_t);
