...
private:
    ...
    la_status update_counters_from_hw(size_t counter_index, la_qos_color_e color);
    la_status update_from_counter_pair_table(size_t counter_index, la_qos_color_e color, size_t& packets, size_t& bytes);

    la_status wait_until_meter_is_full();

    /* The memory line containing packet count is wider than an unsigned long type, but
     * LSBs can be discarded since they represent packet fractions, so it can be fitted
     * into a size_t type */
    size_t form_packet_count(size_t lower_bits, size_t upper_bits);
    ...
    // We need to simulate operation of the clear-on-read present in counter banks
    // We will increment our cached values with the difference between two reads
    // To get the difference we need to keep the last value we read from the HW
    std::vector<colored_counter_values_t> m_previous_hw_packets;
    std::vector<colored_counter_values_t> m_previous_hw_bytes;
