...
public:
    ...
    virtual la_status initialize(la_object_id_t oid, la_meter_set::type_e meter_type, size_t size);
    la_status destroy();
    ...
    /* Statistical meters on GB can meter and counter in packet counting mode
     * at an increased precision compared to Pacific stat meters, but as a consequnce
     * tokens are consumed at 1024 times the rate. To compensate this we configure
     * meters to a higher rate in case they are of la_meter_set::STATISTICAL type
     * and the attached meter profile counting type is la_meter_profile::PACKET */
    static constexpr size_t STAT_PKT_FRACTION_COMPENSATION = 1024;
...
protected:
    ...
    static constexpr size_t STAT_PKT_FRACTION_SHIFT = 10;
