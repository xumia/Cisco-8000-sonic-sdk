...
la_meter_profile_impl::destroy()
{
    ...
    // We clear allocation and tables once there are no meters that use this profile, so if the profile is not in use
    // this means that it is already released the allocation and deleted itself from tables.
...
la_meter_profile_impl::set_cbs(la_uint64_t cbs)
{
    ...
    if (cbs == 0) {
        log_err(HLD, "CBS value must be greater than 0.");
        return LA_STATUS_EINVAL;
    }
...
la_meter_profile_impl::set_ebs_or_pbs(la_uint64_t ebs_or_pbs)
{
    ...
    if (ebs_or_pbs == 0) {
        log_err(HLD, "EBS/PBS value must be greater than 0.");
        return LA_STATUS_EINVAL;
    }
...
la_meter_profile_impl::set_cbs(la_slice_ifg ifg, la_uint64_t cbs)
{
    ...
    if (cbs < burst_size_resolution) {
        log_err(HLD, "The size must be greater than burst size resolution.");
        return LA_STATUS_EINVAL;
    }
                                                                ...
                                                                _Payload& payload) const
{
    ...
    payload.cbs.value = data.cbs / CBS_RESOLUTION;
    payload.ebs.value = data.ebs_or_pbs / EBS_OR_PBS_RESOLUTION;
}
...
la_meter_profile_impl::attach_statistical_meter()
{
    ...
    /* Since we now know we are attaching this profile to a statistical meter we
     * need to perform an additional validation of the CBS/EBS we set for this
     * profile */
    status = validate_cbs_and_ebs_for_stat_meter();
    return_on_error(status);
...
la_status
la_meter_profile_impl::attach_exact_meter()
{
    la_status status;
    la_uint64_t cbs_or_ebs = 0;
    la_uint64_t max_burst_size = 0;

    if (m_type == la_meter_profile::type_e::PER_IFG) {
        return LA_STATUS_SUCCESS;
    }
    ...
    for (size_t index = 0; index < NUM_IFGS_PER_DEVICE; ++index) {
        // Validating CBS first
        cbs_or_ebs = m_ifg_data[index].cbs;
        status = validate_cbs_or_ebs_for_exact_meter(cbs_or_ebs);
        return_on_error(status);
        if (cbs_or_ebs > max_burst_size) {
            log_err(HLD, "CBS is larger than max burst size.");
            return LA_STATUS_EINVAL;
        }

        // Validating EBS/PBS
        cbs_or_ebs = m_ifg_data[index].ebs_or_pbs;
        status = validate_cbs_or_ebs_for_exact_meter(cbs_or_ebs);
        return_on_error(status);
        if (cbs_or_ebs > max_burst_size) {
            log_err(HLD, "EBS/PBS is larger than max burst size.");
            return LA_STATUS_EINVAL;
        }
    }

    if (m_type == la_meter_profile::type_e::GLOBAL) {
        status = validate_cbs_or_ebs_for_exact_meter(m_stat_bank_data.cbs);
        return_on_error(status);

        status = validate_cbs_or_ebs_for_exact_meter(m_stat_bank_data.ebs_or_pbs);
        return_on_error(status);
    }
...
la_status
la_meter_profile_impl::validate_cbs_or_ebs_for_exact_meter(la_uint64_t cbs_or_ebs)
{
    // validate_burst_size
    double burst_size_resolution;
    la_status status = m_device->get_precision(la_precision_type_e::METER_PROFILE__CBS_RESOLUTION, burst_size_resolution);
    return_on_error(status);
    if (cbs_or_ebs < burst_size_resolution) {
        log_err(HLD, "CBS and EBS must be greater than burst size resolution.");
        return LA_STATUS_EINVAL;
    }
...
la_status
la_meter_profile_impl::validate_cbs_and_ebs_for_stat_meter()
{
    la_uint64_t max_burst_size;
    m_device->get_limit(limit_type_e::STAT_METER_PROFILE__MAX_BURST_SIZE, max_burst_size);

    for (size_t index = 0; index < NUM_IFGS_PER_DEVICE; ++index) {
        if (m_ifg_data[index].cbs > max_burst_size) {
            log_err(HLD, "CBS of %llu on IFG %lu is too large when attached to a statistical meter.", m_ifg_data[index].cbs, index);
            return LA_STATUS_EINVAL;
        }
    }

    if (m_stat_bank_data.cbs > max_burst_size) {
        log_err(HLD, "CBS of %llu is too large when attached to a statistical meter.", m_stat_bank_data.cbs);
        return LA_STATUS_EINVAL;
    }

    for (size_t index = 0; index < NUM_IFGS_PER_DEVICE; ++index) {
        if (m_ifg_data[index].ebs_or_pbs > max_burst_size) {
            log_err(HLD,
                    "EBS of %llu on IFG %lu is too large when attached to a statistical meter.",
                    m_ifg_data[index].ebs_or_pbs,
                    index);
            return LA_STATUS_EINVAL;
        }
    }

    if (m_stat_bank_data.ebs_or_pbs > max_burst_size) {
        log_err(HLD, "EBS of %llu is too large when attached to a statistical meter.", m_stat_bank_data.ebs_or_pbs);
        return LA_STATUS_EINVAL;
    }
...
la_meter_profile_impl::statistical_meter_profile_table_configure_entries(size_t bank_index)
{
    ...
    auto& payload = v.payloads.rx_meter_meter_profile_result;
    auto stat_meter_data = m_stat_bank_data;

    stat_meter_data.cbs *= la_meter_set_impl::STAT_PKT_FRACTION_COMPENSATION;
    stat_meter_data.ebs_or_pbs *= la_meter_set_impl::STAT_PKT_FRACTION_COMPENSATION;
    populate_meter_profile_table_key_payload(bank_index, stat_meter_data, k, payload);
