...
la_meter_set_exact_impl::do_set_meter_profile(size_t meter_index, const la_meter_profile_impl_wptr& meter_profile_impl)
{
    ...
    la_status status = add_current_ifgs(this->m_ifg_use_count.get(), meter_profile_impl);
    return_on_error(status);

    status = meter_profile_impl->attach_exact_meter();
    return_on_error(status);
...
la_meter_set_exact_impl::configure_meter_state_entry(la_slice_ifg ifg, size_t meter_index)
{
    ...
    const lld_memory_scptr& state_table = (*m_device->m_gb_tree->rx_meter->block[block_index]->meters_state_table)[table_index];
    gibraltar::rx_meter_block_meters_state_table_memory entry;
...
la_meter_set_exact_impl::configure_meters_table_entry(la_slice_ifg ifg, size_t meter_index)
{
    ...
    const lld_memory_scptr& meters_table = (*m_device->m_gb_tree->rx_meter->block[block_index]->meters_table)[table_index];
    status = do_configure_meters_table_entry<gibraltar::rx_meter_block_meters_table_memory>(
        ifg, meter_index, meters_table, line_index);
    return status;
}
...
{
    gibraltar::rx_meter_block_meters_table_memory entry;
    la_status status;
    ...
    for (size_t meter_index = 0; meter_index < m_set_size; ++meter_index) {
        ...
        lld_memory_scptr meters_table = (*m_device->m_gb_tree->rx_meter->block[block_index]->meters_table)[table_index];
