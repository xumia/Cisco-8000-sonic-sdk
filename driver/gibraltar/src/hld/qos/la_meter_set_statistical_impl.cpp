...
la_meter_set_statistical_impl::initialize(la_object_id_t oid, type_e meter_type, size_t size)
{
    ...
    m_previous_hw_packets.resize(size, {{}});
    m_previous_hw_bytes.resize(size, {{}});
...
la_meter_set_statistical_impl::get_meters_token_table_entry_details(size_t meter_index) const
{
    ...
    constexpr size_t ENTRY_LENGTH = gibraltar::rx_meter_meters_token_table_memory::fields::TABLE_ENTRY0_TOKEN_SIZE_WIDTH;
...
la_meter_set_statistical_impl::configure_meters_token_table(size_t meter_index)
{
    ...
    const lld_memory_scptr& tokens_table = (*m_device->m_gb_tree->rx_meter->top->meters_token_table)[m_bank_index];
    ...
    entry.resize(gibraltar::rx_meter_meters_token_table_memory::SIZE_IN_BITS_WO_ECC);
...
la_meter_set_statistical_impl::configure_meter_state_entry(la_slice_ifg ifg, size_t meter_index)
{
    ...
    for (la_slice_id_t sid : m_device->get_used_slices()) {
        ...
        const lld_memory_scptr& state_table = (*m_device->m_gb_tree->rx_meter->top->meters_state_table)[table_instance];
        gibraltar::rx_meter_meters_state_table_memory entry;
...
{
    const lld_memory_scptr& meters_table = (*m_device->m_gb_tree->rx_meter->top->meters_table)[m_bank_index];
    size_t line_index = (m_set_base_index + meter_index) / NUM_METERS_PER_LINE_IN_METERS_TABLE;

    la_status status
        = do_configure_meters_table_entry<gibraltar::rx_meter_meters_table_memory>(ifg, meter_index, meters_table, line_index);
    return status;
}
...
la_meter_set_statistical_impl::do_allocation()
{
    ...
    for (size_t bank_idx = 0; bank_idx < NUM_STATISTICAL_METER_BANKS; bank_idx++) {
        status = m_device->m_index_generators.statistical_meter_id[bank_idx].allocate(allocation_size, m_set_base_index);
        if (status == LA_STATUS_SUCCESS) {
            m_bank_index = bank_idx;
            break;
        }
    }
...
la_meter_set_statistical_impl::alloc_exact_meter_set_as_counter(la_meter_set_exact_impl_wptr& out_exact_meter_set_impl)
{
    ...
    return_on_error(txn.status);
    txn.on_fail([=]() { m_device->do_destroy(exact_meter_set_impl); });
...
{
    la_status status;
    bool use_new_read_mechanism = false;
    status = m_device->get_bool_property(la_device_property_e::STATISTICAL_METER_COUNTING, use_new_read_mechanism);
    return_on_error(status);
    ...
    return_on_error(txn.status);
    txn.on_fail([=]() {
        /* This lambda is bogus, it shouldn't have a return type usually,
         * in the old implementation where we manually delete the exact meter
         * no status was returned. Arguablly, the importat things are:
         * 1) the exact meter gets cleaned up properly lest we have seg faults in la_device::get_object()
         * 2) this code will be going away anyway
         * so I think that the return value shouldn't cause problems */
        m_allocations[SINGLE_ALLOCATION_IFG].reset();
        if (m_exact_meter_set_impl != nullptr) {
            la_status status = m_device->do_destroy(m_exact_meter_set_impl);
            return_on_error(status);
    ...
    if (!use_new_read_mechanism) {
        if ((user->type() == la_object::object_type_e::DEVICE) || // Meter is attached to trap/redirect and mirrors
            (user->type() == la_object::object_type_e::L2_MIRROR_COMMAND)) {
            if (m_exact_meter_set_impl == nullptr) {
                                    ...
                                    size_t& out_bytes)
{
    ...
    if (!use_new_read_mechanism) {
        if (m_exact_meter_set_impl != nullptr) {
            la_status status
                = m_exact_meter_set_impl->read(counter_index, force_update, clear_on_read, color, out_packets, out_bytes);
            return_on_error(status);
            return LA_STATUS_SUCCESS;
        } else {
            // Only users la_object.type_DEVICE and la_object.type_L2_MIRROR_COMMAND have an associated per-ifg exact meter.
            log_err(HLD, "No per-ifg exact meter set associated with this statistical meter");
            return LA_STATUS_ENOTIMPLEMENTED;
        }
    }

    if (force_update) {
        status = update_counters_from_hw(counter_index, color);
        return_on_error(status);
    }
...
la_status
la_meter_set_statistical_impl::update_counters_from_hw(size_t counter_index, la_qos_color_e color)
{
    la_status status;
    size_t bytes = 0;
    size_t packets = 0;

    size_t final_bytes_addition = 0;
    size_t final_packet_addition = 0;

    size_t packets_max_value_shift
        = gibraltar::rx_meter_counter_pair_table_memory::fields::GREEN_PKT_COUNT_WIDTH - STAT_PKT_FRACTION_SHIFT;
    size_t bytes_max_value_shift = gibraltar::rx_meter_counter_pair_table_memory::fields::GREEN_BYTE_COUNT_WIDTH;

    status = update_from_counter_pair_table(counter_index, color, packets, bytes);
    return_on_error(status);

    /* Checking if HW wrapped around */
    if (bytes < m_previous_hw_bytes[counter_index][(size_t)color]) {
        final_bytes_addition += (1UL << bytes_max_value_shift) - m_previous_hw_bytes[counter_index][(size_t)color];
        final_bytes_addition += bytes;
    } else {
        final_bytes_addition = bytes - m_previous_hw_bytes[counter_index][(size_t)color];
    }
    m_cached_bytes[counter_index][(size_t)color] += final_bytes_addition;
    m_previous_hw_bytes[counter_index][(size_t)color] = bytes;

    if (packets < m_previous_hw_packets[counter_index][(size_t)color]) {
        final_packet_addition += (1UL << packets_max_value_shift) - m_previous_hw_packets[counter_index][(size_t)color];
        final_packet_addition += packets;
    } else {
        final_packet_addition += packets - m_previous_hw_packets[counter_index][(size_t)color];
    }
    m_cached_packets[counter_index][(size_t)color] += final_packet_addition;
    m_previous_hw_packets[counter_index][(size_t)color] = packets;

    return LA_STATUS_SUCCESS;
}

la_status
la_meter_set_statistical_impl::update_from_counter_pair_table(size_t counter_index,
                                                              la_qos_color_e color,
                                                              size_t& packets,
                                                              size_t& bytes)
{
    ...
    const lld_memory_scptr& counter_pair_table = (*m_device->m_gb_tree->rx_meter->top->counter_pair_table)[m_bank_index];
    gibraltar::rx_meter_counter_pair_table_memory entry;

    status = m_device->m_ll_device->read_memory(counter_pair_table, m_set_base_index + counter_index, entry);
    return_on_error(status);

    switch (color) {
    case la_qos_color_e::GREEN:
        bytes = entry.fields.green_byte_count;
        packets = form_packet_count(entry.fields.green_pkt_count_p0, entry.fields.green_pkt_count_p1);
        break;
    case la_qos_color_e::YELLOW:
        bytes = entry.fields.yellow_byte_count;
        packets = form_packet_count(entry.fields.yellow_pkt_count_p0, entry.fields.yellow_pkt_count_p1);
        break;
    case la_qos_color_e::RED:
        bytes = entry.fields.red_byte_count;
        packets = form_packet_count(entry.fields.red_pkt_count_p0, entry.fields.red_pkt_count_p1);
        break;
    default:
...
size_t
la_meter_set_statistical_impl::form_packet_count(size_t lower_bits, size_t upper_bits)
{
    size_t packet_count = 0;
    packet_count = lower_bits >> STAT_PKT_FRACTION_SHIFT;
    packet_count = packet_count | (upper_bits << (sizeof(size_t) * bit_utils::BITS_IN_BYTE - STAT_PKT_FRACTION_SHIFT));
...
{
    const lld_memory_scptr& meters_table = (*m_device->m_gb_tree->rx_meter->top->meters_table)[m_bank_index];
    gibraltar::rx_meter_meters_table_memory entry;
    la_status status;
    ...
    for (size_t meter_index = 0; meter_index < m_set_size; ++meter_index) {
        ...
        // Store old CIR data in case meter is re-attached with no changes.
