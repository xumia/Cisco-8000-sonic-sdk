...
{
    return (ifg * tm_utils::TM_IFG_SYSTEM_PORT_SCHEDULERS + serdes) * NUM_TC_CLASSES;
}
    ...
    : m_device(device), m_hr_management_mode(la_rx_cgm_headroom_mode_e::TIMER)
{
    ...
    m_slice_id_manager = device->get_slice_id_manager();
}
...
rx_cgm_handler::set_rx_cgm_sms_bytes_quantization(const la_rx_cgm_sms_bytes_quantization_thresholds& thresholds)
{
    ...
    auto counter_thresholds_reg_1 = m_device->m_gb_tree->rx_pdr->counters_thresholds_reg1;
...
rx_cgm_handler::set_rx_cgm_sqg_thresholds(la_uint_t sqg_index, const la_rx_cgm_sqg_thresholds& thresholds)
{
    ...
    for (la_slice_id_t slice : nw_slices) {
        auto sqg_profile_lookup_table = (*m_device->m_gb_tree->rx_cgm->sq_group_profile_lut)[slice];
        gibraltar::rx_cgm_sq_group_profile_lut_memory sqg_profile = {{0}};
...
{
    auto sqg_profile_lookup_table = (*m_device->m_gb_tree->rx_cgm->sq_group_profile_lut)[0];
    gibraltar::rx_cgm_sq_group_profile_lut_memory sqg_profile = {{0}};
                                                 ...
                                                 const la_rx_cgm_sq_profile_thresholds& thresholds)
{
    ...
    auto sq_profile_lookup_table = (*m_device->m_gb_tree->rx_cgm->sq_profile_lut)[slice];
    gibraltar::rx_cgm_sq_profile_lut_memory profile_lut_entry;
...
static int
get_policy_map_index(bool flow_control, bool drop_yellow, bool drop_green, bool fc_trig)
{
    // Default configuration for SourceCGMPolicyLUTMap:
    // 0 - Trigger Xon, no drops
    // 1 - Do not change Xon/Xoff, no drops
    // 2 - Do not change Xon/Xoff, drop yellow
    // 3 - Do not change Xon/Xoff, drop yellow+green
                                             ...
                                             bool fc_trig)
{
    ...
    if (drop_green && !drop_yellow) {
        // In GB, the result of the SourceCgmPolicyLUT indexes into a seperate map register.
        // There are no map entries that correspond to drop green but no drop yellow by default, so that configuration is not
        // supported
        return LA_STATUS_EINVAL;
    }

    auto source_cgm_policy_lut = (*m_device->m_gb_tree->rx_cgm->source_cgm_policy_lut)[slice];
    ...
    gibraltar::rx_cgm_source_cgm_policy_lut_memory source_cgm_policy_line;
    status = m_device->m_ll_device->read_memory(source_cgm_policy_lut, line, source_cgm_policy_line);
    return_on_error(status);

    la_uint_t map_index = get_policy_map_index(flow_control, drop_yellow, drop_green, fc_trig);

    bit_vector bv = bit_vector(source_cgm_policy_line);
    bv.set_bits((3 * line_idx) + 2, 3 * line_idx, map_index);
...
rx_cgm_handler::set_rx_cgm_hr_timer_or_threshold_value(la_slice_id_t slice, la_uint_t profile_id, la_uint_t hr_value)
{
    ...
    if (hr_value > bit_utils::ones(gibraltar::rx_cgm_sq_profile_lut_memory::fields::SLICE_HR_THRESHOLD_OR_TIMER_MAX_WIDTH)) {
        return LA_STATUS_EINVAL;
    }
                                              ...
                                              la_uint_t profile_id)
{
    ...
    gibraltar::rx_cgm_profile_map_table_memory map_table_line;
...
rx_cgm_handler::set_rx_cgm_hr_management_mode(la_rx_cgm_headroom_mode_e mode)
{
    auto nw_slices = get_slices(m_device, la_slice_mode_e::NETWORK);
    for (la_slice_id_t slice : nw_slices) {
        auto rx_cgm_global_config = (*m_device->m_gb_tree->rx_cgm->global_configuration)[slice];
        gibraltar::rx_cgm_global_configuration_register reg;
...
rx_cgm_handler::read_rx_cgm_drop_counter(la_slice_id_t slice, la_uint_t counter_index, la_uint_t& out_packet_count)
{
    ...
    auto rx_cgm_drop_counters_reg = (*m_device->m_gb_tree->rx_cgm->rx_cgm_drop_counters1)[idx];
...
la_status
rx_cgm_handler::read_profile_map_table_line(la_slice_id_t slice,
                                            la_uint_t line,
                                            gibraltar::rx_cgm_profile_map_table_memory& out_mem)
{
    auto profile_map_table = (*m_device->m_gb_tree->rx_cgm->profile_map_table)[slice];
    gibraltar::rx_cgm_profile_map_table_memory profile_map_line;
...
la_status
rx_cgm_handler::write_profile_map_table_line(la_slice_id_t slice, la_uint_t line, gibraltar::rx_cgm_profile_map_table_memory& mem)
{
    auto profile_map_table = (*m_device->m_gb_tree->rx_cgm->profile_map_table)[slice];
...
{
    la_uint_t threshold_max = bit_utils::ones(gibraltar::rx_cgm_sq_group_profile_lut_memory::fields::SLICE_SQ_GROUP_THR0_WIDTH);
    la_uint64_t threshold_max_bytes = threshold_max * la_device_impl::SMS_BLOCK_SIZE_IN_BYTES;
...
{
    la_uint_t threshold_max = bit_utils::ones(gibraltar::rx_cgm_sq_profile_lut_memory::fields::SLICE_SQ_THR0_WIDTH);
    la_uint64_t threshold_max_bytes = threshold_max * la_device_impl::SMS_BLOCK_SIZE_IN_BYTES;
...
{
    la_uint_t threshold_max
        = bit_utils::ones(gibraltar::rx_cgm_sq_profile_lut_memory::fields::SLICE_HR_THRESHOLD_OR_TIMER_MAX_WIDTH);
    la_uint64_t threshold_max_bytes = threshold_max * la_device_impl::SMS_BLOCK_SIZE_IN_BYTES;
                                           ...
                                           size_t& out_buffers)
{
    ...
    auto sq_counters_table = (*m_device->m_gb_tree->rx_cgm->sq_counters_table)[slice];
    gibraltar::rx_cgm_sq_counters_table_memory sq_counters_table_memory;
