...
la_rx_cgm_sq_profile_impl::initialize(la_object_id_t oid, bool is_default)
{
    ...
    la_uint_t default_threshold_value = bit_utils::ones(gibraltar::rx_cgm_sq_profile_lut_memory::fields::SLICE_SQ_THR0_WIDTH)
                                        * la_device_impl::SMS_BLOCK_SIZE_IN_BYTES;
    ...
    // If default profile, update HW on init
    if (is_default) {
        ...
        for (la_slice_id_t slice : nw_slices) {
            ...
            status = set_slice_thresholds(slice, m_thresholds);
            return_on_error(status);
