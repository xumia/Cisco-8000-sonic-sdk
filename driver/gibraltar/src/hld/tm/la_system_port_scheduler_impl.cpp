...
la_system_port_scheduler_impl::get_priority_propagation(bool& out_enabled) const
{
    ...
    gibraltar::sch_tpse_general_configuration_register reg;
    la_status stat = m_device->m_ll_device->read_register(*m_sch_tpse_general_configuration, reg);
    return_on_error(stat);

    out_enabled = reg.fields.get_tpse_priority_propagation(m_sp_sch_id);
...
la_system_port_scheduler_impl::set_priority_propagation(bool enabled)
{
    ...
    gibraltar::sch_tpse_general_configuration_register sch_reg;
    la_status stat = m_device->m_ll_device->read_register(*m_sch_tpse_general_configuration, sch_reg);
    return_on_error(stat);

    sch_reg.fields.set_tpse_priority_propagation(m_sp_sch_id, enabled);

    stat = m_device->m_ll_device->write_register(*m_sch_tpse_general_configuration, sch_reg);
    return_on_error(stat);

    // Set field TpsePriorityPropagation[m_sp_sch_id] of register TpseGeneralConfiguration[m_ifg_id]
    gibraltar::pdoq_tpse_general_configuration_register pdoq_reg;

    stat = m_device->m_ll_device->read_register(
        (*m_device->m_gb_tree->slice[m_slice_id]->pdoq->top->tpse_general_configuration)[m_ifg_id], pdoq_reg);
    return_on_error(stat);

    pdoq_reg.fields.set_tpse_priority_propagation(m_sp_sch_id, enabled);
...
la_system_port_scheduler_impl::set_oq_priority_group(la_oq_id_t oid, priority_group_e pg)
{
    ...
    if (s_oq_msb_bit[oid] < BITS_PER_PORT) {
        ...
        stat = m_device->m_ll_device->read_modify_write_register(
            *(*m_device->m_gb_tree->slice[m_slice_id]->pdoq->top->tpse_oqpg_mapping_configuration)[m_ifg_id],
            base + s_oq_msb_bit[oid],
            base + s_oq_lsb_bit[oid],
            s_oq_oqpg_value[oid][(size_t)pg]);
        return_on_error(stat);
...
la_system_port_scheduler_impl::set_credit_pir(la_oq_id_t oid, la_rate_t rate)
{
    ...
    status = tm_utils::set_oqcs_rate(m_device,
                                     ...
                                     mem_line,
                                     m_sch_tpse_pir_shaper_update_cfg,
                                     rate,
                                     m_port_speed,
                                     tpse_pir,
...
la_system_port_scheduler_impl::get_credit_pir_burst_size(la_oq_id_t oid, size_t& out_burst) const
{
    ...
    size_t mem_line = m_sp_sch_id * tm_utils::NUM_OQS_PER_SYSTEM_PORT_SCH + oid;
    gibraltar::sch_oq_pir_token_bucket_cfg_memory token_bucket;
...
la_system_port_scheduler_impl::do_set_credit_pir_burst_size(la_oq_id_t oid, size_t burst)
{
    ...
    la_status status = tm_utils::set_burst_size(
        m_device, m_sch_oq_pir_token_bucket_cfg, m_sch_oq_pir_token_bucket, mem_line, m_sch_tpse_pir_shaper_update_cfg, burst);
    return_on_error(status);
...
la_system_port_scheduler_impl::get_transmit_pir(la_oq_id_t oid, la_rate_t& out_rate) const
{
    ...
    gibraltar::pdoq_oq_pir_token_bucket_cfg_memory token_bucket_cfg;
    size_t mem_line = m_sp_sch_id * tm_utils::NUM_OQS_PER_SYSTEM_PORT_SCH + oid;
    la_status status = m_device->m_ll_device->read_memory(
        (*m_device->m_gb_tree->slice[m_slice_id]->pdoq->top->oq_pir_token_bucket_cfg)[m_ifg_id], mem_line, token_bucket_cfg);
    return_on_error(status);
...
la_system_port_scheduler_impl::set_transmit_pir(la_oq_id_t oid, la_rate_t rate)
{
    ...
    const auto& pdoq_top = m_device->m_gb_tree->slice[m_slice_id]->pdoq->top;
    status = tm_utils::set_oqcs_rate(m_device,
                                     (*pdoq_top->oq_pir_token_bucket_cfg)[m_ifg_id],
                                     (*pdoq_top->oq_pir_token_bucket)[m_ifg_id],
                                     mem_line,
                                     (*pdoq_top->tpse_pir_shaper_update)[m_ifg_id],
                                     rate,
                                     m_port_speed,
                                     ifg_rate,
                                     cached_transmit_oq_pir_burst_size);
    return_on_error(status);
...
la_system_port_scheduler_impl::get_transmit_pir_burst_size(la_oq_id_t oid, size_t& out_burst) const
{
    ...
    gibraltar::pdoq_oq_pir_token_bucket_cfg_memory token_bucket;
    size_t mem_line = m_sp_sch_id * tm_utils::NUM_OQS_PER_SYSTEM_PORT_SCH + oid;
    la_status stat = m_device->m_ll_device->read_memory(
        (*m_device->m_gb_tree->slice[m_slice_id]->pdoq->top->oq_pir_token_bucket_cfg)[m_ifg_id], mem_line, token_bucket);
    return_on_error(stat);
...
la_system_port_scheduler_impl::do_set_transmit_pir_burst_size(la_oq_id_t oid, size_t burst)
{
    size_t mem_line = m_sp_sch_id * tm_utils::NUM_OQS_PER_SYSTEM_PORT_SCH + oid;
    const auto& pdoq_top = m_device->m_gb_tree->slice[m_slice_id]->pdoq->top;
    la_status status = tm_utils::set_burst_size(m_device,
                                                (*pdoq_top->oq_pir_token_bucket_cfg)[m_ifg_id],
                                                (*pdoq_top->oq_pir_token_bucket)[m_ifg_id],
                                                mem_line,
                                                (*pdoq_top->tpse_pir_shaper_update)[m_ifg_id],
...
{
    gibraltar::pdoq_uc_mc_wfq_cfg_memory uc_mc_wfq_cfg;
    ...
    std::vector<la_rate_t> rates = tm_utils::convert_weight_2_rate_vector(
        m_uc_mc_weights[oid], gibraltar::pdoq_uc_mc_wfq_cfg_memory::fields::UC_WFQ_WEIGHT_WIDTH);
    ...
    return m_device->m_ll_device->write_memory(
        (*m_device->m_gb_tree->slice[m_slice_id]->pdoq->top->uc_mc_wfq_cfg)[m_ifg_id], mem_line, uc_mc_wfq_cfg);
}
...
la_system_port_scheduler_impl::get_priority_group_credit_cir(priority_group_e pg, la_rate_t& out_rate) const
{
    ...
    gibraltar::sch_oqpg_cir_token_bucket_cfg_memory token_bucket_cfg;
...
la_system_port_scheduler_impl::set_priority_group_credit_cir(priority_group_e pg, la_rate_t rate)
{
    ...
    // The value is stored as a relative to interface CIR, the ratio is as defined below
    la_rate_t tpse_cir = 0;
    ...
    status = tm_utils::set_oqcs_rate(m_device,
                                     ...
                                     mem_line,
                                     m_sch_tpse_cir_shaper_update_cfg,
                                     rate,
                                     m_port_speed,
                                     tpse_cir,
...
la_system_port_scheduler_impl::get_priority_group_credit_burst_size(priority_group_e pg, size_t& out_burst) const
{
    ...
    size_t mem_line = m_sp_sch_id * (size_t)priority_group_e::NONE + (size_t)pg;
    gibraltar::sch_oqpg_cir_token_bucket_cfg_memory token_bucket;
...
la_system_port_scheduler_impl::do_set_priority_group_credit_burst_size(size_t pg, size_t burst)
{
    ...
    la_status status = tm_utils::set_burst_size(
        m_device, m_sch_oqpg_cir_token_bucket_cfg, m_sch_oqpg_cir_token_bucket, mem_line, m_sch_tpse_cir_shaper_update_cfg, burst);
    return_on_error(status);
...
la_system_port_scheduler_impl::get_priority_group_transmit_cir(priority_group_e pg, la_rate_t& out_rate) const
{
    ...
    gibraltar::pdoq_oqpg_cir_token_bucket_cfg_memory token_bucket_cfg;
    size_t mem_line = m_sp_sch_id * (size_t)priority_group_e::NONE + (size_t)pg;
    la_status status = m_device->m_ll_device->read_memory(
        (*m_device->m_gb_tree->slice[m_slice_id]->pdoq->top->oqpg_cir_token_bucket_cfg)[m_ifg_id], mem_line, token_bucket_cfg);
    return_on_error(status);
...
la_system_port_scheduler_impl::set_priority_group_transmit_cir(priority_group_e pg, la_rate_t rate)
{
    ...
    const auto& pdoq_top = m_device->m_gb_tree->slice[m_slice_id]->pdoq->top;
    status = tm_utils::set_oqcs_rate(m_device,
                                     (*pdoq_top->oqpg_cir_token_bucket_cfg)[m_ifg_id],
                                     (*pdoq_top->oqpg_cir_token_bucket)[m_ifg_id],
                                     mem_line,
                                     (*pdoq_top->tpse_cir_shaper_update)[m_ifg_id],
                                     rate,
                                     m_port_speed,
                                     ifg_rate,
                                     cached_transmit_oqpg_cir_burst_size);
    return_on_error(status);
...
la_system_port_scheduler_impl::get_priority_group_eir_actual_weight(priority_group_e pg, la_wfq_weight_t& out_weight) const
{
    ...
    size_t lsb = (size_t)pg * tm_utils::TPSE_WFQ_WEIGHT_WIDTH;
    size_t msb = lsb + tm_utils::TPSE_WFQ_WEIGHT_WIDTH - 1;
...
la_system_port_scheduler_impl::set_priority_group_eir_weight(priority_group_e pg, la_wfq_weight_t weight)
{
    ...
    std::vector<la_rate_t> rates = tm_utils::convert_weight_2_rate_vector(m_pg_weights, tm_utils::TPSE_WFQ_WEIGHT_WIDTH);

    bit_vector bv_rates;
    for (size_t i = 0; i < size_t(priority_group_e::NONE); i++) {
        size_t lsb = i * tm_utils::TPSE_WFQ_WEIGHT_WIDTH;
        size_t msb = lsb + tm_utils::TPSE_WFQ_WEIGHT_WIDTH - 1;
    ...
    stat = m_device->m_ll_device->write_memory(
        (*m_device->m_gb_tree->slice[m_slice_id]->pdoq->top->tpse_wfq_cfg)[m_ifg_id], m_sp_sch_id, bv_rates);
    return_on_error(stat);
