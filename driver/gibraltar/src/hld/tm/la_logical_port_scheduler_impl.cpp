...
la_logical_port_scheduler_impl::get_attached_oqcs(la_oq_pg_vec_t& out_oq_vector) const
{
    ...
    for (const auto& oqcs : m_oq_sch_set) {
        ...
        la_status status = m_device->m_ll_device->read_memory(
            m_device->m_gb_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch->oqse_map_cfg, oqse_id, tmp_bv);
...
la_logical_port_scheduler_impl::do_attach_oqcs(const la_output_queue_scheduler_impl_wptr& oqcs_impl, size_t group_id)
{
    ...
    la_status status = m_device->m_ll_device->write_memory(
        *m_device->m_gb_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch->oqse_map_cfg, oqcs_id, 1, sizeof(val), &val);
    return_on_error(
...
la_logical_port_scheduler_impl::get_group_actual_cir_weight(la_vsc_gid_t group_id, la_wfq_weight_t& out_weight) const
{
    ...
    la_status stat = m_device->m_ll_device->read_memory(
        m_device->m_gb_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch->lpse_wfq_weight_map, m_tid, tmp_bv);

    return_on_error(stat);

    // Get proper values at line of tid in LpseWfqWeightMap memory of SCH block
    size_t lsb = group_id * gibraltar::sch_lpse_wfq_weight_map_memory::fields::LPSE_CIR_WEIGHT0_WIDTH;
    size_t msb = lsb + gibraltar::sch_lpse_wfq_weight_map_memory::fields::LPSE_CIR_WEIGHT0_WIDTH - 1;
...
la_logical_port_scheduler_impl::set_group_cir_weight(la_vsc_gid_t group_id, la_wfq_weight_t weight)
{
    ...
    std::vector<la_rate_t> rates = tm_utils::convert_weight_2_rate_vector(
        m_groups_cir_weights, gibraltar::sch_lpse_wfq_weight_map_memory::fields::LPSE_CIR_WEIGHT0_WIDTH);

    bit_vector bv_rates;
    la_status stat = m_device->m_ll_device->read_memory(
        m_device->m_gb_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch->lpse_wfq_weight_map, m_tid, bv_rates);
    return_on_error(stat);

    // Set proper values at line of tid in LpseWfqWeightMap memory of SCH block
    for (size_t i = 0; i < NUM_OF_LPCS_GROUPS; i++) {
        size_t lsb = i * gibraltar::sch_lpse_wfq_weight_map_memory::fields::LPSE_CIR_WEIGHT0_WIDTH;
        size_t msb = lsb + gibraltar::sch_lpse_wfq_weight_map_memory::fields::LPSE_CIR_WEIGHT0_WIDTH - 1;
...
la_logical_port_scheduler_impl::get_group_actual_eir_weight(la_vsc_gid_t group_id, la_wfq_weight_t& out_weight) const
{
    ...
    // Get proper values at line of tid in LpseWfqWeightMap memory of SCH block
    size_t lsb
        = group_id * gibraltar::sch_lpse_wfq_weight_map_memory::fields::LPSE_CIR_WEIGHT0_WIDTH + tm_utils::EIR_WEIGHT_MAP_OFFSET;
    size_t msb = lsb + gibraltar::sch_lpse_wfq_weight_map_memory::fields::LPSE_CIR_WEIGHT0_WIDTH - 1;
    la_wfq_weight_t temp_out_weight = (la_wfq_weight_t)tmp_bv.bits(msb, lsb).get_value();
    if (temp_out_weight > tm_utils::TM_WEIGHT_MAX || temp_out_weight == 0) {
        return LA_STATUS_EUNKNOWN;
...
la_logical_port_scheduler_impl::set_group_eir_weight(la_vsc_gid_t group_id, la_wfq_weight_t weight)
{
    ...
    std::vector<la_rate_t> rates = tm_utils::convert_weight_2_rate_vector(
        m_groups_eir_weights, gibraltar::sch_lpse_wfq_weight_map_memory::fields::LPSE_CIR_WEIGHT0_WIDTH);
    ...
    for (size_t i = 0; i < NUM_OF_LPCS_GROUPS; i++) {
        size_t lsb
            = i * gibraltar::sch_lpse_wfq_weight_map_memory::fields::LPSE_CIR_WEIGHT0_WIDTH + tm_utils::EIR_WEIGHT_MAP_OFFSET;
        size_t msb = lsb + gibraltar::sch_lpse_wfq_weight_map_memory::fields::LPSE_CIR_WEIGHT0_WIDTH - 1;
        bv_rates.set_bits(msb, lsb, rates[i]);
    }
...
la_logical_port_scheduler_impl::do_get_oqcs_cir(const la_output_queue_scheduler_wptr& oqcs, la_rate_t& out_rate) const
{
    ...
    la_status status = m_device->m_ll_device->read_memory(
        m_device->m_gb_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch->oqse_cir_token_bucket_cfg, oqcs_id, token_bucket_cfg);
...
la_logical_port_scheduler_impl::set_oqcs_cir(la_output_queue_scheduler* oqcs, la_rate_t rate)
{
    ...
    const auto& sch = m_device->m_gb_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch;
    status = tm_utils::set_oqcs_rate(m_device,
                                     sch->oqse_cir_token_bucket_cfg,
                                     sch->oqse_cir_token_bucket,
                                     oqcs_id,
                                     sch->lpse_cir_shaper_update,
                                     rate,
                                     m_port_speed,
                                     tpse_rate,
...
la_logical_port_scheduler_impl::do_set_oqcs_burst_size(const la_output_queue_scheduler_wptr& oqcs, size_t burst)
{
    ...
    la_status status = tm_utils::set_burst_size(
        m_device, sch->oqse_cir_token_bucket_cfg, sch->oqse_cir_token_bucket, mem_line, sch->lpse_cir_shaper_update, burst);
    return_on_error(status);
                                                       ...
                                                       bool& out_is_eir) const
{
    ...
    gibraltar::sch_oqse_eir_token_bucket_cfg_memory token_bucket_cfg;
    la_status status = m_device->m_ll_device->read_memory(
        m_device->m_gb_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch->oqse_eir_token_bucket_cfg, oqcs_id, token_bucket_cfg);
    return_on_error(status, HLD, ERROR, "%s::%s(...); failed to read rate ratio", silicon_one::to_string(this).c_str(), __func__);

    lld_memory_sptr oqse_eir_pir_shaper_cfg(m_device->m_gb_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch->oqse_eir_pir_shaper_cfg);
    size_t line = oqcs_id / gibraltar::sch_oqse_eir_pir_shaper_cfg_memory::SIZE_IN_BITS;
    size_t index_in_line = oqcs_id % gibraltar::sch_oqse_eir_pir_shaper_cfg_memory::SIZE_IN_BITS;
    bit_vector mode_bv;
    status = m_device->m_ll_device->read_memory(oqse_eir_pir_shaper_cfg, line, mode_bv);
    return_on_error(status, HLD, ERROR, "%s::%s(...); failed to read is_eir", silicon_one::to_string(this).c_str(), __func__);
    out_is_eir = !mode_bv.bit(index_in_line);
...
la_logical_port_scheduler_impl::set_oqcs_eir_or_pir(la_output_queue_scheduler* oqcs, la_rate_t rate, bool is_eir)
{
    ...
    const auto& sch = m_device->m_gb_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch;
    status = tm_utils::set_oqcs_rate(m_device,
                                     sch->oqse_eir_token_bucket_cfg,
                                     sch->oqse_eir_token_bucket,
                                     oqcs_id,
                                     sch->lpse_eir_shaper_update,
                                     rate,
                                     m_port_speed,
                                     tpse_rate,
    ...
    size_t line = oqcs_id / gibraltar::sch_oqse_eir_pir_shaper_cfg_memory::SIZE_IN_BITS;
    size_t index_in_line = oqcs_id % gibraltar::sch_oqse_eir_pir_shaper_cfg_memory::SIZE_IN_BITS;
    status = m_device->m_ll_device->read_modify_write_memory(*oqse_eir_pir_shaper_cfg, line, index_in_line, index_in_line, !is_eir);
...
la_logical_port_scheduler_impl::do_set_oqcs_eir_or_pir_burst_size(const la_output_queue_scheduler_wptr& oqcs, size_t burst)
{
    ...
    const auto& sch = m_device->m_gb_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch;
    la_status status = tm_utils::set_burst_size(
        m_device, sch->oqse_eir_token_bucket_cfg, sch->oqse_eir_token_bucket, mem_line, sch->lpse_eir_shaper_update, burst);
    return_on_error(status);
