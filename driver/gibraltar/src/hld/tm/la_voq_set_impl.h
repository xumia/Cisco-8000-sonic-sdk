...
public:
    ...
    virtual la_status set_cgm_profile(size_t voq_index, la_voq_cgm_profile* cgm_profile) override;
    virtual la_status get_cgm_profile(size_t voq_index, la_voq_cgm_profile*& out_cgm_profile) const override;
    virtual la_status flush(bool block) override;
    ...
    virtual la_status set_fabric_priority(size_t voq_index, bool is_high_priority) override;
    virtual la_status get_fabric_priority(size_t voq_index, bool& out_is_high_priority) const override;
    ...
    virtual la_status force_local_voq_enable(bool enable);
...
protected:
    ...
    // Helper functions
    virtual uint64_t get_voq_cgm_profile_id(size_t voq_index) const;
...
private:
    ...
    enum {
        ...
        // Disabled PIF used for flushing on standalone systems
        DISABLED_PIF = 1,
    };
    ...
    la_status acquire_auto_credit_fsm(size_t drain_rate_gbps, size_t start, size_t end);
    la_status release_auto_credit_fsm(size_t offset);

    la_status map_voq_to_vsc();
