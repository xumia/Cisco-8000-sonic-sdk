...
// la_ac_port_common models the shared part betweeen la_l2_service_port_base and la_l3_ac_port_impl
...
public:
    ...
    la_status set_relay_id(la_slice_pair_id_t slice_pair_idx, uint32_t relay_id);

    const la_device_impl* get_device() const;

    uint64_t get_local_slp_id(la_slice_pair_id_t slice_pair_idx) const;
    ...
    la_status allocate_local_slp_id(la_slice_pair_id_t slice_pair_idx);
    la_status deallocate_local_slp_id(la_slice_pair_id_t slice_pair_idx);
    la_status configure_slice_ac_tcam_attributes(la_slice_pair_id_t slice_pair_idx,
...
private:
    ...
    la_status configure_slice_ac_attributes(la_slice_pair_id_t slice_pair_idx);
    la_status erase_slice_ac_attributes(la_slice_pair_id_t slice_pair_idx);

    la_status configure_slice_ac_port_table(la_slice_pair_id_t slice_pair_idx);
    la_status configure_slice_ac_port_tag_table(la_slice_pair_id_t slice_pair_idx);
    la_status configure_slice_ac_port_tag_fallback_table(la_slice_pair_id_t slice_pair_idx, la_vlan_id_t vid, bool update_entry);
    la_status configure_slice_ac_port_tag_tag_table(la_slice_pair_id_t slice_pair_idx,
                                                    la_vlan_id_t vid1,
                                                    la_vlan_id_t vid2,
                                                    bool update_entry);

    la_status configure_slice_ac_port_tcam_table(la_slice_pair_id_t slice_pair_idx,
                                                 const npl_mac_lp_attributes_payload_t& payload,
                                                 const uint32_t relay_id);
    la_status configure_slice_ac_port_tcam_tag_table(la_slice_pair_id_t slice_pair_idx,
                                                     const npl_mac_lp_attributes_payload_t& payload,
                                                     const uint32_t relay_id);
    la_status configure_slice_ac_port_tcam_tag_tag_table(la_slice_pair_id_t slice_pair_idx,
                                                         const npl_mac_lp_attributes_payload_t& payload,
                                                         const uint32_t relay_id);
    ...
    la_status add_service_mapping_vid_per_slice_pair(la_slice_pair_id_t slice_pair_idx, la_vlan_id_t vid);
    la_status remove_service_mapping_vid_per_slice_pair(la_slice_pair_id_t slice_pair_idx, la_vlan_id_t vid);
    la_status erase_slice_pair_service_mapping_vid(la_slice_pair_id_t slice_pair_idx);
    la_status configure_slice_pair_service_mapping_vid(la_slice_pair_id_t slice_pair_idx);

    la_uint_t get_slice_relay_id(la_slice_pair_id_t slice_pair_idx);
    la_status set_destination_p2p_pwe_per_slice(la_pwe_gid_t pwe_gid, la_slice_pair_id_t slice_pair_idx);
    la_status set_destination_p2p_pwe_per_slice_ac_port(la_pwe_gid_t pwe_gid, la_slice_pair_id_t slice_pair_idx);
    la_status set_destination_p2p_pwe_per_slice_ac_port_tag(la_pwe_gid_t pwe_gid, la_slice_pair_id_t slice_pair_idx);
    la_status set_destination_p2p_pwe_per_slice_ac_port_tag_tag(la_pwe_gid_t pwe_gid, la_slice_pair_id_t slice_pair_idx);
...
private:
    struct slice_pair_data {
        ...
        npl_service_mapping_tcam_ac_port_table_entry_wptr_t tcam_ac_entry = nullptr;

        /// TCAM AC port: (Port, VID) -> (Relay ID, LP ID)
        npl_service_mapping_tcam_ac_port_tag_table_entry_wptr_t tcam_ac_tag_entry = nullptr;

        /// TCAM AC port: (Port, VID, VID) -> (Relay ID, LP ID)
        npl_service_mapping_tcam_ac_port_tag_tag_table_entry_wptr_t tcam_ac_tag_tag_entry = nullptr;
