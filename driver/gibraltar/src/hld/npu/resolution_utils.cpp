...
{
    const la_l2_destination* out_l2_destination_raw = nullptr;
...
la_status
get_underlying_ethernet_port(const la_l2_destination_wcptr& l2_dest, la_ethernet_port_wcptr& out_ethernet_port)
{
    ...
    case la_object::object_type_e::L2_SERVICE_PORT: {
        ...
        auto l2_service_port_impl = l2_dest.weak_ptr_static_cast<const la_l2_service_port_base>();

        la_l2_service_port_base::port_type_e port_type = l2_service_port_impl->get_port_type();
        if (port_type != la_l2_service_port::port_type_e::AC) {
            return LA_STATUS_ENOTIMPLEMENTED;
        }
        out_ethernet_port = srvp->get_ethernet_port();
    } break;

    case la_object::object_type_e::ETHERNET_PORT: {
        out_ethernet_port = l2_dest.weak_ptr_static_cast<const la_ethernet_port_base>();
    } break;
                ...
                bool& out_is_aggregate)
{
    ...
    la_status status = get_underlying_ethernet_port(l2_dest, ep);
    return_on_error(status);

    auto epi = ep.weak_ptr_static_cast<const la_ethernet_port_base>();
    out_is_aggregate = epi->is_aggregate();
    out_gid = device->get_l2_destination_gid(ep);
...
{
    if ((lpm_dest.val & NPL_DESTINATION_MASK_VRF_REDIRECT) == NPL_DESTINATION_MASK_VRF_REDIRECT) {
        return true;
    }
...
const la_system_port_base_wcptr get_actual_dsp(const la_system_port_wcptr& dsp) // PACKET-DMA-WA
{
    ...
    // punt recycle port might be null depending on asic
    auto punt_rcy_port = idsp->get_punt_recycle_port();

    return (punt_rcy_port == nullptr) ? idsp : punt_rcy_port;
}
...
get_destination_id(const la_object* dest_object, resolution_step_e prev_step)
{
    ...
    case la_object::object_type_e::L3_AC_PORT: {
        ...
        if (port->is_stack_remote_lp_queueing_enabled()) {
            auto la_dev = static_cast<const la_device_impl*>(dest_object->get_device());
            return la_dev->get_stack_remote_resolution_destination_id();
        }
        const la_ethernet_port_base* ep = static_cast<const la_ethernet_port_base*>(port->get_ethernet_port());
        if (ep->is_aggregate()) {
            return get_destination_id(ep, RESOLUTION_STEP_STAGE2_NH);
        } else {
            la_voq_set* vs = port->get_voq_set();
            if (vs == nullptr) {
                return DESTINATION_ID_INVALID;
            }
            return get_destination_id(port->get_voq_set(), RESOLUTION_STEP_STAGE2_NH);
        }
    }
    ...
    case la_object::object_type_e::PBTS_GROUP: {
        const la_pbts_group_impl* group = static_cast<const la_pbts_group_impl*>(dest_object);
        return group->get_destination_id(prev_step);
    }
    ...
    case la_object::object_type_e::L2_PUNT_DESTINATION: {
        const la_l2_punt_destination_impl* l2_punt_destination = static_cast<const la_l2_punt_destination_impl*>(dest_object);
        return l2_punt_destination->get_destination_id(prev_step);
    }
    ...
    case la_object::object_type_e::VRF_REDIRECT_DESTINATION: {
        const la_vrf_redirect_destination_impl* vrf_redirect = static_cast<const la_vrf_redirect_destination_impl*>(dest_object);
        return vrf_redirect->get_destination_id(prev_step);
    }
...
get_lpm_destination_id(const la_object_wcptr& dest_object, resolution_step_e prev_step)
{
    ...
    case la_object::object_type_e::PBTS_GROUP: {
        const auto& group = dest_object.weak_ptr_static_cast<const la_pbts_group_impl>();
        return group->get_lpm_destination_id(prev_step);
    }
    ...
    case la_object::object_type_e::VRF_REDIRECT_DESTINATION: {
        const auto& vrf_redirect = dest_object.weak_ptr_static_cast<const la_vrf_redirect_destination_impl>();
        return vrf_redirect->get_lpm_destination_id(prev_step);
    }
...
instantiate_resolution_object(const la_object* obj, resolution_step_e prev_step)
{
    ...
    case la_object::object_type_e::PBTS_GROUP: {
        la_pbts_group_impl* group = const_cast<la_pbts_group_impl*>(static_cast<const la_pbts_group_impl*>(obj));
        la_status status = group->instantiate(prev_step);
        return status;
    }
    ...
    case la_object::object_type_e::MULTICAST_PROTECTION_GROUP: {
        return LA_STATUS_SUCCESS; // Multicast protection group is an entirely egress construct, so requires no instantiation
    }
...
la_status
instantiate_resolution_object(const la_object* obj, resolution_step_e prev_step, const la_object* dep_obj)
{
    la_object::object_type_e obj_type = obj->type();
...
{
    return instantiate_resolution_object(obj.get(), prev_step, dep_obj.get());
}

la_status
instantiate_resolution_object(const la_object_wcptr& obj, resolution_step_e prev_step, const la_object* prev_obj)
{
    return instantiate_resolution_object(obj.get(), prev_step, prev_obj);
}
...
uninstantiate_resolution_object(const la_object* obj, resolution_step_e prev_step)
{
    ...
    case la_object::object_type_e::PBTS_GROUP: {
        la_pbts_group_impl* group = const_cast<la_pbts_group_impl*>(static_cast<const la_pbts_group_impl*>(obj));
        la_status status = group->uninstantiate(prev_step);
        return status;
    }
...
get_destination_type(destination_id dest_id)
{
    if (does_destination_match_prefix(dest_id.val, NPL_DESTINATION_L2_DLP_PREFIX, NPL_DESTINATION_L2_DLP_PREFIX_LEN)) {
        ...
        return DESTINATION_TYPE_L3;
                   ...
                   dest_id.val, NPL_DESTINATION_LEVEL2_ECMP_PREFIX, NPL_DESTINATION_LEVEL2_ECMP_PREFIX_LEN)) {
        return DESTINATION_TYPE_L3;
    } else if (does_destination_match_prefix(
        ...
        return DESTINATION_TYPE_L3;
    } else if (does_destination_match_prefix(dest_id.val, NPL_DESTINATION_P_L3_NH_PREFIX, NPL_DESTINATION_P_L3_NH_PREFIX_LEN)) {
        return DESTINATION_TYPE_L3;
    }
...
l3_destination_gid_2_lpm_destination_id(la_l3_destination_gid_t dest_id)
{
    ...
    } else if (does_destination_match_prefix(dest_id, NPL_DESTINATION_LEVEL2_ECMP_PREFIX, NPL_DESTINATION_LEVEL2_ECMP_PREFIX_LEN)) {
        retval.val = (dest_id & ~NPL_DESTINATION_MASK_LEVEL2_ECMP) | NPL_LPM_COMPRESSED_DESTINATION_STAGE2_ECMP_MASK;
    } else if (does_destination_match_prefix(
...
int
res_step_to_stage(resolution_step_e res_step)
{
    switch (res_step) {
    case RESOLUTION_STEP_FEC:
    case RESOLUTION_STEP_STAGE0_ECMP:
    case RESOLUTION_STEP_STAGE0_CE_PTR:
    case RESOLUTION_STEP_STAGE0_L2_LP:
    case RESOLUTION_STEP_STAGE0_PROTECTION:
        return 0;

    case RESOLUTION_STEP_STAGE1_ECMP:
    case RESOLUTION_STEP_STAGE1_L2_DLPA:
    ...
    case RESOLUTION_STEP_STAGE3_DSPA:
        return 3;

    default:
        dassert_crit(false);
        return RESOLUTION_INVALID_STAGE;
    }
}
...
hw_load_balancing_populate_bit_vector(const la_lb_vector_t& lb_vector, bit_vector384_t& bit_vector)
{
    ...
    if (lb_vector.type == LA_LB_VECTOR_IPV4_TCP_UDP) {
        ...
        // Reg: m_gb_tree->slice[0-5].npu.rxpp_fwd.rxpp_fwd.res_lb_profile_fs_insturctions_reg[1]
        ...
        // The following bit set relies on the configuration of:
        // Reg: m_gb_tree->slice[0-5].npu.rxpp_fwd.rxpp_fwd.res_lb_profile_fs_insturctions_reg[2]
        // Field: res_lb_key_fs0_instruction
        // The configuration is made in 'la_device_impl::init_load_balancing_keys()'
        ...
        // The following bit set relies on the configuration of:
        // Reg: m_gb_tree->slice[0-5].npu.rxpp_fwd.rxpp_fwd.res_lb_profile_fs_insturctions_reg[7]
        // Field: res_lb_key_fs0_instruction
        // The configuration is made in 'la_device_impl::init_load_balancing_keys()'
        // --> dst: 'bit_vector' fs0 (bit_vector[347:316]), src/value: packet portion as defined in profile 7, fs0_instruction
        msb = HW_LOAD_BALANCE_VECTOR_BITS - 33;
        lsb = msb - 32 + 1;
        value = bit_utils::get_bits(lb_vector.ethernet.sa.flat, 39, 8);
        bit_vector.set_bits(msb, lsb, value);
        ...
        // The following bit set relies on the configuration of:
        // Reg: m_gb_tree->slice[0-5].npu.rxpp_fwd.rxpp_fwd.res_lb_profile_fs_insturctions_reg[3]
        // Field: res_lb_key_fs0_instruction
        // The configuration is made in 'la_device_impl::init_load_balancing_keys()'
        // --> dst: 'bit_vector' fs0 (bit_vector[347:316]), src/value: packet portion as defined in profile 3, fs0_instruction
        msb = HW_LOAD_BALANCE_VECTOR_BITS - 33;
        lsb = msb - 32 + 1;
        value = bit_utils::get_bits(lb_vector.ethernet.sa.flat, 31, 0);
        bit_vector.set_bits(msb, lsb, value);

        msb = HW_LOAD_BALANCE_VECTOR_BITS - 1;
        lsb = msb - 32 + 1;
        value = bit_utils::get_bits(lb_vector.ethernet.da.flat, 31, 0);
        bit_vector.set_bits(msb, lsb, value);
...
populate_soft_lb_vector_ipv4(const la_lb_vector_t& lb_vector, bit_vector384_t& bit_vector)
{
    ...
    // Set bit 54 of soft vector remaining
    msb -= (64 + 53);
...
populate_soft_lb_vector_ipv6(const la_lb_vector_t& lb_vector, bit_vector384_t& bit_vector)
{
    ...
    msb -= 32;
    set_src_to_dst_vector_in_reverse(bit_vector, msb, dip_vector_3, 0, 5);
...
populate_soft_lb_vector_eth_non_vlan(const la_lb_vector_t& lb_vector, bit_vector384_t& bit_vector)
{
    ...
    // Set bit 54 of soft vector remaining
    msb = SOFT_LOAD_BALANCE_VECTOR_BITS - 1 - 64 - 53;
    bit_vector.set_bit(msb, 1);
...
{
    std::vector<uint64_t> crc_divisors{NPL_LB_CRC_DIVISOR_5,
                                       NPL_LB_CRC_DIVISOR_4,
                                       NPL_LB_CRC_DIVISOR_3,
                                       NPL_LB_CRC_DIVISOR_2,
                                       NPL_LB_CRC_DIVISOR_1,
                                       NPL_LB_CRC_DIVISOR_0};

    std::rotate(crc_divisors.begin(), crc_divisors.begin() + shift_amount, crc_divisors.end());
...
calculate_lb_hash_for_step(const la_lb_pak_fields_vec& lb_vector, resolution_step_e step, uint16_t seed, uint16_t shift_amount)
{
    ...
    case RESOLUTION_STEP_STAGE0_ECMP: {
        // LB_KEY 0
        hw_lb_key = load_balancing_hash(seed, crc_divisors[5], hw_lb_vector, false /* is_soft */);
    ...
    case RESOLUTION_STEP_STAGE1_ECMP: {
        // LB_KEY 1
        hw_lb_key = load_balancing_hash(seed, crc_divisors[4], hw_lb_vector, false /* is_soft */);
        hw_lb_key += NPL_LB_KEY_CONST_ADD;
