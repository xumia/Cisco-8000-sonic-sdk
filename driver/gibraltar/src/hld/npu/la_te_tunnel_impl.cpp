...
la_te_tunnel_impl::clear_nh_lsp_properties(const la_next_hop* nh)
{
    ...
    const auto& te_em_map_entry_it = m_te_em_entry_map.find(next_hop);
    if (te_em_map_entry_it == m_te_em_entry_map.end()) {
        return LA_STATUS_ENOTFOUND;
    }
    // Always clear the Large-EM table programmed with the Tunnel labels
                                                 ...
                                                 const la_counter_set_wptr& counter)
{
    ...
    entry_info.ifgs = make_unique<ifg_use_count>(m_device->get_slice_id_manager());
...
{
    if (prev_step < RESOLUTION_STEP_STAGE0_L2_LP) {
...
la_te_tunnel_impl::add_ifg(const la_next_hop_base_wcptr& nh, la_slice_ifg ifg)
{
    ...
    const auto& te_em_map_entry_it = m_te_em_entry_map.find(nh);
    if (te_em_map_entry_it == m_te_em_entry_map.end()) {
...
la_te_tunnel_impl::get_lb_resolution(const la_lb_pak_fields_vec& lb_vector, size_t& member, const la_object*& out_object) const
{
    ...
    resolution_step_e step = RESOLUTION_STEP_STAGE2_ECMP;
