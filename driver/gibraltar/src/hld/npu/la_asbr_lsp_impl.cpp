...
la_status
la_asbr_lsp_impl::get_resolution_cfg_handle(const resolution_cfg_handle_t*& out_cfg_handle) const
{
    const la_l3_destination* in_stage_dest = get_destination();

    if (in_stage_dest->type() == la_object::object_type_e::L3_PROTECTION_GROUP) {
        auto l3_protection_group = static_cast<const la_l3_protection_group_impl*>(in_stage_dest);
        la_status status = l3_protection_group->get_resolution_cfg_handle(out_cfg_handle);
        return status;
    }
...
la_asbr_lsp_impl::set_destination(const la_l3_destination* destination)
{
    ...
    la_status status = check_asbr_and_destination(m_asbr, destination_sp);
    return_on_error(status);

    status = m_device->check_asbr_lsps(m_asbr, destination_sp);
    if (status == LA_STATUS_EEXIST) {
        return status;
    }

    status = m_device->update_asbr_lsp(m_asbr, destination_sp, m_device->get_sptr(this));
    return_on_error(status);
    ...
    status = notify_asbr_about_lsp_destination(m_asbr, destination_sp, true);
    return_on_error(status);
    ...
    status = m_device->clear_asbr_lsp(m_asbr, old_destination);
    return_on_error(status);

    status = notify_asbr_about_lsp_destination(m_asbr, old_destination, false);
    return_on_error(status);
...
la_asbr_lsp_impl::set_asbr(const la_prefix_object* asbr)
{
    ...
    la_status status = check_asbr_and_destination(asbr_sp, m_destination);
    return_on_error(status);

    status = m_device->check_asbr_lsps(asbr_sp, m_destination);
    if (status == LA_STATUS_EEXIST) {
        return status;
    }

    status = m_device->update_asbr_lsp(asbr_sp, m_destination, m_device->get_sptr(this));
    return_on_error(status);
    ...
    status = notify_asbr_about_lsp_destination(asbr_sp, m_destination, true);
    ...
    status = m_device->clear_asbr_lsp(old_asbr, m_destination);
    return_on_error(status);

    status = notify_asbr_about_lsp_destination(old_asbr, m_destination, false);
    return_on_error(status);
