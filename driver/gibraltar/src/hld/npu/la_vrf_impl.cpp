...
void
la_vrf_impl::update_em_entry_shadow(la_ipv4_addr_t addr, const la_l3_destination_wcptr& dest, la_user_data_t user_data)
{
    m_ipv4_em_entries[addr] = {dest, user_data};
}

void
la_vrf_impl::update_em_entry_shadow(la_ipv6_addr_t addr, const la_l3_destination_wcptr& dest, la_user_data_t user_data)
{
    m_ipv6_em_entries[addr] = {dest, user_data};
}
...
la_vrf_impl::get_route_info_from_em_shadow(la_ipv4_addr_t addr, la_ip_route_info& out_ip_route_info) const
{
    ...
    out_ip_route_info.l3_dest = shadow_it->second.dest.get();
    out_ip_route_info.user_data = shadow_it->second.user_data;
...
la_vrf_impl::initialize(la_object_id_t oid, la_vrf_gid_t gid)
{
    ...
    la_status status = add_lpm_entry(m_device->m_tables.ipv4_lpm_table,
                                     prefix_v4,
                                     default_dest,
                                     LA_CLASS_ID_DEFAULT,
                                     0 /* user_data */,
    ...
    status = add_em_entry(
        m_device->m_tables.ipv4_vrf_dip_em_table, illegal_dip_v4, illegal_dip_dest, nullptr /* l3_port */, LA_CLASS_ID_DEFAULT);
    return_on_error(status);
    ...
    status = add_em_entry(
        m_device->m_tables.ipv6_vrf_dip_em_table, illegal_dip_v6, illegal_dip_dest, nullptr /* l3_port */, LA_CLASS_ID_DEFAULT);
    return_on_error(status);
                              ...
                              bool clear_catch_all_entry)
{
    ...
    if ((prefix_length != 0) || clear_catch_all_entry) {
        ...
        new_value.payloads.lpm_payload.class_id.id = LA_CLASS_ID_DEFAULT;
                           ...
                           bool latency_sensitive)
{
    ...
    value.payloads.lpm_payload.class_id.id = class_id;
                          ...
                          la_user_data_t user_data)
{
    ...
    status = add_em_entry(table, addr, dest, m_device->get_sptr(l3_port), class_id);
    return_on_error(status);

    update_em_entry_shadow(addr, destination, user_data);
                          ...
                          la_class_id_t class_id)
{
    ...
    if (l3_port != nullptr) {
        ...
        l3_dlp_with_prefix.msbs.l3_dlp_msbs = get_l3_lp_msb(l3_dlp_gid);
    ...
    //  value.payloads.em_lookup_result.result_type = NPL_IP_EM_LPM_RESULT_TYPE_DESTINATION_FROM_EM; // TODO Igor - commented to
    //  compile.
    //  value.payloads.em_lookup_result.result.em_dest.dest = dest;
    value.payloads.em_lookup_result.result_type = NPL_IP_UC_EM_RESULT_TYPE_HOST_PTR_AND_L3_DLP;
    value.payloads.em_lookup_result.result.ptr_and_l3_dlp.destination = dest;
    value.payloads.em_lookup_result.result.em_dest.dest = dest;
    // Temp patch to force alignment.
    value.payloads.em_lookup_result.result.em_dest.em_rpf_src = l3_dlp_with_prefix;
    value.payloads.em_lookup_result.result.em_dest.class_id.id = class_id;
...
la_vrf_impl::clear_all_ip_em_routes(npl_ipv6_vrf_dip_em_table_sptr_t v6_em_table)
{
    ...
    for (const auto& prefix : prefixes_to_remove) {
        la_status status = do_ipv6_route_action(la_route_entry_action_e::DELETE,
                                                prefix,
                                                nullptr,
                                                false /* class_id_set */,
                                                LA_CLASS_ID_DEFAULT,
...
la_vrf_impl::get_route_info_from_table_entry(weak_ptr_unsafe<_EntryType> entry, la_ip_route_info& out_ip_route_info) const
{
    ...
    if ((lpm_dest_id.val >> (NPL_LPM_COMPRESSED_DESTINATION_LEN - NPL_DESTINATION_VRF_REDIRECT_PREFIX_LEN))
        == NPL_DESTINATION_VRF_REDIRECT_PREFIX) {
        out_ip_route_info.is_host = false;
        out_ip_route_info.user_data = entry->user_data();
        la_vrf_gid_t vrf_gid = lpm_dest_id.val & ~NPL_DESTINATION_MASK_VRF_REDIRECT;
        la_status status = m_device->get_vrf_redirect_destination_by_id(vrf_gid, out_ip_route_info.l3_dest);
        return_on_error(status);
        return LA_STATUS_SUCCESS;
    }
    ...
    if (is_l3_lpm_destination(lpm_dest)) {
        ...
        out_ip_route_info.class_id = entry->value().payloads.lpm_payload.class_id.id;
        out_ip_route_info.user_data = entry->user_data();
    } else {
...
la_vrf_impl::delete_ipv4_route(la_ipv4_prefix_t prefix)
{
    ...
    return do_ipv4_route_action(la_route_entry_action_e::DELETE,
                                prefix,
                                nullptr,
                                false /* class_id_set */,
                                LA_CLASS_ID_DEFAULT,
...
la_vrf_impl::common_pre_bulk_update(const std::shared_ptr<_LpmTableType>& lpm_table,
                                    ...
                                    la_class_id_t& out_old_class_id,
                                       ...
                                       npl_destination_t& out_lpm_dest)
{
    ...
    case la_route_entry_action_e::DELETE:
        ...
        route_entry.class_id = LA_CLASS_ID_DEFAULT;
        route_entry.is_class_id_set = true;
...
la_vrf_impl::do_lpm_pre_bulk_update(const std::shared_ptr<_LpmTableType>& lpm_table,
                                    ...
                                    la_user_data_t& out_old_user_data)
{
    la_status status = common_pre_bulk_update(
        lpm_table, em_table, route_entry, out_lpm_dest, old_destination, out_old_class_id, out_old_user_data);
    return_on_error(status);
                                  ...
                                  size_t& out_count_success)
{
    ...
    la_class_id_t old_class_id = LA_CLASS_ID_DEFAULT;
    ...
    for (uint32_t i = start_batch; i < end_batch; i++) {
        ...
        status = do_lpm_pre_bulk_update(
            lpm_table, em_table, route_entry_vec[i], lpm_dest, old_destination, old_class_id, old_user_data);
        if (status != LA_STATUS_SUCCESS) {
        ...
        value.payloads.lpm_payload.class_id.id = (route_entry_vec[i].is_class_id_set) ? route_entry_vec[i].class_id : old_class_id;
                               ...
                               size_t& out_count_success)
{
    ...
    status
        = common_pre_bulk_update(lpm_table, em_table, route_entry_vec[i], lpm_dest, old_destination, old_class_id, old_user_data);
    if (status != LA_STATUS_SUCCESS) {
        return status;
    }
    ...
    case la_route_entry_action_e::ADD:
        if (route_entry_vec[i].is_class_id_set) {
            status = add_em_entry(em_table,
                                  route_entry_vec[i].prefix.addr,
                                  m_device->get_sptr(route_entry_vec[i].destination),
                                  route_entry_vec[i].class_id,
                                  route_entry_vec[i].user_data);
        } else {
            status = add_em_entry(em_table,
                                  route_entry_vec[i].prefix.addr,
                                  m_device->get_sptr(route_entry_vec[i].destination),
    ...
    case la_route_entry_action_e::MODIFY:
        if (route_entry_vec[i].is_class_id_set) {
            status = modify_em_entry(em_table,
                                     route_entry_vec[i].prefix.addr,
                                     m_device->get_sptr(route_entry_vec[i].destination),
                                     route_entry_vec[i].class_id,
                                     route_entry_vec[i].user_data);
        ...
        if (is_em) {
            ...
            if (start_batch < cur) {
                ...
                status = ip_lpm_bulk_updates(
                    lpm_table, em_table, route_entry_vec, lpm_entries_bulk, start_batch, cur, successful_lpm_routes);
...
la_vrf_impl::ipv4_route_bulk_updates(la_ipv4_route_entry_parameters_vec route_entry_vec, size_t& out_count_success)
{
    ...
    if ((status == LA_STATUS_SUCCESS) && (route_entry_vec.size() > 1) && (out_count_success == 0)) {
        ...
        case la_route_entry_action_e::ADD:
            status = do_ipv4_route_action(la_route_entry_action_e::ADD,
                                          route_entry_vec[0].prefix,
                                          m_device->get_sptr(route_entry_vec[0].destination),
                                          true /* class_id_set */,
                                          route_entry_vec[0].class_id,
                                          true /* user_data_set */,
        ...
        case la_route_entry_action_e::MODIFY:
            status = do_ipv4_route_action(la_route_entry_action_e::MODIFY,
                                          route_entry_vec[0].prefix,
                                          m_device->get_sptr(route_entry_vec[0].destination),
                                          route_entry_vec[0].is_class_id_set /* class_id_set */,
                                          route_entry_vec[0].is_class_id_set ? route_entry_vec[0].class_id : LA_CLASS_ID_DEFAULT,
                                          route_entry_vec[0].is_user_data_set /* user_data_set */,
                                          route_entry_vec[0].user_data,
                                          false);
            break;

        case la_route_entry_action_e::DELETE:
            status = do_ipv4_route_action(la_route_entry_action_e::DELETE,
                                          route_entry_vec[0].prefix,
                                          nullptr,
                                          false /* class_id_set */,
                                          LA_CLASS_ID_DEFAULT,
...
la_vrf_impl::do_ipv4_route_action(const la_route_entry_action_e action,
                                  ...
                                  const la_l3_destination_wcptr& destination,
                                  const bool is_class_id_set,
                                  const la_class_id_t class_id,
                                  ...
                                  const bool latency_sensitive)
{
    ...
    route_entry_vec[0].is_class_id_set = is_class_id_set;
                                 ...
                                 bool enable_rpf_check)
{
    ...
    populate_mc_result_payload(
        mcg, rpf, false, punt_and_forward, use_rpfid, rpfid, value.payloads.em_lookup_result.result.mc_result, enable_rpf_check);
    value.payloads.em_lookup_result.result_type = NPL_IP_UC_EM_RESULT_TYPE_HOST_PTR_AND_L3_DLP;
                                         ...
                                         la_counter_set* counter)
{
    ...
    status = verify_mc_route_parameters(
...
la_vrf_impl::ipv6_route_bulk_updates(la_ipv6_route_entry_parameters_vec route_entry_vec, size_t& out_count_success)
{
    ...
    if ((status == LA_STATUS_SUCCESS) && (route_entry_vec.size() > 1) && (out_count_success == 0)) {
        switch (route_entry_vec[0].action) {
        case la_route_entry_action_e::ADD:
            status = do_ipv6_route_action(la_route_entry_action_e::ADD,
                                          route_entry_vec[0].prefix,
                                          m_device->get_sptr(route_entry_vec[0].destination),
                                          true /* class_id_set */,
                                          route_entry_vec[0].class_id,
        ...
        case la_route_entry_action_e::MODIFY:
            status = do_ipv6_route_action(la_route_entry_action_e::MODIFY,
                                          route_entry_vec[0].prefix,
                                          m_device->get_sptr(route_entry_vec[0].destination),
                                          route_entry_vec[0].is_class_id_set /* class_id_set */,
                                          route_entry_vec[0].is_class_id_set ? route_entry_vec[0].class_id : LA_CLASS_ID_DEFAULT,
        ...
        case la_route_entry_action_e::DELETE:
            status = do_ipv6_route_action(la_route_entry_action_e::DELETE,
                                          route_entry_vec[0].prefix,
                                          nullptr,
                                          false /* class_id_set */,
                                          LA_CLASS_ID_DEFAULT,
...
la_vrf_impl::delete_ipv6_route(la_ipv6_prefix_t prefix)
{
    ...
    return do_ipv6_route_action(la_route_entry_action_e::DELETE,
                                prefix,
                                nullptr,
                                false /* class_id_set */,
                                LA_CLASS_ID_DEFAULT,
                                         ...
                                         la_counter_set* counter)
{
    ...
    // unregister this route with the multicast group
    auto mcg_base = desc.mcg.weak_ptr_static_cast<la_ip_multicast_group_base>();
    la_vrf_impl_sptr vrf_impl_sptr = m_device->get_sptr(this);
    status = mcg_base->unregister_mc_ipv6_vrf_route(vrf_impl_sptr, saddr, gaddr);
    return_on_error(status);
    ...
    if (group_prefix.length == default_prefix.length) {
        ...
        if (status == LA_STATUS_ENOTFOUND) {
            status = add_lpm_entry(
                table, group_prefix, destination, LA_CLASS_ID_DEFAULT, 0 /* user_data */, false /* latency_sensitive */);
            return_on_error(status);
        } else {
            status = entry->update(value, 0 /* user_data */);
            return_on_error(status);
...
la_vrf_impl::update_ip_subnet(const std::shared_ptr<_TableType>& table, _PrefixType subnet, const la_l3_port_wcptr& l3_port)
{
    ...
    value.payloads.lpm_payload.class_id.id = LA_CLASS_ID_DEFAULT;

    // Update table entry
    return entry->update(value, entry->user_data());
}
...
la_vrf_impl::add_ipv4_subnet(la_ipv4_prefix_t subnet, const la_l3_port_wptr& l3_port)
{
    ...
    la_status status = add_lpm_entry(table, subnet, dest, LA_CLASS_ID_DEFAULT, 0 /* user_data */, false /* latency_sensitive */);
    return_on_error(status);
                                    ...
                                    la_ipv6_route_entry_vec& out_route_entries)
{
    ...
    for (const auto& addr_dest : m_ipv6_em_entries) {
        ...
        tmp_entry.route_info.user_data = addr_dest.second.user_data;
        out_route_entries.push_back(tmp_entry);
    }
...
static void
populate_em_table_key(la_vrf_gid_t vrf_gid, la_ipv4_addr_t ip_addr, npl_ipv4_sgt_em_table_key_t& out_em_key)
{
    out_em_key.vrf_id = vrf_gid;
    out_em_key.ip_address_31_20 = (ip_addr.s_addr >> 20) & 0xfff;
    out_em_key.ip_address_19_0 = ip_addr.s_addr & 0xfffff;
}

static void
populate_em_table_key(la_vrf_gid_t vrf_gid, la_ipv6_addr_t ip_addr, npl_ipv6_sgt_em_table_key_t& out_em_key)
{
    out_em_key.vrf_id = vrf_gid;
    out_em_key.ip_address[0] = ip_addr.q_addr[0];
    out_em_key.ip_address[1] = ip_addr.q_addr[1];
}

static void
populate_lpm_table_key(la_vrf_gid_t vrf_gid, la_ipv4_addr_t ip_addr, npl_ipv4_sgt_lpm_table_key_t& out_lpm_key)
{
    out_lpm_key.vrf_id = vrf_gid;
    out_lpm_key.ip_address = ip_addr.s_addr;
}

static void
populate_lpm_table_key(la_vrf_gid_t vrf_gid, la_ipv6_addr_t ip_addr, npl_ipv6_sgt_lpm_table_key_t& out_lpm_key)
{
    out_lpm_key.vrf_id = vrf_gid;
    out_lpm_key.ip_address[0] = ip_addr.q_addr[0];
    out_lpm_key.ip_address[1] = ip_addr.q_addr[1];
}

template <class _TableType, class _AddrType>
la_status
la_vrf_impl::add_em_entry(const std::shared_ptr<_TableType>& table, _AddrType addr, la_sgt_t sgt)
{
    typename _TableType::key_type key;
    typename _TableType::value_type value;
    typename _TableType::entry_type* entry = nullptr;

    populate_em_table_key(m_gid, addr, key);

    value.payloads.ip_sgt_em_result.result.sgt_data.security_group_tag = sgt;
    value.payloads.ip_sgt_em_result.result.sgt_data.valid_group = 1;
...
template <class _TableType, class _AddrType>
la_status
la_vrf_impl::modify_em_entry(const std::shared_ptr<_TableType>& table, _AddrType addr, la_sgt_t sgt)
{
    typename _TableType::key_type key;
    typename _TableType::value_type value;
    typename _TableType::entry_type* entry = nullptr;
...
template <class _TableType, class _AddrType>
la_status
la_vrf_impl::get_security_group_tag_info_for_host(const std::shared_ptr<_TableType>& table, _AddrType addr, la_sgt_t& out_sgt) const
{
    typename _TableType::key_type key;
    typename _TableType::value_type value;
    typename _TableType::entry_type* entry = nullptr;
    ...
    value = entry->value();
    out_sgt = value.payloads.ip_sgt_em_result.result.sgt_data.security_group_tag;
                           ...
                           bool latency_sensitive)
{
    ...
    populate_lpm_table_key(m_gid, prefix.addr, key);

    if (prefix.length == 0) {
        status = table->lookup(key, entry);
        if (status == LA_STATUS_SUCCESS) {
            value = entry->value();
            if (value.payloads.sgt_data.security_group_tag != sgt) {
                value.payloads.sgt_data.security_group_tag = sgt;
                status = entry->update(value, 0 /* user_data */);
            }
            return LA_STATUS_SUCCESS;
        }
    ...
    value.payloads.sgt_data.security_group_tag = sgt;
    value.payloads.sgt_data.valid_group = prefix.length ? 1 : 0;

    status = table->insert(key, prefix.length, value, user_data, latency_sensitive, entry);
...
template <class _TableType, class _PrefixType>
la_status
la_vrf_impl::modify_lpm_entry(const std::shared_ptr<_TableType>& table, _PrefixType prefix, la_sgt_t sgt)
{
    typename _TableType::key_type key;
    typename _TableType::value_type value;
    typename _TableType::entry_type* entry = nullptr;
...
template <class _TableType, class _PrefixType>
la_status
la_vrf_impl::delete_lpm_entry(const std::shared_ptr<_TableType>& table, _PrefixType prefix)
{
    typename _TableType::key_type key;

    populate_lpm_table_key(m_gid, prefix.addr, key);

    la_status status = table->erase(key, prefix.length);
...
template <class _TableType, class _PrefixType>
la_status
la_vrf_impl::get_security_group_tag_info_for_prefix(const std::shared_ptr<_TableType>& table,
                                                    _PrefixType prefix,
                                                    la_sgt_t& out_sgt) const
{
    typename _TableType::key_type key;
    typename _TableType::value_type value;
    typename _TableType::entry_type* entry = nullptr;
    ...
    value = entry->value();
    out_sgt = value.payloads.sgt_data.security_group_tag;
...
{
    start_api_call("prefix=", prefix, "sgt=", sgt);

    la_status status;
    if (prefix.length == 32) {
        status = add_em_entry(m_device->m_tables.ipv4_sgt_em_table, prefix.addr, sgt);
    } else {
        status = add_lpm_entry(m_device->m_tables.ipv4_sgt_lpm_table, prefix, sgt, 0 /* user_data */, false);
    }
    return_on_error(status);
...
la_vrf_impl::modify_security_group_tag(la_ipv4_prefix_t prefix, la_sgt_t sgt)
{
    ...
    la_status status;
    if (prefix.length == 32) {
        status = modify_em_entry(m_device->m_tables.ipv4_sgt_em_table, prefix.addr, sgt);
    } else {
        status = modify_lpm_entry(m_device->m_tables.ipv4_sgt_lpm_table, prefix, sgt);
    }
    return_on_error(status);
...
la_vrf_impl::delete_security_group_tag(la_ipv4_prefix_t prefix)
{
    ...
    la_status status;
    if (prefix.length == 32) {
        status = delete_em_entry(m_device->m_tables.ipv4_sgt_em_table, prefix.addr);
    } else {
        status = delete_lpm_entry(m_device->m_tables.ipv4_sgt_lpm_table, prefix);
    }
    return_on_error(status);
...
la_vrf_impl::get_security_group_tag(la_ipv4_prefix_t prefix, la_sgt_t& out_sgt) const
{
    ...
    la_status status;
    if (prefix.length == 32) {
        status = get_security_group_tag_info_for_host(m_device->m_tables.ipv4_sgt_em_table, prefix.addr, out_sgt);
    } else {
        status = get_security_group_tag_info_for_prefix(m_device->m_tables.ipv4_sgt_lpm_table, prefix, out_sgt);
    }
    return_on_error(status);
...
la_vrf_impl::add_security_group_tag(la_ipv6_prefix_t prefix, la_sgt_t sgt)
{
    ...
    if (prefix.length == 128) {
        status = add_em_entry(m_device->m_tables.ipv6_sgt_em_table, prefix.addr, sgt);
    } else {
        status = add_lpm_entry(m_device->m_tables.ipv6_sgt_lpm_table, prefix, sgt, 0 /* user_data */, false);
    }
    return_on_error(status);
...
la_vrf_impl::modify_security_group_tag(la_ipv6_prefix_t prefix, la_sgt_t sgt)
{
    ...
    la_status status;
    if (prefix.length == 128) {
        status = modify_em_entry(m_device->m_tables.ipv6_sgt_em_table, prefix.addr, sgt);
    } else {
        status = modify_lpm_entry(m_device->m_tables.ipv6_sgt_lpm_table, prefix, sgt);
    }
    return_on_error(status);
...
la_vrf_impl::delete_security_group_tag(la_ipv6_prefix_t prefix)
{
    ...
    la_status status;
    if (prefix.length == 128) {
        status = delete_em_entry(m_device->m_tables.ipv6_sgt_em_table, prefix.addr);
    } else {
        status = delete_lpm_entry(m_device->m_tables.ipv6_sgt_lpm_table, prefix);
    }
    return_on_error(status);
...
la_vrf_impl::get_security_group_tag(la_ipv6_prefix_t prefix, la_sgt_t& out_sgt) const
{
    ...
    la_status status;
    if (prefix.length == 128) {
        status = get_security_group_tag_info_for_host(m_device->m_tables.ipv6_sgt_em_table, prefix.addr, out_sgt);
    } else {
        status = get_security_group_tag_info_for_prefix(m_device->m_tables.ipv6_sgt_lpm_table, prefix, out_sgt);
    }
    return_on_error(status);
