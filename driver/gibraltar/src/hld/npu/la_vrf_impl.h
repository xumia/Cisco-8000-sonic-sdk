...
public:
    ...
    la_status get_max_vrf_gids(unsigned int& out_max_vrf_gids) const;
...
private:
    ...
    struct ip_em_entry {
        la_l3_destination_wcptr dest;
        la_user_data_t user_data;
    };
    CEREAL_SUPPORT_PRIVATE_CLASS(ip_em_entry);
    ...
    std::map<la_ipv4_addr_t, ip_em_entry, ipv4_less_op> m_ipv4_em_entries;
    std::map<la_ipv6_addr_t, ip_em_entry, ipv6_less_op> m_ipv6_em_entries;
...
private:
    ...
    la_status add_em_entry(const std::shared_ptr<_TableType>& table,
                           _AddrType addr,
                           const la_l3_destination_wcptr& dest,
                           la_class_id_t class_id,
                           la_user_data_t user_data);
    ...
    template <class _TableType, class _AddrType>
    la_status modify_em_entry(const std::shared_ptr<_TableType>& table,
    ...
    template <class _TableType, class _AddrType>
    la_status add_em_entry(const std::shared_ptr<_TableType>& table, _AddrType addr, la_sgt_t sgt);

    template <class _TableType, class _AddrType>
    la_status modify_em_entry(const std::shared_ptr<_TableType>& table, _AddrType addr, la_sgt_t sgt);

    template <class _TableType, class _AddrType>
    la_status get_security_group_tag_info_for_host(const std::shared_ptr<_TableType>& table,
                                                   _AddrType addr,
    ...
    template <class _TableType, class _PrefixType>
    la_status modify_lpm_entry(const std::shared_ptr<_TableType>& table, _PrefixType prefix, la_sgt_t sgt);

    template <class _TableType, class _PrefixType>
    la_status delete_lpm_entry(const std::shared_ptr<_TableType>& table, _PrefixType prefix);

    template <class _TableType, class _PrefixType>
    la_status get_security_group_tag_info_for_prefix(const std::shared_ptr<_TableType>& table,
                                                     _PrefixType prefix,
                                                     la_sgt_t& out_sgt) const;
    ...
    void update_em_entry_shadow(la_ipv4_addr_t addr, const la_l3_destination_wcptr& dest, la_user_data_t user_data);
    void update_em_entry_shadow(la_ipv6_addr_t addr, const la_l3_destination_wcptr& dest, la_user_data_t user_data);
