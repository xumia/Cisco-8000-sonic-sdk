                             ...
                             la_egress_qos_profile_impl* egress_qos_profile_impl)
{
    ...
    slice_ifg_vec_t slice_ifgs = get_all_network_ifgs(m_device);
    for (la_slice_ifg ifg : slice_ifgs) {
    ...
    txn.status = m_device->m_ipv4_tunnel_ep_manager->add_local_ep_entry(
        m_local_ip_prefix, m_underlay_vrf, m_sip_index->id(), NPL_PROTOCOL_TYPE_GRE, NPL_TERMINATION_SIP_DIP_INDEX_LDB);
    return_on_error(txn.status,
                    HLD,
                    ERROR,
...
la_gre_port_impl::destroy()
{
    ...
    auto status = m_device->m_ipv4_tunnel_ep_manager->remove_local_ep_entry(
        m_local_ip_prefix, m_underlay_vrf, NPL_PROTOCOL_TYPE_GRE, m_sip_index->id());
    return_on_error(status);
...
la_gre_port_impl::set_underlay_vrf(const la_vrf* underlay_vrf)
{
    ...
    // add the new tunnel endpoint to the endpoint database
...
la_gre_port_impl::set_local_ip_prefix(const la_ipv4_prefix_t local_ip_prefix)
{
    ...
    // allocate SIP index
    auto status = m_device->m_ipv4_sip_index_manager->allocate_sip_index(local_ip_prefix, sip_index);
    return_on_error(status);

    // remove the existing local IP address from the tunnel endpoint database
...
la_gre_port_impl::configure_ip_tunnel_dlp_table(la_slice_pair_id_t pair_idx)
{
    ...
    // attrib.qos_and_acl_info.demux_count = demux_count ? 1 : 0;
    ;
...
{
    for (la_slice_id_t slice_id : get_slices(m_device, la_slice_mode_e::NETWORK)) {
        la_status status
...
la_gre_port_impl::teardown_lp_attributes_table()
{
    for (la_slice_id_t slice_id : get_slices(m_device, la_slice_mode_e::NETWORK)) {
        if (m_slice_data[slice_id].ipv4_gre_tunnel_termination_sip_dip_index_tt0_table_entry != nullptr) {
            la_status status = teardown_lp_attributes_table(slice_id);
            return_on_error(status);
        }
    }
    return LA_STATUS_SUCCESS;
