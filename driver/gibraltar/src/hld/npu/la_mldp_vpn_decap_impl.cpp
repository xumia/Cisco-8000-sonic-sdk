...
la_mldp_vpn_decap_impl::add_to_mldp_termination_table(const la_counter_set_wcptr& counter)
{
    for (la_slice_pair_id_t slice_pair : get_slice_pairs(m_device, la_slice_mode_e::NETWORK)) {
        ...
        la_status status = table->set(key, value, m_slice_pair_data[slice_pair].m_mldp_termination_entry);
...
la_mldp_vpn_decap_impl::update_mldp_termination_table(la_mpls_label label, const la_vrf_wcptr& vrf, la_uint_t rpfid, bool bud_node)
{
    ...
    for (la_slice_pair_id_t slice_pair = 0; slice_pair < NUM_SLICE_PAIRS_PER_DEVICE; slice_pair++) {
    ...
    for (la_slice_pair_id_t slice_pair = 0; slice_pair < NUM_SLICE_PAIRS_PER_DEVICE; slice_pair++) {
        ...
        result.pwe_vpn_mldp_info.l3vpn_info.l3_relay_id.id = vrf_gid;
        result.pwe_vpn_mldp_info.l3vpn_info.vpn_p_counter
            = populate_counter_ptr_slice_pair(m_counter, slice_pair, COUNTER_DIRECTION_INGRESS);
        result.pwe_vpn_mldp_info.l3vpn_info.vpn_mldp_info.mldp_info.rpf_id = m_rpfid;
...
la_mldp_vpn_decap_impl::remove_from_mldp_termination_table()
{
    for (la_slice_pair_id_t slice_pair : get_slice_pairs(m_device, la_slice_mode_e::NETWORK)) {
        const auto& table(m_device->m_tables.mpls_termination_em1_table[slice_pair]);
        npl_mpls_termination_em1_table_key_t key = m_slice_pair_data[slice_pair].m_mldp_termination_entry->key();

        la_status status = table->erase(key);
        return_on_error(status);

        m_slice_pair_data[slice_pair].m_mldp_termination_entry = nullptr;
    }
    return LA_STATUS_SUCCESS;
}
