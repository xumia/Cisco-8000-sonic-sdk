...
{
public:
    static const size_t PREFIX_LEN = 12; // 12 bits for vrf ID
...
template <>
class table_init_helper<npl_ipv4_sgt_lpm_table_t, TABLE_TYPE_LPM>
{
public:
    static const size_t PREFIX_LEN = 12; // 12 bits for vrf ID
    static const lpm_ip_protocol_e IP_PROTOCOL = lpm_ip_protocol_e::IPV4;

    la_status init_table(npl_ipv4_sgt_lpm_table_t& table, ra_translator_creator& creator, const std::vector<size_t>& indices)
    {
        typename npl_ipv4_sgt_lpm_table_t::table_translator_sptr_vec_t translator_vec;
        ...
        std::unique_ptr<lpm_db> db = creator.create_lpm_table(PREFIX_LEN, IP_PROTOCOL);
        dassert_crit(db);
        translator_vec.push_back(std::make_shared<ra_lpm_translator<npl_ipv4_sgt_lpm_table_t::trait_type> >(std::move(db)));
        table.initialize(translator_vec);
...
template <>
class table_init_helper<npl_ipv6_sgt_lpm_table_t, TABLE_TYPE_LPM>
{
public:
    static const size_t PREFIX_LEN = 12; // 12 bits for vrf ID
    static const lpm_ip_protocol_e IP_PROTOCOL = lpm_ip_protocol_e::IPV6;

    la_status init_table(npl_ipv6_sgt_lpm_table_t& table, ra_translator_creator& creator, const std::vector<size_t>& indices)
    {
        typename npl_ipv6_sgt_lpm_table_t::table_translator_sptr_vec_t translator_vec;
        ...
        std::unique_ptr<lpm_db> db = creator.create_lpm_table(PREFIX_LEN, IP_PROTOCOL);
        dassert_crit(db);
        translator_vec.push_back(std::make_shared<ra_lpm_translator<npl_ipv6_sgt_lpm_table_t::trait_type> >(std::move(db)));
        table.initialize(translator_vec);
...
private:
    enum {
        ...
        PHYSICAL_TABLE_PAYLOAD_WIDTH = gibraltar::ifgb_24p_tc_lut_mem_memory::SIZE_IN_BITS_WO_ECC,
    };
};
...
private:
    enum {
        PORT_PACKET_INDEX_KEY_PART_WIDTH = 8,
        G_IFG_INDEX_KEY_PART_WIDTH = 4,
        FULL_KEY_WIDTH = PORT_PACKET_INDEX_KEY_PART_WIDTH + G_IFG_INDEX_KEY_PART_WIDTH,
        PHYSICAL_TABLE_PAYLOAD_WIDTH = gibraltar::rx_meter_rate_limiter_shaper_configuration_table_memory::SIZE_IN_BITS_WO_ECC,
    };
};
