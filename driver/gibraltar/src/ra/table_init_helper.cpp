                                                                          ...
                                                                          const std::vector<size_t>& indices)
{
    ...
    auto gt = ldevice->get_gibraltar_tree();
    size_t first_slice = indices[0];
    const size_t NUM_MEMORIES_PER_IFG = gt->slice[first_slice]->ifg[0]->ifgb->tc_lut_mem->size();
        ...
        for (size_t ifg_id = 0; ifg_id < engine_block_mapper::NUM_IFGS_PER_SLICE; ifg_id++) {
            ...
            for (size_t mem_idx = 0; mem_idx < NUM_MEMORIES_PER_IFG; mem_idx++) {
                ...
                physical_sram sram = {.start_line = 0,
                                      ...
                                      .memories = {(*gt->slice[slice_id]->ifg[ifg_id]->ifgb->tc_lut_mem)[mem_idx]}};
    ...
    const std::vector<size_t>& indices)
{
    ...
    auto ldevice = creator.get_ll_device();
    auto gt = ldevice->get_gibraltar_tree();
    const size_t NUM_TABLES_PER_DEVICE = gt->rx_meter->top->rate_limiter_shaper_configuration_table->size();
    const size_t NUM_TABLE_ENTRIES = (*gt->rx_meter->top->rate_limiter_shaper_configuration_table)[0]->get_desc()->entries;
    ...
    for (size_t table_idx = 0; table_idx < NUM_TABLES_PER_DEVICE; table_idx++) {
        ...
        physical_sram sram = {.start_line = 0,
                              .offset = 0,
                              .width = PHYSICAL_TABLE_PAYLOAD_WIDTH,
                              .memories = {(*gt->rx_meter->top->rate_limiter_shaper_configuration_table)[table_idx]}};
