...
def read_tables(npl_tables, database_resources_map, ret):
    ...
    # Temp - Manually define key/payload widths
    # For the following tables the NPL widths did not match the SDK widths (that were defined in external tables file).
    # When resolved, the link to script in pacific path can be restored.
    ret['egress_nh_and_svi_direct0_table']['translated_key_width'] = 13
    ret['egress_nh_and_svi_direct1_table']['translated_key_width'] = 12
    ret['stage0_group_size_table']['translated_key_width'] = 15
    #ret['hmc_cgm_cgm_lut_table']['translated_key_width'] = 13
    #ret['hmc_cgm_cgm_lut_table']['translated_payload_width'] = 48
    #ret['hmc_cgm_profile_global_table']['translated_payload_width'] = 540
