...
{
    return std::make_shared<silicon_one::nsim_translator_creator>(lld, npl_context_slices);
}

silicon_one::device_simulator*
create_nsim_simulator(const char* socket_addr, size_t port, const char* sdk_version)
{
    silicon_one::simulator_client* sim = new silicon_one::simulator_client();

    if (!sim->initialize(socket_addr, port, sdk_version)) {
        delete sim;
        return nullptr;
    }
