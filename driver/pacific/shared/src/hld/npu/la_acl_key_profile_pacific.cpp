...
#include "la_acl_key_profile_pacific.h"
...
la_acl_key_profile_pacific::la_acl_key_profile_pacific(const la_device_impl_wptr& device) : la_acl_key_profile_base(device)
{
}

la_acl_key_profile_pacific::~la_acl_key_profile_pacific()
{
}

int8_t
la_acl_key_profile_pacific::get_vlan_outer_offset() const
{
    return VLAN_OUTER_OFFSET;
}

int8_t
la_acl_key_profile_pacific::get_vlan_inner_offset() const
{
    return VLAN_INNER_OFFSET;
}

la_status
la_acl_key_profile_pacific::fill_ethernet_udk_components(std::vector<udk_component>& udk_components,
...
la_status
la_acl_key_profile_pacific::fill_v4_udk_components(std::vector<udk_component>& udk_components, const la_acl_key_def_vec_t& key_def)
{
    ...
    if ((dst_bincode) && (src_bincode)) {
        ...
        log_err(HLD, "la_acl_key_profile_pacific: og acl, must configure both src&dst bincodes");
        return LA_STATUS_EINVAL;
    }
    ...
    if (key_size > 320) {
        log_err(HLD, "la_acl_key_profile_pacific::%s Aggregated ipv4 acl key size %d > 320", __func__, static_cast<int>(key_size));
        return LA_STATUS_EINVAL;
    }
...
la_status
la_acl_key_profile_pacific::fill_v6_udk_components(std::vector<udk_component>& udk_components, const la_acl_key_def_vec_t& key_def)
{
    uint64_t udk_comp_size = key_def.size() + 1; // +1 for acl_id field
    udk_components.resize(udk_comp_size, udk_component());
    uint64_t key_size = 0;
    ...
    if (key_size > 320) {
        log_err(HLD, "la_acl_key_profile_pacific::%s Aggregated ipv6 acl key size %d > 320", __func__, static_cast<int>(key_size));
        return LA_STATUS_EINVAL;
    }
