...
#include "la_l2_multicast_group_akpg.h"
...
la_l2_multicast_group_akpg::la_l2_multicast_group_akpg(la_device_impl_wptr device) : la_l2_multicast_group_base(device)
{
}

la_l2_multicast_group_akpg::~la_l2_multicast_group_akpg()
{
}

la_status
la_l2_multicast_group_akpg::get_mc_copy_id(const member_t& member,
...
la_status
la_l2_multicast_group_akpg::add_to_mc_copy_id_table(const member_t& member, const la_system_port_wcptr& dsp)
{
    return LA_STATUS_SUCCESS;
}

la_status
la_l2_multicast_group_akpg::remove_from_mc_copy_id_table(const member_t& member, const la_system_port_wcptr& dsp)
{
    return LA_STATUS_SUCCESS;
}

la_status
la_l2_multicast_group_akpg::allocate_mc_copy_id(const la_l2_destination* member,
...
la_status
la_l2_multicast_group_akpg::release_mc_copy_id(const member_t& member, const la_system_port_wcptr& dsp_sptr)
{
    la_status status;
    auto adsp = get_actual_dsp(dsp_sptr);
    la_slice_id_t dest_slice = adsp->get_slice();
...
la_status
la_l2_multicast_group_akpg::add(const la_l2_destination* vxlan_port, la_next_hop* next_hop, const la_system_port* dsp)
{
    transaction txn;

    auto vxlan_port_sptr = m_device->get_sptr(vxlan_port);
    auto dsp_sptr = m_device->get_sptr(dsp);
...
la_status
la_l2_multicast_group_akpg::remove_cud_table_entry(const la_l2_destination* destination, const la_system_port_wcptr& dsp)
{
    return LA_STATUS_ENOTIMPLEMENTED;
}

la_status
la_l2_multicast_group_akpg::configure_stack_copy_cud_mapping(la_slice_id_t slice, uint64_t mc_copy_id)
{
    ...
    payload.overload_union_l2_encapsulation_type_app_defined.l2_encapsulation_type = NPL_NPU_ENCAP_L2_HEADER_TYPE_SVL;
    payload.overload_union_l2_ac_encdap_app_defined.l2_ac_encdap.l2_dlp.l2_dlp.id = 0;
