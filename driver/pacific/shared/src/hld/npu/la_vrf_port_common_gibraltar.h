...
#ifndef __LA_VRF_PORT_COMMON_GIBRALTAR_H__
#define __LA_VRF_PORT_COMMON_GIBRALTAR_H__
...
class la_vrf_port_common_gibraltar : public la_vrf_port_common_pacgb
{
    /////// Cereal ///////////////
    CEREAL_SUPPORT_PRIVATE_MEMBERS
    la_vrf_port_common_gibraltar() = default;
    //////////////////////////////

public:
    la_vrf_port_common_gibraltar(const la_device_impl_wptr& device, la_l3_port_wptr parent);
    virtual ~la_vrf_port_common_gibraltar();
...
// Get Mac address based on host encoding with Class ID or without Class ID
static inline void
populate_mac_address(const _ValueType& value, la_mac_addr_t& out_mac_address)
{
    auto dest_type = value.payloads.em_lookup_result.result.dsp_host_w_class.dest_type_or_has_class.dest_type;
    if (dest_type & 0x4) {
        out_mac_address.flat = value.payloads.em_lookup_result.result.dsp_host_w_class.host_mac_lsb;
        out_mac_address.bytes[5] = value.payloads.em_lookup_result.result.dsp_host_w_class.host_mac_msb << 1;
    } else {
        out_mac_address.flat = value.payloads.em_lookup_result.result.dsp_host_wo_class.host_mac_lsb;
        out_mac_address.bytes[5] = value.payloads.em_lookup_result.result.dsp_host_wo_class.host_mac_msb << 1;
    }
}

template <class _ValueType>
// Get Mac address and Class ID based on host encoding with Class ID or without Class ID
static inline void
populate_mac_address_and_class_id(const _ValueType& value, la_mac_addr_t& out_mac_address, la_class_id_t& out_class_id)
{
    auto dest_type = value.payloads.em_lookup_result.result.dsp_host_w_class.dest_type_or_has_class.dest_type;
    if (dest_type & 0x4) {
        out_mac_address.flat = value.payloads.em_lookup_result.result.dsp_host_w_class.host_mac_lsb;
...
#endif // __LA_VRF_PORT_COMMON_GIBRALTAR_H__
