...
la_svi_port_pacific::la_svi_port_pacific(const la_device_impl_wptr& device) : la_svi_port_base(device)
{
}

la_svi_port_pacific::~la_svi_port_pacific()
{
}

la_status
la_svi_port_pacific::add_ipv4_host(la_ipv4_addr_t ip_addr, la_mac_addr_t mac_addr, la_class_id_t class_id)
{
    ...
    return la_svi_port_base::add_ipv4_host(ip_addr, mac_addr);
}

la_status
la_svi_port_pacific::modify_ipv4_host(la_ipv4_addr_t ip_addr, la_mac_addr_t mac_addr, la_class_id_t class_id)
{
    start_api_call("ip_addr=", ip_addr, "mac_addr=", mac_addr, "class_id=", class_id);
    ...
    return la_svi_port_base::modify_ipv4_host(ip_addr, mac_addr);
}

la_status
la_svi_port_pacific::add_ipv6_host(la_ipv6_addr_t ip_addr, la_mac_addr_t mac_addr, la_class_id_t class_id)
{
    start_api_call("ip_addr=", ip_addr, "mac_addr=", mac_addr, "class_id=", class_id);
    ...
    return la_svi_port_base::add_ipv6_host(ip_addr, mac_addr);
}

la_status
la_svi_port_pacific::modify_ipv6_host(la_ipv6_addr_t ip_addr, la_mac_addr_t mac_addr, la_class_id_t class_id)
{
    start_api_call("ip_addr=", ip_addr, "mac_addr=", mac_addr, "class_id=", class_id);
    ...
    return la_svi_port_base::modify_ipv6_host(ip_addr, mac_addr);
}

la_status
la_svi_port_pacific::populate_recycled_inject_up_info_table(const la_l2_service_port_base_wptr& inject_up_port)
{
    ...
    if (is_recycle_ac) {
        key.tx_to_rx_rcy_data = NPL_TX2RX_SCHED_RCY_DATA_OBM_TO_INJECT_UP & 0x3f;
    } else {
        key.tx_to_rx_rcy_data = NPL_TX2RX_RCY_DATA_OBM_TO_INJECT_UP & 0x3f;
    }

    auto& first_table = m_device->m_tables.recycled_inject_up_info_table[m_device->first_active_slice_id()];
    la_status status = first_table->lookup(key, entry);
...
la_status
la_svi_port_pacific::clear_recycled_inject_up_info_table()
{
    npl_recycled_inject_up_info_table_key_t key;
    key.tx_to_rx_rcy_data = NPL_TX2RX_RCY_DATA_OBM_TO_INJECT_UP & 0x3f;
    for (la_slice_id_t slice : m_device->get_used_slices()) {
...
la_status
la_svi_port_pacific::update_additional_l3_lp_attributes(const npl_l3_lp_additional_attributes_t& additional_attribs)
{
    ...
    npl_l3_lp_additional_attributes_t& current_additional_attribs(
        value.payloads.relay.relay_table_payload.relay_attr.l3_lp_additional_attributes);
...
void
la_svi_port_pacific::fill_npl_mac_termination_em_table_key(la_switch_gid_t sw_gid,
                                                           const la_mac_addr_t& mac_addr,
                                                           uint64_t prefix,
                                                           npl_mac_termination_em_table_key_t& out_key)
{
    out_key.relay_id.id = sw_gid;
    out_key.ethernet_header_da_18_0_ = mac_addr.flat & ((1ULL << 19) - 1);
