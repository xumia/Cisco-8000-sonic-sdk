...
#include "la_acl_key_profile_gibraltar.h"
...
la_acl_key_profile_gibraltar::la_acl_key_profile_gibraltar(const la_device_impl_wptr& device) : la_acl_key_profile_base(device)
{
}

la_acl_key_profile_gibraltar::~la_acl_key_profile_gibraltar()
{
}

int8_t
la_acl_key_profile_gibraltar::get_vlan_outer_offset() const
{
    return VLAN_OUTER_OFFSET;
}

int8_t
la_acl_key_profile_gibraltar::get_vlan_inner_offset() const
{
    return VLAN_INNER_OFFSET;
}

la_status
la_acl_key_profile_gibraltar::fill_ethernet_udk_components(std::vector<udk_component>& udk_components,
...
la_status
la_acl_key_profile_gibraltar::fill_v4_udk_components(std::vector<udk_component>& udk_components,
                                                     const la_acl_key_def_vec_t& key_def)
{
    ...
    if ((dst_bincode) && (src_bincode)) {
        ...
        log_err(HLD, "la_acl_key_profile_gibraltar: og acl, must configure both src&dst bincodes");
        return LA_STATUS_EINVAL;
    }
    ...
    if (key_size > 320) {
        log_err(
            HLD, "la_acl_key_profile_gibraltar::%s Aggregated ipv4 acl key size %d > 320", __func__, static_cast<int>(key_size));
        return LA_STATUS_EINVAL;
    }
...
la_status
la_acl_key_profile_gibraltar::fill_v6_udk_components(std::vector<udk_component>& udk_components,
                                                     const la_acl_key_def_vec_t& key_def)
{
    uint64_t udk_comp_size = key_def.size() + 1; // +1 for acl_id field
    udk_components.resize(udk_comp_size, udk_component());
    ...
    if (key_size > 320) {
        log_err(
            HLD, "la_acl_key_profile_gibraltar::%s Aggregated ipv6 acl key size %d > 320", __func__, static_cast<int>(key_size));
        return LA_STATUS_EINVAL;
    }
