...
#include "la_ethernet_port_akpg.h"
...
la_ethernet_port_akpg::la_ethernet_port_akpg(const la_device_impl_wptr& device) : la_ethernet_port_base(device)
{
}

la_ethernet_port_akpg::~la_ethernet_port_akpg()
{
}

la_status
la_ethernet_port_akpg::set_service_mapping_type(service_mapping_type_e type)
{
...
la_status
la_ethernet_port_akpg::set_source_pif_entry(const la_ac_profile_impl* ac_profile)
{
    ...
    npl_ifg0_ssp_mapping_table_value_t source_pif_value;
    source_pif_value.payloads.init_rx_data.initial_rx_data.init_fields = populate_initial_pd_nw_rx_data(ac_profile);

    source_pif_value.action = NPL_IFG0_SSP_MAPPING_TABLE_ACTION_INIT_RX_DATA;
    ...
    source_pif_value.payloads.init_rx_data.fi_macro_id = NPL_FI_MACRO_ID_ETH | FI_MACRO_RTC_STAGE;
    ...
    // New NP2 Fields
    source_pif_value.payloads.init_rx_data.mtu = m_mtu;
    source_pif_value.payloads.init_rx_data.mtu_violation = 0;
    source_pif_value.payloads.init_rx_data.slice_source_system_port.value = 0;
    ...
    if (m_spa_port != nullptr) {
        auto spa_port_akpg = m_spa_port.weak_ptr_static_cast<la_spa_port_akpg>();
        status = spa_port_akpg->set_source_pif_table(source_pif_value);
        ...
        if (sys_port_type != la_system_port_base::port_type_e::REMOTE) {
            ...
            status = system_port_akpg->set_source_pif_table(source_pif_value);
        }
    }
...
la_status
la_ethernet_port_akpg::erase_source_pif_entry()
{
    la_status status = LA_STATUS_SUCCESS;
    if (m_spa_port != nullptr) {
        ...
        if (sys_port_type != la_system_port_base::port_type_e::REMOTE) {
            auto system_port_akpg = m_system_port.weak_ptr_static_cast<la_system_port_akpg>();
            status = system_port_akpg->erase_source_pif_table_entries();
        }
    }
...
la_ethernet_port_akpg::populate_mac_af_npp_attributes() const
{
...
la_status
la_ethernet_port_akpg::update_npp_sgt_attributes()
{
    la_status status = LA_STATUS_SUCCESS;
    ...
    value.payloads.npp_sgt_attributes.security_group = m_security_group_tag;

    if (m_spa_port != nullptr) {
        // TODO: adhawane, support for SPA port.
    } else if (m_system_port != nullptr) {
        la_system_port_base::port_type_e sys_port_type = m_system_port->get_port_type();
        if (sys_port_type != la_system_port_base::port_type_e::REMOTE) {
            // Create the key
            key.npp_attributes_index = m_system_port->get_npp_attributes_index();

            // Update NPP SGT attributes table
            auto slice_id = m_system_port->get_slice();
...
la_status
la_ethernet_port_akpg::set_security_group_tag(la_sgt_t sgt)
{
...
la_status
la_ethernet_port_akpg::get_security_group_tag(la_sgt_t& out_sgt) const
{
...
la_status
la_ethernet_port_akpg::update_dsp_sgt_attributes()
{
    la_status status = LA_STATUS_SUCCESS;
    ...
    value.payloads.dsp_group_policy.enable = m_security_group_policy_enforcement;

    if (m_system_port != nullptr) {
        la_system_port_base::port_type_e sys_port_type = m_system_port->get_port_type();
        if (sys_port_type != la_system_port_base::port_type_e::REMOTE) {
            // Create the key
            key.dsp_index = m_system_port->get_gid();
...
la_status
la_ethernet_port_akpg::set_security_group_policy_enforcement(bool enforcement)
{
...
la_status
la_ethernet_port_akpg::get_security_group_policy_enforcement(bool& out_enforcement) const
{
    start_api_getter_call();
...
la_status
la_ethernet_port_akpg::configure_security_group_policy_attributes()
{
    transaction txn;
