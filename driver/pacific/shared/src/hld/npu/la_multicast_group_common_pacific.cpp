...
#include "hld_utils.h"
#include "la_multicast_group_common_pacific.h"
...
la_multicast_group_common_pacific::la_multicast_group_common_pacific(const la_device_impl_wptr& device)
    : la_multicast_group_common_base(device)
{
}

la_multicast_group_common_pacific::~la_multicast_group_common_pacific()
{
}

la_status
la_multicast_group_common_pacific::initialize(la_multicast_group_gid_t multicast_gid,
...
la_status
la_multicast_group_common_pacific::destroy()
{
    const auto& tables(m_device->m_tables.mc_slice_bitmap_table);
...
la_status
la_multicast_group_common_pacific::configure_egress_rep_common(const group_member_desc& member,
                                                               const la_system_port_wcptr& dsp_in,
                                                               uint64_t mc_copy_id)
{
    transaction txn;

    if ((dsp_in != nullptr) && (is_dsp_remote(dsp_in))) {
...
la_status
la_multicast_group_common_pacific::do_add_mc_em_db_entry_egress_rep(const la_system_port_wcptr& dsp_in,
                                                                    uint64_t member_index,
                                                                    uint64_t mc_copy_id,
                                                                    npl_mc_em_db_entry_wptr_t& out_entry)
{
    if (is_dsp_remote(dsp_in)) {
        return LA_STATUS_EINVAL;
    }
    auto dsp = la_system_port_base::upcast_from_api(m_device, dsp_in);

    auto dspi = std::static_pointer_cast<const la_system_port_base>(dsp.lock());
...
la_status
la_multicast_group_common_pacific::do_add_mc_em_db_entry_egress_rep(la_slice_ifg slice_ifg,
                                                                    la_uint_t base_pif,
                                                                    uint64_t member_index,
                                                                    uint64_t mc_copy_id,
                                                                    npl_mc_em_db_entry_wptr_t& out_entry)
{
    ...
    la_oq_id_t oq = slice_ifg.ifg * NUM_OQ_PER_IFG + base_pif * NUM_TC_CLASSES;
    la_oq_id_t oqg = oq / 2;
...
la_status
la_multicast_group_common_pacific::add_mc_em_db_entry_egress_rep(const la_system_port_wcptr& dsp_in,
                                                                 ...
                                                                 uint64_t mc_copy_id)
{
    ...
    if (member.counter != nullptr) { // MCG counter special member case
        la_status status = do_add_mc_em_db_entry_egress_rep(member.counter_slice_ifg, HOST_PIF_ID, member_index, mc_copy_id, entry);
        return_on_error(status);
    } else {
...
la_status
la_multicast_group_common_pacific::set_member_dsp(const group_member_desc& member,
                                                  ...
                                                  uint64_t new_mc_copy_id)
{
    ...
    if ((curr_dsp == nullptr) || (new_dsp == nullptr)) {
...
la_status
la_multicast_group_common_pacific::insert_entry_to_mc_em_db_tx_format_0(uint64_t member_index,
                                                                        uint64_t dest_slice,
                                                                        uint64_t tc_map_profile,
                                                                        uint64_t oq_group,
                                                                        uint64_t mc_copy_id,
...
la_status
la_multicast_group_common_pacific::remove_entry_from_mc_em_db_tx_format_0(size_t member_index, npl_mc_em_db_key_t key)
{
    const auto& table(m_device->m_tables.mc_em_db);
    npl_mc_em_db_entry_wptr_t entry;
    bool is_0 = ((member_index % 2) == 0);
...
la_status
la_multicast_group_common_pacific::update_entry_in_mc_em_db_tx_format_0(npl_mc_em_db_entry_wptr_t dst_entry,
...
la_status
la_multicast_group_common_pacific::remove_mc_em_db_entry_egress_rep(const la_system_port_wcptr& dsp_in,
...
la_status
la_multicast_group_common_pacific::teardown_egress_rep_common(const group_member_desc& member, const la_system_port_wcptr& dsp_in)
{
    auto dsp = la_system_port_base::upcast_from_api(m_device, dsp_in);
    transaction txn;
    la_slice_id_t slice = (member.counter != nullptr) ? member.counter_slice_ifg.slice : dsp->get_slice();
...
la_status
la_multicast_group_common_pacific::verify_dsp(const la_ethernet_port_wcptr& eth, const la_system_port_wcptr& dsp) const
{
    if (dsp == nullptr) {
...
la_status
la_multicast_group_common_pacific::update_member_slice_data(const group_member_desc& old_member,
...
bool
la_multicast_group_common_pacific::is_dsp_remote(const la_system_port_wcptr& dsp) const
{
    auto dspi = std::static_pointer_cast<const la_system_port_base>(dsp.lock());
    la_system_port_base::port_type_e dsp_type = dspi->get_port_type();
...
void
la_multicast_group_common_pacific::set_local_mcid(la_multicast_group_gid_t local_mcid)
{
    m_local_mcid = local_mcid;
    m_mc_slice_bitmap_table_key.rxpp_pd_fwd_destination_15_0_ = local_mcid;
}
...
la_status
la_multicast_group_common_pacific::configure_ingress_rep_common(const group_member_desc& member, la_slice_id_t slice)
{
...
la_status
la_multicast_group_common_pacific::add_mc_em_db_entry_ingress_rep(const group_member_desc& member,
                                                                  la_slice_id_t slice,
...
la_status
la_multicast_group_common_pacific::do_add_mc_em_db_entry_ingress_rep(uint64_t member_index,
                                                                     uint64_t slice,
                                                                     uint64_t member_mcid,
                                                                     npl_mc_em_db_entry_wptr_t& out_entry)
{
...
la_status
la_multicast_group_common_pacific::insert_entry_to_mc_em_db_rx_result(uint64_t member_index,
                                                                      uint64_t slice,
                                                                      uint64_t tc_map_profile,
                                                                      uint64_t base_voq_nr,
                                                                      uint64_t member_mcid,
...
void
la_multicast_group_common_pacific::populate_mc_em_db_rx_result_value(bool is_0,
                                                                     uint64_t tc_map_profile,
                                                                     uint64_t base_voq_nr,
                                                                     uint64_t member_mcid,
                                                                     npl_mc_em_db_value_t& out_value)
...
la_status
la_multicast_group_common_pacific::teardown_ingress_rep_common(const group_member_desc& member, la_slice_id_t slice)
{
    transaction txn;
    // always use local_mcid
    // for non scale mode, local mcid is the same as global mcid
...
la_status
la_multicast_group_common_pacific::remove_mc_em_db_entry_ingress_rep(const group_member_desc& member, la_slice_id_t slice)
{
    la_multicast_group_gid_t mcid = get_local_mcid(member);
    if (mcid == ((la_multicast_group_gid_t)-1)) {
        return LA_STATUS_EINVAL;
...
la_status
la_multicast_group_common_pacific::update_entry_in_mc_em_db_rx_result(la_slice_id_t slice,
                                                                      size_t dst_index,
...
la_status
la_multicast_group_common_pacific::remove_entry_from_mc_em_db_rx_result(la_slice_id_t slice,
                                                                        size_t member_index,
                                                                        npl_mc_em_db_entry_wptr_t& entry)
{
    const auto& table(m_device->m_tables.mc_em_db);
...
la_status
la_multicast_group_common_pacific::configure_cud_mapping(const group_member_desc& member,
                                                         ...
                                                         uint64_t mc_copy_id)
{
    ...
    if ((l2_ac != nullptr) && (l2_ac->get_port_type() == la_l2_service_port_base::port_type_e::PWE)) {

        const auto& table(m_device->m_tables.cud_wide_hw_table[dest_slice]);
        npl_cud_wide_hw_table_key_t key;
        npl_cud_wide_hw_table_value_t value;
        npl_cud_wide_hw_table_entry_wptr_t entry;

        value.action = NPL_CUD_WIDE_HW_TABLE_ACTION_WRITE;
        npl_npu_l3_encap_header_t& payload(value.payloads.cud_mapping_local_vars_wide_mc_cud.app_mc_cud.npu_encap_data.l3);
        ...
        key.cud_mapping_local_vars_mc_copy_id_12_1_ = (mc_copy_id >> 1);
        status = table->set(key, value, entry);
        return_on_error(status);
        ...
        const auto& table(m_device->m_tables.cud_narrow_hw_table[dest_slice]);
        npl_cud_narrow_hw_table_key_t key;
        npl_cud_narrow_hw_table_value_t value;
        npl_cud_narrow_hw_table_entry_wptr_t entry;

        uint64_t l2_port_gid = (l2_ac != nullptr) ? l2_ac->get_gid() : 0;

        value.action = NPL_CUD_NARROW_HW_TABLE_ACTION_WRITE;
        npl_npu_ip_collapsed_mc_encap_header_t payload;
        uint64_t l3_port_gid = (member.l3_port != nullptr) ? member.l3_port->get_gid() : 0;
        if ((member.l3_port != nullptr) && member.l3_port->type() == la_object::object_type_e::SVI_PORT) {
            payload.collapsed_mc_encap_type = NPL_NPU_ENCAP_L3_HEADER_TYPE_COLLAPSED_MC;
        } else {
            payload.collapsed_mc_encap_type = NPL_NPU_ENCAP_L3_HEADER_TYPE_ETHERNET_NH;
        }

        payload.l3_dlp = get_l3_dlp_encap(l3_port_gid);
        payload.l3_dlp.properties.monitor_or_l3_dlp_ip_type.l3_dlp_ip_type = NPL_IPV4_L3_DLP;
        ...
        key.cud_mapping_local_vars_mc_copy_id_12_0_ = mc_copy_id;
        value.payloads.cud_mapping_local_vars_narrow_mc_cud = payload.pack().get_value();
...
la_status
la_multicast_group_common_pacific::teardown_cud_mapping(const group_member_desc& member,
                                                        la_slice_id_t dest_slice,
                                                        uint64_t mc_copy_id)
{
    if ((member.l3_port != nullptr) && (member.l3_port->type() == la_object::object_type_e::L3_AC_PORT)
...
la_status
la_multicast_group_common_pacific::reconfigure_mcemdb_entry(group_member_desc member,
