...
#ifndef __IFG_HANDLER_GIBRALTAR_H__
#define __IFG_HANDLER_GIBRALTAR_H__
...
class ifg_handler_gibraltar : public ifg_handler_ifg
{

    CEREAL_SUPPORT_PRIVATE_MEMBERS

public:
    explicit ifg_handler_gibraltar(la_device_impl_wptr device, la_slice_id_t slice_id, la_ifg_id_t ifg_id);
    ~ifg_handler_gibraltar() override;
    ...
    la_status reset_read_schedule_weight(la_uint_t mac_lane_base_id, size_t serdes_count) override;

    la_status set_port_periodic_timer_value(la_uint_t mac_lane_base_id, size_t serdes_count, la_uint_t timer_value) override;
    la_status set_port_periodic_int_enable(la_uint_t mac_lane_base_id, size_t serdes_count, bool enable) override;
    ...
    la_status modify_port_tc_tpid(la_uint_t mac_lane_base_id, size_t serdes_count, la_uint_t idx, la_tpid_t tpid) override;
    la_status remove_port_tc_tpid(la_uint_t mac_lane_base_id, size_t serdes_count, la_uint_t idx) override;
    ...
    la_status remove_port_tc_custom_protocol(la_uint_t mac_lane_base_id, size_t serdes_count, la_uint_t idx) override;
...
private:
    enum class fifo_memory_reset_state_e {
        RESET_ALL = 0,        ///< All ports are in reset state.
        ACTIVATE_INBAND_ONLY, ///< All ports, except Inband port are reset, Inband port active.
    };

    la_status reset_config();
    la_status reset_fifo_memory_lc56(size_t mac_lane_base, size_t serdes_count, la_mac_port_base::mac_reset_state_e reset);
    ...
    la_status configure_rx_cgm(size_t mac_lane_base, size_t serdes_count, la_mac_port::port_speed_e speed) override;
    la_status configure_rx_pma();

    la_status reset_read_schedule_weight();
    la_status configure_read_schedule_weight(la_uint_t mac_lane_base_id, size_t serdes_count, uint64_t read_weight);

    la_status set_reset_fifo_memory(fifo_memory_reset_state_e state);
    ...
    la_status set_fc_mode_periodic(la_uint_t mac_lane_base_id, size_t serdes_count, bool enable) override;
    la_status set_fc_mode_fabric_extraction(la_uint_t mac_lane_base_id, bool enable) override;
    ...
    la_status reset_oob_inj_credits(size_t mac_lane_base, int val) override;

    gibraltar_tree_scptr m_gibraltar_tree;

    size_t m_fabric_port_base;
    lld_register_scptr m_serdes_rx_lane_swap_config;
    lld_register_scptr m_serdes_tx_lane_swap_config;
    lld_register_array_scptr m_serdes_status;
    lld_register_array_scptr m_serdes_pll_status;
    lld_register_scptr m_serdes_an_master_config;
    lld_register_scptr m_serdes_an_bitmap_config;

    // For serialization purposes only
    ifg_handler_gibraltar() = default;
};

} // namespace silicon_one

#endif // __IFG_HANDLER_GIBRALTAR_H__
