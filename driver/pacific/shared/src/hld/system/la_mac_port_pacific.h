...
#ifndef __LA_MAC_PORT_PACIFIC_H__
#define __LA_MAC_PORT_PACIFIC_H__
...
class la_mac_port_pacific : public la_mac_port_pacgb
{
    CEREAL_SUPPORT_PRIVATE_MEMBERS
    la_mac_port_pacific() = default; // Needed for cereal

public:
    explicit la_mac_port_pacific(const la_device_impl_wptr& device);
    ~la_mac_port_pacific() override;

    la_status do_reset() override;
    la_status do_reset_fabric_port_pacific_a0();

    // PFC
    la_status set_pfc_enable(la_uint8_t tc_bitmap) override;
    la_status get_pfc_enabled(bool& out_enabled, la_uint8_t& out_tc_bitmap) const override;
...
private:
    ...
    la_status update_pfc_tc_table(la_pfc_priority_t pfc_priority);
    la_status init_pfc_quanta_table();
...
#endif // __LA_MAC_PORT_PACIFIC_H__
