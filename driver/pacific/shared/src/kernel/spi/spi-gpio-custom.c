...
/* Support old kernels which don't have those defines */
#ifndef SPI_GPIO_NO_MISO
#define SPI_GPIO_NO_MISO ((unsigned long)-1l)
#endif

#ifndef SPI_GPIO_NO_MOSI
...
static int
spi_gpio_custom_get_slave_mode(unsigned int id, unsigned int* params, int slave_index)
{
    int param_index;
...
static int
spi_gpio_custom_get_slave_maxfreq(unsigned int id, unsigned int* params, int slave_index)
{
    int param_index;
...
static int
spi_gpio_custom_get_slave_cs(unsigned int id, unsigned int* params, int slave_index)
{
    int param_index;
...
static int
spi_gpio_custom_check_params(unsigned int id, unsigned int* params)
{
    int i;
...
static int __init
spi_gpio_custom_add_one(unsigned int id, unsigned int* params)
{
    ...
    if (num_cs == 0) {
        /* Even if no CS is used, spi modules expect at least 1 (unused) */
        num_cs = 1;
    }

    pdata.sck = params[BUS_PARAM_SCK];
    pdata.mosi = gpio_is_valid(params[BUS_PARAM_MOSI]) ? params[BUS_PARAM_MOSI] : SPI_GPIO_NO_MOSI;
    pdata.miso = gpio_is_valid(params[BUS_PARAM_MISO]) ? params[BUS_PARAM_MISO] : SPI_GPIO_NO_MISO;
    ...
    for (i = 0; i < BUS_SLAVE_COUNT_MAX; i++) {
        ...
        slave_info.controller_data = (void*)((cs >= 0) ? cs : SPI_GPIO_NO_CHIPSELECT);
