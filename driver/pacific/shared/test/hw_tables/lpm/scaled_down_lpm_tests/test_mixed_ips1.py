...
@unittest.skipIf(ASIC_GB, "Test is skipped in GB sanity")
@unittest.skipIf(lpm_test_utils.is_valgrind(),
                 "Test is skipped in valgrind environment due to long running time")
class mixed_ips1(lpm_scaled_down_test):

    @unittest.skipIf(lpm_test_utils.is_valgrind(),
                     "Test is skipped in valgrind environment due to long running time")
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_mixed_ips1(self):
        self.perform_test_many_lpm_instances_with_entries_from_file(
            "shared/test/hw_tables/lpm/inputs/lpm_data.mixed_ips1.txt.gz", "OLD_FORMAT", sys.maxsize)
