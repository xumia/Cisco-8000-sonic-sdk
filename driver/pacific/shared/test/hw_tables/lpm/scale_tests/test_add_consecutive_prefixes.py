...
@unittest.skipUnless(lpm_test_utils.is_run_slow(), "Slow test not part of sanity")
class test_add_consecutive_prefixes(scale_test_base):

    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_insert_consecutive_1_5m_ipv4(self):
    ...
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_insert_consecutive_450k_ipv6(self):
        prefixes = lpm_test_utils.generate_consecutive_prefixes(
    ...
    def disabled_test_insert_consecutive_450k_ipv6_shuffled(self):
        prefixes = lpm_test_utils.generate_consecutive_prefixes(
            num_entries=450000, step=1, vrf=0, base_prefix=lpm_test_utils.BASE_IPV6_PREFIX)
        lpm_test_utils.randomize_list(prefixes)
        instructions = lpm_test_utils.populate_with_actions(
    ...
    def disabled_test_insert_consecutive_1_5m_ipv4_step_3(self):
        prefixes = lpm_test_utils.generate_many_groups_of_consecutive_prefixes(
            num_entries=1500000, step=3, num_groups=1, group_prefix_length=1, vrf=0, base_prefix=lpm_test_utils.BASE_IPV4_PREFIX)
        lpm_test_utils.randomize_list(prefixes)
        instructions = lpm_test_utils.populate_with_actions(
            self.logical_lpm, prefixes, action=lpm_test_utils.lpm_instruction.INSERT)
        lpm_test_utils.execute_one_by_one(self.logical_lpm, instructions)

    def disabled_test_insert_consecutive_350k_ipv6_step_3(self):
        prefixes = lpm_test_utils.generate_many_groups_of_consecutive_prefixes(
            num_entries=350000, step=3, num_groups=1, group_prefix_length=1, vrf=0, base_prefix=lpm_test_utils.BASE_IPV6_PREFIX)
        lpm_test_utils.randomize_list(prefixes)
        instructions = lpm_test_utils.populate_with_actions(
            self.logical_lpm, prefixes, action=lpm_test_utils.lpm_instruction.INSERT)
        lpm_test_utils.execute_one_by_one(self.logical_lpm, instructions)
