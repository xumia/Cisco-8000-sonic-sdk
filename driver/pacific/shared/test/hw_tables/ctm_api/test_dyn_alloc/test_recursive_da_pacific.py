...
#
# Test description: Testing recursive allocation in pacific standalone
#
...
class test_recursive_reallocation(unified_table_test_case_base):
    ...
    # Testing recursive allocation Tx wide -> Fw0 (allocate for Fw0)
    def test_alloc_narrow_for_wide(self):
        tables = [
        ...
        tables_to_insert = {
            "INGRESS_IPV4_SEC_TABLE": 1500,
            "EGRESS_IPV6_SEC_TABLE": 600,
        }
        ...
        self.assertEqual(tables["EGRESS_IPV6_SEC_TABLE"].do_get_count(), tables_to_insert["EGRESS_IPV6_SEC_TABLE"])

        for table in tables:
            tables[table].detach_default()

    # Testing recursive allocation Tx wide -> Fw wide (reallocate Fw0)
    def test_alloc_narrow_from_wide_for_wide(self):
        tables = [
            "INGRESS_IPV4_SEC_TABLE",
            "EGRESS_IPV6_SEC_TABLE",
        ...
        tables_to_init = {
            "INGRESS_IPV6_SEC_TABLE": 1500,
            "EGRESS_IPV6_SEC_TABLE": 500,
        }
        ...
        self.erase_entries_regular(tables["INGRESS_IPV6_SEC_TABLE"], 1000, 1, 500)

        table_name = "INGRESS_IPV4_SEC_TABLE"
        self.insert_entries_n(table_name, tables[table_name], generators[table_name], 400)
    ...
    # Testing recursive allocation Fx wide -> Fw0 (reallocate Tx0)
    def test_realloc_narrow_for_wide(self):
        tables = [
            "INGRESS_IPV4_SEC_TABLE",
            "INGRESS_IPV6_SEC_TABLE",
        ...
        tables_to_init = {
            "INGRESS_IPV6_SEC_TABLE": 1000,
            "INGRESS_IPV4_SEC_TABLE": 500,
            "EGRESS_IPV4_SEC_TABLE": 1500,
        }
        ...
        table_name = "INGRESS_IPV4_SEC_TABLE"
        self.insert_entries_n(table_name, tables[table_name], generators[table_name], 512)

        self.erase_entries_regular(tables["EGRESS_IPV4_SEC_TABLE"], 0, 1, 1024)

        self.fill_table(tables["INGRESS_IPV6_SEC_TABLE"], generators["INGRESS_IPV6_SEC_TABLE"])

        self.assertTrue(tables["INGRESS_IPV6_SEC_TABLE"].do_get_count() > 1024)

        for table in tables:
            tables[table].detach_default()

    # Testing recursive allocation Fx wide -> Fw0 (reallocate Tx0)
    def test_realloc_2_narrow_for_wide(self):
        tables = [
            "INGRESS_IPV4_SEC_TABLE",
            "EGRESS_IPV6_SEC_TABLE",
        ]
        ...
        self.erase_entries_regular(tables["INGRESS_IPV4_SEC_TABLE"], 0, 1, 1024)
