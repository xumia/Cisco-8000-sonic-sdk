...
class ipv4_l3_ac_test(mpls_to_mpls_headend_base_disable_mpls_sr):
    ...
    INPUT_PACKET_BASE = \
        Ether(dst=T.RX_L3_AC_MAC.addr_str, src=mpls_to_mpls_headend_base_disable_mpls_sr.SA.addr_str, type=U.Ethertype.QinQ.value) / \
        ...
        IP(src=SIP.addr_str, dst=DIP.addr_str, ttl=mpls_to_mpls_headend_base_disable_mpls_sr.IP_TTL)
    ...
    INPUT_PACKET_DOUBLE_LABEL_BASE = \
        ...
        Dot1Q(vlan=T.RX_L3_AC_PORT_VID1, type=U.Ethertype.Dot1Q.value) / \
        Dot1Q(vlan=T.RX_L3_AC_PORT_VID2) / \
        MPLS(label=mpls_to_mpls_headend_base_disable_mpls_sr.INPUT_LABEL0.label, ttl=mpls_to_mpls_headend_base_disable_mpls_sr.MPLS_TTL - 2, s=0) / \
    ...
    EXPECTED_OUTPUT_POP_UNIFORM_PACKET_BASE = \
        Ether(dst=T.NH_L3_AC_REG_MAC.addr_str, src=T.TX_L3_AC_REG_MAC.addr_str, type=U.Ethertype.MPLS.value) / \
        MPLS(label=mpls_to_mpls_headend_base_disable_mpls_sr.INPUT_LABEL1.label, ttl=mpls_to_mpls_headend_base_disable_mpls_sr.MPLS_TTL - 3) / \
        IP(src=SIP.addr_str, dst=DIP.addr_str, ttl=mpls_to_mpls_headend_base_disable_mpls_sr.IP_TTL)

    EXPECTED_OUTPUT_PHP_UNIFORM_PACKET_BASE = \
        Ether(dst=T.NH_L3_AC_REG_MAC.addr_str, src=T.TX_L3_AC_REG_MAC.addr_str) / \
        IP(src=SIP.addr_str, dst=DIP.addr_str, ttl=mpls_to_mpls_headend_base_disable_mpls_sr.MPLS_TTL - 1)
    ...
    def test_sr_global_per_protocol_counters(self):
        self._test_sr_global_per_protocol_counters(sdk.la_l3_protocol_counter_e_MPLS)
