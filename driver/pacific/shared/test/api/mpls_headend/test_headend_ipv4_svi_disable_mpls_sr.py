...
class ipv4_svi(mpls_headend_base_disable_mpls_sr):
    ...
    EXPECTED_OUTPUT_PACKET_UPDATED_MPLS_BASE = \
        Ether(dst=T.NH_SVI_REG_MAC.addr_str, src=T.TX_SVI_MAC.addr_str, type=U.Ethertype.MPLS.value) / \
        MPLS(label=mpls_headend_base_disable_mpls_sr.NEW_LDP_LABEL.label, ttl=mpls_headend_base_disable_mpls_sr.MPLS_TTL) / \
        IP(src=SIP.addr_str, dst=DIP.addr_str, ttl=mpls_headend_base_disable_mpls_sr.IP_TTL - 1)
