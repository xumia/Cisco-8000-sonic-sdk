...
HW_EXPECTED_MEM_CONSUMPTION = 550000
SIM_EXPECTED_MEM_CONSUMPTION = 740000
STANDALONE_EXPECTED_MEM_CONSUMPTION = HW_EXPECTED_MEM_CONSUMPTION if decor.is_hw_device() else SIM_EXPECTED_MEM_CONSUMPTION
...
class test_mem_consumption_standalone(mem_consumption_base):

    @unittest.skipIf(decor.is_asic4(), "Test is not yet enabled on PL")
    @unittest.skipIf(decor.is_asic5(), "Test is not yet enabled on AR")
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_mem_sa(self):
        import sim_utils
        self._test_mem(sim_utils.STANDALONE_DEV, STANDALONE_EXPECTED_MEM_CONSUMPTION)
