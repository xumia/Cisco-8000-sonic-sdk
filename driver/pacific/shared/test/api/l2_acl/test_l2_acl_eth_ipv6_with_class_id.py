...
class test_l2_acl_eth_ipv6_with_class_id(test_l2_acl_ipv_base):
    ...
    def device_config_func(device, state):
        if state == sdk.la_device.init_phase_e_DEVICE:
            ...
            tcam_pool_id = 0
            test_l2_acl_eth_ipv6_with_class_id.acl_profile = device.create_acl_key_profile(
                key_type, direction, udk, tcam_pool_id)

    @classmethod
    def setUpClass(cls):
        super(test_l2_acl_eth_ipv6_with_class_id, cls).setUpClass(
            device_config_func=test_l2_acl_eth_ipv6_with_class_id.device_config_func)
    ...
    def create_class_id_acl(self):
        ...
        f0.val.da.flat = 0xcafecafecafe
        f0.mask.da.flat = 0xffffffffffff
        k0.append(f0)

        k1 = []
        f1 = sdk.la_acl_field()
        f1.type = sdk.la_acl_field_type_e_CLASS_ID
        f1.val.class_id = test_l2_acl_eth_ipv6_with_class_id.CLASS_ID
    ...
    def test_ipv6_acl(self):
        ...
        eth_acls = []
        eth_acls.append(acl1)
        acl_group = []
        acl_group = self.device.create_acl_group()
        acl_group.set_acls(sdk.la_acl_packet_format_e_ETHERNET, eth_acls)
