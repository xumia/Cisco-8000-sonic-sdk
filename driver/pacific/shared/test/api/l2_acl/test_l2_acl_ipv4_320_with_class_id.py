...
class test_l2_acl_ipv4_320_with_class_id(test_l2_acl_ipv_base):
    ...
    def device_config_func(device, state):
        if state == sdk.la_device.init_phase_e_DEVICE:
            ...
            udf2.type = sdk.la_acl_field_type_e_SA
            udk.append(udf2)
            udf3 = sdk.la_acl_field_def()
            udf3.type = sdk.la_acl_field_type_e_IPV4_DIP
            udk.append(udf3)
            udf4 = sdk.la_acl_field_def()
            udf4.type = sdk.la_acl_field_type_e_PROTOCOL
            udk.append(udf4)
            udf5 = sdk.la_acl_field_def()
            udf5.type = sdk.la_acl_field_type_e_SPORT
            udk.append(udf5)
            udf6 = sdk.la_acl_field_def()
            udf6.type = sdk.la_acl_field_type_e_DPORT
            udk.append(udf6)
            udf7 = sdk.la_acl_field_def()
            udf7.type = sdk.la_acl_field_type_e_TOS
            udk.append(udf7)
            udf8 = sdk.la_acl_field_def()
            udf8.type = sdk.la_acl_field_type_e_CLASS_ID
            udk.append(udf8)
            key_type = sdk.la_acl_key_type_e_IPV4
            direction = sdk.la_acl_direction_e_INGRESS
            tcam_pool_id = 0
            test_l2_acl_ipv4_320_with_class_id.acl_profile_ipv4_320_class_id = device.create_acl_key_profile(
                key_type, direction, udk, tcam_pool_id)

    @classmethod
    def setUpClass(cls):
        super(test_l2_acl_ipv4_320_with_class_id, cls).setUpClass(
            device_config_func=test_l2_acl_ipv4_320_with_class_id.device_config_func)
    ...
    def create_mac_class_id_unified_acl(self):
        ''' Create simple unified ACL with a Class ID field type. '''

        acl1 = self.device.create_acl(
            test_l2_acl_ipv4_320_with_class_id.acl_profile_ipv4_320_class_id,
        ...
        k3 = []
        f3 = sdk.la_acl_field()
        f3.type = sdk.la_acl_field_type_e_SA
        f3.val.sa.flat = 0xdeaddead0000
        f3.mask.sa.flat = 0xffffffff0000
        k3.append(f3)

        k2 = []
        f2 = sdk.la_acl_field()
        f2.type = sdk.la_acl_field_type_e_CLASS_ID
        f2.val.class_id = test_l2_acl_ipv4_320_with_class_id.CLASS_ID
    ...
    def test_acl_force_destination(self):
        acl1 = self.create_mac_class_id_unified_acl()
