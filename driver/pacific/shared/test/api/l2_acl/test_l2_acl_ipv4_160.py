...
@unittest.skipUnless(decor.is_gibraltar(), "UDK tests supported only on GB")
class test_l2_acl_ipv4_160(test_l2_acl_ipv_base):

    acl_profile_ipv4_160 = None

    @staticmethod
    def device_config_func(device, state):
        if state == sdk.la_device.init_phase_e_DEVICE:
            device.set_bool_property(sdk.la_device_property_e_ENABLE_CLASS_ID_ACLS, False)
            udk = []
            ...
            tcam_pool_id = 0
            test_l2_acl_ipv4_160.acl_profile_ipv4_160 = device.create_acl_key_profile(
                key_type, direction, udk, tcam_pool_id)

    @classmethod
    def setUpClass(cls):
        super(test_l2_acl_ipv4_160, cls).setUpClass(
            device_config_func=test_l2_acl_ipv4_160.device_config_func)
    ...
    def create_ipv4_unified_acl(self):
        ''' Create simple unified ACL. '''

        acl1 = self.device.create_acl(
            test_l2_acl_ipv4_160.acl_profile_ipv4_160,
    ...
    def test_ipv4_acl(self):
        acl1 = self.create_ipv4_unified_acl()
