...
from ip_routing.ipv4_l3_ac_routing_base import *


@unittest.skipIf(decor.is_hw_asic3(), "Test is not yet enabled on GR-HW")
class test_ipv4_ptp(ipv4_l3_ac_routing_base):
    ...
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_ptp_transient(self):
        self.topology.rx_eth_port.hld_obj.set_transparent_ptp_enabled(False)
        self.INPUT_PACKET_BASE /= S.UDP(sport=2048) / P.PTPv2() / P.PTPSync()
        self.EXPECTED_OUTPUT_PACKET_BASE /= S.UDP(sport=2048) / P.PTPv2() / P.PTPSync()
        self.INPUT_PACKET, self.EXPECTED_OUTPUT_PACKET = U.pad_input_and_output_packets(
            self.INPUT_PACKET_BASE, self.EXPECTED_OUTPUT_PACKET_BASE)
    ...
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_ptp_no_stamp(self):
        self.topology.rx_eth_port.hld_obj.set_transparent_ptp_enabled(True)
        self.INPUT_PACKET_BASE /= S.UDP(sport=2048, chksum=0) / P.PTPv2(correction_field=0x1122334455667788) / P.PTPDelayResp()
        self.EXPECTED_OUTPUT_PACKET_BASE /= S.UDP(sport=2048,
                                                  chksum=0) / P.PTPv2(correction_field=0x1122334455667788) / P.PTPDelayResp()
        self.INPUT_PACKET, self.EXPECTED_OUTPUT_PACKET = U.pad_input_and_output_packets(
            self.INPUT_PACKET_BASE, self.EXPECTED_OUTPUT_PACKET_BASE)
