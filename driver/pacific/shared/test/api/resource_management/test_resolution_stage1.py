...
class resolution_stage1(resource_handler_base):
    ...
    def test_resolution_stage1(self):
        rd_def = sdk.la_resource_descriptor()
        rd_def.m_resource_type = sdk.la_resource_descriptor.type_e_STAGE1_LB_GROUP
        group_resource_init_status = self.device.get_resource_usage(rd_def)
        rd_def.m_resource_type = sdk.la_resource_descriptor.type_e_STAGE1_LB_MEMBER
        member_resource_init_status = self.device.get_resource_usage(rd_def)

        ecmp = self.device.create_ecmp_group(sdk.la_ecmp_group.level_e_LEVEL_1)
