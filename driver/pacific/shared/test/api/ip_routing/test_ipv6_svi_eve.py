...
class test_ipv6_svi_eve(ip_routing_svi_eve_base):
    ip_impl = ip_test_base.ipv6_test_base
    SVI_HOST_IP_ADDR = T.ipv6_addr('1111:0db8:0a0b:12f0:0000:0000:0000:1111')
    SVI_TRUNK_HOST_IP_ADDR = T.ipv6_addr('1111:0db8:0a0b:12f0:0000:0000:0000:1122')
    SVI1_HOST_IP_ADDR = T.ipv6_addr('2222:0db8:0a0b:12f0:0000:0000:0000:2222')
    SVI1_TRUNK_HOST_IP_ADDR = T.ipv6_addr('2222:0db8:0a0b:12f0:0000:0000:0000:2233')
    SVI2_TRUNK_HOST_IP_ADDR = T.ipv6_addr('3333:0db8:0a0b:12f0:0000:0000:0000:3344')
    base = ip_routing_svi_eve_base
    TTL = 64

    rx_svi_access_in_packet = \
        S.Ether(dst=T.RX_SVI_MAC.addr_str, src=base.SVI_HOST_MAC.addr_str) / \
        S.IPv6(dst=SVI1_HOST_IP_ADDR.addr_str, src=SVI_HOST_IP_ADDR.addr_str, hlim=TTL, plen=40) / \
        S.TCP()

    rx_svi_access_in_packet2 = \
        S.Ether(dst=T.RX_SVI_MAC.addr_str, src=base.SVI_HOST_MAC.addr_str) / \
        S.IPv6(dst=SVI1_TRUNK_HOST_IP_ADDR.addr_str, src=SVI_HOST_IP_ADDR.addr_str, hlim=TTL, plen=40) / \
        S.TCP()
    ...
    rx_svi1_trunk_in_packet = \
        ...
        S.Dot1Q(vlan=30) / \
        S.IPv6(dst=SVI_HOST_IP_ADDR.addr_str, src=SVI1_TRUNK_HOST_IP_ADDR.addr_str, hlim=TTL, plen=40) / \
        S.TCP()

    rx_svi1_trunk_in_packet2 = \
        S.Ether(dst=T.RX_SVI_MAC1.addr_str, src=base.SVI1_TRUNK_HOST_MAC.addr_str, type=U.Ethertype.Dot1Q.value) / \
        S.Dot1Q(vlan=base.SVI1_VLAN) / \
        S.IPv6(dst=SVI_TRUNK_HOST_IP_ADDR.addr_str, src=SVI1_TRUNK_HOST_IP_ADDR.addr_str, hlim=TTL, plen=40) / \
        S.TCP()

    rx_svi1_trunk_in_packet3 = \
        S.Ether(dst=T.RX_SVI_MAC1.addr_str, src=base.SVI1_TRUNK_HOST_MAC.addr_str, type=U.Ethertype.Dot1Q.value) / \
        S.Dot1Q(vlan=base.SVI1_VLAN) / \
        S.IPv6(dst=SVI2_TRUNK_HOST_IP_ADDR.addr_str, src=SVI1_TRUNK_HOST_IP_ADDR.addr_str, hlim=TTL, plen=40) / \
        S.TCP()

    rx_svi1_access_out_packet = \
        S.Ether(dst=base.SVI1_HOST_MAC.addr_str, src=T.RX_SVI_MAC1.addr_str) / \
        S.IPv6(dst=SVI1_HOST_IP_ADDR.addr_str, src=SVI_HOST_IP_ADDR.addr_str, hlim=TTL - 1, plen=40) / \
        S.TCP()
    ...
    rx_svi1_trunk_out_packet = \
        ...
        S.Dot1Q(vlan=base.SVI1_VLAN) / \
        S.IPv6(dst=SVI1_TRUNK_HOST_IP_ADDR.addr_str, src=SVI_HOST_IP_ADDR.addr_str, hlim=TTL - 1, plen=40) / \
        S.TCP()
    ...
    rx_svi_access_out_packet = \
        ...
        S.IPv6(dst=SVI_HOST_IP_ADDR.addr_str, src=SVI1_TRUNK_HOST_IP_ADDR.addr_str, hlim=TTL - 1, plen=40) / \
        S.TCP()
    ...
    rx_svi_trunk_out_packet = \
        ...
        S.IPv6(dst=SVI_TRUNK_HOST_IP_ADDR.addr_str, src=SVI1_TRUNK_HOST_IP_ADDR.addr_str, hlim=TTL - 1, plen=40) / \
        S.TCP()
    ...
    rx_svi2_trunk_out_packet = \
        ...
        S.IPv6(dst=SVI2_TRUNK_HOST_IP_ADDR.addr_str, src=SVI1_TRUNK_HOST_IP_ADDR.addr_str, hlim=TTL - 1, plen=40) / \
        S.TCP()
