...
class fallback(vlan_vlan_with_fallback_base):
    ...
    def test_fallback(self):
        INPUT_PACKET_BASE = Ether(dst=self.DST_MAC, src=self.SRC_MAC, type=Ethertype.QinQ.value) / \
            ...
            Dot1Q(prio=2, id=1, vlan=self.DUMMY_VID2) / \
            IP() / TCP()
        ...
        rx_fallback_ac_port = T.l2_ac_port(self, self.device, self.RX_FALLBACK_AC_PORT_GID, self.topology.filter_group_def, None,
                                           self.rx_eth_port, None, self.RX_AC_PORT_VID1)

        rx_fallback_ac_port.hld_obj.set_destination(self.topology.tx_l2_ac_port_reg.hld_obj)
