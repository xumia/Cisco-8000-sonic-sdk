...
class basic_lpts(ipv4_lpts_base):
    ...
    def test_basic_lpts(self):
        ...
        with self.assertRaises(sdk.OutOfRangeException) as cm:
            self.push_lpts_entry(lpts, 0, k1, result)
        e = cm.exception
        print("push_lpts_entry failed because of out-of-range flow_type: " + e.status.message())
    ...
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_ipv4_src_dst_lpts_urpf(self):
        lpts = self.create_lpts_instance()
        self.setup_forus_dest()
