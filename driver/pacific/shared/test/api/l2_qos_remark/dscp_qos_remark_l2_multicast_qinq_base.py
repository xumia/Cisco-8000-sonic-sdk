...
class test_l2_multicast_qos_remark(sdk_test_case_base):
    ...
    def create_packets(self):
        ...
        out_packet_with_dscp_remark_base = Ether(dst=DST_MAC, src=SRC_MAC, type=Ethertype.QinQ.value) / \
            Dot1QPrio(vlan=VLAN, pcpdei=OUT_PCPDEI.flat, type=Ethertype.Dot1Q.value) / \
            Dot1QPrio(vlan=VLAN + 1, pcpdei=IN_PCPDEI.flat) / \
            IPvX(ipvx=self.ipvx, src=self.SIP.addr_str, dst=self.DIP.addr_str,
...
class dscp_qos_remarking_l2_multicast_qinq_test(test_l2_multicast_qos_remark):
