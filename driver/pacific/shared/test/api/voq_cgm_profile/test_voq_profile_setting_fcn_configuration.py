...
class voq_profile_setting_fcn_configuration(voq_cgm_profile_base):

    @unittest.skipIf(decor.is_asic4(), "Test is not yet enabled on PL")
    @unittest.skipIf(decor.is_asic5(), "Test is not yet enabled on AR")
    @unittest.skipIf(decor.is_gibraltar(), "Test is not yet enabled on GB")
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_voq_profile_setting_fcn_configuration(self):
        ...
        for enabled in [True, False]:
            self.voq_cgm_profile.set_fcn_configuration(enabled, probabilities)
            (res_enabled, res_probabilities) = self.voq_cgm_profile.get_fcn_configuration()
            self.assertEqual(res_enabled, enabled)

        # Set/get check probabilities
        self.voq_cgm_profile.set_fcn_configuration(True, probabilities)

        (res_enabled, res_probabilties) = self.voq_cgm_profile.get_fcn_configuration()
