...
from ipv6_ingress_acl_class_id_160_base import *
import sim_utils
import topology as T


@unittest.skipUnless(decor.is_gibraltar(), "Class ID tests supported only on GB")
class unified_acl(ipv6_ingress_acl_class_id_160_base):

    @unittest.skipIf(decor.is_pacific(), "Not valid for Pacific.")
    @unittest.skipUnless(decor.is_gibraltar(), "Class ID tests supported only on GB")
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_unified_acl_with_host_class_id(self):
        ...
        expected_output_packet = self.EXPECTED_OUTPUT_HOST_PACKET.copy()
        expected_output_packet[IPv6].tc = (QOS_MARK_DSCP << 2)
        run_and_compare_inner_fields(self, self.device,
                                     self.INPUT_HOST_PACKET, T.RX_SLICE, T.RX_IFG, T.FIRST_SERDES,
        ...
        assertPacketLengthIngress(self, self.INPUT_HOST_PACKET, T.RX_SLICE, byte_count)
