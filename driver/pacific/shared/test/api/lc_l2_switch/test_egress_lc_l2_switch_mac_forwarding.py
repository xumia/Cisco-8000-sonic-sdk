...
class egress_lc_l2_switch_mac_forwarding(lc_l2_switch_base):

    @unittest.skipIf(decor.is_asic4(), "Test is not yet enabled on PL")
    @unittest.skipIf(decor.is_asic5(), "Test is not yet enabled on AR")
    @unittest.skipIf(decor.is_asic3(), "Test is not yet enabled on GR")
    def test_egress_lc_l2_switch_mac_forwarding(self):
        ...
        out_tx_ac_port = T.l2_ac_port(self, self.device, AC_PORT_GID_BASE +
                                      1, self.topology.filter_group_def, sw, out_tx_eth_port, dest_mac, VLAN, 0x0)
