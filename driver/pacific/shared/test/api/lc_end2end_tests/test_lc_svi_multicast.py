...
SW_GID = 0xa0e
SVI_GID = 0x732
SVI_MAC = T.mac_addr('20:22:23:24:25:27')
TXSW_GID = 0xa0f
TXSVI_GID = 0x733
TXSVI_MAC = T.mac_addr('20:22:23:24:25:28')
...
class test_ip_multicast_lc(sdk_test_case_base):
    ...
    def create_network_ports_and_topology(self):
        ...
        self.switch = T.switch(self, self.device, SW_GID)
        self.svi = T.svi_port(self, self.device, SVI_GID, self.switch, self.vrf, SVI_MAC)
        self.svi.hld_obj.set_protocol_enabled(sdk.la_l3_protocol_e_IPV4_UC, True)
        self.svi.hld_obj.set_protocol_enabled(sdk.la_l3_protocol_e_IPV4_MC, True)

        self.tx_switch = T.switch(self, self.device, TXSW_GID)
        self.tx_svi = T.svi_port(self, self.device, TXSVI_GID, self.tx_switch, self.vrf, TXSVI_MAC)
        ...
        self.in_ac_port = T.l2_ac_port(self, self.device, AC_PORT_GID_BASE, None, self.switch, self.in_eth_port, T.RX_MAC, VLAN1, 0)
        ...
        self.out_eth_port1 = T.ethernet_port(
            ...
            OUT_SERDES_FIRST1,
            OUT_SERDES_LAST1)
        self.out_ac_port1 = T.l2_ac_port(
            self,
            self.device,
            AC_PORT_GID_BASE + 1,
            None,
        ...
        self.out_eth_port2 = T.ethernet_port(
            ...
            OUT_SERDES_FIRST2,
            OUT_SERDES_LAST2)
        self.out_ac_port2 = T.l2_ac_port(
            self,
            self.device,
            AC_PORT_GID_BASE + 2,
            None,
        ...
        self.mc_group.add(self.tx_svi.hld_obj, self.out_ac_port1.hld_obj, self.out_sys_port1)
        self.mc_group.add(self.tx_svi.hld_obj, self.out_ac_port2.hld_obj, self.out_sys_port2)
    ...
    def create_packets(self):
        ...
        out_packet_base = Ether(dst=test_ip_multicast_lc.get_mc_sa_addr_str(MC_GROUP_ADDR), src=TXSVI_MAC.addr_str) / \
    ...
    def test_ip_multicast(self):
        ...
        # test1 - with ingress and svi sub groups
        # test empty member group
        self.sub_group1 = self.device.create_l2_multicast_group(0x14, sdk.la_replication_paradigm_e_EGRESS)
        self.mc_ingress_group.add(self.tx_svi.hld_obj, self.sub_group1)
        ingress_packet = {'data': self.in_packet, 'slice': IN_SLICE, 'ifg': IN_IFG, 'pif': IN_SERDES_FIRST}
        expected_packets = []
        run_and_compare_list(self, self.device, ingress_packet, expected_packets)

        # test port addition/removal notification in member group
        self.sub_group1.add(self.out_ac_port1.hld_obj, self.out_sys_port1)
        self.sub_group1.add(self.out_ac_port2.hld_obj, self.out_sys_port2)
        self.do_test_route()
        self.sub_group1.remove(self.out_ac_port1.hld_obj)
        self.sub_group1.remove(self.out_ac_port2.hld_obj)
        self.mc_ingress_group.remove(self.tx_svi.hld_obj, self.sub_group1)
        ...
        # test2 - with ingress and ports
        self.mc_ingress_group.add(self.tx_svi.hld_obj, self.out_ac_port1.hld_obj, self.out_sys_port1)
        self.mc_ingress_group.add(self.tx_svi.hld_obj, self.out_ac_port2.hld_obj, self.out_sys_port2)
        self.do_test_route()
        self.mc_ingress_group.set_replication_paradigm(sdk.la_replication_paradigm_e_EGRESS)
        self.do_test_route()
        self.mc_ingress_group.remove(self.tx_svi.hld_obj, self.out_ac_port1.hld_obj)
        self.mc_ingress_group.remove(self.tx_svi.hld_obj, self.out_ac_port2.hld_obj)

        self.vrf.hld_obj.delete_ipv4_multicast_route(sdk.LA_IPV4_ANY_IP, MC_GROUP_ADDR.hld_obj)
        self.device.destroy(self.mc_ingress_group)

        # test3 - Change replication paradigm
        self.vrf.hld_obj.add_ipv4_multicast_route(sdk.LA_IPV4_ANY_IP, MC_GROUP_ADDR.hld_obj,
                                                  self.mc_group, None, False, False, None)
        self.do_test_route()
        self.mc_group.set_replication_paradigm(sdk.la_replication_paradigm_e_INGRESS)
        self.mc_group.add(self.tx_svi.hld_obj, self.sub_group1)
        self.do_test_route()
        self.mc_group.remove(self.tx_svi.hld_obj, self.sub_group1)
