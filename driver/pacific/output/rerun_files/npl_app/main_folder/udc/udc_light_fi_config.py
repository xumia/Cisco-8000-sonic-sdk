...
def config_light_fi_stages_cfg_table():
    ...
    for line in udc_data_light_fi_stages_cfg_table:
        ...
        # init value
        val = line["value"]
        conf_data = light_fi_stage_cfg_t(update_current_header_info=val[0],
                                         size_width=val[1],
                                         size_offset=val[2],
                                         next_protocol_or_type_width=val[3],
                                         next_protocol_or_type_offset=val[4])
...
def config_light_fi_fabric_table():
    ...
    for line in udc_data_light_fi_fabric_table:
        ...
        # init value
        val = line["value"]
        next_header_format = header_format_t(flags=val[6][0], type=val[6][1])
        header_format = header_format_t(flags=val[7][0], type=val[7][1])
        value = light_fi_fabric_table_value_t(use_additional_size=val[0],
                                              base_size=val[1],
                                              is_protocol_layer=val[2],
                                              next_fi_macro_id=val[3],
                                              npe_macro_id=val[4],
                                              npe_macro_id_valid=val[5],
    ...
    udc_data_light_fi_npu_base_table = [
        # UDC doesn't care about the forwarding type. Size of selector is 0. Only one entry is needed.
        {
            ...
            "value": [
                ...
                0,  # GB 0->1                               # npe_macro_id                        : 8;
                0,  # GB 0->1                               # npe_macro_id_valid                  : 1; # macro-id is chosen in npe-macro_id-resolution
                [0, PROTOCOL_TYPE_UNKNOWN],                    # next_header_format                  : header_format_t; (8)
...
def config_light_fi_nw_0_table():
    ...
    for line in udc_data_light_fi_nw_0_table:
        ...
        value = light_fi_nw_0_table_value_t(next_stage_size_width=val[0],
                                            ...
                                            base_size=val[4],
                                            is_protocol_layer=val[5],
                                            next_fi_macro_id=val[6],
                                            next_header_format=next_header_format,
                                            header_format=val[8])
        # insert to table
