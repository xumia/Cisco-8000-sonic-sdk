...
def config_tables():
    ...
    config_service_relay_id_static_table()
    config_link_relay_id_static_table()
...
def config_service_relay_id_static_table():
    table = service_relay_id_static_table
    table_config = DirectTableConfig("service_relay_id_static_table")

    table_data = [{"key": ["relay_id"] , "value": ["relay_id_or_l3_lp_add_attr"]}]
    table_size = 1024*12 # 12k
    for id in range(0, table_size):
        table_data.append({"key": [id], "value": [id]})
    table_config.create_table(table_data, NETWORK_CONTEXT)


def config_link_relay_id_static_table():
    table = link_relay_id_static_table
    table_config = DirectTableConfig("link_relay_id_static_table")

    table_data = [{"key": ["service_relay_attributes_table_key_11_0_"] , "value": ["relay_id_or_l3_lp_add_attr"]}]
    table_size = 1024*4 # 4k
    for id in range(0, table_size):
        table_data.append({"key": [id], "value": [id]})
    table_config.create_table(table_data, NETWORK_CONTEXT)
...
def config_select_mac_forwarding_static_table():
    table_config = DirectTableConfig("select_mac_forwarding_static_table")
    table_size = table_config.get_table_size()

    table_data = [{"key": ["next_proto_type", "use_metedata_table_per_packet_format" ] , "value": ["use_metadata_table"]}]
    for line in range(0, table_size):
        use_metedata_table_per_packet_format = line & 0b11
        use_metadata_table_for_non_ip_packet = line & 0b1
        use_metadata_table_for_ip_packet = (line >> 1) & 0b1 # it should be aligned with use_metedata_table_per_packet_format_t header_type
        next_proto_type = (line >> 2) & 0b1111
        use_metadata_table = 0b0
        if ((next_proto_type == PROTOCOL_TYPE_IPV4_SUFFIX) or (next_proto_type == PROTOCOL_TYPE_IPV6_SUFFIX)):
            if use_metadata_table_for_ip_packet == 0b1:
                use_metadata_table = 0b1
        elif (use_metadata_table_for_non_ip_packet == 0b1):
            use_metadata_table = 0b1

        table_data.append({"key": [next_proto_type, use_metedata_table_per_packet_format], "value": [use_metadata_table]})
    table_config.create_table(table_data, NETWORK_CONTEXT, init_table=True)
...
def config_sgt_vxlan_termination_table():
    ...
    table_data = [
        ...
        {"key": [PROTOCOL_TYPE_VXLAN, 1],      "value": [1]},
...
# #
# # for GB, align the counter_0_offset to 4 bits
# #
...
def config_probe_marker_1_static_table():
    table_config = TcamTableConfig("probe_marker_1_static_table")
    table_data = [
        {"key": ["probe_marker_1"], "value": ["probe_marker_1_match"]},
        {"key": [0x01020304],     "value": [1]}
    ]
    table_config.create_table(table_data, NETWORK_CONTEXT, init_table=True)


def config_probe_marker_2_static_table():
    table_config = TcamTableConfig("probe_marker_2_static_table")
    table_data = [
        {"key": ["type", "probe_marker_2"], "value": ["probe_marker_2_match"]},
        {"key": [PROTOCOL_TYPE_IPV4_L4, 0x05060708],      "value": [1]}
    ]
    table_config.create_table(table_data, NETWORK_CONTEXT, init_table=True)
