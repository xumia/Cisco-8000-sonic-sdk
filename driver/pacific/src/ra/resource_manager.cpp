...
size_t NUM_SLICE_PAIRS_PER_DEVICE = 3;
//******************************
// AUX functions
//******************************
resolution_lp_config_sptr
create_native_lp_config(const ll_device_sptr ldevice)
{
    std::vector<lld_memory_scptr> config_mems;
    pacific_tree_scptr device_tree = ldevice->get_pacific_tree_scptr();
    for (size_t slice_pair = 0; slice_pair < NUM_SLICE_PAIRS_PER_DEVICE; ++slice_pair) {
        config_mems.push_back(device_tree->slice_pair[slice_pair]->idb->res->dest_decoding_for_l2_and_l3_table_index);
    }

    resolution_lp_config::entry_widths widths = {.sram_width = 98, .protected_width = 97, .wide_width = 48, .narrow_width = 24};

    resolution_lp_config_sptr config
        = std::make_shared<resolution_lp_config>(config_mems, widths, 6 /*base_width*/, 6 /*mask_width*/);

    // Configure this table for 128k entries with a key size of 17b.
    config->add_table(NPL_TABLES_NATIVE_CE_PTR_TABLE,
                      NPL_DESTINATION_CE_PTR_PREFIX,
                      NPL_DESTINATION_CE_PTR_PREFIX_LEN,
                      0 /*base*/,
                      0x1f /*mask = 11111 for key 17 bits*/,
                      2 /*shift*/);

    config->add_table(NPL_TABLES_NATIVE_L2_LP_TABLE,
                      NPL_DESTINATION_L2_DLP_PREFIX,
                      NPL_DESTINATION_L2_DLP_PREFIX_LEN,
                      16 /*base*/,
                      0x3f /*mask = 111111 for key 18 bits*/,
                      2 /*shift*/);
...
resolution_lp_config_sptr
create_path_lp_config(const ll_device_sptr ldevice)
{
    std::vector<lld_memory_scptr> config_mems;
    pacific_tree_scptr device_tree = ldevice->get_pacific_tree_scptr();
    for (size_t slice_pair = 0; slice_pair < NUM_SLICE_PAIRS_PER_DEVICE; ++slice_pair) {
        config_mems.push_back(device_tree->slice_pair[slice_pair]->idb->res->dest_decoding_for_tunnel_dlp_table_index);
    }

    resolution_lp_config::entry_widths widths = {.sram_width = 84, .protected_width = 83, .wide_width = 40, .narrow_width = 20};

    resolution_lp_config_sptr config
        = std::make_shared<resolution_lp_config>(config_mems, widths, 3 /*base_width*/, 3 /*mask_width*/);

    config->add_table(NPL_TABLES_PATH_LP_TABLE,
                      NPL_DESTINATION_STAGE2_P_NH_PREFIX,
                      NPL_DESTINATION_STAGE2_P_NH_PREFIX_LEN,
                      0 /*base*/,
                      0x7 /*mask = 000111 for key 15 bits*/,
                      0 /*shift*/);
...

{
    ...
    // native
    m_resolution_lp_config[RESOLUTION_LP_DB_NATIVE] = create_native_lp_config(ldevice);

    m_resolution_lp_config[RESOLUTION_LP_DB_PATH] = create_path_lp_config(ldevice);

    m_service_lp_attribute_config = std::make_shared<const service_lp_attribute_config>();

    m_service_mapping_config = std::make_shared<const service_mapping_config>();

    m_large_enc_db_config = std::make_shared<const large_enc_db_config>();
...
resource_manager::init_ctm(bool is_linecard_mode, size_t number_of_slices)
{
    m_ctm_mgr = std::make_shared<ctm_mgr_tcam>(m_ll_device,
                                               ...
                                               engine_block_mapper(m_ll_device->get_pacific_tree_scptr()),
                                               number_of_slices);
...
resource_manager::pre_table_init()
{
    ...
    log_debug(RA, "resource_manager::pre_table_init Update service_lp_attribute configuration");
    status = m_service_lp_attribute_config->configure_hw(m_ll_device);
    return_on_error(status);

    log_debug(RA, "resource_manager::pre_table_init Update large_enc_em_db configuration");
    status = m_large_enc_db_config->configure_hw(m_ll_device);
    return_on_error(status);

    log_debug(RA, "resource_manager::pre_table_init Update service_mapping configuration");
    status = m_service_mapping_config->configure_hw(m_ll_device);
    return_on_error(status);

    log_debug(RA, "resource_manager::pre_table_init Update native_lp configuration");
    status = m_resolution_lp_config[RESOLUTION_LP_DB_NATIVE]->configure_hw(m_ll_device);
    return_on_error(status);

    log_debug(RA, "resource_manager::pre_table_init Update path_lp configuration");
    status = m_resolution_lp_config[RESOLUTION_LP_DB_PATH]->configure_hw(m_ll_device);
    return_on_error(status);
