...
{
    auto tree = ldevice->get_pacific_tree_scptr();
    if (table_id == NPL_TABLES_MII_LOOPBACK_TABLE) {
        m_registers = {{tree->slice[slice]->ifg[0]->mac_pool8[0]->mac_lanes_loopback_register,
                        ...
                        tree->slice[slice]->ifg[0]->mac_pool2->mac_lanes_loopback_register,
                        ...
                        tree->slice[slice]->ifg[1]->mac_pool2->mac_lanes_loopback_register}};
...
loopback_table_sram::write(size_t line, const bit_vector& value)
{
    ...
    const lld_register_array_container& reg_array = *m_registers[ifg * NUM_REGS_PER_IFG + pool_index];
    const lld_register& reg(*reg_array[index_in_pool]);
