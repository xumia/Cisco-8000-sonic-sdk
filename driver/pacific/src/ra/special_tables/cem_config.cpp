...
cem_config::configure_hw(const ll_device_sptr& ldevice) const
{
    ...
    // TODO: Testing implementation: 256 groups - 16 cores. Each 16 consecutive groups are mapped to the same core.
    ...
    for (size_t group_idx = 0; group_idx < tree->cdb->top->cem_group_map_table->size(); ++group_idx) {
        cdb_top_cem_group_map_table_register core_map = {.u8 = {0}};
    ...
    cdb_top_learn_manager_ldb_cfg_register learn_mgr_cfg_val = {.u8 = {0}};
    ...
    //
    cdb_top_learn_manager_cfg_max_learn_type_register learn_mgr_type_val = {.u8 = {0}};
    ...
    for (size_t age_value_idx = 0; age_value_idx < tree->cdb->top->age_refresh_value_reg->size(); ++age_value_idx) {
        cdb_top_age_refresh_value_reg_register refresh_value = {.u8 = {0}};
    ...
    for (auto& core : tree->cdb->core_reduced) {
        log_debug(RA, "cem_config::configure_hw configure reduced core");
        status = write_cdb_core_config(ldevice, core, key_width_reg);
        return_on_error(status);
    }
...
cem_config::write_cdb_core_config(const ll_device_sptr& ldevice, const CDB_CORE& core, const bit_vector& key_width_reg) const
{
    ...
    cdb_core_em_configurations_register em_config_reg;
    em_config_reg.fields.auto_bubble_req_en = 1;
