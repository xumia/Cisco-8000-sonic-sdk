...
microcode_parser::get_database_enum(const std::string& database_name, database_e& ret)
{
    static const std::map<std::string, database_e> database_enum_map{
        {"external_resolution_native_lp", DATABASE_NATIVE_LP_SRAM},
        {"external_resolution_path_lp", DATABASE_PATH_LP_SRAM},
        ...
        {"external_resolution_native_lb", DATABASE_RESOLUTION_NATIVE_LB_EM},
        {"external_resolution_path_lb", DATABASE_RESOLUTION_PATH_LB_EM},
        {"external_resolution_port_dspa", DATABASE_RESOLUTION_PORT_DSPA_EM},
        {"external_resolution_port_npp_lb", DATABASE_RESOLUTION_PORT_NPP_LB_EM},
        // the name of the database has changed from port_npp_lb to stage3_lb, while both are mapped to the same resource.
        {"external_resolution_stage3_lb", DATABASE_RESOLUTION_PORT_NPP_LB_EM},
...
microcode_parser::get_database_block_enum(const std::string& block_name, database_block_e& ret)
{
    static const std::map<std::string, database_block_e> db_block_enum_map{
        ...
        {"cdb_core_reduced", DATABASE_BLOCK_EXTERNAL_CDB_CORE_REDUCED},
        {"idb_top", DATABASE_BLOCK_EXTERNAL_IDB_TOP},
        {"idb_res", DATABASE_BLOCK_EXTERNAL_IDB_RES},
        {"sdb_mac", DATABASE_BLOCK_EXTERNAL_SDB_MAC},
        {"sdb_enc", DATABASE_BLOCK_EXTERNAL_SDB_ENC},
        ...
        {"ene_cluster", DATABASE_BLOCK_EXTERNAL_CLUSTER},
        {"fi_stage", DATABASE_BLOCK_EXTERNAL_FI_STAGE},
        ...
        {"hmc_cgm", DATABASE_BLOCK_EXTERNAL_HMC_CGM},
...
microcode_parser::read_tables(json_t* root, size_t& table_count)
{
    ...
    {
        ret &= read_table(table_name, data);
        table_count++;
    }
...
microcode_parser::read_ucode_for_context(const char* context_name, json_t* ucode_context_root)
{
    ...
    if (m_device_revision == la_device_revision_e::PACIFIC_A0) {
        default_root = json_object_get(ucode_context_root, "pacific_revision_a");
        ...
        log_err(RA, "Unsupported device revision %d", (int)m_device_revision);
        return false;
    }
