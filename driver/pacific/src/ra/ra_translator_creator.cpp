...
#include "special_tables/large_enc_db_config.h"
...
#include "special_tables/service_mapping_config.h"
...
{
    ra_translator_creator_sptr creator
        = std::make_shared<silicon_one::ra_translator_creator>(resource_mgr, lld, npl_context_slices, trans_info);
...
ra_translator_creator::initialize()
{
    ...
    m_block_mapper = std::make_shared<engine_block_mapper>(ldevice->get_pacific_tree_scptr());
...
{
    for (size_t i = 0; i <= pacific_tree::LLD_MEMORY_LAST; ++i) {
        const lld_memory_desc_t& mem_desc = pacific_tree::get_memory_desc(i);
        memory_desc_map_entry_t entry(mem_desc.name, (pacific_tree::lld_memory_e)i);

        m_memory_desc_map.insert(entry);
    }

    for (size_t i = 0; i <= pacific_tree::LLD_REGISTER_LAST; ++i) {
        const lld_register_desc_t& reg_desc = pacific_tree::get_register_desc(i);
        register_desc_map_entry_t entry(reg_desc.name, (pacific_tree::lld_register_e)i);
...
ra_translator_creator::get_memory_enum(const lld_block_scptr& block, const std::string& res_name)
{
    ...
    pacific_tree::lld_memory_e ret = pacific_tree::LLD_MEMORY_LAST;
...
ra_translator_creator::get_register_enum(const lld_block_scptr& block, const std::string& res_name)
{
    ...
    pacific_tree::lld_register_e ret = pacific_tree::LLD_REGISTER_LAST;
...
ra_translator_creator::get_memories(const lld_block_vec_t& blocks, const std::string& mem_name, size_t arr_idx)
{
    ...
    const ll_device_sptr& ldevice = get_ll_device();
    const pacific_tree* ptree = ldevice->get_pacific_tree();

    for (auto& block : blocks) {
        pacific_tree::lld_memory_e mem_id = get_memory_enum(block, mem_name);
        if (arr_idx != microcode_parser::ALL_ARRAY_IDXS) {
            // push single index
            const auto& mem = ptree->get_memory(block->get_block_id(), mem_id, arr_idx);
            dassert_crit(mem);
            ret.push_back(mem);
            continue;
        }
        ...
        for (size_t curr_arr_idx = 0; curr_arr_idx < array_size; ++curr_arr_idx) {
            const auto& mem = ptree->get_memory(block->get_block_id(), mem_id, curr_arr_idx);
            dassert_crit(mem);
            ret.push_back(mem);
        }
    }
...
ra_translator_creator::get_registers(const lld_block_vec_t& blocks, const std::string& reg_name, size_t arr_idx)
{
    ...
    for (const lld_block_scptr& block : blocks) {
        pacific_tree::lld_register_e reg_id = get_register_enum(block, reg_name);
        if (arr_idx != microcode_parser::ALL_ARRAY_IDXS) {
            // push single index
            auto reg = ptree->get_register(block->get_block_id(), reg_id, arr_idx);
            dassert_crit(reg);
            ret.push_back(reg);
            continue;
        }
        ...
        for (size_t curr_arr_idx = 0; curr_arr_idx < array_size; ++curr_arr_idx) {
            auto reg = ptree->get_register(block->get_block_id(), reg_id, curr_arr_idx);
            dassert_crit(reg);
            ret.push_back(reg);
        }
    }
...
{
    const ll_device_sptr& ldevice = get_ll_device();
    const pacific_tree* pt = ldevice->get_pacific_tree();
    const size_t NUM_FI_ENG_PER_SLICE = array_size(pt->slice[0]->npu->rxpp_term->fi_eng);

    switch (db) {
    case DATABASE_LIGHT_FI_NPU_BASE_SRAM:
        ret.push_back(pt->slice[slice_id]->npu->txpp->txpp->light_fi_npu_base_lookup);
        return true;

    case DATABASE_LIGHT_FI_NPU_ENCAP_SRAM: {
        const size_t NUM_MEMORIES_PER_BLOCK = pt->slice[0]->npu->txpp->txpp->light_fi_npu_encap_lookup->size();
        for (size_t mem_idx = 0; mem_idx < NUM_MEMORIES_PER_BLOCK; mem_idx++) {
            ret.push_back((*pt->slice[slice_id]->npu->txpp->txpp->light_fi_npu_encap_lookup)[mem_idx]);
        }
        return true;
    }
...
ra_translator_creator::get_fi_registers(database_e db, la_slice_id_t slice_id, lld_register_vec_t& ret)
{
    ...
    case DATABASE_LIGHT_FI_FABRIC_SRAM:
        ret.push_back(pt->slice[slice_id]->npu->txpp->txpp->light_fi_fabric_lookup);
        return true;

    case DATABASE_LIGHT_FI_TM_SRAM:
        ret.push_back(pt->slice[slice_id]->npu->txpp->txpp->light_fi_tm_lookup);
        return true;
                                                          ...
                                                          const std::vector<size_t>& indices)
{
    ...
    if (desc.database_id == DATABASE_EGRESS_LARGE_EM || desc.database_id == DATABASE_MAC_SERVICE_MAPPING_0_EM
        || desc.database_id == DATABASE_MAC_SERVICE_MAPPING_1_EM
        || desc.database_id == DATABASE_MAC_SERVICE_LP_SRAM) {

        // Special case: database is shared between all slice pairs/slices, but the block is one per device
    ...
    if (num_blocks != engine_block_mapper::ASIC_MAX_SLICES_PER_DEVICE_NUM && table_allocation == ALLOCATION_SLICE) {
        ...
        // This is HW speciality. For IDB per-slice tables, for IDB[0] only (slices 0, 1), the slices are inverted in HW.
        static const size_t idb_slice_indices[engine_block_mapper::ASIC_MAX_SLICES_PER_DEVICE_NUM] = {1, 0, 2, 3, 4, 5};

        size_t tables_per_block = engine_block_mapper::ASIC_MAX_SLICES_PER_DEVICE_NUM / num_blocks;
        for (size_t idx : indices) {
            if (desc.block_id == DATABASE_BLOCK_EXTERNAL_IDB_TOP || desc.block_id == DATABASE_BLOCK_EXTERNAL_IDB_RES) {
...
ra_translator_creator::create_mc_emdb_physical_ems(const microcode_parser::npl_table_translator_desc& desc)
{
    ...
    // 1. EM configuration register (shared_db_per_em_reg)
    for (size_t tbl_idx = 0; tbl_idx < array_size(ptree->rx_pdr_mc_db); tbl_idx++) {
        physical_em em;
        em.config_regs = {(*ptree->rx_pdr_mc_db[tbl_idx]->shared_db_per_em_reg)[0]};

        // 2. Per-bank configuration register base address + index (shared_db_per_bank_reg)
        // 3. Four banks (shared_db_verifier)
        for (size_t bank_idx = 0; bank_idx < ptree->rx_pdr_mc_db[tbl_idx]->shared_db_verifier->size(); bank_idx++) {
            physical_em::bank bnk;
            bnk.config_reg = (*ptree->rx_pdr_mc_db[tbl_idx]->shared_db_per_bank_reg)[bank_idx];
            bnk.memory = (*ptree->rx_pdr_mc_db[tbl_idx]->shared_db_verifier)[bank_idx];
            bnk.is_active = true;
            em.banks.push_back(bnk);
        }

        // 4. CAM (shared_db_cam)
        em.cams = {ptree->rx_pdr_mc_db[tbl_idx]->shared_db_cam};
...
// Service Mapping DB contains 8 EM cores. For each slice, only several cores out of 8 are used.
// Moreover, even if core is used, some of its banks are inactive.
// The function does the following:
// 1. Filters our only relevant cores (using config object).
// 2. Active banks are updated per port_idx.
//      - It's important to keep the core, even if all its banks are inactive, because its CAM still can be used.
void
ra_translator_creator::update_cores_for_service_mapping_em(size_t slice_idx, size_t port_idx, std::vector<physical_em>& em_cores)
{
    ...
    const auto& config = m_resource_manager->get_service_mapping_config();
    std::vector<size_t> em_core_idxs = config->get_em_cores(slice_idx);

    for (size_t em_core_idx : em_core_idxs) {

        bit_vector active_banks = config->get_active_banks(em_core_idx, port_idx);
        dassert_crit(active_banks.get_width() == em_cores[em_core_idx].banks.size());
...
// Large Enc DB contains 4 EM cores. For each slice pair, only several cores out of 2 are used.
// Moreover, even if core is used, some of its banks are inactive.
// The function does the following:
// 1. Filters our only relevant cores (using config object).
void
ra_translator_creator::update_cores_for_large_enc_db_em(size_t slice_pair_idx, std::vector<physical_em>& em_cores)
{
    std::vector<physical_em> ret_em_cores;

    const auto& config = m_resource_manager->get_large_enc_db_config();
    std::vector<size_t> em_core_idxs = config->get_em_cores(slice_pair_idx);
...
ra_translator_creator::update_cores_for_special_em_tables(database_e database_id, size_t index, std::vector<physical_em>& em_cores)
{
    ...
    case DATABASE_EGRESS_LARGE_EM:
        update_cores_for_large_enc_db_em(index, em_cores);
        break;
    default:
        // nothing to update
        break;
...
ra_translator_creator::create_memory_sram(const microcode_parser::npl_table_translator_desc& desc,
                                          ...
                                          const std::vector<size_t>& indices)
{
    // 1. manually create memory sram for special tables
    ...
    const ll_device_sptr& ldevice = get_ll_device();
    std::unique_ptr<logical_sram> lsram = silicon_one::make_unique<memory_sram>(ldevice, desc.payload_width, sections);
                                          ...
                                          const std::vector<size_t>& indices)
{
    ...
    const ll_device_sptr& ldevice = get_ll_device();
    logical_tcam_sptr ltcam(new memory_tcam(ldevice, desc.key_width, desc.payload_width, sections));
...
std::unique_ptr<logical_sram>
ra_translator_creator::create_service_lp_attribute_sram(const microcode_parser::npl_table_translator_desc& desc,
                                                        const std::vector<size_t>& indices)
{
    // Require each table to be defined for a single slice pair
    dassert_crit(indices.size() == 1);
    size_t slice_pair_idx = indices.back();
    ...
    dassert_crit(resource_vec.size() == service_lp_attribute_config::NUM_CORES);

    const auto& config = m_resource_manager->get_service_lp_attribute_config();
    std::vector<sram_section> sections;

    for (size_t section_idx = 0; section_idx < service_lp_attribute_config::NUM_CORES; ++section_idx) {
        size_t sram_core_idx = config->get_sram_core_idx(slice_pair_idx, section_idx);
        if (sram_core_idx == service_lp_attribute_config::IDX_NOVAL) {
            break;
        }

        const microcode_parser::table_resource_desc& res = resource_vec[sram_core_idx];
        if (res.type_id != RESOURCE_TYPE_SRAM) {
            log_err(RA, "Only SRAM resources are supported for table %s", desc.npl_table_name.c_str());
            return nullptr;
        }
        ...
        section.size = service_lp_attribute_config::SECTION_SIZE;
        section.entries_per_line = 1;
        section.is_valid = true;
        physical_sram sram = create_physical_sram(res, blocks);
        sram.start_line = config->get_section_start_line(sram_core_idx, section_idx);
        section.srams.push_back(sram);

        sections.push_back(section);
    }

    dassert_crit(sections.size() != 0);
    bool resources_ok = check_sram_sections(desc, sections);
    if (!resources_ok) {
        return nullptr;
    }
...
std::unique_ptr<logical_sram>
ra_translator_creator::create_resolution_lp_sram(const microcode_parser::npl_table_translator_desc& desc,
                                                 const std::vector<size_t>& indices,
                                                 ...
                                                 size_t table_id)
{
    ...
    bool success = get_blocks_and_resources(desc, indices, 0 /*no multi-instance*/, blocks, resource_vec);
    // check there is one resource
    if (!success || resource_vec.size() != 1) {
        return nullptr;
    }
    ...
    const auto& config = m_resource_manager->get_resolution_lp_config(db);
    dassert_crit(config);
    std::unique_ptr<logical_sram> lsram
        = silicon_one::make_unique<resolution_lp_sram>(ldevice, sram.memories, config.get(), table_id);
...
{
    const ll_device_sptr& ldevice = get_ll_device();
    pacific_tree_scptr pt = ldevice->get_pacific_tree_scptr();
    const size_t NUM_REGS_PER_BLOCK = pt->slice[0]->npu->txpp->txpp->light_fi_stages_cfg->size();
    ...
    for (size_t reg_idx = 0; reg_idx < NUM_REGS_PER_BLOCK; reg_idx++) {
        ...
        for (size_t slice_id : indices) {
            mem_line.push_back((*pt->slice[slice_id]->npu->txpp->txpp->light_fi_stages_cfg)[reg_idx]);
        }
        regs.memories.push_back(mem_line);
    }
...
ra_translator_creator::create_fi_core_tcam(const std::vector<size_t>& indices)
{
    ...
    logical_tcam_sptr ltcam(new memory_tcam(ldevice, tcam_width, sram_width, sections));
...
ra_translator_creator::create_light_fi_nw_tcam(size_t nw_id, const std::vector<size_t>& indices)
{
    ...
    lld_memory_vec_t sram_memories;
    for (size_t slice_id : indices) {
        sram_memories.push_back((*pt->slice[slice_id]->npu->txpp->txpp->light_fi_nw_lookup_table_tcam_mem)[nw_id]);
    }
    ...
    lld_memory_vec_t tcam_memories;
    for (size_t slice_id : indices) {
        tcam_memories.push_back((*pt->slice[slice_id]->npu->txpp->txpp->light_fi_nw_lookup_table_tcam)[nw_id]);
    }
