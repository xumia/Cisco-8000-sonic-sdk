...
public:
    ...
    using lld_block_vec_t = lld_block::lld_block_vec_t;

    /// @brief C'tor
    ///
    /// @param[in]  ptree                   Pointer to a device tree for the low level device.
    engine_block_mapper(pacific_tree_scptr tree);
    ...
    /// @param[in]  subring_idx             Subring index, in pacific subring is always 0.
    ...
    engine_block_mapper() = default; // For serialization purposes only
private:
    // Device tree pointer
