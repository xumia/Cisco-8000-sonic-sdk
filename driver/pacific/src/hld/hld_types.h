...
enum {
    ...
    RECYCLE_PIF_ID = 19,
    NUM_OQ_PER_PIF = 8,
    NUM_OQ_PER_IFG = NUM_OQ_PER_PIF * NUM_PIF_PER_IFG + NUM_OQ_PER_PIF /* Rcy */ + NUM_OQ_PER_PIF /* host */,
    ...
    NUM_HBM_DRAM_BUFFER_CELLS = 64, ///< Number of cells in HBM DRAM buffer
    ...
    NUM_FABRIC_PORTS_IN_DEVICE = NUM_IFGS_PER_DEVICE * NUM_FABRIC_PORTS_IN_NORMAL_IFG,
    ...
    CALCULATED_VALUES_DEVICE_FREQUENCY = 1200000,
    MAX_DEVICE_FREQUENCY = 1400000,
    MIN_DEVICE_FREQUENCY = 1000000,
    ...
    SYNCE_DIVIDER_DEFAULT = 5,

    // IFG buffer allocation constants
    TX_FIFO_LINES_MAIN_PIF = (1232 / 16),

    // Static VSC GIDs dedicated for granting credits during VOQ flush.
    LC_VOQ_FLUSH_VSC_RANGE_START = round_up(FABRIC_MC_VSC_RANGE_END, NATIVE_VOQ_SET_SIZE),                                  // 48
    LC_VOQ_FLUSH_VSC_RANGE_END = LC_VOQ_FLUSH_VSC_RANGE_START + (3 * NATIVE_VOQ_SET_SIZE) - 1,                              // 95
    SA_VOQ_FLUSH_VSC_RANGE_START = round_up(SA_MC_VSC_RANGE_END, NATIVE_VOQ_SET_SIZE),                                      // 96
    SA_VOQ_FLUSH_VSC_RANGE_END = SA_VOQ_FLUSH_VSC_RANGE_START + (ASIC_MAX_SLICES_PER_DEVICE_NUM * NATIVE_VOQ_SET_SIZE) - 1, // 191

    LC_LAST_RESERVED_VSC = LC_VOQ_FLUSH_VSC_RANGE_END, // 95
    SA_LAST_RESERVED_VSC = SA_VOQ_FLUSH_VSC_RANGE_END, // 191

    // MC-COPY-ID prefixes
    L2_AC_MC_COPY_ID_PREFIX = 0x0,
    L3_AC_MC_COPY_ID_PREFIX = 0x3,
    MCG_COUNTER_MC_COPY_ID_PREFIX = 0xb,
    CUD_MAP_PREFIX = 0x15,
};
...
enum resolution_step_e {
    ...
    RESOLUTION_STEP_NATIVE_FEC,
    RESOLUTION_STEP_NATIVE_LB,
    RESOLUTION_STEP_NATIVE_CE_PTR,
    RESOLUTION_STEP_NATIVE_L2_LP,
    RESOLUTION_STEP_NATIVE_L3_LP,
    RESOLUTION_STEP_NATIVE_FRR,
    RESOLUTION_STEP_STAGE1_PROTECTION,
    RESOLUTION_STEP_STAGE2_LB,
    RESOLUTION_STEP_PATH_LP,
    RESOLUTION_STEP_STAGE2_PROTECTION,
    RESOLUTION_STEP_STAGE3_LB,
    RESOLUTION_STEP_PORT_NPP_PROTECTION,
    RESOLUTION_STEP_PORT_DSPA,
...
enum class attribute_management_op : la_uint64_t {
    ...
    PREFIX_OBJECT_VPN_PROPERTY_CHANGED = (1 << 6),
    VOQ_CHANGED = (1 << 7),
    MAC_MOVED = (1 << 8),
    L3_AC_PORT_MAC_CHANGED = (1 << 9),
    L3_PROT_GROUP_DESTINATION_CHANGED = (1 << 10),
    SERVICE_MAPPING_TYPE_CHANGED = (1 << 11),
    PORT_SPEED_CHANGED = (1 << 12),
    MIRROR_COMMAND_ATTR_UPDATE = (1 << 13),
    EGRESS_SFLOW_CHANGED = (1 << 14),
    L3_PORT_ATTR_CHANGED = (1 << 15),
    L2_DLP_ATTRIB_CHANGED = (1 << 16),
    MULTICAST_PROTECTION_GROUP_CHANGED = (1 << 17),
    PWE_L3_DESTINATION_ATTRIB_CHANGED = (1 << 18),
    REMOTE_VOQ_CHANGED = (1 << 19),
    ACL_GROUP_CHANGED = (1 << 20),
    MCG_MEMBER_LIST_CHANGED = (1 << 21),
};
...
static const uint16_t LA_L3_MTU_MAX = (LA_MTU_MAX);
