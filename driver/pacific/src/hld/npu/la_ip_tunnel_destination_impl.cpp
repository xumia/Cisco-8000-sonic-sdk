...
la_ip_tunnel_destination_impl::get_next_resolution_step(resolution_step_e prev_step) const
{
    ...
    if (prev_step == RESOLUTION_STEP_NATIVE_LB || prev_step == RESOLUTION_STEP_FORWARD_L3) {
                                          ...
                                          const la_l3_destination_wcptr& underlay_destination)
{
    ...
    if (ip_tunnel_destination_gid % 4 != 0 && ip_tunnel_destination_gid % 4 != 1) {
        log_err(HLD,
                "gid for ip tunnel destination has to be multiple of 4 or multiple of 4 plus 1, i.e. mod(gid, 4) = 0 or "
                "mod(gid, 4) = 1, gid=%u",
                ip_tunnel_destination_gid);
    }
...
la_ip_tunnel_destination_impl::destroy()
{
    ...
    status = uninstantiate_resolution_object(m_underlay_destination, RESOLUTION_STEP_NATIVE_CE_PTR);
    return_on_error(status);
                                                  ...
                                                  bool is_init)
{
    ...
    la_device_impl::resolution_lp_table_format_e format = la_device_impl::resolution_lp_table_format_e::WIDE;
    ...
    switch (underlay_dest_type) {
    case object_type_e::NEXT_HOP: {
        txn.status = instantiate_resolution_object(underlay_destination, RESOLUTION_STEP_NATIVE_CE_PTR);
        return_on_error(txn.status);
        break;
    }
    ...
    case object_type_e::ECMP_GROUP: {
        ...
        txn.status = instantiate_resolution_object(underlay_destination, RESOLUTION_STEP_NATIVE_CE_PTR, this);
        return_on_error(txn.status);
        break;
    }
    ...
    txn.on_fail([=]() { uninstantiate_resolution_object(underlay_destination, RESOLUTION_STEP_NATIVE_CE_PTR); });
...
la_ip_tunnel_destination_impl::configure_ip_tunnel_destination_table()
{
    ...
    destination_id underlay_destination_id = silicon_one::get_destination_id(m_underlay_destination, RESOLUTION_STEP_NATIVE_CE_PTR);

    key.ce_ptr = m_ip_tunnel_destination_gid;
    ...
    value.payloads.wide_entry.entry.destination_ip_tunnel.destination = underlay_destination_id.val;
    value.payloads.wide_entry.entry.destination_ip_tunnel.ip_tunnel = ip_tunnel_gid;
    value.payloads.wide_entry.entry.destination_ip_tunnel.type = NPL_NATIVE_L2_LP_ENTRY_TYPE_NATIVE_L2_LP_DESTINATION_IP_TUNNEL;
    value.payloads.wide_entry.entry.destination_ip_tunnel.enc_type = NPL_NPU_ENCAP_L3_HEADER_TYPE_GRE;

    la_status status = table->set(key, value, entry);
    return_on_error(status, HLD, ERROR, "native_ce_ptr_table program failed");
    return status;
}
...
la_ip_tunnel_destination_impl::teardown_ip_tunnel_destination_table()
{
    ...
    return_on_error(status, HLD, ERROR, "native_ce_ptr_table erase failed");
    return status;
}
