...
validate_key(const la_acl_key& key_val)
{
    ...
    for (const auto acl_field : key_val) {
        ...
        case la_acl_field_type_e::PROTOCOL:
            if (acl_field.mask.protocol != 0x00 && acl_field.mask.protocol != 0xff) {
                log_err(HLD, "la_acl_delegate:unsupported protocol mask, protocol mask must be 0x00 or 0xff");
                return LA_STATUS_EINVAL;
            }
            break;
        ...
        for (auto cmd : acl_cmd_actions) {
            ...
            case la_acl_action_type_e::DO_MIRROR:
                ...
                if (cmd.data.do_mirror == la_acl_mirror_src_e::DO_MIRROR_FROM_LP) {
                    ...
                    rtf_result_profile_0.mirror_cmd_or_offset.mirror_offset = 0;
                } else if (!found_mirror_cmd) {
                    return LA_STATUS_EINVAL;
                }
                break;
            case la_acl_action_type_e::MIRROR_CMD:
                // Mirror Command from ACE
            ...
            default:
                log_err(HLD,
                        "la_acl_delegate::%s received unsupported acl action type (%s) for rtf_result_profile_0",
                        __func__,
...
{
    // out_acl_cmd = la_acl_command();

    // out_acl_cmd.type = la_acl_cmd_type_e::SGACL;
    // out_acl_cmd.data.sgacl.drop = npl_sgacl.drop;
}
...
{
    // if (acl_cmd.type != la_acl_cmd_type_e::SGACL) {
    //     log_err(HLD, "Invalid command type in SGACL %s", silicon_one::to_string(m_parent).c_str());
    //     return LA_STATUS_EINVAL;
    // }

    // npl_sgacl.drop = acl_cmd.data.sgacl.drop;
    return LA_STATUS_SUCCESS;
}
