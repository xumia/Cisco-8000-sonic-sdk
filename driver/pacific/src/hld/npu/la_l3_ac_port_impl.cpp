...
la_l3_ac_port_impl::la_l3_ac_port_impl(const la_device_impl_wptr& device)
    : m_device(device), m_slice_data(ASIC_MAX_SLICES_PER_DEVICE_NUM, slice_data()), m_ac_port_common(device)
{
    m_mac_addr.flat = 0;
}
                               ...
                               la_egress_qos_profile_impl* egress_qos_profile_impl)
{
    ...
    la_status status = ethernet_port->get_service_mapping_type(type);
    return_on_error(status);
    m_service_mapping_type = type;

    status = m_ac_port_common.initialize(m_device->get_sptr(this), gid, m_ethernet_port, vid1, vid2);
    return_on_error(status);
...
la_l3_ac_port_impl::destroy()
{
    ...
    for (auto sp_voq_pair = m_voq_map.begin(); sp_voq_pair != m_voq_map.end(); ++sp_voq_pair) {
        ...
        auto sys_port_base_sptr = sys_port.weak_ptr_static_cast<const la_system_port_base>();
        status = sys_port_base_sptr->clear_voq_mapping(voq_set);
        return_on_error(status);
        if (is_agg) {
            // clear lp_over_lag_table
            status = clear_lp_over_lag_table(sys_port_base_sptr);
            return_on_error(status);
        }
    }
...
la_l3_ac_port_impl::add_ifg(la_slice_ifg ifg)
{
    ...
    if (slice_added) {
        txn.status = configure_lp_attributes_table(ifg.slice);
        return_on_error(txn.status);
        txn.on_fail([=]() { teardown_lp_attributes_table(ifg.slice); });
    }
...
la_status
la_l3_ac_port_impl::configure_lp_attributes_table(la_slice_id_t slice_idx)
{
    ...
    k.service_lp_attributes_table_key.id = m_ac_port_common.get_local_slp_id(slice_idx);
    ...
    status = table->insert(k, v, m_slice_data[slice_idx].lp_attributes_entry);

    return status;
}

la_status
la_l3_ac_port_impl::update_l3_lp_attributes_tcam(la_slice_id_t slice,
                                                 ...
                                                 const uint32_t relay_id)
{
    ...
    la_status status = m_ac_port_common.configure_slice_ac_tcam_attributes(slice, payload, relay_id);
    return_on_error(status);
...
la_status
la_l3_ac_port_impl::update_l3_lp_attributes(la_slice_id_t slice,
                                            ...
                                            const npl_l3_lp_additional_attributes_t& additional_attribs)
{
    ...
    const auto& table(m_device->m_tables.service_lp_attributes_table[pair_idx]);
    const auto& entry = m_slice_data[slice].lp_attributes_entry;
    ...
    la_status status = table->set_entry_value(entry, value);
    ...
    status = m_ac_port_common.set_relay_id(slice, relay_id);

    // in case of TCAM service mapping, update the l3 attributes in the TCAM
    // service mapping table
    status = update_l3_lp_attributes_tcam(slice, payload, relay_id);
    return_on_error(
        status, HLD, ERROR, "la_l3_ac_port_impl::update_l3_lp_attributes: update l3 attribute in slice %d tcam failed", slice);

    return LA_STATUS_SUCCESS;
}

la_status
la_l3_ac_port_impl::teardown_lp_attributes_table(la_slice_id_t slice_idx)
{
    la_slice_pair_id_t pair_idx = slice_idx / 2;
    const auto& table(m_device->m_tables.service_lp_attributes_table[pair_idx]);
    npl_service_lp_attributes_table_key_t k = m_slice_data[slice_idx].lp_attributes_entry->key();

    la_status status = table->erase(k);
    return_on_error(status);

    m_slice_data[slice_idx].lp_attributes_entry = nullptr;
...
la_l3_ac_port_impl::add_ipv4_host(la_ipv4_addr_t ip_addr, la_mac_addr_t mac_addr, la_class_id_t class_id)
{
    ...
    if (class_id != LA_CLASS_ID_DEFAULT) {
        return LA_STATUS_EINVAL;
    }
...
la_status
la_l3_ac_port_impl::get_ipv6_host(la_ipv6_addr_t ip_addr, la_mac_addr_t& out_mac_address) const
{
    la_status status = m_vrf_port_common->get_ipv6_host(ip_addr, out_mac_address);
...
la_l3_ac_port_impl::configure_lp_over_lag_table(const la_system_port_base_wcptr& sp, const la_voq_set_wptr& voq_set)
{
    ...
    k.l3_dlp_msbs.no_acls = get_l3_lp_msb(m_gid);
    // DSP of member as destination
    k.destination = (silicon_one::get_destination_id(sp, RESOLUTION_STEP_STAGE3_LB)).val;
    // l3_dlp of self
    v.payloads.bvn_destination = (silicon_one::get_destination_id(voq_set, RESOLUTION_STEP_STAGE3_LB)).val;
...
la_l3_ac_port_impl::set_mac(const la_mac_addr_t& mac_addr)
{
    ...
    for (auto& slice_data : m_slice_data) {
        auto entry = slice_data.lp_attributes_entry;
...
la_l3_ac_port_impl::set_service_mapping_type()
{
    ...
    // program service mapping table
    // TBD, do the delete in the previous type
    // 1. Get the L3 SLP attribute
    m_service_mapping_type = type;
