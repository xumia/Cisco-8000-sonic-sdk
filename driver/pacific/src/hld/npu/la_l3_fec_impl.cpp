...
la_status
la_l3_fec_impl::config_native_fec_table(npl_native_fec_table_value_t value)
{
    const auto& table(m_device->m_tables.native_fec_table);
    npl_native_fec_table_key_t key;
    key.fec.id = m_gid;

    la_status status = table->set(key, value, m_native_fec_table_entry);
...
{
    if (m_device->is_in_use(shared_from_this())) {
...
void
la_l3_fec_impl::register_attribute_dependency(const la_l3_destination_wptr& destination)
{
    if (destination->type() == object_type_e::PREFIX_OBJECT) {
        ...
        m_device->add_attribute_dependency(destination, shared_from_this(), registered_attributes);
    }
}

void
la_l3_fec_impl::deregister_attribute_dependency(const la_l3_destination_wptr& destination)
{
    if (destination->type() == object_type_e::PREFIX_OBJECT) {
        bit_vector registered_attributes((la_uint64_t)attribute_management_op::PREFIX_OBJECT_VPN_PROPERTY_CHANGED);
        m_device->remove_attribute_dependency(destination, shared_from_this(), registered_attributes);
    }
}
...
la_l3_fec_impl::update_dependent_attributes(dependency_management_op op)
{
    ...
    case (attribute_management_op::PREFIX_OBJECT_VPN_PROPERTY_CHANGED): {
        la_prefix_object_base_wptr pfx_obj = m_l3_destination.weak_ptr_static_cast<la_prefix_object_base>();
...
la_l3_fec_impl::update_fec(const la_l3_destination_wptr& destination)
{
    ...
    if (dest_type == la_object::object_type_e::ECMP_GROUP) {
        auto ecmp_group = destination.weak_ptr_static_cast<la_ecmp_group_impl>();
    ...
    la_status status = instantiate_resolution_object(destination, RESOLUTION_STEP_NATIVE_FEC);
    return_on_error(status);

    switch (dest_type) {
    case la_object::object_type_e::NEXT_HOP: {
        la_next_hop_pacific_wptr nh = destination.weak_ptr_static_cast<la_next_hop_pacific>();
        la_status status = configure_basic_routing(nh);
        return_on_error(status);
    } break;
    case la_object::object_type_e::ECMP_GROUP: {
        la_ecmp_group_impl_wptr ecmp = destination.weak_ptr_static_cast<la_ecmp_group_impl>();
        la_status status = configure_basic_routing(ecmp);
        return_on_error(status);
    } break;
    case la_object::object_type_e::PREFIX_OBJECT: {
        la_prefix_object_base_wptr pfx_obj = destination.weak_ptr_static_cast<la_prefix_object_base>();
        la_status status = configure_basic_routing(pfx_obj);
        return_on_error(status);
    } break;
    default:
    ...
    status = uninstantiate_resolution_object(m_l3_destination, RESOLUTION_STEP_NATIVE_FEC);
    return status;
}
...
la_l3_fec_impl::update_fec(const la_l2_destination_wptr& destination)
{
    ...
    status = uninstantiate_resolution_object(m_l2_destination, RESOLUTION_STEP_NATIVE_FEC);
    return status;
}
...
la_l3_fec_impl::do_set_destination(const weak_ptr_unsafe<_DestinationType>& destination)
{
    ...
    if (!of_same_device(shared_from_this(), destination)) {
...
la_l3_fec_impl::set_destination(la_l3_destination* destination)
{
    ...
    return do_set_destination(destination_wptr);
}
...
la_status
la_l3_fec_impl::configure_basic_routing(const la_next_hop_pacific_wptr& nh)
{
    npl_native_fec_table_value_t value;
    npl_destination_t rpf_fec_table_dest = {.val = 0};
    la_status status = nh->get_fec_table_value(value, rpf_fec_table_dest);
    return_on_error(status);

    // Configure the RPF FEC table first so that it is ready when there's a hit in the native FEC table
    status = config_rpf_fec_table(rpf_fec_table_dest);
    return_on_error(status);

    // Configure the native FEC table
    status = config_native_fec_table(value);
...
{
    npl_native_fec_table_value_t value;
    auto pac_pfx_obj = pfx_obj.weak_ptr_static_cast<la_prefix_object_pacific>();
    la_status status = pac_pfx_obj->get_fec_table_value(value);
    return_on_error(status);
...
{
    npl_native_fec_table_value_t value;
    auto status = vxlan_port.weak_ptr_static_cast<la_l2_service_port_pacific>()->get_fec_table_value(value);
    return_on_error(status);
...
{
    la_status status = teardown_resolution_step_native_fec();
    return_on_error(status);
...
la_status
la_l3_fec_impl::teardown_resolution_step_native_fec()
{
    if (m_native_fec_table_entry != nullptr) {
        const auto& table(m_device->m_tables.native_fec_table);
        npl_native_fec_table_key_t key = m_native_fec_table_entry->key();
        la_status status = table->erase(key);
        return_on_error(status);

        m_native_fec_table_entry = nullptr;
    }
