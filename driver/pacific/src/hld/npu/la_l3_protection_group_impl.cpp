                                        ...
                                        const la_protection_monitor_wcptr& protection_monitor)
{
    ...
    status = instantiate_resolution_object(protection_monitor, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);

    status = instantiate_resolution_object(primary_destination, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);

    status = instantiate_resolution_object(backup_destination, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);
...
la_l3_protection_group_impl::configure_resolution_step()
{
    ...
    npl_path_lp_table_key_t k;
    npl_path_lp_table_value_t v;

    // Set key
    k.tunnel_dlp.id = m_gid;

    // Set value
    npl_path_lp_table_result_protected_t& protection_data(v.payloads.protected_entry.data);
    npl_path_lp_table_protection_entry_t& primary_entry_value(protection_data.primary);
    npl_path_lp_table_protection_entry_t& protecting_entry_value(protection_data.protecting);

    status = get_path_lp_table_protection_member_value(m_primary_destination, primary_entry_value);
    return_on_error(status);

    status = get_path_lp_table_protection_member_value(m_backup_destination, protecting_entry_value);
    return_on_error(status);
    ...
    v.action = NPL_PATH_LP_TABLE_ACTION_PROTECTED_ENTRY;

    // Write to table
    npl_path_lp_table_entry_t* new_entry_ptr = nullptr;
    status = m_device->m_tables.path_lp_table->set(k, v, new_entry_ptr);

    return status;
}

la_status
la_l3_protection_group_impl::get_path_lp_table_protection_member_value(const la_l3_destination_wcptr& protection_member_dest,
                                                                       npl_path_lp_table_protection_entry_t& value)
{
    ...
    if (type == object_type_e::NEXT_HOP) {
        ...
        npl_path_lp_stage3_nh1_t& nh_value(value.stage3_nh1);

        nh_value.stage3_nh = protection_member_nh->get_destination_id(RESOLUTION_STEP_PATH_LP).val;
        nh_value.type = NPL_PATH_LP_ENTRY_TYPE_PATH_LP_STAGE3_NH1;
        return LA_STATUS_SUCCESS;
    }
    ...
    if (type == object_type_e::TE_TUNNEL) {
        ...
        npl_path_lp_stage3_nh_te_tunnel16b_t& nh_value(value.stage3_nh_te_tunnel16b);

        nh_value.stage3_nh = te_tunnel_impl->get_destination_id(RESOLUTION_STEP_PATH_LP).val;
        nh_value.te_tunnel16b = te_tunnel_impl->get_gid();
        nh_value.type = NPL_PATH_LP_ENTRY_TYPE_PATH_LP_STAGE3_NH_TE_TUNNEL16B;
        return LA_STATUS_SUCCESS;
    }
...
la_l3_protection_group_impl::destroy()
{
    ...
    status = uninstantiate_resolution_object(m_backup_destination, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);

    status = uninstantiate_resolution_object(m_primary_destination, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);

    status = uninstantiate_resolution_object(m_protection_monitor, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);
...
{
    const auto& table(m_device->m_tables.path_lp_table);
    npl_path_lp_table_key_t key;

    // Set key
    key.tunnel_dlp.id = m_gid;
...
la_l3_protection_group_impl::get_destination_id(resolution_step_e prev_step) const
{
    ...
    switch (cur_step) {
    case silicon_one::RESOLUTION_STEP_PATH_LP: {
        return destination_id(NPL_DESTINATION_MASK_STAGE2_P_NH | m_gid);
    }
                                                     ...
                                                     const la_protection_monitor* protection_monitor)
{
    ...
    la_l3_destination_wcptr primary_destination_sp = m_device->get_sptr<la_l3_destination>(primary_destination);
    la_l3_destination_wcptr backup_destination_sp = m_device->get_sptr<la_l3_destination>(backup_destination);
    la_protection_monitor_wcptr protection_monitor_sp = m_device->get_sptr<la_protection_monitor>(protection_monitor);

    la_status status = check_destination(m_gid, primary_destination_sp, backup_destination_sp, protection_monitor_sp);
    return_on_error(status);

    status = instantiate_resolution_object(protection_monitor_sp, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);

    status = instantiate_resolution_object(primary_destination_sp, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);

    status = instantiate_resolution_object(backup_destination_sp, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);

    la_l3_destination_wcptr old_primary_destination = m_primary_destination;
    la_l3_destination_wcptr old_backup_destination = m_backup_destination;
    la_protection_monitor_wcptr old_protection_monitor = m_protection_monitor;
    ...
    status = uninstantiate_resolution_object(old_backup_destination, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);

    status = uninstantiate_resolution_object(old_primary_destination, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);

    status = uninstantiate_resolution_object(old_protection_monitor, RESOLUTION_STEP_PATH_LP);
    return_on_error(status);
