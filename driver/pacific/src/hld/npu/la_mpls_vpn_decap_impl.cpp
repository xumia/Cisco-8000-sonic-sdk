    ...
    for (la_slice_id_t slice : get_slices(m_device, la_slice_mode_e::NETWORK)) {
        ...
        if (m_vrf != nullptr) {
            ...
            result.pwe_vpn_mldp_info.l3vpn_info.vpn_p_counter
                = populate_counter_ptr_slice(counter, slice, COUNTER_DIRECTION_INGRESS);
        ...
        la_status status = table->set(key, value, m_slice_data[slice].m_mpls_termination_entry);
...
la_mpls_vpn_decap_impl::remove_from_mpls_termination_table()
{
    for (la_slice_id_t slice : get_slices(m_device, la_slice_mode_e::NETWORK)) {
        const auto& table(m_device->m_tables.mpls_termination_em1_table[slice]);
        npl_mpls_termination_em1_table_key_t key = m_slice_data[slice].m_mpls_termination_entry->key();

        la_status status = table->erase(key);
        return_on_error(status);

        m_slice_data[slice].m_mpls_termination_entry = nullptr;
    }
