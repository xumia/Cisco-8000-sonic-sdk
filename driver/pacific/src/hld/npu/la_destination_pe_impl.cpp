...
la_destination_pe_impl::set_destination(const la_l3_destination* destination)
{
    ...
    la_l3_destination_wcptr destination_sp = m_device->get_sptr<la_l3_destination>(destination);
...
la_destination_pe_impl::update_destination(const la_l3_destination_wcptr& destination, bool is_init)
{
    ...
    status = instantiate_resolution_object(destination, RESOLUTION_STEP_NATIVE_CE_PTR, this);
    return_on_error(status);
...
la_status
la_destination_pe_impl::configure_native_prefix_table()
{
...
la_status
la_destination_pe_impl::configure_native_ce_ptr_to_ecmp_group_value()
{
    ...
    destination_id dest_id = silicon_one::get_destination_id(m_destination, RESOLUTION_STEP_NATIVE_CE_PTR);

    key.ce_ptr = m_gid;

    value_pfx.action = NPL_NATIVE_CE_PTR_TABLE_ACTION_NARROW_ENTRY;
    value_pfx.payloads.narrow_entry.entry.stage2_ecmp_vpn_inter_as.stage2_ecmp = dest_id.val;
    value_pfx.payloads.narrow_entry.entry.stage2_ecmp_vpn_inter_as.vpn_inter_as = (m_vpn_enabled << 1 | 0x1);
    value_pfx.payloads.narrow_entry.entry.stage2_ecmp_vpn_inter_as.type
        = NPL_NATIVE_L2_LP_ENTRY_TYPE_NATIVE_L2_LP_STAGE2_ECMP_VPN_INTER_AS;
...
la_status
la_destination_pe_impl::teardown_native_prefix_table()
{
    const auto& table(m_device->m_tables.native_ce_ptr_table);
    npl_native_ce_ptr_table_key_t key;
...
la_status
la_destination_pe_impl::clear_vrf_properties(const la_vrf* vrf, la_ip_version_e ip_version)
{
...
la_status
la_destination_pe_impl::get_vrf_properties(const la_vrf* vrf, la_ip_version_e ip_version, la_mpls_label_vec_t& out_labels) const
{
    la_vrf_impl_wcptr vrf_impl = m_device->get_sptr<const la_vrf_impl>(vrf);
...
la_status
la_destination_pe_impl::set_vrf_properties(const la_vrf* vrf, la_ip_version_e ip_version, const la_mpls_label_vec_t& labels)
{
    ...
    if (vpn_map_entry_it == m_vpn_entry_map.end()) {
        ...
        m_device->add_object_dependency(vrf_impl, this);
    }
