...
{
    npl_rewrite_sa_prefix_index_table_key_t key;
    npl_rewrite_sa_prefix_index_table_value_t value;
    const auto& table(m_device->m_tables.rewrite_sa_prefix_index_table);
    npl_rewrite_sa_prefix_index_table_entry_t* dummy;

    key.rewrite_sa_index = index;
    value.payloads.sa_msb = get_msbits(mac_addr);
