...
private:
    ...
    void register_attribute_dependency(const la_l3_destination_wptr& destination);
    void deregister_attribute_dependency(const la_l3_destination_wptr& destination);
    ...
    // Object ID
    la_object_id_t m_oid = LA_OBJECT_ID_INVALID; // FEC ID
    la_fec_gid_t m_gid;
    ...
    // Native FEC table entry
    npl_native_fec_table_entry_wptr_t m_native_fec_table_entry;

    // RPF FEC table entry
    npl_rpf_fec_table_entry_wptr_t m_rpf_fec_table_entry;

    // Table configuration helper functions
    la_status configure_basic_routing(const la_next_hop_pacific_wptr& nh);
    ...
    la_status config_native_fec_table(npl_native_fec_table_value_t value);
