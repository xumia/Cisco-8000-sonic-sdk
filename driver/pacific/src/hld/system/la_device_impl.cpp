...
enum {
    ...
    PACIFIC_EXTENSION_HBM = 1, ///< Device Efuse information, extension indicating Pacific with HBM.
    ...
    PFC_PILOT_MIRROR_GID = la_device_impl::MIRROR_GID_INGRESS_OFFSET + 28,
...
la_device_impl::is_voq_id_in_range(la_slice_id_t slice_id, la_voq_gid_t voq_id)
{
    la_voq_gid_t voqs_per_slice
        = is_multi_device_aware_slice(slice_id) ? MAX_VOQS_PER_SLICE_IN_LINECARD_DEVICE : MAX_VOQS_PER_SLICE_IN_STANDALONE_DEVICE;

    // The last VOQ is used by the HW for invalid indication. Reserve the whole native VOQ set size.
    la_voq_gid_t last_available_base_voq = voqs_per_slice - NATIVE_VOQ_SET_SIZE;

    return FIRST_AVAILABLE_BASE_VOQ <= voq_id && voq_id < last_available_base_voq;
}

bool
la_device_impl::is_dsp_in_range(la_system_port_gid_t system_port_gid)
{
...
{
    la_device_impl_base::pre_initialize(std::make_shared<slice_id_manager_base>() /* slice_id_manager */);
    m_pacific_tree = m_ll_device->get_pacific_tree_scptr();
    ...
    // Revision
    m_revision = m_pacific_tree->get_revision();

    m_notification = std::make_shared<hld_notification_pacific>(shared_from_this());
    ...
    m_pvt_handler = silicon_one::make_unique<pacific_pvt_handler>(shared_from_this());
    m_pvt_handler->initialize();

    m_hbm_aapl_handlers.resize(NUM_HBM_INTERFACES);
    ...
    for (la_slice_id_t sid : m_slice_id_manager->get_all_possible_slices()) {
        for (la_ifg_id_t ifg_id = 0; ifg_id < NUM_IFGS_PER_SLICE; ifg_id++) {
            m_ifg_handlers[sid][ifg_id] = silicon_one::make_unique<ifg_handler_pacific>(shared_from_this(), sid, ifg_id);
        }
    }
    ...
    m_arc_hdlr = silicon_one::make_unique<arc_handler_pacific>(shared_from_this());
    m_npu_host_eventq = silicon_one::make_unique<npu_host_event_queue_pacific>(shared_from_this());
    ...
    // Make the lower 4 dwords of efuse value visible through sbif.efuse_userbits_reg0,1,2,3.
    la_status status = reload_fuse_userbits(m_fuse_userbits);
...
la_device_impl::initialize_resolution_index_generators()
{
    ...
    m_index_generators.ecmp_groups[RESOLUTION_STEP_NATIVE_LB] = ranged_index_generator(0, MAX_ECMP_GROUP_NATIVE_LB_ID);
    m_index_generators.ecmp_groups[RESOLUTION_STEP_STAGE2_LB] = ranged_index_generator(0, MAX_ECMP_GROUP_STAGE2_LB_ID);
    m_index_generators.ecmp_groups[RESOLUTION_STEP_STAGE3_LB] = ranged_index_generator(0, MAX_ECMP_GROUP_STAGE3_LB_ID);
}
...
la_device_impl::initialize_phase_topology(const translator_creator_sptr& creator)
{
    ...
    // Initialize bitmap_oqg_map_table for simulator.
    retval = initialize_bitmap_oqg_map_table();
    return_on_error(retval);
    ...
    // Initialize default values for path_lp_is_pbts_prefix table.
    retval = initialize_path_lp_is_pbts_prefix_default_values();
    return_on_error(retval);
    ...
    if (std::find(m_slice_mode.begin(), m_slice_mode.end(), la_slice_mode_e::NETWORK) != m_slice_mode.end()) {
    ...
    m_counter_bank_manager = silicon_one::make_unique<counter_manager>(shared_from_this());
    ...
    m_fabric_init_handler = silicon_one::make_unique<fabric_init_handler>(shared_from_this());

    retval = m_fabric_init_handler->create_fabric_mc_voq_set(m_egress_multicast_fabric_replication_voq_set);
    return_on_error(retval);
    if (m_egress_multicast_fabric_replication_voq_set != nullptr) {
        m_is_builtin_objects[m_egress_multicast_fabric_replication_voq_set->oid()] = true;
    }

    retval = m_fabric_init_handler->configure_phase_topology_pre_soft_reset();
    return_on_error(retval);
    ...
    retval = m_fabric_init_handler->configure_phase_topology_dynamic_memories();
    return_on_error(retval);
    ...
    retval = m_fabric_init_handler->configure_phase_topology_post_soft_reset();
    return_on_error(retval);

    retval = apply_topology_post_soft_reset_workaround();
    return_on_error(retval);

    retval = apply_fe_light_fi_workaround();
    return_on_error(retval);
    ...
    // Lookup error WA - PACIFIC_B0_WA
    // Lookup Error wait for result per macro
    retval = init_npe2dbc_thread_ready_indication();
    return_on_error(retval);
    ...
    retval = clear_meter_block_memories();
    return_on_error(retval);
    ...
    // TODO: Remove this function when this issue is handled by NPSuite.
    retval = init_lookup_error_macros_ids();
    return_on_error(retval);
    ...
    retval = configure_meters_for_ecn_workaround_in_lc();
    return_on_error(retval);
    // end WORKAROUND

    // Prepare dedicated VSCs for credits grant during VOQ flush, and configure shapers accordingly.
    if (m_device_mode != device_mode_e::FABRIC_ELEMENT) {
        retval = prepare_dedicated_vsc_for_voq_flush();
        return_on_error(retval);
    ...
    // Initialize PFC SW latency tables
    retval = init_pfc_sw_tables();
    return_on_error(retval);
    ...
    // B1 P4 overwrite static config
    retval = post_topology_p4_overrides();
    return_on_error(retval);
...
la_device_impl::is_vsc_id_in_range(la_slice_id_t slice_id, la_vsc_gid_t vsc_id)
{
    ...
    bool is_within_upper_bound = vsc_id < vscs_per_ifg;

    // Check lower boundry
    la_uint64_t first_vsc = 0;
    bool is_within_lower_bound = true;
    get_limit(limit_type_e::DEVICE__MIN_ALLOCATABLE_VSC, first_vsc);
    is_within_lower_bound = vsc_id >= first_vsc;

    if (is_within_upper_bound && is_within_lower_bound) {
        return true;
    } else {
        return false;
    }
...
{
    // pacific doesn't support SVL (datapath)
    return dest_id;
}
                                                                        ...
                                                                        uint64_t key_sched_rcy)
{
    ...
    la_status status = per_slice_tables_erase(m_slice_mode, tables, {la_slice_mode_e::NETWORK, la_slice_mode_e::UDC}, k);
...
la_status
la_device_impl::configure_overhead_accounting(int overhead)
{
    int compensation = overhead + ETHERNET_OVERHEAD - NPU_HEADER_SIZE;

    if ((compensation >= OVERHEAD_ACCOUTING_LIMIT) || (compensation <= -OVERHEAD_ACCOUTING_LIMIT)) {
        return LA_STATUS_EINVAL;
    }

    lld_register_value_list_t reg_val_list;

    la_uint64_t interface_bitmap = 0xFFFFF00000ULL;
    for (size_t slice = 0; slice < array_size(m_pacific_tree->slice); slice++) {
        pdvoq_shared_mma_compensation_per_ifg_register comp_reg;
        comp_reg.fields.ifg_per_source_if = interface_bitmap;
        comp_reg.fields.ifg0_compensation = compensation;
        comp_reg.fields.ifg1_compensation = compensation;
        comp_reg.fields.use_sch_comp_for_counters = 0;
        if (m_slice_mode[slice] == la_slice_mode_e::CARRIER_FABRIC) {
            comp_reg.fields.fabric_slice = 1;
        } else {
            comp_reg.fields.fabric_slice = 0;
        }

        reg_val_list.push_back({(*m_pacific_tree->pdvoq_shared_mma->compensation_per_ifg)[slice], comp_reg});

        ics_slice_compensation_per_ifg_register ics_comp_reg;
        ics_comp_reg.fields.ifg_per_source_if = interface_bitmap;
        ics_comp_reg.fields.ifg0_compensation = compensation;
        ics_comp_reg.fields.ifg1_compensation = compensation;
        reg_val_list.push_back({(m_pacific_tree->slice[slice]->ics->compensation_per_ifg), ics_comp_reg});
    }

    for (la_slice_id_t slice : m_slice_id_manager->get_slices_by_fabric_type(fabric_slices_type_e::HW_NON_FABRIC)) {
        pdvoq_slice_compensation_per_ifg_register comp_reg;
        comp_reg.fields.ifg_per_source_if = interface_bitmap;
        comp_reg.fields.ifg0_compensation = compensation;
        comp_reg.fields.ifg1_compensation = compensation;
        if (m_slice_mode[slice] == la_slice_mode_e::CARRIER_FABRIC) {
        ...
        reg_val_list.push_back({(m_pacific_tree->slice[slice]->pdvoq->compensation_per_ifg), comp_reg});
    }

    for (la_slice_id_t slice : m_slice_id_manager->get_slices_by_fabric_type(fabric_slices_type_e::HW_FABRIC)) {
        pdvoq_slice5_compensation_per_ifg_register comp_reg;
        comp_reg.fields.ifg_per_source_if = interface_bitmap;
        comp_reg.fields.ifg0_compensation = compensation;
        comp_reg.fields.ifg1_compensation = compensation;
        if (m_slice_mode[slice] == la_slice_mode_e::CARRIER_FABRIC) {
        ...
        reg_val_list.push_back({(m_pacific_tree->slice[slice]->fabric_pdvoq->compensation_per_ifg), comp_reg});
    }
...
la_device_impl::retrieve_overhead_accounting(int& out_overhead) const
{
    ...
    // ASIC_MAX_SLICES_PER_DEVICE_NUM -- this is bad
    la_status status
        = m_ll_device->read_register(*m_pacific_tree->slice[ASIC_MAX_SLICES_PER_DEVICE_NUM - 1]->fabric_pdvoq->compensation_per_ifg,
                                     pdvoq_slice5_compensation_per_ifg_register::SIZE,
                                     &comp_reg);
    return_on_error(status);

    // The compensation value is 6 bit signed value - translate the 6 bit to signed integer.
    out_overhead = comp_reg.fields.ifg0_compensation < OVERHEAD_ACCOUTING_LIMIT
                       ? comp_reg.fields.ifg0_compensation
                       : -(OVERHEAD_ACCOUTING_LIMIT - ((OVERHEAD_ACCOUTING_LIMIT - 1) & comp_reg.fields.ifg0_compensation));

    out_overhead -= (ETHERNET_OVERHEAD - NPU_HEADER_SIZE);
...
la_device_impl::create_security_group_cell(la_sgt_t sgt,
                                           ...
                                           la_security_group_cell*& out_sg_cell)
{
    return LA_STATUS_ENOTIMPLEMENTED;
}
...
la_device_impl::configure_redirect_eth_encap(uint64_t encap_ptr, la_mac_addr_t da, la_mac_addr_t sa, la_vlan_tag_tci_t vlan_tag)
{
    ...
    v.payloads.found.punt_eth_or_npu_host_encap.punt_eth_nw_encap_data.sa_or_npuh.punt_if_sa.mac_address = sa.flat;
...
{
    {
        ...
        v.action = NPL_TX_PUNT_ETH_ENCAP_TABLE_ACTION_FOUND;
        v.payloads.found.wide_bit = 1;
        v.payloads.found.punt_eth_or_npu_host_encap.punt_eth_nw_encap_data.sa_or_npuh.punt_npu_host_data.npu_host_macro_data
            .first_fi_macro_id
            = fi_macro;
        v.payloads.found.punt_eth_or_npu_host_encap.punt_eth_nw_encap_data.sa_or_npuh.punt_npu_host_data.npu_host_macro_data
            .first_npe_macro_id
                                             ...
                                             la_uint_t encap_ptr)
{
    ...
    la_slice_ifg s_ifg = m_slice_id_manager->get_npu_host_port_ifg();
    {
        ...
        case LA_EVENT_OAMP_PFC_LOOKUP_FAILED:
            if (redirect_dest == RX_NOT_CNT_DROP_DSP) {
                                        ...
                                        la_traffic_class_t tc)
{
    ...
    for (la_slice_id_t slice : get_used_slices()) {
        tcv.payloads.counter_meter_found.counter_ptr = populate_counter_ptr_slice(tcv_meter, slice, COUNTER_DIRECTION_INGRESS);
        status = m_tables.inject_down_tx_redirect_counter_table[slice]->set(tck, tcv, tce);
        return_on_error(status);
    }
...
la_device_impl::configure_static_tables()
{
    ...
    status = configure_lpm_destination_prefix_map_table();
    return_on_error(status);
    ...
    status = configure_fabric_filb_voq_mapping();
    return_on_error(status);
...
la_device_impl::configure_static_resolution_destination_decoding_table()
{
    for (uint64_t index = 0; index < (1 << RESOLUTION_ENCODING_PREFIX_MAX_LEN); ++index) {
        npl_destination_decoding_table_t::key_type k;
        npl_destination_decoding_table_t::value_type v;

        k.destination_encoding = index;

        npl_destination_decoding_table_result_t destination_decoding_table_entry
            = get_resolution_destination_decoding_value(k.destination_encoding);
        v.action = NPL_DESTINATION_DECODING_TABLE_ACTION_WRITE;
        v.payloads.destination_decoding_table_result = destination_decoding_table_entry;

        npl_destination_decoding_table_t::entry_type* dummy_entry;
        la_status status = m_tables.destination_decoding_table->insert(k, v, dummy_entry);
        return_on_error(status);
    }
    return LA_STATUS_SUCCESS;
}

npl_destination_decoding_table_result_t
la_device_impl::get_resolution_destination_decoding_value(uint64_t destination_encoding)
{
    npl_destination_type_e destination_type = get_resolution_destination_type(destination_encoding);
    ...
    const npl_resolution_stage_type_e bypass_resulution_stage = NPL_RESOLUTION_STAGE_TYPE_NATIVE;

    const npl_destination_decoding_table_result_t destination_decoding_value_arr[NPL_DESTINATION_TYPE_UNKNOWN + 1]
        = {// lb_table_behavior, resolution_table, resolution_stage
               // end of resolution
               [NPL_DESTINATION_TYPE_BVN] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                             NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                             NPL_RESOLUTION_TABLE_TYPE_UNKNOWN,
                                             bypass_resulution_stage},
               [NPL_DESTINATION_TYPE_MC] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                            NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                            NPL_RESOLUTION_TABLE_TYPE_UNKNOWN,
                                            bypass_resulution_stage},
               [NPL_DESTINATION_TYPE_FLBG] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                              NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                              NPL_RESOLUTION_TABLE_TYPE_UNKNOWN,
                                              bypass_resulution_stage},
               [NPL_DESTINATION_TYPE_DSP] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                             NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                             NPL_RESOLUTION_TABLE_TYPE_UNKNOWN,
                                             bypass_resulution_stage},

               // native resolution
               [NPL_DESTINATION_TYPE_L2_DLP] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                                NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                                NPL_RESOLUTION_TABLE_TYPE_NATIVE_L2_LP,
                                                NPL_RESOLUTION_STAGE_TYPE_NATIVE},
               [NPL_DESTINATION_TYPE_L2_DLPA_OR_ECMP] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                                         NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                                         NPL_RESOLUTION_TABLE_TYPE_NATIVE_LB,
                                                         NPL_RESOLUTION_STAGE_TYPE_NATIVE},
               [NPL_DESTINATION_TYPE_FEC] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                             NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                             NPL_RESOLUTION_TABLE_TYPE_NATIVE_FEC,
                                             NPL_RESOLUTION_STAGE_TYPE_NATIVE},
               [NPL_DESTINATION_TYPE_FRR] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                             NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                             NPL_RESOLUTION_TABLE_TYPE_NATIVE_FRR,
                                             NPL_RESOLUTION_STAGE_TYPE_NATIVE},
               [NPL_DESTINATION_TYPE_CE_PTR] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                                NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                                NPL_RESOLUTION_TABLE_TYPE_NATIVE_L2_LP,
                                                NPL_RESOLUTION_STAGE_TYPE_NATIVE},

               // path resolution
               [NPL_DESTINATION_TYPE_STAGE2_ECMP] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                                     NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                                     NPL_RESOLUTION_TABLE_TYPE_STAGE2_LB,
                                                     NPL_RESOLUTION_STAGE_TYPE_PATH},
               [NPL_DESTINATION_TYPE_STAGE2_P_NH] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                                     NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                                     NPL_RESOLUTION_TABLE_TYPE_PATH_LP,
                                                     NPL_RESOLUTION_STAGE_TYPE_PATH},

               // port resolution
               [NPL_DESTINATION_TYPE_NPP] = {NPL_CHECK_NPP_RANGE_MODE_ENABLED,
                                             NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                             NPL_RESOLUTION_TABLE_TYPE_STAGE3_LB,
                                             NPL_RESOLUTION_STAGE_TYPE_PORT},
               [NPL_DESTINATION_TYPE_STAGE3_NH] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                                   NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                                   NPL_RESOLUTION_TABLE_TYPE_STAGE3_LB,
                                                   NPL_RESOLUTION_STAGE_TYPE_PORT},
               [NPL_DESTINATION_TYPE_DSPA] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                              NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                              NPL_RESOLUTION_TABLE_TYPE_PORT_DSPA,
                                              NPL_RESOLUTION_STAGE_TYPE_PORT},

               // unknown prefix
               [NPL_DESTINATION_TYPE_UNKNOWN] = {NPL_CHECK_NPP_RANGE_MODE_DISABLED,
                                                 NPL_LB_TABLE_BEHAVIOR_TYPE_LB,
                                                 NPL_RESOLUTION_TABLE_TYPE_UNKNOWN,
                                                 NPL_RESOLUTION_STAGE_TYPE_RETURN}};
...
la_device_impl::get_resolution_destination_type(uint64_t destination_encoding)
{
    ...
    if (does_encoding_match_prefix(
            destination_encoding, NPL_DESTINATION_STAGE2_P_NH_PREFIX, NPL_DESTINATION_STAGE2_P_NH_PREFIX_LEN)) {
        return NPL_DESTINATION_TYPE_STAGE2_P_NH;
    }
    if (does_encoding_match_prefix(
            destination_encoding, NPL_DESTINATION_STAGE2_ECMP_PREFIX, NPL_DESTINATION_STAGE2_ECMP_PREFIX_LEN)) {
        return NPL_DESTINATION_TYPE_STAGE2_ECMP;
    }

    if (does_encoding_match_prefix(destination_encoding, NPL_DESTINATION_STAGE3_NH_PREFIX, NPL_DESTINATION_STAGE3_NH_PREFIX_LEN)) {
        return NPL_DESTINATION_TYPE_STAGE3_NH;
    }
...
la_status
la_device_impl::initialize_path_lp_is_pbts_prefix_default_values()
{
    const size_t PATH_LP_IS_PBTS_PREFIX_TABLE_SIZE = 32;
    for (size_t i = 0; i < PATH_LP_IS_PBTS_PREFIX_TABLE_SIZE; ++i) {
        npl_path_lp_is_pbts_prefix_table_t::key_type pbts_key;
        npl_path_lp_is_pbts_prefix_table_t::value_type pbts_value;
        npl_path_lp_is_pbts_prefix_table_entry_t* pbts_dummy_ret = nullptr;

        pbts_key.prefix = i;
        pbts_value.action = NPL_PATH_LP_IS_PBTS_PREFIX_TABLE_ACTION_WRITE;
        pbts_value.payloads.path_lp_is_pbts_prefix_table_result.val = npl_is_pbts_prefix_e::NPL_IS_PBTS_PREFIX_FALSE;
        la_status status = m_tables.path_lp_is_pbts_prefix_table->insert(pbts_key, pbts_value, pbts_dummy_ret);
        return_on_error(status);
    }
...
{
    npl_rewrite_sa_prefix_index_table_t::entry* dummy_ret = nullptr;
    npl_rewrite_sa_prefix_index_table_t::key_type key;
    npl_rewrite_sa_prefix_index_table_t::value_type value;

    key.rewrite_sa_index = 0;
    value.action = NPL_REWRITE_SA_PREFIX_INDEX_TABLE_ACTION_WRITE;
    value.payloads.sa_msb = 0;

    la_status status = m_tables.rewrite_sa_prefix_index_table->insert(key, value, dummy_ret);
    return status;
}
...
la_device_impl::configure_cud_is_multicast_bitmap_table()
{
    ...
    status = configure_cud_is_multicast_bitmap_entry(NPL_TX_CUD_RESERVED_PREFIX, NPL_TX_CUD_RESERVED_PREFIX_LEN, false);
    return_on_error(status);
...
la_status
la_device_impl::do_set_lpm_destination_prefix_map(uint64_t compressed_prefix,
                                                  size_t compressed_prefix_len,
                                                  uint64_t prefix,
                                                  size_t prefix_len,
                                                  bool is_default)
{
    const auto& table = m_tables.lpm_destination_prefix_map_table;
    uint64_t default_prefix_mask = is_default ? 1 << (COMPRESSED_DESTINATION_PREFIX_LENGTH - 1) : 0;
    size_t num_of_free_bits = COMPRESSED_DESTINATION_PREFIX_LENGTH - compressed_prefix_len;

    // The actual non-compressed prefix can be narrower than the full prefix width. In this case
    // the bits in the gap between the full-width and the actual-width need to be kept. This is
    // done by adding several entries for each prefix - one for each possible combination of bits
    // in the gap
    for (size_t j = 0; j < (1ULL << num_of_free_bits); j++) {
        npl_lpm_destination_prefix_map_table_key_t k;
        npl_lpm_destination_prefix_map_table_value_t v;
        npl_lpm_destination_prefix_map_table_entry_t* e = nullptr;

        // 'j' is used as the bit combination in the gap between the full and actual prefix width
        uint64_t key_prefix
            = default_prefix_mask | compressed_prefix << (COMPRESSED_DESTINATION_PREFIX_LENGTH - compressed_prefix_len) | j;
        uint64_t value_prefix = (prefix << (COMPRESSED_DESTINATION_PREFIX_LENGTH - prefix_len)) | j;
        // Pacific WA: Force Caching default routes. Use destination[19] as default route indicator
        // make sure that destination[19] is unused
        dassert_crit((value_prefix & default_prefix_mask) == 0);
        value_prefix |= default_prefix_mask;

        k.lpm_prefix_map_input_prefix = key_prefix;
        v.payloads.lpm_prefix_map_output.prefix = value_prefix;
        // Pacific WA: Force Caching default routes. Never set the is_default indication
        v.payloads.lpm_prefix_map_output.is_default = 0;
...
la_status
la_device_impl::set_lpm_destination_prefix_map(uint64_t compressed_prefix,
                                               size_t compressed_prefix_len,
                                               uint64_t prefix,
                                               size_t prefix_len,
                                               bool do_default,
                                               bool do_non_default)
{
    if (do_non_default) {
        la_status status
            = do_set_lpm_destination_prefix_map(compressed_prefix, compressed_prefix_len, prefix, prefix_len, false /*is_default*/);
        return_on_error(status);
    }

    if (do_default) {
        la_status status
            = do_set_lpm_destination_prefix_map(compressed_prefix, compressed_prefix_len, prefix, prefix_len, true /*is_default*/);
        return_on_error(status);
    }
...
la_status
la_device_impl::configure_lpm_destination_prefix_map_table()
{
    la_status status;

    status = set_lpm_destination_prefix_map(NPL_LPM_COMPRESSED_DESTINATION_CE_PTR_PREFIX,
                                            NPL_LPM_COMPRESSED_DESTINATION_CE_PTR_PREFIX_LEN,
                                            NPL_DESTINATION_CE_PTR_PREFIX,
                                            NPL_DESTINATION_CE_PTR_PREFIX_LEN,
                                            true /* do_default */,
                                            true /* do_non_default */);
    return_on_error(status);

    status = set_lpm_destination_prefix_map(NPL_LPM_COMPRESSED_DESTINATION_ECMP_PREFIX,
                                            NPL_LPM_COMPRESSED_DESTINATION_ECMP_PREFIX_LEN,
                                            NPL_DESTINATION_ECMP_PREFIX,
                                            NPL_DESTINATION_ECMP_PREFIX_LEN,
                                            true /* do_default */,
                                            true /* do_non_default */);
    return_on_error(status);

    status = set_lpm_destination_prefix_map(NPL_LPM_COMPRESSED_DESTINATION_STAGE2_ECMP_PREFIX,
                                            NPL_LPM_COMPRESSED_DESTINATION_STAGE2_ECMP_PREFIX_LEN,
                                            NPL_DESTINATION_STAGE2_ECMP_PREFIX,
                                            NPL_DESTINATION_STAGE2_ECMP_PREFIX_LEN,
                                            true /* do_default */,
                                            true /* do_non_default */);
    return_on_error(status);

    status = set_lpm_destination_prefix_map(NPL_LPM_COMPRESSED_DESTINATION_FEC_PREFIX,
                                            NPL_LPM_COMPRESSED_DESTINATION_FEC_PREFIX_LEN,
                                            NPL_DESTINATION_FEC_PREFIX,
                                            NPL_DESTINATION_FEC_PREFIX_LEN,
                                            true /* do_default */,
                                            true /* do_non_default */);
    return_on_error(status);

    status = set_lpm_destination_prefix_map(NPL_LPM_COMPRESSED_DESTINATION_LPTS_PREFIX,
                                            NPL_LPM_COMPRESSED_DESTINATION_LPTS_PREFIX_LEN,
                                            NPL_DESTINATION_LPTS_PREFIX,
                                            NPL_DESTINATION_LPTS_PREFIX_LEN,
                                            true /* do_default */,
                                            true /* do_non_default */);
    return_on_error(status);

    status = set_lpm_destination_prefix_map(NPL_LPM_COMPRESSED_DESTINATION_L3_DLP_SUBNET_PREFIX,
                                            NPL_LPM_COMPRESSED_DESTINATION_L3_DLP_SUBNET_PREFIX_LEN,
                                            NPL_DESTINATION_L3_DLP_SUBNET_PREFIX,
                                            NPL_DESTINATION_L3_DLP_SUBNET_PREFIX_LEN,
                                            true /* do_default */,
                                            true /* do_non_default */);
    return_on_error(status);
    ...
    for (la_slice_id_t sid : get_used_slices()) {
        for (la_ifg_id_t ifg = 0; ifg < NUM_IFGS_PER_SLICE; ifg++) {
            ...
            case la_slice_mode_e::CARRIER_FABRIC: {
                la_status status = configure_pdoq_oq_ifc_mapping_fabric(sid, ifg);
                return_on_error(status);
                break;
            }
...
{
    la_uint_t oq = ifg * NUM_OQ_PER_IFG;
...
la_status
la_device_impl::configure_pdoq_oq_ifc_mapping_fabric(la_slice_id_t sid, la_ifg_id_t ifg)
{
    // On a fabric slice, each TX port is 100G so it uses a pair of PIFs (serdeses), so nothing will point to an odd PIF. So the
    // config should be such that in even IFG:
    //  OQ=[0..7]   => pif = 0
    //  OQ=[8..15]  => pif = 2
    //  ...
    //  OQ=[64..71] => pif = 16
    ...
    //  OQ=[160..167]   => pif = 0
    //  ...
    //  OQ=[160+64..160+71] => pif = 16
    //
    // In la_device_property_e::LC_56_FABRIC_PORT_MODE, slice/IFG (3/0) and (5/1) logically have one more fabric port
    // (two more serdeses), so
    // slice/IFG (3/0) will have
    //  OQ=[72..79]   =>  pif 19
    // and slice/IFG (5/1) will have
    //  OQ=[160+72..160+79]   =>  pif 19

    la_uint_t oq = ifg * NUM_OQ_PER_IFG;
    size_t num_fabric_ports = num_fabric_ports_in_borrower_ifg(sid, ifg);
    la_uint_t fcn_profile = 0;

    for (size_t fabric_port_num = 0; fabric_port_num < num_fabric_ports; fabric_port_num++) {
        la_uint_t pif;

        if (fabric_port_num < NUM_FABRIC_PORTS_IN_NORMAL_IFG) {
            pif = fabric_port_num * NUM_SERDES_PER_FABRIC_PORT;
        } else {
            pif = IFG_BORROWED_SERDES_ID;
        }
        ...
        for (la_uint_t tc = 0; tc < NUM_TC_CLASSES; tc++) {
            ...
            key.dest_oq = oq;
            // FCN counters is used only for instrumentation. Configure to align to HW init.
            value.payloads.pdoq_oq_ifc_mapping_result.fcn_profile = fcn_profile % 4;

            value.payloads.pdoq_oq_ifc_mapping_result.txpp_map_data.raw = oq % NUM_OQ_PER_IFG;
            value.payloads.pdoq_oq_ifc_mapping_result.dest_pif = pif;
...
la_device_impl::configure_reassembly_source_port_map_table()
{
    for (la_slice_ifg slice_ifg : get_used_ifgs()) {
        ...
        for (la_uint_t serdes = 0; serdes < serdes_count + NUM_INTERNAL_IFCS_PER_IFG; serdes++) {
            const auto& table(m_tables.reassembly_source_port_map_table[slice_ifg.slice]);
            ...
            v.payloads.reassembly_source_port_map_result.tm_ifc = slice_ifg.ifg * tm_utils::IFG_SYSTEM_PORT_SCHEDULERS + serdes;
...
{
    sbif_device_id_status_reg_register dev_id_reg;
    bzero(&dev_id_reg, sbif_device_id_status_reg_register::SIZE);

    la_status status = m_ll_device->read_register(*m_pacific_tree->sbif->device_id_status_reg, dev_id_reg);
    return_on_error(status);

    out_dev_info.family = m_ll_device->get_device_revision();
    out_dev_info.extension = dev_id_reg.fields.device_id_ext;
    out_dev_info.revision = dev_id_reg.fields.device_id_rev_num;
    out_dev_info.part_num = dev_id_reg.fields.device_id_part_num;
...
la_device_impl::get_fabric_mac_ports_mode(fabric_mac_ports_mode_e& out_fabric_mac_ports_mode) const
{
    ...
    out_fabric_mac_ports_mode = fabric_mac_ports_mode_e::E_2x50;
...
la_device_impl::set_fabric_mac_ports_mode(fabric_mac_ports_mode_e fabric_mac_ports_mode)
{
    ...
    if (fabric_mac_ports_mode != fabric_mac_ports_mode_e::E_2x50) {
        return LA_STATUS_EINVAL;
    }
...
la_device_impl::set_is_fabric_time_master(bool is_master)
{
    ...
    fte_fabric_time_force_reg_register reg = {.u8 = {0}};
    reg.fields.fabric_time_sync_force_value = is_master;
    reg.fields.fabric_time_force_value = 0x000ffffff;

    la_status status = m_ll_device->write_register(*m_pacific_tree->dmc->fte->fabric_time_force_reg, reg);
    return_on_error(status);
...
la_device_impl::get_fabric_time_sync_status(bool& out_sync_status) const
{
    ...
    fte_time_status_reg_register reg;

    la_status status = m_ll_device->read_register(*m_pacific_tree->dmc->fte->time_status_reg, reg);
...
{
    la_status stat = m_slice_id_manager->is_slice_ifg_valid(slice_id, ifg_id);
    return_on_error(stat);
                                     ...
                                     std::vector<la_uint_t> serdes_mapping_vec)
{
    ...
    for (size_t serdes_id = 0; serdes_id < serdes_count; serdes_id++) {
        ...
        if (tmp_vec[mapped_serdes]) {
            ...
            log_err(HLD, "%s: serdes_id=%ld, mapped_serdes=%ld, already in use", __func__, mapped_serdes, serdes_count);
            return LA_STATUS_EINVAL;
        }
...
{
    return serdes_idx; // in Pacific each serdes represent 1 pif
}
...
la_device_impl::do_destroy(const la_object_wptr& object)
{
    ...
    case la_object::object_type_e::ETHERNET_PORT: {
        auto port = object.weak_ptr_static_cast<la_ethernet_port_pacific>();
    ...
    case la_object::object_type_e::L2_SERVICE_PORT: {
        auto port = object.weak_ptr_static_cast<la_l2_service_port_pacific>();
    ...
    case la_object::object_type_e::VOQ_SET: {
        ...
        status = destroy_voq_set(set, false /*ignore_active_not_empty*/);
    } break;
    ...
    case la_object::object_type_e::VXLAN_NEXT_HOP: {
        auto hop = object.weak_ptr_static_cast<la_vxlan_next_hop_pacific>();
...
{
    return m_device_port_handler ? m_device_port_handler->get_valid_configs(out_config_vec) : LA_STATUS_ENOTINITIALIZED;
}
                                ...
                                la_mac_port*& out_mac_port)
{
    ...
    auto mac_port = std::make_shared<la_mac_port_pacific>(shared_from_this());
    la_object_id_t oid;
    la_status status = register_object(mac_port, oid);
    return_on_error(status);
                                       ...
                                       la_mac_port*& out_mac_port)
{
    ...
    if (!m_fabric_ports_initialized) {
        la_status status = initialize_fabric_ifgb(fc_mode);
        return_on_error(status);

        if (m_revision == la_device_revision_e::PACIFIC_A0) {
            status = apply_fabric_mac_port_workaround(fc_mode);
            return_on_error(status);
        }
...
la_device_impl::create_fabric_port(la_mac_port* fabric_mac_port, la_fabric_port*& out_fabric_port)
{
    ...
    la_uint_t fabric_port_num = fabric_port->get_fabric_port_num();
...
la_device_impl::destroy_fabric_port(const la_fabric_port_impl_wptr& fabric_port)
{
    ...
    auto status = fabric_port->destroy();
    return_on_error(status);
...
la_device_impl::create_pci_port(la_slice_id_t slice_id, la_ifg_id_t ifg_id, bool skip_kernel_driver, la_pci_port*& out_pci_port)
{
    ...
    auto pci_port = std::make_shared<la_pci_port_pacific>(shared_from_this(), skip_kernel_driver);

    la_object_id_t oid;
    la_status status = register_object(pci_port, oid);
    return_on_error(status);
...
la_device_impl::create_recycle_port(la_slice_id_t slice_id, la_ifg_id_t ifg_id, la_recycle_port*& out_recycle_port)
{
    ...
    auto recycle_port = std::make_shared<la_recycle_port_pacific>(shared_from_this());
                                   ...
                                   la_system_port*& out_system_port)
{
    ...
    auto system_port = std::make_shared<la_system_port_pacific>(shared_from_this());
...
la_device_impl::create_spa_port(la_spa_port_gid_t spa_port_gid, la_spa_port*& out_spa_port)
{
    ...
    auto spa_port = std::make_shared<la_spa_port_pacific>(shared_from_this());
    la_object_id_t oid;
                                        ...
                                        la_punt_inject_port*& out_punt_inject_port)
{
    ...
    auto punt_inject_port = std::make_shared<la_punt_inject_port_pacific>(shared_from_this());
                                     ...
                                     la_npu_host_port*& out_npu_host_port)
{
    ...
    auto npu_host_port = std::make_shared<la_npu_host_port_pacific>(shared_from_this());
    la_object_id_t oid;
                                             ...
                                             la_erspan_mirror_command*& out_mirror_cmd)
{
    ...
    auto erspan_mirror_command = std::make_shared<la_erspan_mirror_command_pacific>(shared_from_this());
    la_object_id_t oid;
                                            ...
                                            la_l2_mirror_command*& out_mirror_cmd)
{
    ...
    if ((pi_port != nullptr) && !of_same_device(pi_port, this)) {
        return LA_STATUS_EDIFFERENT_DEVS;
    }

    if ((system_port != nullptr) && !of_same_device(system_port, this)) {
        return LA_STATUS_EDIFFERENT_DEVS;
    }

    // Create and initialize L2 punt/inject destination
    auto l2_mirror_command = std::make_shared<la_l2_mirror_command_pacific>(shared_from_this());
    la_object_id_t oid;
    la_status status = register_object(l2_mirror_command, oid);
    return_on_error(status);

    auto pi_port_impl = get_sptr<la_punt_inject_port_base>(pi_port);
    auto system_port_base = get_sptr<la_system_port_base>(system_port);
    la_meter_set_wcptr meter_wptr = get_sptr(meter);
    auto non_const_meter_wptr = meter_wptr.weak_ptr_const_cast<la_meter_set>();
    status = l2_mirror_command->initialize(
        oid, mirror_gid, pi_port_impl, system_port_base, mac_addr, vlan_tag, voq_offset, non_const_meter_wptr, probability);
    if (status != LA_STATUS_SUCCESS) {
        deregister_object(oid);
        return status;
    }
                                         ...
                                         la_l2_mirror_command_wptr& out_mirror_cmd)
{
    ...
    if (npu_host_port->get_device() != this) {
        return LA_STATUS_EDIFFERENT_DEVS;
    }

    // Create and initialize NPU host destination
    auto l2_mirror_command = std::make_shared<la_l2_mirror_command_pacific>(shared_from_this());
    la_object_id_t oid;
    la_status status = register_object(l2_mirror_command, oid);
    return_on_error(status);

    status = l2_mirror_command->initialize(oid, mirror_gid, npu_host_port, voq_offset, probability);
                                         ...
                                         la_l2_mirror_command*& out_mirror_cmd)
{
    ...
    la_ethernet_port_pacific* eth_port_impl = static_cast<la_ethernet_port_pacific*>(eth_port);
    la_system_port_base* system_port_base = static_cast<la_system_port_base*>(system_port);
                                                    ...
                                                    la_traffic_class_t tc)
{
    ...
    destination_id sp_destination = RX_NOT_CNT_DROP_DSP;
                                          ...
                                          la_traffic_class_t tc)
{
    ...
    if (destination != nullptr) {
        ...
        npl_punt_nw_encap_type_e encap_type;
        ...
        if (destination->type() == la_object::object_type_e::NPU_HOST_DESTINATION) {
            // If its an L2 control trap and its destined to the NPU host, assume its handled by PFC.
            ...
            encap_type = NPL_PUNT_NW_NPU_HOST_ENCAP_TYPE;
        } else {
            punt_dest_impl = destination.weak_ptr_static_cast<const la_l2_punt_destination_impl>();
            encap_ptr = punt_dest_impl->get_gid();
            encap_type = NPL_PUNT_NW_ETH_ENCAP_TYPE;
        }

        status = configure_redirect_code(
            redirect_code, disable_snoop, is_l3_trap, counter_or_meter, sp_destination, encap_type, encap_ptr, overwrite_phb, tc);
        return_on_error(status);
    ...
    if ((counter_or_meter != nullptr) && (counter_or_meter->type() == la_object::object_type_e::METER_SET)) {
        ...
        if (meter_impl->get_type() == la_meter_set::type_e::STATISTICAL) {
            const auto s_meter_set_impl = meter_impl.weak_ptr_static_cast<const la_meter_set_statistical_impl>();
...
la_device_impl::do_clear_trap_configuration(la_event_e trap)
{
    ...
    if (is_oam_trap(trap)) {
        ...
        // Clear RX OBM Table for Ingress/Egress sources
        status = clear_rx_obm_punt_src_and_code(redirect_code, NPL_PUNT_SRC_INGRESS_TRAP);
        return_on_error(status);

        status = clear_rx_obm_punt_src_and_code(redirect_code, NPL_PUNT_SRC_EGRESS_TRAP);
        return_on_error(status);
    }
                                     ...
                                     la_ethernet_port*& out_ethernet_port)
{
    ...
    auto ethernet_port = std::make_shared<la_ethernet_port_pacific>(shared_from_this());
    la_object_id_t oid;
...
la_status
la_device_impl::destroy_ethernet_port(const la_ethernet_port_pacific_wptr& ethernet_port)
{
                                          ...
                                          la_l2_service_port*& out_l2_service_port)
{
    ...
    const la_ethernet_port_pacific* ethernet_port_impl = static_cast<const la_ethernet_port_pacific*>(ethernet_port);
    ...
    auto service_port_impl = std::make_shared<la_l2_service_port_pacific>(shared_from_this());
    la_object_id_t oid;
...
la_device_impl::create_rate_limiter(silicon_one::la_system_port* system_port, la_rate_limiter_set*& out_rate_limiter_set)
{
    ...
    auto rl_impl = std::make_shared<la_rate_limiter_set_pacific>(shared_from_this());
    la_object_id_t oid;
...
la_status
la_device_impl::destroy_l2_service_port(const la_l2_service_port_pacific_wptr& port)
{
    if (port == nullptr) {
        return LA_STATUS_EINVAL;
    }
...
la_device_impl::create_multicast_protection_monitor(la_multicast_protection_monitor*& out_protection_monitor)
{
    ...
    auto protection_monitor = std::make_shared<la_multicast_protection_monitor_base>(this->shared_from_this());
...
la_device_impl::create_stack_port(la_system_port* system_port, la_stack_port*& out_stack_port)
{
    ...
    auto stack_port = std::make_shared<la_stack_port_pacific>(shared_from_this());
    la_object_id_t oid;
                                           ...
                                           la_l2_protection_group*& out_l2_protection_group)
{
    ...
    auto l2_protection_group = std::make_shared<la_l2_protection_group_pacific>(shared_from_this());
                                          ...
                                          la_l2_multicast_group*& out_l2_multicast_group)
{
    ...
    auto mcg = std::make_shared<la_l2_multicast_group_pacific>(shared_from_this());
...
la_device_impl::do_create_ip_multicast_group(la_multicast_group_gid_t multicast_gid,
                                             la_replication_paradigm_e rep_paradigm,
                                             la_ip_multicast_group_pacific_wptr& out_ip_multicast_group)
{
    auto mcg = std::make_shared<la_ip_multicast_group_pacific>(shared_from_this());
                                          ...
                                          la_ip_multicast_group*& out_ip_multicast_group)
{
    ...
    la_ip_multicast_group_pacific_wptr mcg_wptr;
                                     ...
                                     la_prefix_object*& out_prefix_object)
{
    ...
    auto pfx_obj = std::make_shared<la_prefix_object_pacific>(shared_from_this());
...
la_device_impl::get_asbr_lsp(const la_prefix_object* asbr, const la_l3_destination* destination, la_asbr_lsp*& out_asbr_lsp)
{
    ...
    auto asbr_lsp_map_entry_it = m_asbr_lsp_map.find(std::make_pair(asbr_sp, destination_sp));
    if (asbr_lsp_map_entry_it == m_asbr_lsp_map.end()) {
        return LA_STATUS_ENOTFOUND;
    }
...
la_device_impl::destroy_asbr_lsp(const la_asbr_lsp_impl_wptr& asbr_lsp)
{
    ...
    const auto& asbr = get_sptr(asbr_lsp->get_asbr());
    const auto& destination = get_sptr(asbr_lsp->get_destination());
    m_asbr_lsp_map.erase(std::make_pair(asbr, destination));
...
{
    start_api_call("max_pps_percent=", max_pps_percent);

    size_t period = 100;
    size_t bubble_length = 0;

    if ((m_revision != la_device_revision_e::PACIFIC_B0) && (m_revision != la_device_revision_e::PACIFIC_B1)) {
        // Supported only on Pacific B0 or B1
        return LA_STATUS_SUCCESS;
    }

    if (max_pps_percent < 0 || max_pps_percent > 1) {
        return LA_STATUS_EINVAL;
    }

    if (max_pps_percent < 1) {
        // calculate period (12b) and bubble (4b)
        la_float_t bubble_percent = 1 - max_pps_percent;

        float best_ceil_diff = 1;
        for (la_uint_t bubble = 15; (bubble > 0) && (best_ceil_diff != 0); bubble--) {
            float period_f = (float)bubble / bubble_percent;
            float ceil_diff = (ceil(period_f) - period_f) / period_f;
            if (best_ceil_diff > ceil_diff) {
                best_ceil_diff = ceil_diff;
                bubble_length = bubble;
                period = (int)ceil(period_f);
            }
        }
    }

    log_debug(HLD, "%s: Set Max RxPP PPS %.2f: period %zd, bubble %zd", __func__, max_pps_percent, period, bubble_length);

    for (la_slice_id_t sid : get_used_slices()) {
        if (!is_network_slice(sid)) {
            // Controls only Network slices
            continue;
        }

        bit_vector spare_reg;
        la_status stat = m_ll_device->read_register(*m_pacific_tree->slice[sid]->npu->rxpp_term->rxpp_term->spare_reg, spare_reg);
        return_on_error(stat);

        spare_reg.set_bits(67, 64, bubble_length);
        spare_reg.set_bits(79, 68, period);

        stat = m_ll_device->write_register(*m_pacific_tree->slice[sid]->npu->rxpp_term->rxpp_term->spare_reg, spare_reg);
        return_on_error(stat);
    }
...
la_device_impl::get_ifg_maximum_pps_utilization(la_float_t& out_max_pps_percent) const
{
    ...
    bit_vector spare_reg;
    la_slice_id_t sid = 0; // TODO get the first active slice's id from the slice id manager
    la_status stat = m_ll_device->read_register(*m_pacific_tree->slice[sid]->npu->rxpp_term->rxpp_term->spare_reg, spare_reg);
    return_on_error(stat);

    uint64_t bubble_length = spare_reg.bits(67, 64).get_value();
    uint64_t period = spare_reg.bits(79, 68).get_value();

    if (period == 0) {
        return LA_STATUS_SUCCESS;
    }
    out_max_pps_percent = 1 - (la_float_t)bubble_length / period;
                               ...
                               la_voq_set*& out_voq_set)
{
    ...
    // Verify arguments
    if ((dest_device >= MAX_DEVICES) || (dest_slice >= ASIC_MAX_SLICES_PER_DEVICE_NUM) || (dest_ifg >= NUM_IFGS_PER_SLICE)) {
...
la_status
la_device_impl::destroy_voq_set(const la_voq_set_impl_wptr& voq_set, bool ignore_active_not_empty)
{
    if (voq_set == nullptr) {
        return LA_STATUS_EINVAL;
    }
    ...
    la_voq_gid_t base_voq_id = voq_set->get_base_voq_id();
    la_status status = voq_set->destroy(ignore_active_not_empty);
    return_on_error(status);
...
la_device_impl::remove_egress_multicast_slice_replication_voq_set_from_filb_table(la_slice_id_t dest_slice)
{
    ...
    for (la_slice_id_t table_slice_id : get_used_slices()) {
        if (!is_network_slice(table_slice_id)) {
            continue;
        }
...
la_device_impl::get_system_recycle_port(la_multicast_group_gid_t smcid, la_system_port_wcptr& out_sys_recycle_port)
{
    ...
    if ((m_revision == la_device_revision_e::PACIFIC_A0) || (m_revision == la_device_revision_e::PACIFIC_B0)) {
        // A0 and B0 boards only have 1 recycle port configured on slice 1
        ifg_idx = m_slice_id_manager->slice_ifg_2_global_ifg(1, 0);
    } else {
        // B1 boards have 6 recycle ports configured, 1 per IFG, per slice
        switch (smcid) {
...
la_device_impl::allocate_punt_recycle_port(const la_system_port_base_wcptr& target_port)
{
    ...
    if (target_port->get_port_type() == la_system_port_base::port_type_e::PCI) {
        ...
        possible_ifgs.push_back({.slice = target_port->get_slice(), .ifg = RCYCLE_PORT_IFG});
...
la_device_impl::create_multicast_scale_reserved_groups()
{
    ...
    if (m_device_mode == device_mode_e::LINECARD) {
        ...
        for (auto smcid : reserved_smcids) {
            la_ip_multicast_group_pacific_wptr ip_mcg;
...
la_device_impl::configure_tr_lc_sa_configuration_registers()
{
    ...
    for (la_slice_id_t slice : get_used_slices()) {
        reg_val_list.push_back({(*m_pacific_tree->slice_pair[slice / 2]->rx_pdr->tr_lc_sa_configurations_reg1)[slice % 2], reg1});
        reg_val_list.push_back({(*m_pacific_tree->slice_pair[slice / 2]->rx_pdr->tr_lc_sa_configurations_reg2)[slice % 2], reg2});
        reg_val_list.push_back({(*m_pacific_tree->slice_pair[slice / 2]->rx_pdr->tr_lc_sa_configurations_reg3)[slice % 2], reg3});
        reg_val_list.push_back({(*m_pacific_tree->slice_pair[slice / 2]->rx_pdr->tr_lc_sa_configurations_reg4)[slice % 2], reg4});
    }
...
{
    for (la_slice_id_t slice : get_used_slices()) {
        for (size_t engine = 0; engine < NUM_NP_ENGINES_PER_SLICE; engine++) {
            rxpp_fwd_ipv4_error_checks_register reg;

            la_status status
                = m_ll_device->read_register(*m_pacific_tree->slice[slice]->npu->rxpp_term->fi_eng[engine]->ipv4_error_checks, reg);
            return_on_error(status);

            reg.fields.check_ipv4_ihl_error = 0;

            status = m_ll_device->write_register(*m_pacific_tree->slice[slice]->npu->rxpp_term->fi_eng[engine]->ipv4_error_checks,
                                                 reg);
            return_on_error(status);
        }
    }
...
la_device_impl::check_vxrq_dup_range(la_slice_id_t dest_slice, const la_voq_set_wcptr& voq_set_in)
{
    ...
    for (la_slice_id_t src_slice : get_used_slices()) {
        if (!is_network_slice(src_slice)) {
            // skip fabirc slices as only 1 slice is used for multicast
            // traffic and is programmed separately.
            continue;
        }
                                ...
                                la_next_hop*& out_next_hop)
{
    ...
    auto next_hop = std::make_shared<la_next_hop_pacific>(shared_from_this());
                                      ...
                                      la_vxlan_next_hop*& out_vxlan_next_hop)
{
    ...
    auto vxlan_next_hop = std::make_shared<la_vxlan_next_hop_pacific>(shared_from_this());
...
la_status
la_device_impl::destroy_vxlan_next_hop(const la_vxlan_next_hop_pacific_wptr& vxlan_next_hop)
{
                                ...
                                la_svi_port*& out_svi_port)
{
    ...
    auto svi = std::make_shared<la_svi_port_pacific>(shared_from_this());
...
la_status
la_device_impl::warm_boot_post_save()
{
    return LA_STATUS_SUCCESS;
}
...
la_device_impl::get_acl_scaled_enabled(la_slice_pair_id_t slice_pair, la_acl_id_t acl_id, bool& out_enabled)
{
    ...
    k.scaled_ipv4_sec_lv_acl_id = NPL_FWD_INGRESS_ACL_DB_IPV4;
    la_status status = m_tables.scaled_acl_key_type_select_compound_table[slice_pair]->lookup(k, e);
    if (status != LA_STATUS_SUCCESS) {
        return status;
    }
                                       ...
                                       la_acl_key_profile*& out_acl_key_profile)
{
    ...
    auto acl_key_profile = std::make_shared<la_acl_key_profile_pacific>(shared_from_this());
                                    ...
                                    la_acl*& out_acl)
{
    ...
    la_object_id_t oid;
    la_status status = register_object(acl_impl, oid);
    if (status != LA_STATUS_SUCCESS) {
        return status;
    }
...
{
    start_api_call("");
    auto acl_group = std::make_shared<la_acl_group_pacific>(shared_from_this());
...
la_device_impl::destroy_acl_group(const la_acl_group_wptr& acl_group)
{
    ...
    la_acl_group_base* acl_group_pacific = static_cast<la_acl_group_base*>(acl_group.get());
    la_status status = acl_group_pacific->destroy();
    return_on_error(status);
...
la_device_impl::create_copc(la_control_plane_classifier::type_e type, la_control_plane_classifier*& out_copc)
{
    ...
    auto copc = std::make_shared<la_copc_pacific>(shared_from_this());
...
la_device_impl::create_voq_cgm_profile(la_voq_cgm_profile*& out_profile)
{
    ...
    out_profile = voq_cgm_profile.get();
...
la_device_impl::set_rx_pdr_sms_bytes_drop_thresholds(const la_rx_pdr_sms_bytes_drop_thresholds& thresholds)
{
    ...
    auto counter_thresholds_reg_2 = m_pacific_tree->rx_pdr->counters_thresholds_reg2;
    rx_pdr_counters_thresholds_reg2_register reg;
...
la_device_impl::configure_default_rx_cgm_sq_profile()
{
    ...
    status = default_profile->initialize(oid, true /* default profile */);
    if (status != LA_STATUS_SUCCESS) {
        deregister_object(oid);
                                                      ...
                                                      const la_tx_cgm_oq_profile_thresholds& thresholds)
{
    ...
    auto uc_oq_profile_mem = m_pacific_tree->slice[slice]->tx->cgm->uc_oq_profile;
    txcgm_uc_oq_profile_memory mem;
...
{
    if (m_revision != la_device_revision_e::PACIFIC_B0) {
        return LA_STATUS_EINVAL;
    }

    for (size_t recycle_code = 0; recycle_code < 2; recycle_code++) {
        for (size_t recycle_data = 0; recycle_data < 16; recycle_data++) {
            npl_recycle_override_table_t::key_type k;
            npl_recycle_override_table_t::value_type v;
            npl_recycle_override_table_t::entry_pointer_type e = nullptr;

            k.rxpp_npu_input_rcy_code_1_ = recycle_code;
            k.packet_is_rescheduled_recycle = 1;
            k.rxpp_npu_input_tx_to_rx_rcy_data_3_0_ = recycle_data;
            v.action = NPL_RECYCLE_OVERRIDE_TABLE_ACTION_INIT_RX_DATA;
            v.payloads.init_rx_data.override_source_port_table = 0;
            v.payloads.init_rx_data.initial_layer_index = 0;
            v.payloads.init_rx_data.initial_rx_data.init_recycle_fields.initial_is_rcy_if = 1;
            v.payloads.init_rx_data.tag_swap_cmd = NPL_NO_TAG_SWAP;
            v.payloads.init_rx_data.np_macro_id = NPL_NETWORK_RX_MAC_AF_AND_TERMINATION_MACRO;
            ...
            v.payloads.init_rx_data.initial_rx_data.init_recycle_fields.init_data = init_data; /* Initialize init_data with '0' */
            v.payloads.init_rx_data.initial_rx_data.init_recycle_fields.init_data.initial_slice_id = slice_id;
            stat = m_tables.recycle_override_table[slice_id]->set(k, v, e);
            return_on_error(stat);
        }
    }
...
la_device_impl::get_limit(limit_type_e limit_type, la_uint64_t& out_limit) const
{
    ...
    case limit_type_e::DEVICE__MAX_SMS_BYTES_QUANTIZATION_THRESHOLD: {
        ...
        la_uint64_t num_buffers = (1 << pdvoq_shared_mma_cgm_pool_available_region_register::fields::UC_REGION0_WIDTH) - 1;
        out_limit = num_buffers * buffer_size;
        break;
    }
    case limit_type_e::DEVICE__MAX_SMS_PACKETS_QUANTIZATION_THRESHOLD:
        out_limit = (1 << rx_pdr_counters_thresholds_reg1_register::fields::VOQ_CGM_COUNTER_A_THR0_WIDTH) - 1;
        break;
    ...
    case limit_type_e::DEVICE__MAX_HBM_POOL_BYTES_QUANTIZATION_THRESHOLD:
        out_limit = ((1 << hmc_cgm_quant_thresholds_register::fields::SHARED_POOL0_TH_WIDTH) - 1)
    ...
    case limit_type_e::DEVICE__NUM_LOCAL_VOQS:
        // The last VOQ is used by the HW for invalid indication. Reserve the whole native VOQ set size.
        out_limit = MAX_VOQS_PER_SLICE_IN_STANDALONE_DEVICE - NATIVE_VOQ_SET_SIZE;
        break;
    case limit_type_e::DEVICE__NUM_SYSTEM_VOQS:
        // The last VOQ is used by the HW for invalid indication. Reserve the whole native VOQ set size.
        out_limit = MAX_VOQS_PER_SLICE_IN_LINECARD_DEVICE - NATIVE_VOQ_SET_SIZE;
        break;
    ...
    case limit_type_e::DEVICE__MAX_ALLOCATABLE_VSC:
        if (m_device_mode == device_mode_e::INVALID) {
            ...
            out_limit = MAX_VSCS_PER_IFG_IN_STANDALONE_DEVICE;
        } else if (m_device_mode == device_mode_e::LINECARD) {
            out_limit = MAX_VSCS_PER_IFG_IN_LINECARD_DEVICE;
        } else {
            return LA_STATUS_EINVAL;
        }
        break;
    ...
    case limit_type_e::METER_PROFILE__MAX_BURST_SIZE:
        ...
        break;
    case limit_type_e::METER_PROFILE__MAX_PPS_BURST_SIZE:
        // PPS burst-size in B1 is divided by a factor
        {
            size_t max_bucket_size = ((1 << la_meter_profile_impl::BURST_SIZE_WIDTH) - 1)
                                     / get_meter_cir_eir_factor(la_meter_set::type_e::STATISTICAL);
            size_t max_update_chunk_size = la_meter_profile_impl::TOKEN_RESOLUTION * la_meter_profile_impl::TOKEN_PARTS;
            out_limit = max_bucket_size - max_update_chunk_size;
        }
        break;
...
la_device_impl::get_precision(la_precision_type_e precision_type, double& out_precision) const
{
    ...
    case la_precision_type_e::METER_PROFILE__EBS_RESOLUTION:
        ...
        break;
    case la_precision_type_e::METER_PROFILE__STATISTICAL_METER_CBS_RESOLUTION:
        out_precision = la_meter_profile_impl::CBS_RESOLUTION / get_meter_cir_eir_factor(la_meter_set::type_e::STATISTICAL);
        break;
    case la_precision_type_e::METER_PROFILE__STATISTICAL_METER_EBS_RESOLUTION:
        out_precision = la_meter_profile_impl::EBS_OR_PBS_RESOLUTION / get_meter_cir_eir_factor(la_meter_set::type_e::STATISTICAL);
        break;
...
la_device_impl::get_int_property(la_device_property_e device_property, int& out_property_value) const
{
    ...
    if (m_init_phase == init_phase_e::CREATED) {
        if (device_property == la_device_property_e::EFUSE_REFCLK_SETTINGS) {
            log_err(HLD,
                    "Property cannot be retrieved in this device phase %s (device_property %s)",
                    silicon_one::to_string(m_init_phase).c_str(),
                    silicon_one::to_string(device_property).c_str());
            return LA_STATUS_EINVAL;
        }
...
la_device_impl::is_allow_modify_property_at_phase(la_device_property_e device_property) const
{
    if (m_init_phase != init_phase_e::CREATED) {
        if ((device_property == la_device_property_e::ENABLE_HBM)
            ...
            || (device_property == la_device_property_e::AAPL_IFG_DELAY_BEFORE_EXEC)
            || (device_property == la_device_property_e::AAPL_HBM_DELAY_BEFORE_EXEC)
            || (device_property == la_device_property_e::AAPL_IFG_DELAY_BEFORE_POLL)
            || (device_property == la_device_property_e::AAPL_HBM_DELAY_BEFORE_POLL)
            || (device_property == la_device_property_e::LPM_TCAM_BANK_SIZE)
            || (device_property == la_device_property_e::LPM_TCAM_NUM_BANKSETS)
            || (device_property == la_device_property_e::AAPL_IFG_DELAY_IN_POLL)
            || (device_property == la_device_property_e::AAPL_IFG_POLL_TIMEOUT)
    ...
    if (m_init_phase == init_phase_e::TOPOLOGY) {
        if ((device_property == la_device_property_e::LC_FORCE_FORWARD_THROUGH_FABRIC_MODE)
            ...
            || (device_property == la_device_property_e::DEV_REFCLK_SEL)
                                   ...
                                   la_bfd_session*& out_bfd_session)
{
    ...
    auto session = std::make_shared<la_bfd_session_pacific>(shared_from_this());
...
la_device_impl::destroy()
{
    ...
    m_arc_hdlr->reset_arc_cpus();

    destroy_aapl_handlers();
...
la_device_impl::get_ifg_aapl_handler(la_slice_id_t slice_id, la_ifg_id_t ifg_id, Aapl_t*& out_aapl)
{
    ...
    // If we are called for the 1st time, AAPL handler will get allocated and initialized.
    // This requires HW access, which is disabled during reconnect.
    if (m_reconnect_handler->is_reconnect_in_progress()) {
        log_err(HLD, "%s: reconnect in progress", __func__);
    ...
    stat = m_serdes_device_handler->get_ifg_aapl_handler(slice_id, ifg_id, out_aapl);
    return_on_error(stat);
...
la_device_impl::get_native_sbus_aapl_handler(la_slice_id_t slice_id, la_ifg_id_t ifg_id, Aapl_t*& out_aapl)
{
    ...
    stat = m_serdes_device_handler->get_native_sbus_aapl_handler(slice_id, ifg_id, out_aapl);
    return_on_error(stat);
...
{
    if (m_pci_aapl_handler.handler == nullptr) {
        Aapl_t* aapl_handler = aapl_construct(); /* Create AAPL struct */
        std::shared_ptr<la_aapl_user_pci> aapl_user = std::make_shared<la_aapl_user_pci>(shared_from_this());

        silicon_one::aapl_client_data_struct<la_aapl_user>* pci_struct = new silicon_one::aapl_client_data_struct<la_aapl_user>();
        pci_struct->default_ptr = std::static_pointer_cast<la_aapl_user>(aapl_user);
        aapl_bind_set(aapl_handler, pci_struct);
        ...
        m_pci_aapl_handler.handler = aapl_handler;
        m_pci_aapl_handler.user = aapl_user;
    }

    out_aapl = m_pci_aapl_handler.handler;
...
la_device_impl::get_hbm_aapl_handler(size_t hbm_interface, Aapl_t*& out_aapl)
{
    ...
    if (m_hbm_aapl_handlers[hbm_interface].handler == nullptr) {
        Aapl_t* aapl_handler = aapl_construct(); /* Create AAPL struct */
        std::shared_ptr<la_aapl_user_hbm> aapl_user = std::make_shared<la_aapl_user_hbm>(shared_from_this(), hbm_interface);

        silicon_one::aapl_client_data_struct<la_aapl_user>* hbm_struct = new silicon_one::aapl_client_data_struct<la_aapl_user>();
        hbm_struct->default_ptr = std::static_pointer_cast<la_aapl_user>(aapl_user);
        aapl_bind_set(aapl_handler, hbm_struct);
        ...
        aapl_handler->chips = 1;
        aapl_handler->suppress_errors = 1;
        aapl_get_ip_info(aapl_handler, 0);
        aapl_handler->suppress_errors = 0;

        m_hbm_aapl_handlers[hbm_interface].user = aapl_user;
        m_hbm_aapl_handlers[hbm_interface].handler = aapl_handler;
    }

    out_aapl = m_hbm_aapl_handlers[hbm_interface].handler;

    return LA_STATUS_SUCCESS;
}

la_status
la_device_impl::destroy_aapl_handlers()
{
    if (m_pci_aapl_handler.handler != nullptr) {
        aapl_close_connection(m_pci_aapl_handler.handler);
        silicon_one::aapl_client_data_struct<la_aapl_user>* get_client_data_ptr
            = static_cast<silicon_one::aapl_client_data_struct<la_aapl_user>*>(aapl_bind_get(m_pci_aapl_handler.handler));
        get_client_data_ptr->default_ptr.reset();
        get_client_data_ptr->log_buffer.reset();
        delete static_cast<silicon_one::aapl_client_data_struct<la_aapl_user>*>(aapl_bind_get(m_pci_aapl_handler.handler));
        aapl_bind_set(m_pci_aapl_handler.handler, nullptr);
        aapl_destruct(m_pci_aapl_handler.handler);
        m_pci_aapl_handler.handler = nullptr;
        m_pci_aapl_handler.user = nullptr;
    }

    for (size_t hbm_interface = 0; hbm_interface < NUM_HBM_INTERFACES; hbm_interface++) {
        if (m_hbm_aapl_handlers[hbm_interface].handler != nullptr) {
            aapl_close_connection(m_hbm_aapl_handlers[hbm_interface].handler);
            silicon_one::aapl_client_data_struct<la_aapl_user>* get_client_data_ptr
                = static_cast<silicon_one::aapl_client_data_struct<la_aapl_user>*>(
                    aapl_bind_get(m_hbm_aapl_handlers[hbm_interface].handler));
            get_client_data_ptr->default_ptr.reset();
            get_client_data_ptr->log_buffer.reset();
            delete static_cast<silicon_one::aapl_client_data_struct<la_aapl_user>*>(
                aapl_bind_get(m_hbm_aapl_handlers[hbm_interface].handler));
            aapl_bind_set(m_hbm_aapl_handlers[hbm_interface].handler, nullptr);
            aapl_destruct(m_hbm_aapl_handlers[hbm_interface].handler);
            m_hbm_aapl_handlers[hbm_interface].handler = nullptr;
            m_hbm_aapl_handlers[hbm_interface].user = nullptr;
        }
    }
...
{
    out_cpu2jtag = m_cpu2jtag_handler.get();
...
la_device_impl::do_flush() const
{
    ...
    return m_ll_device->read_register(*(*m_pacific_tree->cdb->top->access_reg)[36], dummy_bv);
}
...
la_device_impl::configure_fabric_tm_headers_table()
{
    ...
    status = configure_fabric_tm_headers_table_prefix_lsb_entries(k, v, NPL_TX_CUD_RESERVED_PREFIX, NPL_TX_CUD_RESERVED_PREFIX_LEN);
    return_on_error(status);
...
la_device_impl::initialize_device_mode()
{
    ...
    for (la_slice_id_t sid : get_used_slices()) {
        bool _is_network_slice = (m_slice_mode[sid] == la_slice_mode_e::NETWORK);
        bool _is_fabric_slice = (m_slice_mode[sid] == la_slice_mode_e::CARRIER_FABRIC);
        bool _is_udc_slice = (m_slice_mode[sid] == la_slice_mode_e::UDC);

        if (!is_network_slice(sid) && (_is_fabric_slice == false)) {
            log_err(HLD, "%s: unexpected slice mode=%s", __func__, silicon_one::to_string(m_slice_mode[sid]).c_str());
            return LA_STATUS_ENOTIMPLEMENTED;
        }

        has_network_slice |= _is_network_slice;
        has_fabric_slice |= _is_fabric_slice;
        has_udc_slice |= _is_udc_slice;
    }
...
la_device_impl::device_mode_optimized_storage_initialization()
{
    if (m_device_mode != device_mode_e::STANDALONE) {
        ...
        if (m_device_mode == device_mode_e::FABRIC_ELEMENT) {
            m_lc_to_min_links.resize(MAX_DEVICES, DEFAULT_MIN_LINKS_THRESHOLD);
    ...
    m_index_generators.voq_cgm_profiles = ranged_index_generator(2, NUM_VOQ_CGM_PROFILES_PER_DEVICE);

    size_t first_stat_meter_index;
    for (size_t i = 0; i < NUM_STATISTICAL_METER_BANKS; i++) {
        // Index ECN_WA_IN_LC_STATISTICAL_METER_OFFSET in LC is reserved in the first two banks (#108 and #109)
        // to the meter used in the ECN workaround.
        if (m_device_mode == device_mode_e::LINECARD && i <= 1) {
            first_stat_meter_index = ECN_WA_IN_LC_STATISTICAL_METER_OFFSET + 1;
        } else {
            first_stat_meter_index = 0;
        }
        m_index_generators.statistical_meter_id[i]
            = ranged_sequential_indices_generator(first_stat_meter_index, la_meter_set_impl::NUM_STATISTICAL_METERS_PER_BANK);
    }
    ...
    for (la_slice_id_t i : m_slice_id_manager->get_all_possible_slices()) {
        ...
        m_index_generators.slice[i].npp_attributes = ranged_index_generator(0, NUM_NPP_ATTRIBUTES_PER_DEVICE);
    }

    for (size_t i : m_slice_id_manager->get_all_possible_slice_pairs()) {
        // Service port SLP-s
    ...
    size_t num_oq_drop_counters = (1 << txcgm_counter_set_map_memory::fields::COUNTER_SET_MAP_DATA_WIDTH);
    for (size_t i : m_slice_id_manager->get_all_possible_slices()) {
...
la_device_impl::initialize_link_up_vector()
{
    ...
    for (int i = 0; i < NUM_FABRIC_PORTS_IN_DEVICE; ++i) {
        value.payloads.link_up_vector_result.link_up[i] = npl_link_state_e::NPL_LINK_STATE_UP;
    }
...
la_device_impl::initialize_all_reachable_vector()
{
    ...
    // value is memset to 0 on construction
...
la_status
la_device_impl::initialize_bitmap_oqg_map_table()
{
    const auto& table(m_tables.bitmap_oqg_map_table);
    npl_bitmap_oqg_map_table_key_t key;
    npl_bitmap_oqg_map_table_value_t value;
    npl_bitmap_oqg_map_table_entry_t* entry;

    value.action = NPL_BITMAP_OQG_MAP_TABLE_ACTION_WRITE;

    size_t entries_nr = m_pacific_tree->slice[0]->tx->pdr->bitmap_oqg_map->get_desc()->entries;
    for (size_t line = 0; line < entries_nr; line++) {
        la_uint_t val = line < 192 ? line : line < 201 ? (line - 192) * 4 : (line - 201) * 4 + 80;

        key.bitmap_oqg_map_index_index = line;
        value.payloads.bitmap_oqg_map_result_oqg_id = val;
...
la_status
la_device_impl::configure_fabric_filb_voq_mapping()
{
    // Configure MS (merge-sort) VOQs -> TX slice. Relevant only on fabric RX-slice.
    // The table resides on an RX slice, and its contents is per TX slice

    for (la_slice_id_t table_slice_id : get_used_slices()) {
        if (m_slice_mode[table_slice_id] != la_slice_mode_e::CARRIER_FABRIC) {
            continue;
        }
        ...
        for (la_slice_id_t dest_slice_id : get_used_slices()) {
            v.payloads.filb_voq_mapping_result.dest_slice = dest_slice_id;
            v.payloads.filb_voq_mapping_result.dest_dev = NPL_LOCAL_DEVICE_ID;
            // Rest of the fields in v.payloads.filb_voq_mapping_result are irrelevant.

            for (size_t fabric_context = 0; fabric_context < NUM_PLB_FABRIC_CONTEXTS; fabric_context++) {
                for (size_t fabric_port_num_in_slice = 0; fabric_port_num_in_slice < MAX_FABRIC_PORTS_IN_SLICE;
                     fabric_port_num_in_slice++) {
                    k.rxpdr_output_voq_nr = (fabric_context * ASIC_MAX_SLICES_PER_DEVICE_NUM * MAX_FABRIC_PORTS_IN_SLICE)
                                            + (dest_slice_id * MAX_FABRIC_PORTS_IN_SLICE) + fabric_port_num_in_slice;
...
la_device_impl::configure_npu_host()
{
...

    {
        ...
        status = m_ll_device->write_register(*m_pacific_tree->npuh->host->mp_ccm_timer, reg);
        return_on_error(status);
    }

    {
        ...
        status = m_ll_device->write_register(*m_pacific_tree->npuh->host->rmep_timer, reg);
        return_on_error(status);
    }
...

    {
        ...
        status = m_ll_device->write_register(*m_pacific_tree->npuh->host->cfg_lri, reg);
        return_on_error(status);
    }
...
{
    // The VOQ CGM profile for drop puposes configuration currently takes place in several functions/files.
    ...
    for (la_quantization_region_t age = 0; age < la_voq_cgm_profile::SMS_NUM_AGE_QUANTIZATION_REGIONS; age++) {
        behavior_val.payloads.voq_cgm_slice_buffers_consumption_lut_for_enq_result.drop_g.drop_green.drop_color[age].value = true;
        behavior_val.payloads.voq_cgm_slice_buffers_consumption_lut_for_enq_result.drop_y.drop_yellow.drop_color[age].value = true;
    }

    behavior_key.profile_id.value = VOQ_CGM_DROP_PROFILE;
    for (size_t buffer_pool_available_level = 0; buffer_pool_available_level < LA_CGM_NUM_SMS_BYTES_QUANTIZATION_REGIONS;
         buffer_pool_available_level++) {
        behavior_key.buffer_pool_available_level = buffer_pool_available_level;
        for (size_t buffer_voq_size_level = 0; buffer_voq_size_level < la_voq_cgm_profile::SMS_NUM_BYTES_QUANTIZATION_REGIONS;
             buffer_voq_size_level++) {
            behavior_key.buffer_voq_size_level = buffer_voq_size_level;
            for (size_t free_dram_cntx = 0; free_dram_cntx < LA_CGM_NUM_HBM_NUMBER_OF_VOQS_QUANTIZATION_REGIONS; free_dram_cntx++) {
                behavior_key.free_dram_cntx = free_dram_cntx;
    ...
    // Input blocking threshold
    ics_slice_set_queue_blocking_th_reg_register reg;
    reg.fields.set_queue_blocking_th = la_voq_cgm_profile_impl::INPUT_BLOCKING_THRESHOLD_DROP;
    for (size_t i = 0; i < array_size(m_pacific_tree->slice); i++) {
        la_status status = m_ll_device->write_register(
            *(*m_pacific_tree->slice[i]->ics->set_queue_blocking_th_reg)[la_device_impl::VOQ_CGM_DROP_PROFILE], reg);
        return_on_error(status);
    }
...
la_device_impl::configure_static_mc_voqs()
{
    ...
    for (la_slice_id_t dest_slice : get_used_slices()) {
        ...
        m_is_builtin_objects[voq_set->oid()] = true; // These VOQs should always be exist.
...
{
    // This workaround is relevant only for B0 and B1.
    if ((m_revision != la_device_revision_e::PACIFIC_B0) && (m_revision != la_device_revision_e::PACIFIC_B1)) {
        return LA_STATUS_SUCCESS;
    }

    // The workaround affects only transmit stage.
    bit_vector spare_reg_val;
    for (la_slice_id_t slice_id : get_used_slices()) {
        for (auto npe : m_pacific_tree->slice[slice_id]->npu->txpp->npe) {

            la_status status = m_ll_device->read_register(*npe->spare_reg, spare_reg_val);

            spare_reg_val.set_bit(48, 1);
            status = m_ll_device->write_register(*npe->spare_reg, spare_reg_val);
            return_on_error(status);

            spare_reg_val.set_bit(48, 0);
            status = m_ll_device->write_register(*npe->spare_reg, spare_reg_val);
            return_on_error(status);
        }
    }
...
la_device_impl::configure_rx_drop_dsp()
{
    ...
    key.fwd_destination_lsb = RX_DROP_SYSTEM_PORT_GID;
    value.payloads.rxpdr_dsp_lookup_table_result.tc_map_profile = DROP_DSP_TC_PROFILE;
...
la_device_impl::configure_rx_not_cnt_drop_dsp()
{
    ...
    key.fwd_destination_lsb = RX_NOT_CNT_DROP_SYSTEM_PORT_GID;
    value.payloads.rxpdr_dsp_lookup_table_result.tc_map_profile = DROP_DSP_TC_PROFILE;
...
la_status
la_device_impl::init_lookup_error_macros_ids()
{
    lld_register_value_list_t reg_val_list;
    bit_vector fwd_spare_reg;
    bit_vector term_spare_reg;
    bit_vector tran_spare_reg;
    la_status status;

    for (size_t sid : get_used_slices()) {
        for (auto& term_npe : m_pacific_tree->slice[sid]->npu->rxpp_term->npe) {
            status = m_ll_device->read_register(*term_npe->spare_reg, term_spare_reg);
            return_on_error(status);
            term_spare_reg.set_bit(0, 0);
            term_spare_reg.set_bits(8, 1, npl_macro_e::NPL_NETWORK_TERMINATION_ERROR_MACRO);
            reg_val_list.push_back({(term_npe->spare_reg), term_spare_reg});
        }
        for (auto& fwd_npe : m_pacific_tree->slice[sid]->npu->rxpp_fwd->npe) {
            status = m_ll_device->read_register(*fwd_npe->spare_reg, fwd_spare_reg);
            return_on_error(status);
            fwd_spare_reg.set_bit(0, 0);
            fwd_spare_reg.set_bits(8, 1, npl_macro_e::NPL_NETWORK_FORWARDING_ERROR_MACRO);
            reg_val_list.push_back({(fwd_npe->spare_reg), fwd_spare_reg});
        }
        for (auto& tran_npe : m_pacific_tree->slice[sid]->npu->txpp->npe) {
            status = m_ll_device->read_register(*tran_npe->spare_reg, tran_spare_reg);
            return_on_error(status);
            tran_spare_reg.set_bit(0, 0);
            tran_spare_reg.set_bits(8, 1, npl_macro_e::NPL_NETWORK_TRANSMIT_ERROR_MACRO);
            reg_val_list.push_back({(tran_npe->spare_reg), tran_spare_reg});
        }
    }
...
la_status
la_device_impl::configure_meters_for_ecn_workaround_in_lc()
{
    if (m_device_mode != device_mode_e::LINECARD) {
        return LA_STATUS_SUCCESS;
    }
    ...
    // We should allocate a statistical meter and configure it to this table.
    // Configuring entry with bank 108 means there is no congestion
    // Configuring entry with bank 109 means there is a congestion.
    // We reserve statistical meter with offset ECN_WA_IN_LC_STATISTICAL_METER_OFFSET in LC.
    const auto& table(m_tables.get_ecm_meter_ptr_table);
    npl_get_ecm_meter_ptr_table_t::key_type key;
    npl_get_ecm_meter_ptr_table_t::value_type value;
    npl_get_ecm_meter_ptr_table_t::entry_pointer_type e = nullptr;
    value.action = NPL_GET_ECM_METER_PTR_TABLE_ACTION_WRITE;
    value.payloads.stat_meter_ptr.update_or_read = 0;
    value.payloads.stat_meter_ptr.cb_set_base = ECN_WA_IN_LC_STATISTICAL_METER_OFFSET;

    key.tm_h_ecn = 0;
    key.tm_h_dp_0 = 0;
    value.payloads.stat_meter_ptr.cb_id = 108;
    table->set(key, value, e);
    key.tm_h_dp_0 = 1;
    table->set(key, value, e);

    key.tm_h_ecn = 1;
    key.tm_h_dp_0 = 0;
    value.payloads.stat_meter_ptr.cb_id = 109;
    table->set(key, value, e);
    key.tm_h_dp_0 = 1;
    table->set(key, value, e);
    ...
    for (la_slice_id_t sid : get_used_slices()) {
        ...
        m_ll_device->read_register(*(*m_pacific_tree->rx_meter->top->global_conf_reg)[sid], rx_meter_smr);
        rx_meter_smr.fields.slice_bypass_rx_meter = 0;
        reg_val_list.push_back({(*m_pacific_tree->rx_meter->top->global_conf_reg)[sid], rx_meter_smr});

        // We change the map of the meter color to preserve the input color (output color = in color), so:
        // all InColor = 0 (green)  --> Meter result = 0
        // all InColor = 1 (yellow) --> Meter result = 1
        // all InColor = 2 (red)    --> Meter result = 2 (Actually,  it won't happen because:
        //                                                  a) Red packets are dropped on the ingress device and
        //                                                  b) on egress device the NPL re-colors all to green/yellow.)
        // Each 32 bits correspond to one InColor.
        rx_meter_global_conf_register3_register gcr3;
        gcr3.fields.slice_stat_meter_result_map_p0 = 0x5555555500000000;
        gcr3.fields.slice_stat_meter_result_map_p1 = 0xAAAAAAAA;
        reg_val_list.push_back({(*m_pacific_tree->rx_meter->top->global_conf_register3)[sid], gcr3});

        // This table contains 256 entries, and the key to access it is:
        // {stat_meters_bank[1:0], stat_meter_decision_map_profile[1:0], exact_meter_to_stat_meter_color[1:0],
        // stat_meter_result[1:0]}
        // Since `stat_meters_bank` can be, in our case, either 0 or 1 (which are stat-meter banks #108 and #109),
        // only the first 128 entries are relevant (although we configure all the 256).
        // The entries 0-63 are relevant to bank 0 (= stat-meter bank #108) which is accessed if no congestion was experienced,
        // while entries 64-127 are relevant to bank 1 (= stat-meter bank #109) which is accessed if a congestion was
        // experienced.
        rx_meter_stat_meter_decision_mapping_table_memory meter_decision_mapping;
        size_t entries_num = (*m_pacific_tree->rx_meter->top->stat_meter_decision_mapping_table)[sid]->get_desc()->entries;
        for (size_t line = 0; line < entries_num; line++) {
            // We need to preserve the packet's color, so the outgoing color for each line should be the 2 lsb bits
            // of the key (stat_meter_result) because this was the packet's color in TX slice of ingress device.
            // We configure all the lines despite that only lines with indices {4*k, 4*k+1} are relevant, because the packet's
            // color can be either 0 (green) or 1 (yellow).
            meter_decision_mapping.fields.stat_meter_decision_outgoing_color = line % 4;
            if (line < 64) { // accessed by bank 108
                meter_decision_mapping.fields.stat_meter_decision_congestion_experienced = 0;
            } else { // accessed by bank 109 .. 111 (only 109 is relevant here)
                meter_decision_mapping.fields.stat_meter_decision_congestion_experienced = 1;
            }
            meter_decision_mapping.fields.stat_meter_decision_meter_drop = 0;
            meter_decision_mapping.fields.stat_meter_decision_cgm_rx_dp = 0;
            meter_decision_mapping.fields.stat_meter_decision_rx_counter_color = 0;
            mem_line_val_list.push_back(
                {{(*m_pacific_tree->rx_meter->top->stat_meter_decision_mapping_table)[sid], line}, meter_decision_mapping});
        }
    }
...
la_status
la_device_impl::prepare_dedicated_vsc_for_voq_flush()
{
    if ((m_device_mode != device_mode_e::LINECARD) && (m_device_mode != device_mode_e::STANDALONE)) {
        return LA_STATUS_SUCCESS;
    }

    // To perform VOQ flush we need a high stream of credit grants. To do it we create dedicated VSCs and configure shapers
    // accordingly.
    // For LC: on fabric slice 5, ifg 0, serdes 0. For SA: on slice 0, ifg 0, serdes 19 (as no recycle port on even slice).
    // A 400G rate of credits is configured.
    // Create OQ scheduler, wich will hold the required VSCs, so it is sufficient to grant credits for a VOQ_SET of 8 VOQs on
    // each NW slice.
    constexpr size_t IFSE_EIR_SHAPE_MODE_BASE = 124;
    la_slice_id_t vsc_slice_id = 0;
    la_ifg_id_t vsc_ifg_id;
    la_uint_t serdes_id = 0;
    la_uint_t tm_port_id = 0;
    lld_register_array_sptr ifse_cir_shaper_rate_configuration = nullptr;
    lld_register_array_sptr ifse_pir_shaper_configuration = nullptr;
    lld_register_scptr ifse_general_configuration = nullptr;
    lld_memory_scptr oq_pir_token_bucket_cfg = nullptr;
    lld_memory_scptr oq_pir_token_bucket = nullptr;
    lld_memory_scptr oqpg_cir_token_bucket_cfg = nullptr;
    lld_memory_scptr oqpg_cir_token_bucket = nullptr;
    lld_register_scptr tpse_oqpg_mapping_configuration = nullptr;

    if (m_device_mode == device_mode_e::LINECARD) {
        vsc_slice_id = LC_VOQ_FLUSH_VSC_SLICE_ID;
        vsc_ifg_id = LC_VOQ_FLUSH_VSC_IFG_ID;
        serdes_id = 0;
        tm_port_id = serdes_id / NUM_SERDES_PER_FABRIC_PORT; // serdes_id = 0, so tm_port_id will be zero as well.
    } else if (m_device_mode == device_mode_e::STANDALONE) {
        vsc_slice_id = SA_VOQ_FLUSH_VSC_SLICE_ID;
        vsc_ifg_id = SA_VOQ_FLUSH_VSC_IFG_ID;
        serdes_id = RECYCLE_SERDES_ID;
        tm_port_id = serdes_id; // In SA tm_port_id == serdes_id.
    }

    // This oqse should be OQ #7 so flush credits will be granted at highest priority.
    la_uint_t oqse_id = serdes_id * tm_utils::NUM_OQS_PER_SYSTEM_PORT_SCH + 7;

    // Initialize registers
    if (vsc_slice_id < FIRST_HW_FABRIC_SLICE) {
        ifse_cir_shaper_rate_configuration
            = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->sch->ifse_cir_shaper_rate_configuration);
        ifse_pir_shaper_configuration = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->sch->ifse_pir_shaper_configuration);
        ifse_general_configuration = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->sch->ifse_general_configuration);
        oq_pir_token_bucket_cfg = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->sch->oq_pir_token_bucket_cfg);
        oq_pir_token_bucket = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->sch->oq_pir_token_bucket);
        oqpg_cir_token_bucket_cfg = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->sch->oqpg_cir_token_bucket_cfg);
        oqpg_cir_token_bucket = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->sch->oqpg_cir_token_bucket);
        tpse_oqpg_mapping_configuration
            = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->sch->tpse_oqpg_mapping_configuration);
    } else {
        ifse_cir_shaper_rate_configuration
            = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->fabric_sch->ifse_cir_shaper_rate_configuration);
        ifse_pir_shaper_configuration
            = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->fabric_sch->ifse_pir_shaper_configuration);
        ifse_general_configuration = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->fabric_sch->ifse_general_configuration);
        oq_pir_token_bucket_cfg = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->fabric_sch->oq_pir_token_bucket_cfg);
        oq_pir_token_bucket = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->fabric_sch->oq_pir_token_bucket);
        oqpg_cir_token_bucket_cfg = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->fabric_sch->oqpg_cir_token_bucket_cfg);
        oqpg_cir_token_bucket = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->fabric_sch->oqpg_cir_token_bucket);
        tpse_oqpg_mapping_configuration
            = (m_pacific_tree->slice[vsc_slice_id]->ifg[vsc_ifg_id]->fabric_sch->tpse_oqpg_mapping_configuration);
    }

    // Set interface credit cir rate to 400G. Code based on: la_interface_scheduler_impl::set_credit_cir(la_rate_t rate)
    la_rate_t rate = 400ULL * UNITS_IN_GIGA;

    ics_slice_credits_conf_reg_register credits_conf_reg;
    la_status status = m_ll_device->read_register(m_pacific_tree->slice[0]->ics->credits_conf_reg, credits_conf_reg);
    return_on_error(status);

    size_t crdt_in_bytes = credits_conf_reg.fields.crdt_in_bytes;

    uint32_t rate_to_device;
    status = tm_utils::convert_rate_to_device_val(rate, crdt_in_bytes, m_device_frequency_int_khz, rate_to_device);
    return_on_error(status);

    status = m_ll_device->write_register(*(*ifse_cir_shaper_rate_configuration)[tm_port_id], rate_to_device);
    return_on_error(status);

    // Set interface credit pir rate to 400G. Code based on: la_interface_scheduler_impl::set_credit_eir_or_pir(la_rate_t rate,
    // bool is_eir)
    status = m_ll_device->write_register(*(*ifse_pir_shaper_configuration)[tm_port_id], rate_to_device);
    return_on_error(status);

    status = m_ll_device->read_modify_write_register(*ifse_general_configuration,
                                                     tm_port_id + IFSE_EIR_SHAPE_MODE_BASE,
                                                     tm_port_id + IFSE_EIR_SHAPE_MODE_BASE,
                                                     false /* is_eir */);
    return_on_error(status);

    // Create output queue scheduler. Code based on:
    // la_system_port_scheduler_impl::initialize(la_object_id_t oid,la_interface_scheduler* intf_sch)
    la_output_queue_scheduler_impl_sptr voq_flush_oq_sch;
    status = do_create_output_queue_scheduler(vsc_slice_id,
                                              vsc_ifg_id,
                                              index_handle(oqse_id),
                                              la_output_queue_scheduler::scheduling_mode_e::DIRECT_4SP,
                                              voq_flush_oq_sch);
    return_on_error(status);

    m_is_builtin_objects[voq_flush_oq_sch->oid()] = true; // This oqcs should always exist.
    m_voq_flush_oq_sch = voq_flush_oq_sch;

    // Set the priority group mapping. Code based on: la_system_port_scheduler_impl::set_oq_priority_group(...)
    // Credit scheduler priority group mapping
    constexpr size_t OQPG_WIDTH = sch_tpse_oqpg_mapping_configuration_register::fields::TPSE_OQPG_MAP_WIDTH; // 17
    size_t lsb = serdes_id * OQPG_WIDTH;
    size_t msb = lsb + OQPG_WIDTH - 1;
    la_uint64_t val = bit_utils::ones(OQPG_WIDTH); // Setting all ones will map all OQs to OQPG 7
    status = m_ll_device->read_modify_write_register(*tpse_oqpg_mapping_configuration, msb, lsb, val);
    return_on_error(status);

    // Only credit scheduler (SCH) needs to be configure. Transmit scheduler (PDOQ) must not be changed.

    // Disable oqcs shapers. Code based on: la_system_port_scheduler_impl::do_set_credit_pir_burst_size() and
    // do_set_priority_group_credit_burst_size()
    status = tm_utils::set_burst_size(
        shared_from_this(), oq_pir_token_bucket_cfg, oq_pir_token_bucket, oqse_id /* mem_line */, tm_utils::UNLIMITED_BUCKET_SIZE);
    return_on_error(status);
    status = tm_utils::set_burst_size(shared_from_this(),
                                      oqpg_cir_token_bucket_cfg,
                                      oqpg_cir_token_bucket,
                                      oqse_id /* mem_line */,
                                      tm_utils::UNLIMITED_BUCKET_SIZE);
    return_on_error(status);
...
la_device_impl::prepare_dedicated_oq_for_mcg_counter()
{
    ...
    la_slice_id_vec_t nw_slices = get_slices(shared_from_this(), la_slice_mode_e::NETWORK);
    for (auto sid : nw_slices) {
        for (la_ifg_id_t ifg_id = 0; ifg_id < NUM_IFGS_PER_SLICE; ifg_id++) {
            if (((sid == 0) && (ifg_id == 1)) || ((sid == 0) && (ifg_id == 0)) || ((sid == 2) && (ifg_id == 0))
                || ((sid == 4) && (ifg_id == 0))) {
                continue;
            }
            ...
            lld_memory_scptr cfg_mem = (*m_pacific_tree->slice[sid]->pdoq->top->oq_pir_token_bucket_cfg)[ifg_id];
            lld_memory_scptr dynamic_mem = (*m_pacific_tree->slice[sid]->pdoq->top->oq_pir_token_bucket)[ifg_id];
            ...
            for (la_oq_id_t oq = 0; oq < tm_utils::NUM_OQS_PER_SYSTEM_PORT_SCH; oq++) {
                size_t mem_line = HOST_PIF_ID * tm_utils::NUM_OQS_PER_SYSTEM_PORT_SCH + oq;
                status = tm_utils::set_oqcs_rate(shared_from_this(),
                                                 cfg_mem,
                                                 dynamic_mem,
                                                 mem_line,
                                                 port_speed /* requested rate */,
                                                 port_speed,
                                                 ifg_rate,
            ...
            cfg_mem = (*m_pacific_tree->slice[sid]->pdoq->top->oqpg_cir_token_bucket_cfg)[ifg_id];
            dynamic_mem = (*m_pacific_tree->slice[sid]->pdoq->top->oqpg_cir_token_bucket)[ifg_id];
            for (size_t pg = 0; pg < (size_t)la_system_port_scheduler::priority_group_e::NONE; pg++) {
                size_t mem_line = HOST_PIF_ID * (size_t)la_system_port_scheduler::priority_group_e::NONE + pg;
                status = tm_utils::set_oqcs_rate(shared_from_this(),
                                                 cfg_mem,
                                                 dynamic_mem,
                                                 mem_line,
...
void
la_device_impl::periodic_workaround_reset_ics_delete_credits()
{
    ...
    auto nw_slices = get_slices(shared_from_this(), la_slice_mode_e::NETWORK);
    for (auto slice : nw_slices) {
        reg_val_list.push_back({m_pacific_tree->slice[slice]->ics->delete_credits_trig, 0});
        reg_val_list.push_back({m_pacific_tree->slice[slice]->ics->delete_credits_trig, 1});
    }

    la_status status = lld_write_register_list(m_ll_device, reg_val_list);
    if (status != LA_STATUS_SUCCESS) {
        log_err(HLD, "Failed triggering ICS delete_credits_trig register.");
    }
}
...
la_device_impl::periodic_workaround_reset_age_filter_entries()
{
    ...
    la_status status = m_ll_device->read_register(*m_pacific_tree->cdb->top->spare_reg, spare_reg);
    if (status != LA_STATUS_SUCCESS) {
        log_err(HLD, "Failed triggering age filter entries spare register.");
    } else {
        spare_reg.set_bit(1, 0);
        status = m_ll_device->write_register(*m_pacific_tree->cdb->top->spare_reg, spare_reg);
        log_on_error(status, HLD, ERROR, "Failed triggering age filter entries spare register.");
        spare_reg.set_bit(1, 1);
        status = m_ll_device->write_register(*m_pacific_tree->cdb->top->spare_reg, spare_reg);
        log_on_error(status, HLD, ERROR, "Failed triggering age filter entries spare register.");
    }

    for (const auto& slice : m_pacific_tree->slice) {
        status = m_ll_device->read_register(*slice->npu->rxpp_fwd->rxpp_fwd->spare_reg, spare_reg);
        if (status != LA_STATUS_SUCCESS) {
            log_err(HLD, "Failed triggering age filter entries spare register.");
            continue;
        }
        spare_reg.set_bit(3, 0);
        status = m_ll_device->write_register(*slice->npu->rxpp_fwd->rxpp_fwd->spare_reg, spare_reg);
        log_on_error(status, HLD, ERROR, "Failed triggering age filter entries spare register.");
...
la_device_impl::register_pollers()
{
    ...
    m_notification->register_poll_cb(
        [&]() {
            ++m_heartbeat.slow;
            m_ll_device->write_memory(*m_pacific_tree->sbif->css_mem_even,
    ...
    if (m_device_mode != device_mode_e::FABRIC_ELEMENT) {
        // Set NPU host eventq polling to be done by the CPU.
        m_cpu_eventq_polling = true;
        if (is_simulated_device()) {
    ...
    if (!is_simulated_device()) {
        if (m_revision != la_device_revision_e::PACIFIC_B1) {
            m_notification->register_poll_cb([&]() { periodic_workaround_reset_ics_delete_credits(); },
                                             hld_notification_base::poll_interval_e::POLL_INTERVAL_FAST);
        }
        if (m_revision == la_device_revision_e::PACIFIC_B1) {
        ...
        m_notification->register_poll_cb([&]() { m_resource_manager->cem_cam_evacuation(); },
                                         hld_notification_base::poll_interval_e::POLL_INTERVAL_SLOW);
    }
}
...
la_device_impl::configure_device_bool_property(la_device_property_e device_property)
{
    ...
    case la_device_property_e::ENABLE_PACIFIC_SW_BASED_PFC: {
        status = set_pfc_device_tuning_enabled();
        return_on_error(status);

        status = set_sw_pfc_device_tuning_enabled();
        return_on_error(status);

        // Enable the CSS ARC CPUs.
        // Enabling the CSS ARCs is currently only done when Software-based PFC is enabled because of an issue
        // with corrupting an TOD register access.
        status = m_arc_hdlr->configure_css_arc_cpus();
        return_on_error(status);

        // Turn off CPU polling on the eventq and turn on ARC polling
        if (!is_simulated_device()) {
            m_cpu_eventq_polling = false;
        }
        return LA_STATUS_SUCCESS;
    }
...
la_device_impl::configure_device_int_property(la_device_property_e device_property, int old_property_value)
{
    ...
    case la_device_property_e::STATISTICAL_METER_MULTIPLIER: {
        if ((property_value == 0) || (property_value >= (1 << 7))) {
            return LA_STATUS_EOUTOFRANGE;
        }
        rx_meter_meter_block_configuration_register conf_reg;
        status = m_ll_device->read_register(*(*m_pacific_tree->rx_meter->top->meter_block_configuration)[0], conf_reg);
        return_on_error(status);
        conf_reg.fields.block_packet_count_for_stat_update = property_value;
        for (la_slice_id_t s = 0; s < NUM_STATISTICAL_METER_BANKS; s++) {
            status = m_ll_device->write_register(*(*m_pacific_tree->rx_meter->top->meter_block_configuration)[s], conf_reg);
            return_on_error(status);
        }
    ...
    case la_device_property_e::MAC_PORT_SAVE_STATE_SERDES_DEBUG_SNAPSHOTS: {
        ...
        if (property_value < 0 || property_value > MAX_SERDES_DEBUG_CAPTURES) {
            log_err(HLD,
                    "Device property %s only takes values from the range [0-%d].",
                    silicon_one::to_string(la_device_property_e::MAC_PORT_SAVE_STATE_SERDES_DEBUG_SNAPSHOTS).c_str(),
                    (int)MAX_SERDES_DEBUG_CAPTURES);
            return LA_STATUS_EOUTOFRANGE;
        }
        return LA_STATUS_SUCCESS;
    }
    ...
    case la_device_property_e::AAPL_IFG_DELAY_BEFORE_EXEC:
    case la_device_property_e::AAPL_HBM_DELAY_BEFORE_EXEC:
    case la_device_property_e::AAPL_IFG_DELAY_BEFORE_POLL:
    case la_device_property_e::AAPL_HBM_DELAY_BEFORE_POLL:
    case la_device_property_e::AAPL_IFG_DELAY_IN_POLL:
    case la_device_property_e::AAPL_IFG_POLL_TIMEOUT:
        return LA_STATUS_SUCCESS;
    ...
    case la_device_property_e::LINKUP_TIME_BEFORE_SERDES_REFRESH: {
        return LA_STATUS_SUCCESS;
    }

    case la_device_property_e::EFUSE_REFCLK_SETTINGS: {
        return LA_STATUS_EINVAL;
    }

    case la_device_property_e::DEV_REFCLK_SEL: {
        if (property_value & ~(0xfff)) {
            return LA_STATUS_EOUTOFRANGE;
        }
    ...
    case la_device_property_e::PACIFIC_PFC_PILOT_PROBABILITY: {
        status = set_int_device_pfc_pilot_probability(property_value);
        return status;
    }

    case la_device_property_e::PACIFIC_PFC_MEASUREMENT_PROBABILITY: {
        status = set_int_device_pfc_measurement_probability(property_value);
        return status;
    }
...
la_device_impl::set_bool_property_lc_advertise_device_on_fabric_mode(bool property_value)
{
    ...
    frm_frp_enable_reg_register reg;
    reg.fields.frp_packet_gen_en = property_value;

    la_status status = m_ll_device->write_register(*m_pacific_tree->dmc->frm->frp_enable_reg, reg);
...
{
    sbif_msi_master_interrupt_reg_mask_register val{{0}};
    ...
    return m_ll_device->write_register(*m_pacific_tree->sbif->msi_master_interrupt_reg_mask, val);
}
...
la_device_impl::set_int_property_minimum_fabric_ports_for_connectivity(int32_t property_value)
{
    ...
    la_status status = do_set_global_minimum_fabric_links((size_t)property_value);
    return_on_error(status);
...
la_status
la_device_impl::do_set_global_minimum_fabric_links(size_t num_links)
{
    if (num_links > MAX_MIN_LINKS_THRESHOLD) {
        return LA_STATUS_EOUTOFRANGE;
    }

    frm_min_links_threshold_reg_register reg;

    reg.fields.min_links_thr = num_links;

    la_status status = m_ll_device->write_register(m_pacific_tree->dmc->frm->min_links_threshold_reg, reg);
    return_on_error(status);
...
static double
pfc_config_to_mirror_probability(int32_t property_value)
{
    double probability;

    // There is a 1/16 sampling prior to the mirror. So configured prob needs to be multiplied by 16.
    // If its set greater than 6.25% (62) just set the probability to the max.
    if (property_value > 62) {
        probability = 1.0;
    } else {
        probability = property_value * 16 / 1000.0;
    }
    return probability;
}

la_status
la_device_impl::set_int_device_pfc_pilot_probability(int32_t property_value)
{
    if (property_value > 1000) {
        return LA_STATUS_EOUTOFRANGE;
    }

    double pfc_pilot_probability = pfc_config_to_mirror_probability(property_value);

    if (m_mirror_commands.size() < PFC_PILOT_MIRROR_GID) {
        return LA_STATUS_SUCCESS;
    }

    auto pfc_pilot_mirror = m_mirror_commands[PFC_PILOT_MIRROR_GID];
    if (pfc_pilot_mirror == nullptr) {
        // Return ok if mirror has not been added yet. Probability will be
        // set when it is created.
        return LA_STATUS_SUCCESS;
    }

    la_status status = pfc_pilot_mirror->set_probability(pfc_pilot_probability);
    return status;
}

la_status
la_device_impl::set_int_device_pfc_measurement_probability(int32_t property_value)
{
    if (property_value > 1000) {
        return LA_STATUS_EOUTOFRANGE;
    }

    double pfc_measurement_probability = pfc_config_to_mirror_probability(property_value);

    if (m_mirror_commands.size() < PFC_MEASUREMENT_MIRROR_GID) {
        return LA_STATUS_SUCCESS;
    }

    auto pfc_measurement_mirror = m_mirror_commands[PFC_MEASUREMENT_MIRROR_GID];
    ...
    la_status status = pfc_measurement_mirror->set_probability(pfc_measurement_probability);
    return status;
}
...
la_device_impl::set_bool_property_pacific_pfc_hbm_enabled(bool enable)
{
    ...
    if (m_mirror_commands.size() < PFC_PILOT_MIRROR_GID) {
        // Not applicable for FE.
        return LA_STATUS_SUCCESS;
    }

    auto pfc_pilot_mirror = m_mirror_commands[PFC_PILOT_MIRROR_GID].weak_ptr_static_cast<la_l2_mirror_command>();
    ...
    if (!enable) {
        ...
        if ((pfc_pilot_mirror == nullptr) && (pfc_measurement_mirror == nullptr)) {
            return LA_STATUS_SUCCESS;
        }

        status = clear_snoop_configuration(LA_EVENT_ETHERNET_PFC_DIRECT_SAMPLE);
        return_on_error(status);

        status = do_destroy(pfc_pilot_mirror);
        return_on_error(status);

        status = clear_snoop_configuration(LA_EVENT_ETHERNET_PFC_SAMPLE);
        return_on_error(status);
    ...
    // Enable case. If both mirrors are already created, return success.
    if ((pfc_pilot_mirror != nullptr) && (pfc_measurement_mirror != nullptr)) {
        return LA_STATUS_SUCCESS;
    }

    // Create the PFC pilot mirror.
    // Need to retrieve the NPU host port for this device.
    bool found = false;
    la_npu_host_port_base* npu_host_port = nullptr;
    auto npu_host_port_list = get_objects(la_object::object_type_e::NPU_HOST_PORT);
    for (auto object : npu_host_port_list) {
        npu_host_port = static_cast<la_npu_host_port_base*>(object);
        if (!npu_host_port->is_remote() && (npu_host_port->get_system_port() != nullptr)) {
            found = true;
            break;
        }
    }
    ...
    // Create the mirror.
    int prob = 0;
    get_int_property(la_device_property_e::PACIFIC_PFC_PILOT_PROBABILITY, prob);

    double pfc_pilot_probability = pfc_config_to_mirror_probability(prob);
    status = create_l2_mirror_command(PFC_PILOT_MIRROR_GID, get_sptr(npu_host_port), 0, pfc_pilot_probability, pfc_pilot_mirror);
    return_on_error(status);

    // Set the mirror to truncate the packet.
    pfc_pilot_mirror->set_truncate(true);

    // Set the snoop configuration.
    status = set_snoop_configuration(LA_EVENT_ETHERNET_PFC_DIRECT_SAMPLE, 0, false, false, pfc_pilot_mirror.get());
    ...
    prob = 0;
    get_int_property(la_device_property_e::PACIFIC_PFC_MEASUREMENT_PROBABILITY, prob);
    double pfc_measurement_probability = pfc_config_to_mirror_probability(prob);
    ...
    status = set_snoop_configuration(LA_EVENT_ETHERNET_PFC_SAMPLE, 0, false, false, pfc_measurement_mirror.get());
    return status;
}
...
la_device_impl::set_bool_property_fe_per_device_min_links(bool is_enable)
{
    ...
    if (is_enable) { // if we move to per-lc mode, then update all devices' thresholds to be the global threshold.
        for (la_device_id_t device_id = 0; device_id < MAX_DEVICES; device_id++) {
            m_lc_to_min_links[device_id] = m_global_min_fabric_links_threshold;
        }
    }

    la_status status = update_fe_fabric_reachability_advertisement(m_fe_fabric_reachability_enabled, is_enable);
    return_on_error(status);
...
{
    cdb_cache_lpm_cache_disable_cache_em_register_register lpm_em_cache_disable_reg{{0}};
    lpm_em_cache_disable_reg.fields.lpm_cache_disable_cache_em = !is_enable;

    cdb_cache_lpm_cache_disable_cache_tcam_register_register lpm_tcam_cache_disable_reg{{0}};
    lpm_tcam_cache_disable_reg.fields.lpm_cache_disable_cache_tcam = !is_enable;

    cdb_cache_splitter_cache_disable_cache_em_register_register splitter_em_cache_disable_reg{{0}};
    splitter_em_cache_disable_reg.fields.splitter_cache_disable_cache_em = !is_enable;

    cdb_cache_splitter_cache_disable_cache_tcam_register_register splitter_tcam_cache_disable_reg{{0}};
    splitter_tcam_cache_disable_reg.fields.splitter_cache_disable_cache_tcam = !is_enable;

    for (la_slice_id_t slice : get_used_slices()) {
        la_status status = m_ll_device->write_register(
            *m_pacific_tree->slice[slice]->npu->cdb_cache->lpm_cache_disable_cache_em_register, lpm_em_cache_disable_reg);
        return_on_error(status);
        status = m_ll_device->write_register(*m_pacific_tree->slice[slice]->npu->cdb_cache->lpm_cache_disable_cache_tcam_register,
                                             lpm_tcam_cache_disable_reg);
        return_on_error(status);
        status = m_ll_device->write_register(
            *m_pacific_tree->slice[slice]->npu->cdb_cache->splitter_cache_disable_cache_em_register, splitter_em_cache_disable_reg);
        return_on_error(status);
        status
            = m_ll_device->write_register(*m_pacific_tree->slice[slice]->npu->cdb_cache->splitter_cache_disable_cache_tcam_register,
...
weak_ptr_unsafe<hld_notification_base>
...
la_device_impl::initialize_translator_creator(const translator_creator_sptr& creator)
{
    ...
    for (la_slice_id_t sid : get_used_slices()) {
        ...
        if (it == slices_per_context.end()) {
            slices_per_context[slice_context] = {sid};
        } else {
    ...
    // NOTE: it really seems like this should be ASIC_MAX_SLICES_PER_DEVICE_NUM instead of 0
...
la_device_impl::apply_fabric_mac_port_workaround(la_mac_port::fc_mode_e fc_mode)
{
    ...
    // on, only TX-PMA can be set/reset to enable/disable a port.
    ...
    for (la_slice_id_t slice_id : get_used_slices()) {
        for (la_ifg_id_t ifg_id = 0; ifg_id < NUM_IFGS_PER_SLICE; ifg_id++) {
            size_t serdes_count = m_ifg_handlers[slice_id][ifg_id]->get_serdes_count();
            for (la_uint_t serdes = 0; serdes < serdes_count; serdes += NUM_SERDES_PER_FABRIC_PORT) {
                ...
                auto fab_mac_port = std::make_shared<la_mac_port_pacific>(shared_from_this());
                la_object_id_t oid;
                la_status status = register_object(fab_mac_port, oid);
                return_on_error(status);
                status = fab_mac_port->initialize_fabric(
                    oid, slice_id, ifg_id, serdes, NUM_SERDES_PER_FABRIC_PORT, la_mac_port::port_speed_e::E_100G, fc_mode);
                if (status != LA_STATUS_SUCCESS) {
                    deregister_object(oid);
    ...
    // do basic fabric port ifgb allocation
    for (auto& fab_mac_port : fab_mac_ports_vec) {
        auto fab_mac_port_pacific = static_cast<la_mac_port_pacific*>(fab_mac_port.get());
        status = fab_mac_port_pacific->do_reset_fabric_port_pacific_a0();
        return_on_error(status);
    }
    ...
    for (auto& fab_mac_port : fab_mac_ports_vec) {
        ...
        la_status status = fab_mac_port->destroy();
        return_on_error(status);
...
la_device_impl::hbm_exists(bool& out_exists) const
{
    ...
    // Check the eFuse
    la_device_info_t dev_info;
    la_status status = get_device_information(dev_info);
    return_on_error(status);

    if (dev_info.extension == PACIFIC_EXTENSION_HBM) {
        out_exists = true;
    } else {
...
la_device_impl::get_table_allocations(json_t* tables_array_json) const
{
    ...
    table_object = json_object();
    key_str = "ip_prefix_destination_table";
    table_prop_array_json = json_array();
    json_object_set_new(table_object, key_str.c_str(), table_prop_array_json);
    append_table_properties_to_json(m_tables.ip_prefix_destination_table, table_prop_array_json);
    json_array_append_new(tables_array_json, table_object);
...
la_status
la_device_impl::reset_oob_inj_credits(size_t link)
{
    pier_oob_inj_credit_init_reg_register oob_inj_credit_init_reg;
    oob_inj_credit_init_reg.fields.oob_inj_credit_init_en = 1;
    oob_inj_credit_init_reg.fields.oob_inj_credit_init_val = 6;
    oob_inj_credit_init_reg.fields.oob_inj_credit_init_link = link;
    la_status status = m_ll_device->write_register(m_pacific_tree->dmc->pier->oob_inj_credit_init_reg, oob_inj_credit_init_reg);
    return_on_error(status);
    oob_inj_credit_init_reg.fields.oob_inj_credit_init_en = 0;
    status = m_ll_device->write_register(m_pacific_tree->dmc->pier->oob_inj_credit_init_reg, oob_inj_credit_init_reg);
    return_on_error(status);
...
{
    int curr_fabric_link_up_cnt = 0;
    for (auto it : m_mac_ports) {
        bool is_fabric_port_supporting_serdes_en
            = is_fabric_port_supporting_serdes(it.second->get_slice(), it.second->get_ifg(), it.second->get_first_serdes_id());
        bool link_up = it.second->poll_link_state();
        if (is_fabric_port_supporting_serdes_en && link_up) {
            curr_fabric_link_up_cnt++;
        }
    }

    if (curr_fabric_link_up_cnt > 0) {
        pier_pier_oob_counter_reg_register pier_oob_counter_reg;
        m_ll_device->read_register(m_pacific_tree->dmc->pier->pier_oob_counter_reg, pier_oob_counter_reg);
        if (pier_oob_counter_reg.fields.oobi_cnt == 0) {
            log_debug(HLD,
                      "poll_mac_ports: WA reset oob inj credits of link 0: curr_fabric_link_up_cnt=%d oobi_cnt=%ld oobe_cnt=%ld",
                      curr_fabric_link_up_cnt,
                      pier_oob_counter_reg.fields.oobi_cnt,
                      pier_oob_counter_reg.fields.oobe_cnt);
            reset_oob_inj_credits(0);
        }
    }
}
...
la_device_impl::poll_npu_host_event_queue()
{
    ...
    if (m_cpu_eventq_polling) {
        events = m_npu_host_eventq->collect_npu_host_events();
    } else {
        // ARC 0 is monitored.
        events = m_arc_hdlr->collect_arc_events(0);
    }
...
{
    log_debug(HLD, "la_device_impl::poll_fe_routing_table begins.");

    dassert_crit(m_pacific_tree->dmc->frm->fabric_routing_table->get_desc()->width_bits == NUM_FABRIC_PORTS_IN_DEVICE);

    const size_t line_width_total = m_pacific_tree->dmc->frm->fabric_routing_table->get_desc()->width_total;

    bit_vector tmp_fe_routing_table;

    la_status status
        = m_ll_device->read_memory(*m_pacific_tree->dmc->frm->fabric_routing_table, 0, MAX_DEVICES, tmp_fe_routing_table);
    if (status != LA_STATUS_SUCCESS) {
...
la_device_impl::update_on_fabric_links_changed(const la_device_id_vec_t& changed_devices)
{
    const size_t broadcast_bmp_entries
        = rx_pdr_2_slices_fe_configurations_reg1_register::fields::SLICE_FE_VALID_BROADCAST_BMP_VECTOR_WIDTH;
    ...
    bool is_per_device_min_link = false;
    get_bool_property(la_device_property_e::ENABLE_FE_PER_DEVICE_MIN_LINKS, is_per_device_min_link);
    if (is_per_device_min_link && m_fe_fabric_reachability_enabled) {
        configure_reachability_of_lcs(changed_devices);
    }
}
...
la_device_impl::remove_fabric_port_from_bundle(la_uint_t fabric_port_num, la_device_id_t dev_id)
{
    ...
    rx_pdr_2_slices_fb_link_to_link_bundle_table_memory link_to_bundle;

    la_status status = m_ll_device->read_memory(
        (*m_pacific_tree->slice_pair[0]->rx_pdr->fb_link_to_link_bundle_table)[0], fabric_port_num, link_to_bundle);
    return_on_error(status);
    ...
    for (la_slice_id_t slice : get_used_slices()) {
        status
            = m_ll_device->write_memory((*m_pacific_tree->slice_pair[slice / 2]->rx_pdr->fb_link_to_link_bundle_table)[slice % 2],
                                        fabric_port_num,
                                        INVALID_BUNDLE);
        return_on_error(status);
    }
...
la_device_impl::add_fabric_port_to_bundle(la_uint_t fabric_port_num, la_device_id_t dev_id)
{
    ...
    for (size_t link : m_device_to_potential_links[dev_id]) {
        status = m_ll_device->read_memory(
            (*m_pacific_tree->slice_pair[0]->rx_pdr->fb_link_to_link_bundle_table)[0], link, link_to_bundle);
        return_on_error(status);
        ...
        size_t bundle_id = link_to_bundle.fields.table_bundle_num;
        if (m_bundles[bundle_id].size() == 4) {
            continue;
        }
        // Check because of a hardware bug in revisions older then B1
        if (!is_allowed_to_bundle_links(m_bundles[bundle_id][0], fabric_port_num)) {
            continue;
        }
    ...
    for (auto link : links_without_bundle) {
        // Check because of a hardware bug in revisions older then B1
        if (!is_allowed_to_bundle_links(link, fabric_port_num)) {
            continue;
        }
...
{
    constexpr size_t NUM_FABRIC_PORTS_PER_SLICE = NUM_FABRIC_PORTS_IN_NORMAL_IFG * NUM_IFGS_PER_SLICE;
    ...
    la_slice_id_t first_link_slice = bundle[0] / NUM_FABRIC_PORTS_PER_SLICE;
    // Because of a hardware bug in revisions older then B1, a bundle must contain links of the same slice,
    // except of slice 5 whose links can be in a bundle with any slice.
    // In such case, the first link in the bundle must be from the other slice (not 5).
    // If there is link from slice other that 5, we put that link to be first in bundle.
    if (m_revision < la_device_revision_e::PACIFIC_B1 && first_link_slice == 5) {
        for (size_t i = 1; i < bundle.size(); i++) {
            if (bundle[i] / NUM_FABRIC_PORTS_PER_SLICE != 5) {
                std::swap(bundle[0], bundle[i]);
                break;
            }
        }
    }
    ...
    for (la_slice_id_t slice : get_used_slices()) {
        ...
        for (size_t link : bundle) {
            status = m_ll_device->write_memory(
                (*m_pacific_tree->slice_pair[slice / 2]->rx_pdr->fb_link_to_link_bundle_table)[slice % 2], link, link_to_bundle);
            return_on_error(status);
        }
    }
...
la_status
la_device_impl::write_bundle_desc_table_and_verify_write(la_slice_id_t slice,
                                                         size_t bundle_id,
                                                         rx_pdr_2_slices_fe_uc_link_bundle_desc_table_memory& bundle_desc)
{
    rx_pdr_2_slices_fe_uc_link_bundle_desc_table_memory read_bundle_desc;
    ...
    for (size_t i = 0; i < MAX_TRIES_NUM; i++) {
        la_status status = m_ll_device->write_memory(
            (*m_pacific_tree->slice_pair[slice / 2]->rx_pdr->fe_uc_link_bundle_desc_table)[slice % 2], bundle_id, bundle_desc);
        return_on_error(status);

        status = m_ll_device->read_memory(
            (*m_pacific_tree->slice_pair[slice / 2]->rx_pdr->fe_uc_link_bundle_desc_table)[slice % 2], bundle_id, read_bundle_desc);
        return_on_error(status);
...
bool
la_device_impl::is_allowed_to_bundle_links(size_t first_link_in_bundle, size_t new_link) const
{
    // Because of a hardware bug in revisions older then B1, a bundle must contain links of the same slice,
    // except of slice 5 whose links can be in a bundle with any slice.
    // In such case, the first link in the bundle must be from the other slice (not 5).
    ...
    constexpr size_t NUM_FABRIC_PORTS_PER_SLICE = NUM_FABRIC_PORTS_IN_NORMAL_IFG * NUM_IFGS_PER_SLICE;
    la_slice_id_t slice_of_first_link = first_link_in_bundle / NUM_FABRIC_PORTS_PER_SLICE;
    la_slice_id_t slice_of_new_link = new_link / NUM_FABRIC_PORTS_PER_SLICE;

    if (m_revision < la_device_revision_e::PACIFIC_B1 && slice_of_new_link != 5 && slice_of_new_link != slice_of_first_link
        && slice_of_first_link != 5) {
        return false;
    }
...
{
    lld_register_value_list_t reg_val_list;
    auto ptrr = (*m_pacific_tree->slice_pair[slice / 2]->rx_pdr->slice_global_configuration)[slice % 2];
    rx_pdr_2_slices_slice_global_configuration_register rr;
...
{
    lld_register_value_list_t reg_val_list;
    auto pttr = m_pacific_tree->slice[slice]->tx->pdr->mcid_cache_configuration;

    txpdr_mcid_cache_configuration_register tr;
...
la_device_impl::is_fabric_port_supporting_serdes(la_slice_id_t slice_id, la_ifg_id_t ifg_id, la_uint_t first_serdes_id) const
{
    ...
    if (is_network_slice(slice_id) && (is_lender_ifg_en == true) && (is_potentially_lended_port == true)) {
        return true;
    }
...
la_device_impl::set_fabric_sch_valid_links_quantization_thresholds(const la_fabric_valid_links_thresholds& thresholds)
{
    ...
    auto valid_links_mapping_reg = m_pacific_tree->sch_top->frl_valid_links_mapping;
    bit_vector bv(0, sch_top_frl_valid_links_mapping_register::SIZE_IN_BITS);
...
la_device_impl::set_fabric_sch_congested_links_quantization_thresholds(const la_fabric_congested_links_thresholds& thresholds)
{
    ...
    auto congested_links_mapping_reg = m_pacific_tree->sch_top->frl_congested_links_mapping;
    bit_vector bv(0, sch_top_frl_congested_links_mapping_register::SIZE_IN_BITS);
...
la_device_impl::set_fabric_sch_rate_map_entry(la_uint_t index, la_uint_t rate)
{
    ...
    la_uint_t max_rate = bit_utils::ones(sch_top_frl_rate_mapping_register::fields::NUM_LINKS_TO_RATE_MAP_WIDTH / num_indices);
    if (index >= num_indices && rate > max_rate) {
        return LA_STATUS_EINVAL;
    }

    auto rate_mapping_reg = m_pacific_tree->sch_top->frl_rate_mapping;
    sch_top_frl_rate_mapping_register reg;

    la_status status = m_ll_device->read_register(rate_mapping_reg, reg);
    return_on_error(status);

    ics_slice_credits_conf_reg_register credits_conf_reg;
    status = m_ll_device->read_register(m_pacific_tree->slice[0]->ics->credits_conf_reg, credits_conf_reg);
    return_on_error(status);

    size_t crdt_in_bytes = credits_conf_reg.fields.crdt_in_bytes;

    // Convert rate to device val - API takes rate in KB
    uint32_t dev_rate;
    status = tm_utils::convert_rate_to_device_val(((la_rate_t)rate) * 1000, crdt_in_bytes, m_device_frequency_int_khz, dev_rate);
    return_on_error(status);
...
la_device_impl::get_fabric_sch_rate_map_entry(la_uint_t index, la_uint_t& out_rate)
{
    ...
    out_rate = tm_utils::convert_rate_from_device_val(value * 1000, crdt_in_bytes, m_device_frequency_int_khz);
...
la_device_impl::set_fe_fabric_reachability_enabled(bool enabled)
{
    ...
    status = update_fe_fabric_reachability_advertisement(enabled, is_per_device_min_links_mode);
    return_on_error(status);
...
la_status
la_device_impl::update_fe_fabric_reachability_advertisement(bool is_fe_fabric_reachability_enabled,
                                                            bool is_per_device_min_links_mode)
{
    la_status status;
    if (is_fe_fabric_reachability_enabled) {
        if (is_per_device_min_links_mode) { // per device min links mode is set
            // Configure the FE to advertise reachability based on minimum links configured for each LC
            // (see set_minimum_fabric_links_per_lc API).
            // The update of the reachability vector will be done by SDK.
            status = software_reachability_update();
            return_on_error(status);
        } else { // global min links mode is set
            // Configure the FE to advertise reachability based on routing engine and fe routing table updates.
            // The update of the reachability vector will be done by HW.
            status = hardware_reachability_update();
            return_on_error(status);
        }
    } else { // is_fe_fabric_reachability_enabled = false, i.e FE is in isolate mode
        // Configure the FE to advertise reachability of empty list, this will isolate the FE so it will not get any traffic.
        status = advertise_empty_reachability();
        return_on_error(status);
    }
...
la_status
la_device_impl::software_reachability_update()
{
    std::vector<la_device_id_t> all_devices_vec(MAX_DEVICES);
    std::iota(all_devices_vec.begin(), all_devices_vec.end(), 0);

    // disable hw updates to reachability vector.
    la_status status = set_reachable_bitmap_hw_updates_enabled(false);
    return_on_error(status);

    status = configure_reachability_of_lcs(all_devices_vec);
    return_on_error(status);
...
la_device_impl::hardware_reachability_update()
{
    ...
    // Allow the HW to update the reachability vector
...
la_device_impl::trigger_frt_scan()
{
    ...
    frm_debug_frtm_debug_reg_register reg;
    ...
    // Read full register
    la_status status = m_ll_device->read_register(*m_pacific_tree->dmc->frm->debug_frtm_debug_reg, reg);
    return_on_error(status);

    for (size_t i = 0; i < NUM_OF_TRIES; i++) {
        // Write 0
        reg.fields.debug_frtm_generate_frt_scan = 0;
        status = m_ll_device->write_register(*m_pacific_tree->dmc->frm->debug_frtm_debug_reg, reg);
        return_on_error(status);
        ...
        status = m_ll_device->read_register(*m_pacific_tree->dmc->frm->frp_reachable_bitmap12_reg, reachability_bmp);
        return_on_error(status);
...
la_device_impl::advertise_empty_reachability()
{
    ...
    status = m_ll_device->write_register(*m_pacific_tree->dmc->frm->frp_reachable_bitmap12_reg, 0);
    return status;
}
...
{
    frm_debug_frtm_debug_reg_register frtm_debug_reg{{0}};

    la_status status = m_ll_device->read_register(*m_pacific_tree->dmc->frm->debug_frtm_debug_reg, frtm_debug_reg);
    return_on_error(status);
    ...
    status = m_ll_device->write_register(*m_pacific_tree->dmc->frm->debug_frtm_debug_reg, frtm_debug_reg);
    return status;
}
...
la_device_impl::set_global_minimum_fabric_links(size_t num_links)
{
    ...
    la_status status = do_set_global_minimum_fabric_links(num_links);
    return_on_error(status);
...
la_device_impl::set_minimum_fabric_links_per_lc(la_device_id_t device_id, size_t num_links)
{
    ...
    m_lc_to_min_links[device_id] = num_links;
    m_reconnect_handler->update_minimum_fabric_links_per_lc(device_id, num_links);

    // If device is in isolate mode, then we don't update the reachability bitmap.
    if (!m_fe_fabric_reachability_enabled) {
        return LA_STATUS_SUCCESS;
    }

    la_device_id_vec_t devices = {device_id};
    status = configure_reachability_of_lcs(devices);
...
la_device_impl::get_minimum_fabric_links_per_lc(la_device_id_t device_id, size_t& out_num_links) const
{
    ...
    out_num_links = m_lc_to_min_links[device_id];

    return LA_STATUS_SUCCESS;
}

la_status
la_device_impl::configure_reachability_of_lcs(la_device_id_vec_t devices)
{
    bit_vector reachability_bmp;
    la_status status = m_ll_device->read_register(*m_pacific_tree->dmc->frm->frp_reachable_bitmap12_reg, reachability_bmp);
    return_on_error(status);

    for (la_device_id_t dev_id : devices) {
        bool val = (m_device_to_links[dev_id].size() >= m_lc_to_min_links[dev_id]);
        reachability_bmp.set_bit(dev_id, val);
    }
    status = m_ll_device->write_register(*m_pacific_tree->dmc->frm->frp_reachable_bitmap12_reg, reachability_bmp);
...
la_device_impl::configure_fe_configurations_reg1(size_t num_valid_entries)
{
    ...
    lld_register_value_list_t reg_val_list;
    rx_pdr_2_slices_fe_configurations_reg1_register reg;
    ...
    for (la_slice_id_t slice : get_used_slices()) {
        reg_val_list.push_back({(*m_pacific_tree->slice_pair[slice / 2]->rx_pdr->fe_configurations_reg1)[slice % 2], reg});
    }
...
la_status
la_device_impl::apply_fe_light_fi_workaround()
{
    if (m_device_mode != device_mode_e::FABRIC_ELEMENT) {
        return LA_STATUS_SUCCESS;
    }
    ...
    for (la_slice_id_t slice_id : get_used_slices()) {
        txpp_light_fi_fabric_lookup_register reg;

        la_status status
            = m_ll_device->read_register(*m_pacific_tree->slice[slice_id]->npu->txpp->txpp->light_fi_fabric_lookup, reg);
        return_on_error(status);

        // In FE, the ligth-fi should only start either with a drop macro, or a single TX macro.
        // Set all entries to the TX macro
        for (size_t idx = 0; idx < reg.fields.get_fabric_lookup_npe_mid_arr_array_size(); idx++) {
            reg.fields.set_fabric_lookup_npe_mid_arr(idx, NPL_FABRIC_ELEMENT_TX_MACRO);
        }

        status = m_ll_device->write_register(*m_pacific_tree->slice[slice_id]->npu->txpp->txpp->light_fi_fabric_lookup, reg);
        return_on_error(status);
    }
...
la_device_impl::set_learn_mode(learn_mode_e learn_mode)
{
    ...
    status = m_ll_device->write_register(*m_pacific_tree->cdb->top->learn_manager_cfg_max_learn_type, reg);
    return_on_error(status);
...
la_status
la_device_impl::add_to_mc_copy_id_table(la_slice_id_t slice, uint64_t mc_copy_id)
{
    ...
    value.payloads.update.encap_size = NPL_CUD_ENCAP_SIZE_16;
    value.payloads.update.mc_copy_id_msbs = key.cud_mapping_local_vars_mc_copy_id_17_12_ & 0b1111;
    value.payloads.update.encap_type = NPL_NPU_ENCAP_L3_HEADER_TYPE_ETHERNET_NH;
...
la_device_impl::add_to_mc_copy_id_table(la_slice_id_t slice, uint64_t mc_copy_id, size_t bank_id)
{
    ...
    key.cud_mapping_local_vars_mc_copy_id_17_12_ = bit_utils::get_bits(mc_copy_id, 17, 12);
    value.action = NPL_MC_COPY_ID_MAP_ACTION_UPDATE;
    value.payloads.update.encap_size = NPL_CUD_ENCAP_SIZE_20;
    value.payloads.update.mc_copy_id_msbs = bank_id & 0xfe; // 8 bits: 7b bank_id and LSB needs to be zero
...
{
    la_l2_destination_gid_t ac_port_gid = ac_port->get_gid();
    uint64_t ac_port_gid_17_12 = bit_utils::get_bits(ac_port_gid, 17, 12);

    npl_mc_copy_id_map_key_t key;
    npl_mc_copy_id_map_value_t value;

    key.cud_mapping_local_vars_mc_copy_id_17_12_ = ac_port_gid_17_12;
    value.action = NPL_MC_COPY_ID_MAP_ACTION_UPDATE;

    constexpr uint64_t MC_COPY_ID_MSBS_FIELD_WIDTH = 8;
    constexpr uint64_t MC_COPY_ID_MSBS_MASK = NPL_DESTINATION_L2_DLP_PREFIX
                                              << (MC_COPY_ID_MSBS_FIELD_WIDTH - NPL_DESTINATION_L2_DLP_PREFIX_LEN);

    value.payloads.update.encap_size = NPL_CUD_ENCAP_SIZE_20;
    value.payloads.update.mc_copy_id_msbs = MC_COPY_ID_MSBS_MASK | ac_port_gid_17_12;
    value.payloads.update.encap_type = NPL_NPU_ENCAP_L2_HEADER_TYPE_AC;

    la_slice_id_t slice = get_actual_dsp_slice(dsp);
    return do_add_to_mc_copy_id_table(slice, key, value);
}

la_status
la_device_impl::remove_from_mc_copy_id_table(const la_l2_service_port_base_wcptr& ac_port, const la_system_port_wcptr& dsp_in)
{
    la_l2_destination_gid_t ac_port_gid = ac_port->get_gid();
    uint64_t ac_port_gid_17_12 = bit_utils::get_bits(ac_port_gid, 17, 12);
    npl_mc_copy_id_map_key_t key;
    ...
    auto dsp = la_system_port_base::upcast_from_api(get_sptr(this), dsp_in);
...
la_device_impl::get_reachable_devices(bit_vector& out_reachable_dev_bv)
{
    ...
    la_status status = m_ll_device->read_register(*m_pacific_tree->dmc->frm->frp_reachable_bitmap12_reg, out_reachable_dev_bv);
    return_on_error(status);
...
la_device_impl::update_rxpp_lb() const
{
    ...
    rxpp_fwd_res_lb_key_const_config_reg_register hardwired_lb_key_const_config_val;
    la_status status;

    auto nw_slices = get_slices(shared_from_this(), la_slice_mode_e::NETWORK);

    // Place spa hash seed so that after barrel shifing in HW, the seed value will land in res_lb_key_crc_0_init_key (DSPA)
    la_uint16_t shift_amount;
    get_lb_hash_shift_amount(shift_amount);
    std::vector<la_uint16_t> preset_seed(6, m_ecmp_hash_seed);
    preset_seed[0] = m_spa_hash_seed;
    std::rotate(preset_seed.begin(), preset_seed.begin() + preset_seed.size() - shift_amount, preset_seed.end());
    std::reverse(preset_seed.begin(), preset_seed.end());

    for (auto slice : nw_slices) {
        status = m_ll_device->read_register(m_pacific_tree->slice[slice]->npu->rxpp_fwd->rxpp_fwd->res_lb_key_const_config_reg,
                                            hardwired_lb_key_const_config_val);
        return_on_error(status);

        // res_lb_key_crc_0_init_key is used for spa hashing.
        ...
        hardwired_lb_key_const_config_val.fields.res_lb_key_crc_3_init_key = preset_seed[3];
        ...
        reg_val_list.push_back({m_pacific_tree->slice[slice]->npu->rxpp_fwd->rxpp_fwd->res_lb_key_const_config_reg,
                                hardwired_lb_key_const_config_val});

        status = m_ll_device->read_register(
            *m_pacific_tree->slice[slice]->npu->rxpp_fwd->rxpp_fwd->resolution_load_balancing_field_size_conf, field_size_conf_val);
        return_on_error(status);

        field_size_conf_val.fields.field_size_is_16b = 1;

        // lb_key_crc_0_init_key is used for spa hashing.
        field_size_conf_val.fields.lb_key_crc_0_init_key = m_spa_hash_seed;
        ...
        field_size_conf_val.fields.lb_key_crc_3_init_key = m_ecmp_hash_seed;
        ...
        reg_val_list.push_back({m_pacific_tree->slice[slice]->npu->rxpp_fwd->rxpp_fwd->resolution_load_balancing_field_size_conf,
                                field_size_conf_val});
    }
...
bool
la_device_impl::is_fabric_slice(la_slice_id_t slice) const
{
    return (m_slice_mode[slice] == la_slice_mode_e::CARRIER_FABRIC || m_slice_mode[slice] == la_slice_mode_e::DC_FABRIC);
}

enum { FIRST_POSSIBLE_FABRIC_SLICE_IN_LC = 3 };
// this is hard coded connected to ASIC_MAX_SLICES_PER_DEVICE_NUM
bool
la_device_impl::is_fabric_capable_slice(la_slice_id_t slice) const
{
    return slice >= FIRST_POSSIBLE_FABRIC_SLICE_IN_LC;
}
...
la_device_impl::do_read_persistent_token(la_user_data_t& out_token) const
{
    ...
    status = m_ll_device->read_memory(*m_pacific_tree->sbif->css_mem_even,
                                      CSS_MEMORY_PERSISTENT_TOKEN_BASE,
                                      sizeof(out_token) >> 2,
                                      sizeof(out_token),
                                      &out_token);
...
la_device_impl::do_write_persistent_token(la_user_data_t token)
{
    ...
    status = m_ll_device->write_memory(
        *m_pacific_tree->sbif->css_mem_even, CSS_MEMORY_PERSISTENT_TOKEN_BASE, sizeof(token) >> 2, sizeof(token), in_token);
...
la_status
la_device_impl::init_pfc_sw_tables()
{
    la_status status;
    std::chrono::microseconds latency(0);

    for (la_traffic_class_t tc = 0; tc < LA_NUM_EGRESS_TRAFFIC_CLASSES; tc++) {
        status = set_sw_fc_pause_threshold(tc, latency);
        return_on_error(status);
    }
...
la_status
la_device_impl::set_sw_pfc_device_tuning_enabled()
{
    lld_register_value_list_t reg_val_list;
    la_status status;

    // Tuning should only apply on 2.4T LC
    if (m_device_mode != device_mode_e::LINECARD) {
        return LA_STATUS_SUCCESS;
    }
    ...
    // More weight for DRAM write
    size_t tall_channels[] = {2, 3, 4, 5};
    size_t wide_channels[] = {0, 1, 6, 7};
    hbm_chnl_4x_wide_hbm_rd_to_wr_arbitration_config_register rd_to_wr_arb_reg;
    hbm_chnl_4x_wide_hbm_wr_to_rd_arbitration_config_register wr_to_rd_arb_reg;

    status = m_ll_device->read_register(m_pacific_tree->hbm->chnl[0]->wide->hbm_rd_to_wr_arbitration_config, rd_to_wr_arb_reg);
    return_on_error(status);
    status = m_ll_device->read_register(m_pacific_tree->hbm->chnl[0]->wide->hbm_wr_to_rd_arbitration_config, wr_to_rd_arb_reg);
    return_on_error(status);

    rd_to_wr_arb_reg.fields.rd_total_cycles = 512;
    wr_to_rd_arb_reg.fields.wr_total_cycles = 768;

    // Tall channels
    for (size_t i = 0; i < 4; i++) {
        reg_val_list.push_back(
            {m_pacific_tree->hbm->chnl[tall_channels[i]]->tall->hbm_rd_to_wr_arbitration_config, rd_to_wr_arb_reg});
        reg_val_list.push_back(
            {m_pacific_tree->hbm->chnl[tall_channels[i]]->tall->hbm_wr_to_rd_arbitration_config, wr_to_rd_arb_reg});
    }
    // Wide channels
    for (size_t i = 0; i < 4; i++) {
        reg_val_list.push_back(
            {m_pacific_tree->hbm->chnl[wide_channels[i]]->wide->hbm_rd_to_wr_arbitration_config, rd_to_wr_arb_reg});
        reg_val_list.push_back(
            {m_pacific_tree->hbm->chnl[wide_channels[i]]->wide->hbm_wr_to_rd_arbitration_config, wr_to_rd_arb_reg});
    }

    // Set device FC threshold - different value than for HW PFC
    ...
    total_sch_uc_buffers_th.fields.total_sch_uc_buffers_fc_th = 20 * UNITS_IN_KIBI;
    ...
    for (la_slice_id_t slice : get_used_slices()) {
        if (is_network_slice(slice)) {
            ...
            ics_slice_packing_configuration_register packing_config;
            status = m_ll_device->read_register(m_pacific_tree->slice[slice]->ics->packing_configuration, packing_config);
            return_on_error(status);

            packing_config.fields.max_pds_in_pack = 12;
            packing_config.fields.dram_burst_size = 13;

            reg_val_list.push_back({m_pacific_tree->slice[slice]->ics->packing_configuration, packing_config});
        }
    }
...
la_device_impl::set_pfc_device_tuning_enabled()
{
    ...
    // Tuning should only apply on 2.4T LC and FE
    if (m_device_mode == device_mode_e::STANDALONE) {
        return LA_STATUS_SUCCESS;
    }
    ...
    if (m_device_mode == device_mode_e::LINECARD && !is_lc_type_2_4_t) {
        return LA_STATUS_SUCCESS;
    }

    // Set MS VOQ no drop
    if (m_fabric_init_handler != nullptr) {
        status = m_fabric_init_handler->set_pfc();
        return_on_error(status);
    }
    ...
    for (la_slice_id_t slice : get_used_slices()) {
        if (is_network_slice(slice)) {
            ...
            mem_val_list.push_back({m_pacific_tree->slice[slice]->tx->cgm->uc_oqg_profile, uc_oqg_profile});
            ...
            mem_val_list.push_back({m_pacific_tree->slice[slice]->tx->cgm->uc_oq_profile, uc_oq_profile});
        }
    }
    ...
    if (m_device_mode != device_mode_e::FABRIC_ELEMENT) {
        ...
        la_status status = m_ll_device->read_register(m_pacific_tree->pdvoq_shared_mma->cgm_thresholds, cgm_thresholds);
        return_on_error(status);
        ...
        // Set HMC CGM spare reg - controls when to evict back to SMS from VOQs
        bit_vector spare_reg = bit_vector("0x00000000000000000ffffffffffffffe2");
        ...
        total_sch_uc_buffers_th.fields.total_sch_uc_buffers_fc_th = 15 * UNITS_IN_KIBI;
        ...
        for (la_slice_id_t slice : get_used_slices()) {
            if (is_network_slice(slice)) {
                ...
                for (la_ifg_id_t ifg = 0; ifg < NUM_IFGS_PER_SLICE; ifg++) {
                    ...
                    if (slice < FIRST_HW_FABRIC_SLICE) {
                        la_status status = m_ll_device->read_register(
                            m_pacific_tree->slice[slice]->ifg[ifg]->sch->ifse_general_configuration, ifse_general_cfg_reg);
                        return_on_error(status);

                        ifse_general_cfg_reg.fields.ifg_credit_generator_rate = 255;

                        reg_val_list.push_back(
                            {m_pacific_tree->slice[slice]->ifg[ifg]->sch->ifse_general_configuration, ifse_general_cfg_reg});
                    } else {
                        la_status status = m_ll_device->read_register(
                            m_pacific_tree->slice[slice]->ifg[ifg]->fabric_sch->ifse_general_configuration, ifse_general_cfg_reg);
                        return_on_error(status);
                ...
                status = m_ll_device->read_register(m_pacific_tree->slice[slice]->ics->credits_conf_reg, credits_reg);
                return_on_error(status);
                ...
                reg_val_list.push_back({m_pacific_tree->slice[slice]->ics->credits_conf_reg, credits_reg});

                // Set dram eligible threshold
                ics_slice_dram_list_param_reg_register ics_dram_list;
                status = m_ll_device->read_register(m_pacific_tree->slice[slice]->ics->dram_list_param_reg, ics_dram_list);
                return_on_error(status);

                ics_dram_list.fields.dram_eligible_th_empty = 1024;

                reg_val_list.push_back({m_pacific_tree->slice[slice]->ics->dram_list_param_reg, ics_dram_list});

                // Set pre-packet credit balance threshold to 0
                ics_slice_read_pipe_param_reg_register read_pipe_param_reg;
                status = m_ll_device->read_register(m_pacific_tree->slice[slice]->ics->read_pipe_param_reg, read_pipe_param_reg);
                return_on_error(status);

                read_pipe_param_reg.fields.pre_pkt_cb_th = 0;

                reg_val_list.push_back({m_pacific_tree->slice[slice]->ics->read_pipe_param_reg, read_pipe_param_reg});
            }
        }
    }
...
la_device_impl::set_sw_fc_pause_threshold(la_traffic_class_t tc, std::chrono::microseconds latency)
{
    ...
    if (tc >= LA_NUM_EGRESS_TRAFFIC_CLASSES) {
        return LA_STATUS_EOUTOFRANGE;
    }

    // The NPL takes bits 31:16 of the time stamp to calculate the latency. This gives units of
    // 2^16ns ~= 65us.
    constexpr uint64_t NUM_NS_IN_SEC = 999999999;
    constexpr uint64_t TIMESTAMP_SHIFT_VALUE = 16;
    // Max supported latency is 500ms
    constexpr int64_t MAX_PFC_LATENCY = NUM_NS_IN_SEC / 1000 / 2;
    // Take the latency value in us, convert to ns and round up to the units
    uint64_t value = div_round_up((latency.count() * 1000), (1 << TIMESTAMP_SHIFT_VALUE));

    if (latency.count() > MAX_PFC_LATENCY) {
        return LA_STATUS_EOUTOFRANGE;
    }

    // If there was some configured latency, round up to the lowest value.
    if ((latency != chrono::microseconds::zero()) && (value == 0)) {
        value = 1;
    }

    {
        npl_pfc_tc_latency_table_t::key_type k{};
        npl_pfc_tc_latency_table_t::key_type m{};
        npl_pfc_tc_latency_table_t::value_type v{};
        npl_pfc_tc_latency_table_t::entry_pointer_type e = nullptr;
        k.tc = tc;
        m.tc = 0x7;
        v.payloads.pfc_latency_threshold.value = value;

        la_status status = m_tables.pfc_tc_latency_table->set(tc, k, m, v, e);
        return_on_error(status);
    }
    {
        npl_pfc_tc_wrap_latency_table_t::key_type k{};
        npl_pfc_tc_wrap_latency_table_t::key_type m{};
        npl_pfc_tc_wrap_latency_table_t::value_type v{};
        npl_pfc_tc_wrap_latency_table_t::entry_pointer_type e = nullptr;

        k.tc = tc;
        m.tc = 0x7;

        // Calculate a wrap value to be used when the device time and receive time have wrapped.
        uint64_t wrap_value = NUM_NS_IN_SEC / (1 << TIMESTAMP_SHIFT_VALUE) - value;
        v.payloads.pfc_wrap_latency_threshold.value = wrap_value;

        la_status status = m_tables.pfc_tc_wrap_latency_table->set(tc, k, m, v, e);
        return_on_error(status);
    }

    {
        npl_resolution_pfc_select_table_t::key_type k{};
        npl_resolution_pfc_select_table_t::key_type m{};
        npl_resolution_pfc_select_table_t::value_type v{};
        npl_resolution_pfc_select_table_t::entry_pointer_type e = nullptr;

        la_status status;
        k.tc = tc;
        m.tc = 0x7;
        m.rx_time = 0xf;
        for (uint64_t time = 0; time < 2; time++) {
            k.rx_time = time;
            size_t location = 0;
            status = m_tables.resolution_pfc_select_table->find(k, m, e, location);
            if (latency == chrono::microseconds::zero()) {
                // Erase the entries if they are present.
                if (status != LA_STATUS_SUCCESS) {
                    continue;
                }
                status = m_tables.resolution_pfc_select_table->erase(location);
                return_on_error(status);
            } else {
                if (status == LA_STATUS_SUCCESS) {
                    // Its already enabled, nothing to do.
                    continue;
                }
                location = 0;
                status = m_tables.resolution_pfc_select_table->locate_first_free_entry(location);
                v.payloads.update_pfc.pfc_enable.val = NPL_TRUE_VALUE;
                v.payloads.update_pfc.pfc_sample = (time == 0) ? 1 : 0;
                v.payloads.update_pfc.pfc_direct_sample = (time == 0) ? 0 : 1;

                status = m_tables.resolution_pfc_select_table->insert(location, k, m, v, e);
                return_on_error(status);
            }
        }
    }

    m_pfc_tc_latency[tc] = latency;

    {
        ...
        k.is_pfc_enable = 1;
        if (latency != chrono::microseconds::zero()) {
            v.payloads.resolution_set_next_macro.macro_id = NPL_PFC_RESOLUTION_MACRO;
            m_tables.resolution_set_next_macro_table->set(k, v, e);
        } else {
            bool found = false;
            for (la_uint_t ix = 0; ix < LA_NUM_EGRESS_TRAFFIC_CLASSES; ix++) {
                if (m_pfc_tc_latency[tc] != chrono::microseconds::zero()) {
                    found = true;
                    break;
                }
            }
...
la_device_impl::get_sw_fc_pause_threshold(la_traffic_class_t tc, std::chrono::microseconds& out_latency) const
{
    ...
    out_latency = m_pfc_tc_latency[tc];
...
la_device_impl::clear_sw_pfc_congestion_state(la_system_port_gid_t gid, la_traffic_class_t tc)
{
    ...
    arc_cmd_msg_pfc_clear_cong_t pfc_msg;
    pfc_msg.destination = gid;
    pfc_msg.tc = tc;
    ...
    for (la_slice_id_t slice : get_used_slices()) {
        ...
        pfc_msg.slice = slice;
        status = m_arc_hdlr->arc_send_from_cpu_msg(
            0, arc_cmd_type_e::ARC_CMD_NPUH_CLEAR_PFC_CONG, sizeof(pfc_msg), (uint8_t*)&pfc_msg);
        return_on_error(status);
    }
...
la_device_impl::get_cdb_core_lpm_index(lld_register_scptr lpm_shared_sram_interrupt_reg, size_t& out_lpm_index) const
{
    ...
    //   cdb->core_reduced[].lpm_shared_sram_{1,2}b_err_int_reg[lpm_index]
    //
    // Register addresses are identical across 'cdb->core' and across 'cdb->core_reduced' blocks.
    // Hence, the hard-coded core[0] and core_reduced[0].
    la_entry_addr_t addr = lpm_shared_sram_interrupt_reg->get_desc()->addr;
    const lld_register_array_container& core_ecc_1b = *m_pacific_tree->cdb->core[0]->lpm_shared_sram_1b_err_int_reg;
    const lld_register_array_container& core_ecc_2b = *m_pacific_tree->cdb->core[0]->lpm_shared_sram_2b_err_int_reg;
    const lld_register_array_container& core_reduced_ecc_1b = *m_pacific_tree->cdb->core_reduced[0]->lpm_shared_sram_1b_err_int_reg;
    const lld_register_array_container& core_reduced_ecc_2b = *m_pacific_tree->cdb->core_reduced[0]->lpm_shared_sram_2b_err_int_reg;

    for (size_t i = 0; i < core_ecc_1b.size(); ++i) {
        if (addr == core_ecc_1b[i]->get_desc()->addr || addr == core_ecc_2b[i]->get_desc()->addr
            || addr == core_reduced_ecc_1b[i]->get_desc()->addr
            || addr == core_reduced_ecc_2b[i]->get_desc()->addr) {
            out_lpm_index = i;
            return LA_STATUS_SUCCESS;
        }
    }
                                                   ...
                                                   internal_error_stage_e stage)
{
    ...
    status = counter->initialize(oid, tm_utils::IFG_SYSTEM_PORT_SCHEDULERS);
    if (status != LA_STATUS_SUCCESS) {
        deregister_object(oid);
        return status;
    }
...
{
    out_vrf_port_common = std::make_shared<la_vrf_port_common_pacific>(shared_from_this(), parent);
    return LA_STATUS_SUCCESS;
}
...
la_device_impl::read_mc_fe_links_bmp_sram(size_t multicast_gid, bit_vector& out_links_bmp)
{
    ...
    lld_memory_scptr first_mem = (*m_pacific_tree->rx_pdr_mc_db[shared_db_num]->shared_db_verifier)[shared_db_verifier_mem_num];
    lld_memory_scptr second_mem
        = (*m_pacific_tree->rx_pdr_mc_db[shared_db_num]->shared_db_verifier)[shared_db_verifier_mem_num + 1];
...
la_device_impl::restore_fe_smcid_to_mcid_mapping()
{
    ...
    for (la_multicast_group_gid_t smcid = mc_mcid_scale_threshold; smcid < MAX_MC_GROUP_GID;
        ...
        lld_memory_scptr mac_lp_db1 = m_pacific_tree->sdb->mac->lp_table_db1;
        bit_vector line;
        m_ll_device->read_memory(mac_lp_db1, key.system_mcid_17_3, line);
...
la_device_impl::restore_bundles()
{
    ...
    for (la_slice_pair_id_t pair_idx : get_used_slice_pairs()) {
        for (auto idx = 0; idx < 2; idx++) {
            non_volatile_memories.push_back((*m_pacific_tree->slice_pair[pair_idx]->rx_pdr->fb_link_to_link_bundle_table)[idx]);
            non_volatile_memories.push_back((*m_pacific_tree->slice_pair[pair_idx]->rx_pdr->fe_uc_link_bundle_desc_table)[idx]);
        }
    }
    ...
    for (size_t fabric_port = 0; fabric_port < NUM_FABRIC_PORTS_IN_DEVICE; fabric_port++) {
        status = m_ll_device->read_memory(
            (*m_pacific_tree->slice_pair[0]->rx_pdr->fb_link_to_link_bundle_table)[0], fabric_port, link_to_bundle);
        return_on_error(status);
    ...
    for (size_t bundle_id : bundles_in_use) {
        ...
        status = m_ll_device->read_memory(
            (*m_pacific_tree->slice_pair[0]->rx_pdr->fe_uc_link_bundle_desc_table)[0], bundle_id, bundle_desc);
        return_on_error(status);
...
{
    auto port = std::make_shared<la_system_port_pacific>(shared_from_this());
                                            ...
                                            la_sms_packet_counts& out_packet_count)
{
    ...
    sms_main_sms_total_write_pkts_reg_register write_reg;
    sms_main_sms_total_read_pkts_reg_register read_reg;

    if (clear_on_read) {
        status = m_ll_device->read_register((*m_pacific_tree->sms_main->sms_total_write_pkts_reg)[gifg], write_reg);
        return_on_error(status);
        status = m_ll_device->read_register((*m_pacific_tree->sms_main->sms_total_read_pkts_reg)[gifg], read_reg);
        return_on_error(status);
    } else {
        status = m_ll_device->peek_register((*m_pacific_tree->sms_main->sms_total_write_pkts_reg)[gifg], write_reg);
        return_on_error(status);
        status = m_ll_device->peek_register((*m_pacific_tree->sms_main->sms_total_read_pkts_reg)[gifg], read_reg);
        return_on_error(status);
    }
...
{
    sms_main_sms_total_write_error_reg_register total_write_error_reg;
    sms_main_sms_cgm_write_error_reg_register cgm_write_error_reg;
    sms_main_sms_out_of_bank_write_error_reg_register oob_write_error_reg;
    sms_main_sms_read_error_reg_register sms_read_error_reg;
    la_status status;

    if (clear_on_read) {
        status = m_ll_device->read_register(*m_pacific_tree->sms_main->sms_total_write_error_reg, total_write_error_reg);
        return_on_error(status);
        status = m_ll_device->read_register(*m_pacific_tree->sms_main->sms_cgm_write_error_reg, cgm_write_error_reg);
        return_on_error(status);
        status = m_ll_device->read_register(*m_pacific_tree->sms_main->sms_out_of_bank_write_error_reg, oob_write_error_reg);
        return_on_error(status);
        status = m_ll_device->read_register(*m_pacific_tree->sms_main->sms_read_error_reg, sms_read_error_reg);
        return_on_error(status);
    } else {
        status = m_ll_device->peek_register(*m_pacific_tree->sms_main->sms_total_write_error_reg, total_write_error_reg);
        return_on_error(status);
        status = m_ll_device->peek_register(*m_pacific_tree->sms_main->sms_cgm_write_error_reg, cgm_write_error_reg);
        return_on_error(status);
        status = m_ll_device->peek_register(*m_pacific_tree->sms_main->sms_out_of_bank_write_error_reg, oob_write_error_reg);
        return_on_error(status);
        status = m_ll_device->peek_register(*m_pacific_tree->sms_main->sms_read_error_reg, sms_read_error_reg);
        return_on_error(status);
    }
    ...
    out_error_count.read_error_count = sms_read_error_reg.fields.sms_read_error;
...
{
    sms_main_sms_total_free_buff_sum_status_reg_register reg;
    la_status status;

    if (clear_on_read) {
        status = m_ll_device->read_register(*m_pacific_tree->sms_main->sms_total_free_buff_sum_status_reg, reg);
    } else {
        status = m_ll_device->peek_register(*m_pacific_tree->sms_main->sms_total_free_buff_sum_status_reg, reg);
    }
    return_on_error(status);
...
la_device_impl::set_voq_max_negative_credit_balance(la_uint_t balance)
{
    ...
    ics_slice_read_pipe_param_reg_register reg;
    ...
    for (la_slice_id_t slice : get_used_slices()) {
        auto read_pipe_param_reg = m_pacific_tree->slice[slice]->ics->read_pipe_param_reg;
...
la_device_impl::get_voq_max_negative_credit_balance(la_uint_t& out_balance)
{
    ...
    auto read_pipe_param_reg = m_pacific_tree->slice[0]->ics->read_pipe_param_reg;
    ics_slice_read_pipe_param_reg_register reg;
...
{
    pdvoq_shared_mma_cgm_counter_wmk_register reg;
    la_status status;

    status = m_ll_device->read_register(*m_pacific_tree->pdvoq_shared_mma->cgm_counter_wmk, reg);
    return_on_error(status);
...
la_device_impl::set_ip_tunnel_transit_counter(la_counter_set* counter)
{
    ...
    for (la_slice_id_t slice : get_slices(shared_from_this(), la_slice_mode_e::NETWORK)) {
...
la_status
la_device_impl::warm_boot_save_and_destroy(const std::string& warm_boot_file, bool free_objects)
{
    ...
    // TODO: Invoke serialization for 'this' here.
    //

    la_status status = warm_boot_post_save();
    return_on_error(status);

    if (free_objects) {
        // TODO: Free all SDK objects (except for 'this')
        //
    }
...
la_status
la_device_impl::warm_boot_restore(const std::string& device_path,
                                  const std::string& warm_boot_file,
                                  const silicon_one::la_platform_cbs& platform_cbs)
{
    start_api_call_allow_warm_boot("device_path=", device_path, "warm_boot_file=", warm_boot_file, "platform_cbs=", platform_cbs);
    ...
    // TODO invoke deserialization for 'this' (la_device_impl) here.
    //

    la_status status = warm_boot_post_restore();
    return_on_error(status);
...
{
    la_status stat = LA_STATUS_ENOTIMPLEMENTED;
...
{
    out_multicast_group_common = std::make_shared<la_multicast_group_common_pacific>(shared_from_this());
    return LA_STATUS_SUCCESS;
}

size_t
la_device_impl::get_meter_cir_eir_factor(la_meter_set::type_e type) const
{
    return ((m_revision == la_device_revision_e::PACIFIC_B1) && (type == la_meter_set::type_e::STATISTICAL)) ? B1_STAT_METER_FACTOR
                                                                                                             : 1;
}
...
la_device_impl::trigger_mem_protect_error(la_mem_protect_error_e error_type)
{
    ...
    if (error_type == la_mem_protect_error_e::ECC_2B) {
        npu_host_ecc_2b_err_initiate_register_register ecc_reg;
        status = m_ll_device->read_register(m_pacific_tree->npuh->host->ecc_2b_err_initiate_register, ecc_reg);
        return_on_error(status);

        ecc_reg.fields.mp_data_table_ecc_2b_err_initiate = 1;
        status = m_ll_device->write_register(m_pacific_tree->npuh->host->ecc_2b_err_initiate_register, ecc_reg);
        return_on_error(status);

        std::this_thread::sleep_for(std::chrono::milliseconds(100));

        status = m_ll_device->read_memory(m_pacific_tree->npuh->host->mp_data_table, 0, mem_result);
        return_on_error(status);
        ...
        npu_host_ecc_1b_err_initiate_register_register ecc_reg;
        status = m_ll_device->read_register(m_pacific_tree->npuh->host->ecc_1b_err_initiate_register, ecc_reg);
        return_on_error(status);

        ecc_reg.fields.mp_data_table_ecc_1b_err_initiate = 1;
        status = m_ll_device->write_register(m_pacific_tree->npuh->host->ecc_1b_err_initiate_register, ecc_reg);
        return_on_error(status);
                     ...
                     silicon_one::la_device*& out_device)
{
    ...
    // Temporary: needed for warm-boot sequence early testing (without serialziation).
    // auto device = std::make_shared<silicon_one::la_device_impl>();
    silicon_one::la_device_impl_sptr device;
    for (auto& dev : m_devices) {
        if (dev && dev->get_device_path().find(dev_path) != std::string::npos) {
            device = dev;
        }
    }
    if (!device) {
    ...
    // END OF TEMPRARY CODE

    la_status rc = device->warm_boot_restore(dev_path, dev_state_path, platform_cbs);
...
la_warm_boot_save_and_destroy(silicon_one::la_device* device, const char* warm_boot_file, bool free_objects)
{
    ...
    la_status rc = la_device_impl->warm_boot_save_and_destroy(dev_state_path, free_objects);
    if (rc != LA_STATUS_SUCCESS) {
        return rc;
    }
