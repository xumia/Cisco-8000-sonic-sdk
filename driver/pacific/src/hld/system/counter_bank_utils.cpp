...
{
    counters_cpu_read_register read_reg;
    read_reg.fields.read_target = static_cast<size_t>(cmd);
    read_reg.fields.read_reset = clear_on_read;
    ...
    la_status status = device->m_ll_device->write_register(*device->m_pacific_tree->counters->top->cpu_read, (bit_vector)read_reg);
    ...
    do {
        status = device->m_ll_device->read_register(*device->m_pacific_tree->counters->top->cpu_read, sizeof(read_reg), &read_reg);
        if (status != LA_STATUS_SUCCESS) {
