...
la_fabric_port_impl::initialize(la_object_id_t oid, la_mac_port* fabric_mac_port)
{
    ...
    m_slice_id = fabric_mac_port->get_slice();
    m_ifg_id = fabric_mac_port->get_ifg();
    m_serdes_base = fabric_mac_port->get_first_serdes_id();

    la_uint_t fab_intf_id = m_serdes_base / NUM_SERDES_PER_FABRIC_PORT;

    m_mac_port = m_device->get_sptr<la_mac_port_base>(fabric_mac_port);
    m_pif_base = m_mac_port->get_first_pif_id_internal();

    // If LC_56_FABRIC_PORT_MODE the borrowed ports are serviced by the borrowing IFG (which differs from the lender), including the
    // scheduler.
    la_device_impl::lc_56_fabric_port_info fabric_port_info
        = m_device->get_borrowed_fabric_port_info(m_slice_id, m_ifg_id, m_serdes_base);

    if (fabric_port_info.is_lc_56_fabric_port == true) {
        m_slice_id = fabric_port_info.slice_id;
        m_ifg_id = fabric_port_info.ifg_id;
        m_pif_base = m_serdes_base = fabric_port_info.serdes_base_id;
        fab_intf_id = fabric_port_info.fabric_port_num;
        m_is_lc_56_fabric_port = fabric_port_info.is_lc_56_fabric_port;
    }
...
la_fabric_port_impl::get_adjacent_peer_info(adjacent_peer_info& out_adjacent_peer_info) const
{
    ...
    fte_peer_delay_mem_memory peer_delay;
...
la_fabric_port_impl::get_reachable_lc_devices(la_device_id_vec_t& out_device_id_vec) const
{
    ...
    la_uint_t fabric_port_num = get_fabric_port_num();

    bit_vector reachable_devices_bitmap(0, la_device_impl::MAX_DEVICES);
    la_status status = m_device->m_ll_device->read_memory(
        *m_device->m_pacific_tree->dmc->frm->rev_fabric_routing_table, fabric_port_num, reachable_devices_bitmap);
    return_on_error(status);

    for (la_device_id_t dev = 0; dev < la_device_impl::MAX_DEVICES; dev++) {
        bool dev_reachable = reachable_devices_bitmap.bit(dev);
        if (dev_reachable) {
...
la_uint_t
la_fabric_port_impl::get_fabric_port_num() const
{
    // On a device level, the fabric port num is based on its physical location, regargless of LC_56_FABRIC_PORT_MODE
    la_slice_id_t slice_id = m_mac_port->get_slice();
    la_ifg_id_t ifg_id = m_mac_port->get_ifg();
    size_t serdes_base_id = m_mac_port->get_first_serdes_id();

    la_uint_t fabric_port_num = (slice_id * NUM_FABRIC_PORT_PER_NORMAL_SLICE) + (ifg_id * NUM_FABRIC_PORTS_IN_NORMAL_IFG)
                                + (serdes_base_id / NUM_SERDES_PER_FABRIC_PORT);

    return fabric_port_num;
}

la_uint_t
la_fabric_port_impl::get_tsms_tsmon_fabric_port_num_in_slice() const
{
    // On a slice level, a LC_56_FABRIC_PORT_MODE port in TSMS and TSMON uses num==18

    la_uint_t tsms_tsmon_fabric_port_num_in_slice;

    if (m_is_lc_56_fabric_port) {
        tsms_tsmon_fabric_port_num_in_slice = BORROWED_FABRIC_PORT_TSMS_TSMON_ID;
    } else {
        tsms_tsmon_fabric_port_num_in_slice
            = (m_ifg_id * NUM_FABRIC_PORTS_IN_NORMAL_IFG) + (m_serdes_base / NUM_SERDES_PER_FABRIC_PORT);
    }

    return tsms_tsmon_fabric_port_num_in_slice;
}
...
la_fabric_port_impl::configure_frm_db_fabric_routing_table_npl(const la_device_id_vec_t& device_id_vec)
{
    ...
    // TODO - need to add support nsim_device_simulator::handle_special_requests for this table - such that all pacific_tree updates
...
la_fabric_port_impl::configure_frm_db_fabric_routing_table_hardware(const la_device_id_vec_t& device_id_vec)
{
    ...
    for (size_t egress_device_id = 0; egress_device_id < la_device_impl::MAX_DEVICES; egress_device_id++) {
        ...
        const lld_memory& fabric_routing_table(*m_device->m_pacific_tree->dmc->frm->fabric_routing_table);

        // Read current value
        la_status status = m_device->m_ll_device->read_memory(fabric_routing_table, egress_device_id, links_bitmap);
        return_on_error(status);
...
{
    // The rev_fabric_routing_table is used by the HW to get the set of device IDs that a specific link can reach

    bit_vector reachable_device_bitmap(0, la_device_impl::MAX_DEVICES);

    for (const auto device_id : device_id_vec) {
        reachable_device_bitmap.set_bit(device_id, 1);
    }

    la_uint_t fabric_port_num = get_fabric_port_num();

    la_status status = m_device->m_ll_device->write_memory(
        *m_device->m_pacific_tree->dmc->frm->rev_fabric_routing_table, fabric_port_num, reachable_device_bitmap);
    return_on_error(status);
...
la_fabric_port_impl::configure_source_pif_hw_table()
{
    ...
    size_t serdes_count = m_mac_port->get_num_of_serdes();
    if (m_is_lc_56_fabric_port) {
        // If LC_56_FABRIC_PORT_MODE the borrower IFG uses a single serdes towards the NPE
        serdes_count = 1;
    }
    ...
    for (la_uint_t offset = 0; offset < serdes_count; offset++) {
        // Prepare key
...
la_fabric_port_impl::configure_all_reachable_vector(bool all_reachable)
{
    if (m_device->is_simulated_device()) {
        ...
        npl_all_reachable_vector_entry_t* entry{};
        ...
        la_uint_t fabric_port_number = get_fabric_port_num();
        value.payloads.all_reachable_vector_result.reachable[fabric_port_number] = reachable_value;
...
la_fabric_port_impl::get_status(port_status& out_port_status) const
{
    ...
    status = m_device->m_ll_device->peek_register(*(*m_device->m_pacific_tree->ts_mon->link_status_reg)[m_slice_id], tsmon_link);
    return_on_error(status);

    la_uint_t tsms_tsmon_fabric_port_num_in_slice = get_tsms_tsmon_fabric_port_num_in_slice();
    // Verify that the link is usable for all fabric traffic contextes
    bool uch_link_status = bit_utils::get_bit(tsmon_link.fields.uch_link_status, tsms_tsmon_fabric_port_num_in_slice);
    bool ucl_link_status = bit_utils::get_bit(tsmon_link.fields.ucl_link_status, tsms_tsmon_fabric_port_num_in_slice);
    bool mc_link_status = bit_utils::get_bit(tsmon_link.fields.mc_link_status, tsms_tsmon_fabric_port_num_in_slice);
...
{
    la_status status;
    frm_fabric_link_down_transition_reg_register link_down_reg;

    status
        = m_device->m_ll_device->read_register(*m_device->m_pacific_tree->dmc->frm->fabric_link_down_transition_reg, link_down_reg);
    return_on_error(status);
    ...
    status = m_device->m_ll_device->write_register(*m_device->m_pacific_tree->dmc->frm->fabric_link_down_transition_reg,
                                                   link_down_reg);
...
la_fabric_port_impl::do_peer_delay_measurement()
{
    ...
    // Read full register
    status = m_device->m_ll_device->read_register(*m_device->m_pacific_tree->dmc->fte->enable_reg, enable_reg);
    return_on_error(status);

    // Write 0
    enable_reg.fields.peer_delay_req_gen_en = 0;
    status = m_device->m_ll_device->write_register(*m_device->m_pacific_tree->dmc->fte->enable_reg, enable_reg);
    return_on_error(status);
...
la_status
la_fabric_port_impl::get_peer_delay_mem_entry(fte_peer_delay_mem_memory& out_peer_delay_mem_entry) const
{
    la_uint_t fabric_port_num = get_fabric_port_num();

    la_status status = m_device->m_ll_device->read_memory(
        *m_device->m_pacific_tree->dmc->fte->peer_delay_mem, fabric_port_num, out_peer_delay_mem_entry);
...
la_fabric_port_impl::set_keepalive_generation(bool enable)
{
    ...
    status = m_device->m_ll_device->read_register(*m_device->m_pacific_tree->slice[m_slice_id]->ts_ms->keepalive_gen_cfg,
                                                  keepalive_gen_cfg);
    return_on_error(status);
    ...
    la_uint_t tsms_tsmon_fabric_port_num_in_slice = get_tsms_tsmon_fabric_port_num_in_slice();
    keepalive_gen_enable.set_bit(tsms_tsmon_fabric_port_num_in_slice, enable);

    keepalive_gen_cfg.fields.keepalive_gen_enable = keepalive_gen_enable.get_value();

    status = m_device->m_ll_device->write_register(*m_device->m_pacific_tree->slice[m_slice_id]->ts_ms->keepalive_gen_cfg,
                                                   keepalive_gen_cfg);
    return_on_error(status);
...
la_fabric_port_impl::get_keepalive_generation(bool& out_enabled) const
{
    ...
    rc = m_device->m_ll_device->read_register(*m_device->m_pacific_tree->slice[m_slice_id]->ts_ms->keepalive_gen_cfg,
                                              keepalive_gen_cfg);
    if (rc) {
        return rc;
    }
    ...
    bit_vector keepalive_gen_enable(keepalive_gen_cfg.fields.keepalive_gen_enable);
    la_uint_t tsms_tsmon_fabric_port_num_in_slice = get_tsms_tsmon_fabric_port_num_in_slice();
    out_enabled = keepalive_gen_enable.bit(tsms_tsmon_fabric_port_num_in_slice);
...
la_fabric_port_impl::clear_peer_delay_measurement()
{
    ...
    la_status status = m_device->m_ll_device->write_memory(*m_device->m_pacific_tree->dmc->fte->peer_delay_mem, fabric_port_num, 0);
...
la_fabric_port_impl::restore_non_volatile()
{
    ...
    std::vector<lld_register_sptr> non_volatile_registers = {
        m_device->m_pacific_tree->dmc->fte->enable_reg, m_device->m_pacific_tree->dmc->frm->debug_frtm_debug_reg,
    };
    for (la_slice_id_t sid : m_device->get_used_slices()) {
        auto& slice = m_device->m_pacific_tree->slice[sid];
