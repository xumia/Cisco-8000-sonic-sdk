...
enum {
    ...
    RX_PMA_CFG0_SIG_OK_OVRD_ENABLE_OFFSET = 60,
    RX_PMA_CFG0_SIG_OK_OVRD_VALUE_OFFSET = 68,
...
mac_pool8_port::mac_pool8_port(const la_device_impl_wptr& device) : pacific_mac_pool(device)
{
}
...
{
    mac_pool8_tx_mac_lanes_cfg_register tx_reg;
    mac_pool8_rx_mac_lanes_cfg_register rx_reg;
    mac_pool8_mlp_cfg_register mlp_reg;

    bzero(&tx_reg, mac_pool8_tx_mac_lanes_cfg_register::SIZE);
    bzero(&rx_reg, mac_pool8_rx_mac_lanes_cfg_register::SIZE);
    bzero(&mlp_reg, mac_pool8_mlp_cfg_register::SIZE);

    la_status stat;
    uint64_t two_lane_mode = 0;
    if (m_mac_lanes_count < 8) {
        ...
        stat = m_device->m_ll_device->read_register(
            m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index]->tx_mac_lanes_cfg, tx_reg);
        return_on_error(stat);

        two_lane_mode = tx_reg.fields.tx_2_lanes_mode;
        ...
        tx_reg.fields.tx_8_lanes_mode = 0;
        rx_reg.fields.rx_2_lanes_mode = two_lane_mode;
        rx_reg.fields.rx_8_lanes_mode = 0;
    } else {
        tx_reg.fields.tx_2_lanes_mode = 0;
        tx_reg.fields.tx_8_lanes_mode = 1;
        rx_reg.fields.rx_2_lanes_mode = 0;
        rx_reg.fields.rx_8_lanes_mode = 1;
    ...
    stat = m_device->m_ll_device->write_register(
        m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index]->rx_mac_lanes_cfg, rx_reg);
    return_on_error(stat);

    // MLP
    stat = m_device->m_ll_device->write_register(
        m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index]->mlp_cfg, mlp_reg);
    return_on_error(stat);
...
{
    const auto& mac_pool8 = m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index];
...
mac_pool8_port::configure_pma(device_port_handler_base::serdes_config_data config)
{
    ...
    // Get
    stat = m_device->m_ll_device->read_register(
        m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index]->tx_pma_cfg0, bv_tx_pma_cfg0);
    return_on_error(stat);

    stat = m_device->m_ll_device->read_register(
        m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index]->rx_pma_cfg0, bv_rx_pma_cfg0);
    return_on_error(stat);
    ...
    for (size_t serdes_id = 0; serdes_id < m_serdes_count; serdes_id++) {
        ...
        size_t rx_pma_cfg0_dwidth_lsb = (rx_serdes_source % 8) * 2 + RX_PMA_CFG0_DWIDTH_OFFSET;
...
mac_pool8_port::clear_signal_ok_interrupt()
{
    ...
    // Clear
    stat = m_device->m_ll_device->write_register(m_device->m_pacific_tree->slice[m_slice_id]
                                                     ->ifg[m_ifg_id]
...
mac_pool8_port::get_signal_ok_interrupt(bool& out_trapped)
{
    ...
    // Read
    stat = m_device->m_ll_device->read_register(m_device->m_pacific_tree->slice[m_slice_id]
                                                    ->ifg[m_ifg_id]
                                                    ->mac_pool8[m_mac_pool_index]
                                                    ->rx_pma_sig_ok_loss_interrupt_register,
                                                bv_rx_pma_sig_ok_loss);
...
mac_pool8_port::configure_pma_max_burst(uint32_t max_burst)
{
    ...
    la_status status = m_device->m_ll_device->read_register(
        m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index]->rx_pma_max_burst_cfg,
        rx_pma_max_burst);
...
mac_pool8_port::set_rx_reset(la_mac_port_base::mac_reset_state_e state)
{
    ...
    status = m_device->m_ll_device->read_register(
        m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index]->rstn_reg, rstn_reg);
    return_on_error(status);
    ...
    if ((m_device->m_pacific_tree->get_revision() == la_device_revision_e::PACIFIC_A0) && non_mac_rx_active
        ...
        rstn_reg_temp.set_bits(mac_pool8_rstn_reg_register::SIZE_IN_BITS - 1, RX_MAC + STEP - 1, 0);
        // Disable reset - need to toggle the global PMA reset bits. So doing additional write.
        status = m_device->m_ll_device->write_register(
            m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index]->rstn_reg, rstn_reg_temp);
        return_on_error(status);
    }
...
mac_pool8_port::set_rs_fec_debug_enabled()
{
    la_status status = m_device->m_ll_device->write_register(
        m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index]->rx_rsf_dbg_cfg,
        m_serdes_index_in_mac_pool);
    return_on_error(status);
...
{
    mac_pool8_rx_rsf_dbg_cfg_register reg;

    la_status status = m_device->m_ll_device->read_register(
        m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->mac_pool8[m_mac_pool_index]->rx_rsf_dbg_cfg, reg);
    return_on_error(status);
...
la_status
mac_pool8_port::update_rx_krf_config()
{
...
la_status
mac_pool8_port::reset_rx_krf_config()
{
    mac_pool8_rx_krf_cfg_register rx_krf_cfg{{0}};
    la_status stat = LA_STATUS_SUCCESS;
