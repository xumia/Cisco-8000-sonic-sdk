...
enum {
    ...
    VSC_MAP_CFG_BIT_WIDTH = 12, ///< Width in bits of single entry in the VSC map configuration memory.
    VSC_MAP_CNF_IDX = 9,        ///< Map configuration is defined by 9-11 bits
    ...
    VSC_MAP_CFG_DEFAULT_OQSE_ID = 0x1FF,                                ///< Default OQSE id.
    VSC_MAP_CFG_DEFAULT_MAPPING = (size_t)la_oq_vsc_mapping_e::RR1_RR3, ///< Default VSC mapping.
};
...
la_output_queue_scheduler_impl::populate_vsc_voq_mapping_value(la_voq_gid_t ingress_voq, la_slice_id_t ingress_slice)
{
    ...
    if (la_device_impl::is_multi_device_aware_slice(m_slice_id)) {
        ...
        voq_bv.set_bits(10, 0, ingress_voq >> 4);
        voq_bv.set_bits(13, 11, ingress_slice);
    }
...
union vsc_map_cfg_t {
    uint32_t flat = 0;
    struct {
        uint32_t oqse_id : 9;        ///< Map VSC to 1 of 512 OQSEs
        uint32_t oqse_ll_bitmap : 3; ///< Map VSC to 1 or 2 LLs out of 4 or 8 LLs
    } fields;
};
...
la_status
la_output_queue_scheduler_impl::populate_oqse_cfg(scheduling_mode_e mode, sch_oqse_cfg_memory* out_oqse_cfg)
{
    ...
    if (tm_utils::scheduling_mode_is_8p(m_scheduling_mode)) {
        ...
        const size_t topology_field_width = sch_oqse_cfg_memory::fields::OQSE0_TOPOLOGY_WIDTH;
...
la_output_queue_scheduler_impl::get_group_actual_weight(la_oqcs_group_id_t group_id, la_wfq_weight_t& out_weight) const
{
    ...
    size_t lsb = (group_id + group_offset) * sch_oqse_cfg_memory::fields::OQSE_WFQ_WEIGHT0_WIDTH;
    size_t msb = lsb + sch_oqse_cfg_memory::fields::OQSE_WFQ_WEIGHT0_WIDTH - 1;
...
la_output_queue_scheduler_impl::set_group_weight(la_oqcs_group_id_t group_id, la_wfq_weight_t weight)
{
    ...
    std::vector<la_rate_t> rates
        = tm_utils::convert_weight_2_rate_vector(m_groups_weights, sch_oqse_cfg_memory::fields::OQSE_WFQ_WEIGHT0_WIDTH);
    ...
    for (size_t i = 0; i < num_groups; i++) {
        size_t lsb = (i + group_offset) * sch_oqse_cfg_memory::fields::OQSE_WFQ_WEIGHT0_WIDTH;
        size_t msb = lsb + sch_oqse_cfg_memory::fields::OQSE_WFQ_WEIGHT0_WIDTH - 1;
...
la_output_queue_scheduler_impl::get_attached_vscs(la_vsc_oq_vec_t& out_vsc_vector) const
{
    ...
    la_uint64_t vsc = 0;
    m_device->get_limit(limit_type_e::DEVICE__MIN_ALLOCATABLE_VSC, vsc);

    la_oq_id_t oqse_id = m_oqse_id; // OQSE ID, in this case we have 8 OQs per TM port
    for (; m_device->is_vsc_id_in_range(m_slice_id, vsc); ++vsc) {
        ...
        return_on_error(stat);
        // vsc_val is 9 LSB in vsc_map_cfg
        uint32_t vsc_val = tmp_bv.bits(lsb + VSC_MAP_CNF_IDX - 1, lsb).get_value();
        // map_val is 3 MSB in vsc_map_cfg
        la_oq_vsc_mapping_e map_val
            = (la_oq_vsc_mapping_e)(tmp_bv.bits(lsb + VSC_MAP_CFG_BIT_WIDTH - 1, lsb + VSC_MAP_CNF_IDX).get_value());
...
la_output_queue_scheduler_impl::get_attached_csms(la_vsc_oq& attached_vsc) const
{
    ...
    if (la_device_impl::is_multi_device_aware_slice(m_slice_id)) {
        attached_vsc.voq_id = (tmp_bv.bits(10, 0).get_value() << 4) + (attached_vsc.vsc % NATIVE_VOQ_SET_SIZE);
        // 4 LSB in voq value stored in CSMS are masked
        attached_vsc.slice_id = tmp_bv.bits(14, 12).get_value();
    } else {
        attached_vsc.voq_id = (tmp_bv.bits(10, 0).get_value() << 4) + (attached_vsc.vsc % NATIVE_VOQ_SET_SIZE);
        // 4 LSB in voq value stored in CSMS are masked
        attached_vsc.slice_id = tmp_bv.bits(13, 11).get_value();
    }
...
la_vsc_voq_map_t
la_output_queue_scheduler_impl::get_attached_vscs_voqs() const
{
    return m_attached_vscs;
}
                                           ...
                                           la_voq_gid_t ingress_voq_id)
{
    ...
    if (status != LA_STATUS_SUCCESS) {
        dassert_crit(ingress_slice >= ASIC_MAX_SLICES_PER_DEVICE_NUM);
        return LA_STATUS_EINVAL;
    }
    // Check limits
    if ((!m_device->is_vsc_id_in_range(m_slice_id, vsc)) || (ingress_device >= la_device_impl::la_device_impl::MAX_DEVICES)
    ...
    // Due to bug in Pacific when using scheduling_mode LPSE 2P we must use the EIR path.
    // Only OQSE1 on the system_port sch must use only the CIR path.
    if (scheduling_mode_e::FIRST_LPSE_2P_MAP <= m_scheduling_mode && m_scheduling_mode < scheduling_mode_e::FIRST_LPSE_4P_MAP) {
        if (is_system_port_queueing() && (m_oqse_id % NUM_OQ_PER_PIF == 1)) {
            // An SDK-generated OQSE that maps to OQ1 can use only the CIR path, i.e., only RR0, RR1.
            if (la_oq_vsc_mapping_e::RR2 <= mapping) {
                log_err(HLD, "due to LPSE 2P bug VSC-s connected to OQSE1 of the system_port must be mapped to RR0/RR1 (CIR path)");
                return LA_STATUS_EINVAL;
            }
        } else {
            // Any other (both SDK-generated and user-generated) OQSE can use only the EIR path, i.e., only RR2, RR3.
            if (la_oq_vsc_mapping_e::RR2 > mapping) {
                log_err(HLD, "due to LPSE 2P bug VSC must be mapped to RR2/RR3 (EIR path)");
                return LA_STATUS_EINVAL;
            }
        }
    }
...
{
    sch_credit_request_gen_debug_register reg;
    reg.fields.cr_req_gen_trigger = 0;
    ...
    bool is_hw_fabric = m_device->get_slice_id_manager()->is_fabric_type_slice(m_slice_id, fabric_slices_type_e::HW_FABRIC);
    lld_register_scptr p_reg
        = is_hw_fabric ? m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->fabric_sch->credit_request_gen_debug
                       : m_device->m_pacific_tree->slice[m_slice_id]->ifg[m_ifg_id]->sch->credit_request_gen_debug;

    la_status status = m_device->m_ll_device->write_register(p_reg, reg);
    return status;
}
...
{
    sch_credit_request_gen_debug_register reg;
    reg.fields.cr_req_gen_trigger = 2;
    reg.fields.cr_req_gen_acc_bytes = 0;
    reg.fields.cr_req_gen_ib_state = 0;
...
la_output_queue_scheduler_impl::get_vsc_pir(la_vsc_gid_t vsc, la_rate_t& out_rate) const
{
    ...
    if (!(m_device->is_vsc_id_in_range(m_slice_id, vsc))) {
        return LA_STATUS_EINVAL;
    }
    ...
    status = m_device->m_ll_device->read_register(*m_device->m_pacific_tree->slice[m_slice_id]->ics->credits_conf_reg,
                                                  credits_conf_reg);
    return_on_error(status);
    ...
    la_rate_t total_oqse_rate_from_device
        = tm_utils::convert_rate_from_device_val(total_oqse_rate, crdt_in_bytes, m_device->m_device_frequency_int_khz);

    sch_vsc_token_bucket_cfg_memory token_bucket_cfg;
    la_status stat = m_device->m_ll_device->read_memory(*m_sch_vsc_token_bucket_cfg, vsc, token_bucket_cfg);
...
la_output_queue_scheduler_impl::get_vsc_burst_size(la_vsc_gid_t vsc, size_t& out_burst) const
{
    ...
    sch_vsc_map_cfg_memory vsc_map;
    size_t sch_mem_line = vsc / tm_utils::VSC_MAP_CFG_ENTRIES;
...
la_output_queue_scheduler_impl::do_set_vsc_burst_size(la_vsc_gid_t vsc, size_t burst)
{
    ...
    size_t max_bucket_value = (1 << sch_vsc_token_bucket_cfg_memory::fields::VSC_MAX_BUCKET_VALUE_WIDTH) - 1;
    ...
    if (reset_dynamic_memory) {
        sch_vsc_token_bucket_empty_memory reset_cfg;
        size_t reset_mem_line = vsc / sch_vsc_token_bucket_empty_memory::fields::VSC_TOKEN_BUCKET_EMPTY_FLAG_WIDTH;
        size_t reset_mem_idx = vsc % sch_vsc_token_bucket_empty_memory::fields::VSC_TOKEN_BUCKET_EMPTY_FLAG_WIDTH;
...
la_output_queue_scheduler_impl::get_dev_voq_map_info(la_vsc_gid_t vsc,
                                                     lld_memory_scptr& out_dev_mem,
                                                     lld_memory_scptr& out_voq_mem,
                                                     size_t& out_line) const
{
    if (la_device_impl::is_multi_device_aware_slice(m_slice_id)) {
        uint32_t entries_per_ifg_num = la_device_impl::MAX_VSCS_PER_IFG_IN_NETWORK_SLICE / NATIVE_VOQ_SET_SIZE;

        out_dev_mem = (*m_device->m_pacific_tree->csms->dst_dev_map_mem)[m_slice_id];
        out_voq_mem = (*m_device->m_pacific_tree->csms->voq_vsc_dst_map_mem)[m_slice_id];
        out_line = (vsc / NATIVE_VOQ_SET_SIZE) + (m_ifg_id * entries_per_ifg_num);
    } else {
        uint32_t entries_per_ifg_num = la_device_impl::MAX_VSCS_PER_IFG_IN_FABRIC_SLICE / NATIVE_VOQ_SET_SIZE;
        out_voq_mem = (*m_device->m_pacific_tree->csms->vsc_dst_map_mem)[m_slice_id - la_device_impl::MAX_REMOTE_SLICE];
        out_line = (vsc / NATIVE_VOQ_SET_SIZE) + m_ifg_id * entries_per_ifg_num;
    }
}
