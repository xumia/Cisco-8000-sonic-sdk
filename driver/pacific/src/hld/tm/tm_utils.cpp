...
tm_utils::convert_rate_to_device_val(la_rate_t rate, size_t sch_token_size, size_t dev_freq_khz, uint32_t& out_device_rate)
{
    ...
    // TM_CREDIT_RATE = 1000 * SCH_CREDIT_SIZE * 8 * (DEV_FREQ_MHz / 1000) * 16 = 128 * sch_token_size * DEV_FREQ_MHz
                         ...
                         size_t burst)
{
    ...
    static_assert((size_t)sch_oq_pir_token_bucket_cfg_memory::fields::OQ_PIR_MAX_BUCKET_VALUE_WIDTH
                      == (size_t)pdoq_oq_pir_token_bucket_cfg_memory::fields::OQ_PIR_MAX_BUCKET_VALUE_WIDTH,
                  "sch_oq_pir_token_bucket_cfg and pdoq_oq_pir_token_bucket_cfg dont match");
    static_assert((size_t)sch_oq_pir_token_bucket_cfg_memory::fields::OQ_PIR_MAX_BUCKET_VALUE_WIDTH
                      == (size_t)sch_oqpg_cir_token_bucket_cfg_memory::fields::OQPG_CIR_MAX_BUCKET_VALUE_WIDTH,
                  "sch_oq_pir_token_bucket_cfg and pdoq_oq_pir_token_bucket_cfg dont match");
    static_assert((size_t)sch_oq_pir_token_bucket_cfg_memory::fields::OQ_PIR_MAX_BUCKET_VALUE_WIDTH
                      == (size_t)pdoq_oqpg_cir_token_bucket_cfg_memory::fields::OQPG_CIR_MAX_BUCKET_VALUE_WIDTH,
                  "sch_oq_pir_token_bucket_cfg and pdoq_oq_pir_token_bucket_cfg dont match");
    static_assert((size_t)sch_oq_pir_token_bucket_cfg_memory::fields::OQ_PIR_MAX_BUCKET_VALUE_WIDTH
                      == (size_t)sch_oqse_cir_token_bucket_cfg_memory::fields::OQSE_CIR_MAX_BUCKET_VALUE_WIDTH,
                  "sch_oq_pir_token_bucket_cfg and pdoq_oq_pir_token_bucket_cfg dont match");
    static_assert((size_t)sch_oq_pir_token_bucket_cfg_memory::fields::OQ_PIR_MAX_BUCKET_VALUE_WIDTH
                      == (size_t)sch_oqse_eir_token_bucket_cfg_memory::fields::OQSE_EIR_MAX_BUCKET_VALUE_WIDTH,
                  "sch_oq_pir_token_bucket_cfg and pdoq_oq_pir_token_bucket_cfg dont match");

    size_t max_bucket_value = (1 << sch_oq_pir_token_bucket_cfg_memory::fields::OQ_PIR_MAX_BUCKET_VALUE_WIDTH) - 1;
    if (burst > max_bucket_value) {
        return LA_STATUS_EOUTOFRANGE;
    }

    sch_oq_pir_token_bucket_cfg_memory token_bucket_cfg;
    la_status stat = device->m_ll_device->read_memory(*cfg_memory, mem_line, token_bucket_cfg);
    ...
    if (current_burst_size == 0) {
        log_err(HLD,
                "tm_utils::%s(...) =%s; Can't change burst_size from zero to non-zero value",
                __func__,
                la_status2str(LA_STATUS_EACCES).c_str());
    ...
    // 2. Sleep. During this time the shaper should stop using linked lists. It is destructive to configure the
    //    shaper while it is using a link list.
    // 3. Set dynamic_mem.TokenBucketValue = burst
    // 4. Set cfg_mem.MaxBucketValue = burst

    // cfg_mem.MaxBucketValue = UNLIMITED
    token_bucket_cfg.fields.oq_pir_max_bucket_value = UNLIMITED_BUCKET_SIZE;
    stat = device->m_ll_device->write_memory(*cfg_memory, mem_line, token_bucket_cfg);
    return_on_error(stat);

    // Sleep
    bool sleep_in_set_max_burst = false;
    stat = device->get_bool_property(la_device_property_e::SLEEP_IN_SET_MAX_BURST, sleep_in_set_max_burst);
    return_on_error(stat);
    if (sleep_in_set_max_burst) {
        ...
        stat = device->m_ll_device->read_memory(*cfg_memory, mem_line, token_bucket_cfg);
        return_on_error(stat);
    ...
    stat = device->m_ll_device->write_memory(*dynamic_memory, mem_line, burst);
    return_on_error(stat);
                        ...
                        size_t requested_burst_size)
{
    ...
    // If requested rate to be configured is higher than total port speed then shaper is disabled
    // by setting burst size to UNLIMITED.
    ...
    // Need to update the burst size in 2 cases -
    // If the requested rate is in limit and the currently configured burst size is unlimited -
    //    'burst_size' holds the value that was originally requested by the user
    // If the requested rate is above limit and the currently configured burst size is NOT unlimited -
    //    'burst_size' was changed to unlimited just above
    uint64_t curr_burst_size = token_bucket_cfg.fields.oqse_cir_max_bucket_value;
    bool need_to_change_burst_size = (curr_burst_size != burst_size);
    log_debug(HLD, "%s: rate=%llu port_speed=%llu", __func__, rate, port_speed);
    log_debug(HLD,
              "%s: curr_burst_size=%lu burst_size=%lu need_to_change_burst_size=%s",
              __func__,
              curr_burst_size,
              burst_size,
              need_to_change_burst_size ? "true" : "false");
    if (need_to_change_burst_size) {
        status = set_burst_size(device, cfg_memory, dynamic_memory, mem_line, burst_size);
...
la_status
tm_utils::get_vsc_mapping(const la_device_impl_wptr& device, lld_memory_scptr map_cfg_memory, la_vsc_gid_t vsc, size_t& out_oqse_id)
{
    // Verify the given VSC mapped to this OQSE
    size_t sch_mem_line = vsc / VSC_MAP_CFG_ENTRIES;
    sch_vsc_map_cfg_memory vsc_map_cfg;
