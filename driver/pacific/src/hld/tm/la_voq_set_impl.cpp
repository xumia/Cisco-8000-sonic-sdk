...
populate_vsc_voq_mapping_value(la_slice_id_t slice, la_vsc_gid_t vsc, la_slice_id_t dest_slice, la_ifg_id_t dest_ifg)
{
    bit_vector voq_bv(0);
    ...
    if (la_device_impl::is_multi_device_aware_slice(slice)) {
        ...
        voq_bv.set_bits(9, 0, vsc >> 4);
        voq_bv.set_bits(10, 10, dest_ifg);
        voq_bv.set_bits(13, 11, dest_slice);
    }
                            ...
                            la_ifg_id_t dest_ifg)
{
    ...
    la_status status = map_voq_to_vsc(m_base_voq, m_base_vsc_vec, dest_device, dest_slice, dest_ifg);
...
la_status
la_voq_set_impl::destroy(bool ignore_active_not_empty)
{
    ...
    if (m_voq_state == state_e::ACTIVE && !ignore_active_not_empty) {
    ...
    if (!is_voq_empty && !ignore_active_not_empty) {
...
la_status
la_voq_set_impl::map_voq_to_vsc(la_voq_gid_t base_voq_id,
                                la_vsc_gid_vec_t base_vsc_vec,
                                la_device_id_t dest_device,
                                la_slice_id_t dest_slice,
                                la_ifg_id_t dest_ifg)
{
    ...
    for (auto slice : nw_slices) {
        ...
        get_voq_map_info(base_voq_id, slice, voq_mem, line);

        bit_vector voq_bv = populate_vsc_voq_mapping_value(slice, base_vsc_vec[slice], dest_slice, dest_ifg);

        la_status status = m_device->m_ll_device->write_memory(*voq_mem, line, voq_bv);
        return_on_error(status);

        get_dev_dest_map_info(base_voq_id, slice, dev_mem, line);

        if (dev_mem != nullptr) {
            bit_vector dev_bv(dest_device);
            log_debug(HLD,
            ...
            status = m_device->m_ll_device->write_memory(*dev_mem, line, dev_bv);
            return_on_error(status);
        }
    }
...
la_status
la_voq_set_impl::attach_voq_flush_vsc(size_t start, size_t end)
{
    if ((m_device->m_device_mode != device_mode_e::LINECARD) && (m_device->m_device_mode != device_mode_e::STANDALONE)) {
        return LA_STATUS_EUNKNOWN;
    }

    la_device_id_t dev_id = m_device->get_id();
    la_slice_id_vec_t nw_slices = get_slices(m_device, la_slice_mode_e::NETWORK);
    la_vsc_gid_t vsc_base_id;

    if (m_device->m_device_mode == device_mode_e::LINECARD) {
        vsc_base_id = LC_VOQ_FLUSH_VSC_RANGE_START;
    } else { // STANDALONE
        vsc_base_id = SA_VOQ_FLUSH_VSC_RANGE_START;
    }

    size_t base_voq_offset = m_base_voq % NATIVE_VOQ_SET_SIZE;
    la_vsc_gid_t updated_vsc_base_id = vsc_base_id + base_voq_offset;

    la_output_queue_scheduler_impl* flush_oq_sch = m_device->m_voq_flush_oq_sch.get();
    if (flush_oq_sch == nullptr) {
        return LA_STATUS_EUNKNOWN;
    }

    // Verify the dedicated VSCs are not busy granting credits to another VOQ.
    la_vsc_voq_map_t oq_attached_vscs_map = flush_oq_sch->get_attached_vscs_voqs();
    la_status status;

    if (!oq_attached_vscs_map.empty()) {
        for (size_t i = start; i < end; i++) {
            for (auto voq_slice_id : nw_slices) {
                la_vsc_gid_t vsc = (updated_vsc_base_id + (voq_slice_id * NATIVE_VOQ_SET_SIZE) + i);
                auto it = oq_attached_vscs_map.find(vsc);
                if (it == oq_attached_vscs_map.end()) {
                    continue;
                }
                la_voq_gid_t prev_voq_id = it->second.voq_id;
                const auto& prev_voq_set = m_device->m_voq_sets[prev_voq_id]; // m_device->m_voq_sets is initialized to size of
                                                                              // MAX_VOQS_PER_SLICE_IN_LINECARD_DEVICE
                if (prev_voq_set != nullptr) {
                    if (prev_voq_set->is_during_flush()) {
                        bool is_prev_voq_empty;
                        status = prev_voq_set->is_empty(is_prev_voq_empty);
                        return_on_error(status);

                        if (!is_prev_voq_empty) {
                            return LA_STATUS_EAGAIN;
                        }
                    }
                }
    ...
    // Attach the dedicated VSC to this voq_set, and set the VSC to grant credits
    for (size_t i = start; i < end; i++) {
        for (auto voq_slice_id : nw_slices) {
            la_vsc_gid_t vsc = (updated_vsc_base_id + (voq_slice_id * NATIVE_VOQ_SET_SIZE) + i);

            status = flush_oq_sch->do_attach_vsc(vsc, la_oq_vsc_mapping_e::RR0, dev_id, voq_slice_id, m_base_voq + i);
            return_on_error(status);

            status = flush_oq_sch->set_static_go(vsc);
            return_on_error(status);
        }
    }
...
{
    // There is no recycle port on slice 0 (recycle ports only on odd slices).
    // All ports are set to drop by default, so use this port as the required disabled port on SA.
    // On LC: Setting to dest_dev to an invalid dev_id (i.e. MAX_DEVICES == 288) will cause the packets to drop.
    transaction txn;
    txn.on_fail([=]() { m_voq_flush_orig_mappings.clear(); });

    // Following code makes sure the DISABLED_PORT is indeed set to drop.
    if (m_device->m_device_mode == device_mode_e::STANDALONE) {
        auto& oq_drop_bitmap_mem = (m_device->m_pacific_tree->slice[DISABLED_PORT_SLICE_ID]->tx->cgm->oq_drop_bitmap);
        txcgm_oq_drop_bitmap_memory drop_bitmap{{0}};
        la_status status = m_device->m_ll_device->read_memory(oq_drop_bitmap_mem, DISABLED_PORT, drop_bitmap);
        return_on_error(status);
        if (drop_bitmap.fields.oq_drop_bitmap_data != 0xff) {
            // DISABLED_PORT is not disabled.
            log_err(HLD, "DISABLED_PORT(%d) is active", DISABLED_PORT);
            return LA_STATUS_EBUSY;
        }
    }

    // Following code redirects the VOQ to DISABLED_PORT for SA or MAX_DEVICES for LC.
    for (la_slice_id_t slice : get_slices(m_device, {la_slice_mode_e::NETWORK, la_slice_mode_e::UDC})) {
        ...
        for (size_t voq_offset = start; voq_offset < end; voq_offset++) {
            ...
            if (m_device->m_device_mode == device_mode_e::STANDALONE || !is_send_to_fabric()) {
                v.payloads.filb_voq_mapping_result.dest_oq = DISABLED_PORT * NUM_OQ_PER_PIF;
...
la_voq_set_impl::do_flush(size_t voq_index)
{
    ...
    // Attach VOQ to the dedicated VSC to get credits for the flush operation
    status = attach_voq_flush_vsc(voq_index, voq_index + 1);
    return_on_error(status);
...
la_voq_set_impl::do_flush()
{
    ...
    // Attach VOQ to the dedicated VSC to get credits for the flush operation
    status = attach_voq_flush_vsc(0, m_set_size);
    return_on_error(status);
...
la_voq_set_impl::return_from_flush(size_t start, size_t end)
{
    ...
    // Stop static-go on the dedicated voq flush VSC.
    la_output_queue_scheduler_impl* flush_oq_sch = m_device->m_voq_flush_oq_sch.get();
    if (flush_oq_sch == nullptr) {
        return LA_STATUS_EUNKNOWN;
    }
    la_vsc_voq_map_t vsc_voq_map = flush_oq_sch->get_attached_vscs_voqs();
    for (const auto& vsc_voq : vsc_voq_map) {
        la_voq_gid_t voq_id = vsc_voq.second.voq_id;
        if ((m_base_voq <= voq_id) && (voq_id < m_base_voq + m_set_size)) {
            la_status status = flush_oq_sch->stop_static_go(vsc_voq.first);
            return_on_error(status);
        }
    }
...
la_voq_set_impl::flush(bool block)
{
    ...
    // Flushed counts for all VOQs in set occupy index m_set_size
...
la_voq_set_impl::read_disabled_oq_drop_counter(size_t& out_packets)
{
    ...
    const auto& counter_set_reg_pd
        = (*m_device->m_pacific_tree->slice[DISABLED_PORT_SLICE_ID]->tx->cgm->uc_pd_counter_set)[FLUSH_OQ_CTR_INDEX];
...
la_voq_set_impl::initialize_flush_counters()
{
    ...
    if (m_device->m_device_mode != device_mode_e::STANDALONE && is_send_to_fabric()) {
        ...
        for (la_slice_id_t slice : fabric_slices) {
            ...
            const auto& tx_pdr_debug_pd_field_value_cfg
                = *m_device->m_pacific_tree->slice[slice]->tx->pdr->debug_pd_field_value_cfg;
            const auto& tx_pdr_debug_pd_field_mask_cfg = *m_device->m_pacific_tree->slice[slice]->tx->pdr->debug_pd_field_mask_cfg;

            // Count packets destined to invalid device ID
            // On Pacific, device ID match field is at bit 105.
            bit_vector value_bv = bit_vector(0, txpdr_debug_pd_field_value_cfg_register::SIZE_IN_BITS);
            bit_vector mask_bv = bit_vector(0, txpdr_debug_pd_field_mask_cfg_register::SIZE_IN_BITS);
            value_bv.set_bits(114, 105, INVALID_DEST_DEV);
            mask_bv.set_bits(114, 105, 0x1FF);
        ...
        // Initialize OQ drop counters for flush OQ (recycle OQ on slice 0)
        for (size_t queue_num = 0; queue_num < NUM_OQ_PER_PIF; queue_num++) {
            // OQ counter 1 on slice 0 is reserved for flush accounting
            la_status status;
            const lld_memory& counter_set_map(*m_device->m_pacific_tree->slice[DISABLED_PORT_SLICE_ID]->tx->cgm->counter_set_map);
            txcgm_counter_set_map_memory val;
            la_uint_t queue = (DISABLED_PORT * NUM_OQ_PER_PIF) + queue_num;
...
la_voq_set_impl::read_txpdr_debug_pd_counter(size_t& out_packets)
{
    ...
    for (la_slice_id_t slice : fabric_slices) {
        ...
        const auto& tx_pdr_debug_pd_field_status = *m_device->m_pacific_tree->slice[slice]->tx->pdr->debug_pd_field_status;
        txpdr_debug_pd_field_status_register field_status;
...
la_voq_set_impl::get_candidate_context_id(size_t voq_num, la_slice_id_t slice, context_hw_id& out_context_id) const
{
    ...
    if (m_device->get_slice_id_manager()->is_fabric_type_slice(slice, fabric_slices_type_e::HW_FABRIC)) {
        voq2context_mem = (m_device->m_pacific_tree->slice[slice]->fabric_pdvoq->voq2context);
    } else {
        voq2context_mem = (m_device->m_pacific_tree->slice[slice]->pdvoq->voq2context);
    }

    pdvoq_slice_voq2context_memory voq2context;
    la_status status = m_device->m_ll_device->read_memory(*voq2context_mem, voq_num, voq2context);
    return_on_error(status);
...
la_voq_set_impl::verify_context_to_voq(size_t context, size_t voq_num, la_slice_id_t slice, bool& out_match) const
{
    ...
    la_status status
        = m_device->m_ll_device->read_memory(m_device->m_pacific_tree->slice[slice]->ics->context2voq, context, context2voq);
    return_on_error(status);
...
la_voq_set_impl::get_context_size_in_sms(const context_hw_id& context, la_slice_id_t slice, size_t& out_size) const
{
    ...
    ics_slice_last_queue_report_set_memory last_queue_report_set_res;
    la_status status = m_device->m_ll_device->read_memory(
        m_device->m_pacific_tree->slice[slice]->ics->last_queue_report_set, context.line, last_queue_report_set_res);
    return_on_error(status);

    ics_slice_last_queue_report_clr_memory last_queue_report_clr_res;
    status = m_device->m_ll_device->read_memory(
        m_device->m_pacific_tree->slice[slice]->ics->last_queue_report_clr, context.line, last_queue_report_clr_res);
    return_on_error(status);

    uint64_t line_res
        = last_queue_report_set_res.fields.last_queue_report_bmp ^ last_queue_report_clr_res.fields.last_queue_report_clr_bmp;
    ics_slice_last_dequeue_qsize_bytes_memory size_in_bytes;
    if (bit_utils::get_bit(line_res, context.bit)) {
        status = m_device->m_ll_device->read_memory(
            m_device->m_pacific_tree->slice[slice]->ics->last_enqueue_qsize_bytes, context.id, size_in_bytes);
    } else {
        status = m_device->m_ll_device->read_memory(
            m_device->m_pacific_tree->slice[slice]->ics->last_dequeue_qsize_bytes, context.id, size_in_bytes);
    }
...
{
    dics_context_msb_reg_register context_msb_reg;
    context_msb_reg.fields.context_msb = context.line;
    la_status status
        = m_device->m_ll_device->write_register(m_device->m_pacific_tree->slice[slice]->ics->context_msb_reg, context_msb_reg);
    return_on_error(status);

    bit_vector queue_in_dram_bv;
    status = m_device->m_ll_device->read_register(m_device->m_pacific_tree->slice[slice]->ics->queue_in_dram_reg, queue_in_dram_bv);
    return_on_error(status);
                                         ...
                                         size_t& out_size_bytes) const
{
    ...
    ics_slice_queue2_dram_mem_memory ics_slice_queue2_dram_res;
    la_status status = m_device->m_ll_device->read_memory(
        m_device->m_pacific_tree->slice[slice]->ics->queue2_dram_mem, context.id, ics_slice_queue2_dram_res);
    return_on_error(status);

    size_t dram_context = ics_slice_queue2_dram_res.fields.dram_queue_num;
    dvoq_qsm_memory dvoq_qsm_memory_res;
    status = m_device->m_ll_device->read_memory(m_device->m_pacific_tree->dvoq->qsm, dram_context, dvoq_qsm_memory_res);
    return_on_error(status);

    dics_dramcontext2smscontext_memory dramcontext2smscontext;
    status = m_device->m_ll_device->read_memory(
        m_device->m_pacific_tree->dics->dramcontext2smscontext, dram_context, dramcontext2smscontext);
    return_on_error(status);
...
la_voq_set_impl::do_get_voq_age(size_t voq_index, la_slice_id_t slice, size_t& out_age) const
{
    ...
    if (is_in_hbm) {
        log_debug(HLD, "Context is in the HBM, age valid for sms context");
        out_age = 0;
        return LA_STATUS_SUCCESS;
    }
    ...
    lld_memory_sptr voq_tenq_head_rd_mem;
    pdvoq_slice_voq_tenq_head_rd_memory pdvoq_slice_voq_tenq_head_rd_mem;
    if (m_device->get_slice_id_manager()->is_fabric_type_slice(slice, fabric_slices_type_e::HW_FABRIC)) {
        voq_tenq_head_rd_mem = (m_device->m_pacific_tree->slice[slice]->fabric_pdvoq->voq_tenq_head_rd);
    } else {
        voq_tenq_head_rd_mem = (m_device->m_pacific_tree->slice[slice]->pdvoq->voq_tenq_head_rd);
    }

    status = m_device->m_ll_device->read_memory(voq_tenq_head_rd_mem, context.id, pdvoq_slice_voq_tenq_head_rd_mem);
    return_on_error(status);
...
{
    if (la_device_impl::is_multi_device_aware_slice(slice)) {
        out_voq_mem = (*m_device->m_pacific_tree->csms->voq_vsc_dst_map_mem)[slice];
        uint32_t vsc_entries_num = out_voq_mem->get_desc()->entries / 2;
        out_line = (voq / NATIVE_VOQ_SET_SIZE) + vsc_entries_num;
    } else {
        out_voq_mem = (*m_device->m_pacific_tree->csms->voq_dst_map_mem)[slice - la_device_impl::MAX_REMOTE_SLICE];
...
la_voq_set_impl::get_dev_dest_map_info(la_voq_gid_t voq, la_slice_id_t slice, lld_memory_scptr& out_dev_mem, size_t& out_line) const
{
    ...
    out_dev_mem = (*m_device->m_pacific_tree->csms->dst_dev_map_mem)[slice];
...
la_voq_set_impl::set_context_pool_ret_th(la_slice_id_t slice, uint64_t new_val, uint64_t& out_old_val, bool& out_old_val_valid)
{
    ...
    if (m_device->get_slice_id_manager()->is_fabric_type_slice(slice, fabric_slices_type_e::HW_FABRIC)) {
        ...
        status = m_device->m_ll_device->read_register(m_device->m_pacific_tree->slice[slice]->fabric_pdvoq->cmap_th_reg, reg);
        return_on_error(status);
        out_old_val = reg.fields.context_pool_ret_th;
        out_old_val_valid = true;
        reg.fields.context_pool_ret_th = new_val;
        status = m_device->m_ll_device->write_register(m_device->m_pacific_tree->slice[slice]->fabric_pdvoq->cmap_th_reg, reg);
        return_on_error(status);
    } else {
        pdvoq_slice_cmap_th_reg_register reg;
        status = m_device->m_ll_device->read_register(m_device->m_pacific_tree->slice[slice]->pdvoq->cmap_th_reg, reg);
        return_on_error(status);
        out_old_val = reg.fields.context_pool_ret_th;
        out_old_val_valid = true;
        reg.fields.context_pool_ret_th = new_val;
        status = m_device->m_ll_device->write_register(m_device->m_pacific_tree->slice[slice]->pdvoq->cmap_th_reg, reg);
        return_on_error(status);
    }
    return LA_STATUS_SUCCESS;
}
                                               ...
                                               bool& orig_pool_ret_th_valid)
{
    ...
    if (m_device->get_slice_id_manager()->is_fabric_type_slice(slice, fabric_slices_type_e::HW_FABRIC)) {
        voq2context_mem = (m_device->m_pacific_tree->slice[slice]->fabric_pdvoq->voq2context);
        context_allocate_grant_set = (m_device->m_pacific_tree->slice[slice]->fabric_pdvoq->context_allocate_grant_set);
        context_allocate_grant_clr = (m_device->m_pacific_tree->slice[slice]->fabric_pdvoq->context_allocate_grant_clr);
    } else {
        voq2context_mem = (m_device->m_pacific_tree->slice[slice]->pdvoq->voq2context);
        context_allocate_grant_set = (m_device->m_pacific_tree->slice[slice]->pdvoq->context_allocate_grant_set);
        context_allocate_grant_clr = (m_device->m_pacific_tree->slice[slice]->pdvoq->context_allocate_grant_clr);
    }
    la_status status;
    ...
    pdvoq_slice_context_allocate_grant_set_memory allocate_grant_set;
    pdvoq_slice_context_allocate_grant_clr_memory allocate_grant_clr;
    static_assert((uint64_t)pdvoq_slice_context_allocate_grant_set_memory::fields::BITMAP_E_WIDTH
                      == (uint64_t)pdvoq_slice_context_allocate_grant_clr_memory::fields::BITMAP_F_WIDTH,
                  "Misconfiguration");
    status = m_device->m_ll_device->read_memory(*context_allocate_grant_set,
                                                voq_num / pdvoq_slice_context_allocate_grant_set_memory::fields::BITMAP_E_WIDTH,
                                                allocate_grant_set);
    return_on_error_log(status, HLD, NOTICE, "Could not read context_allocate_grant_set table %s", status.message().c_str());
    status = m_device->m_ll_device->read_memory(*context_allocate_grant_clr,
                                                voq_num / pdvoq_slice_context_allocate_grant_clr_memory::fields::BITMAP_F_WIDTH,
                                                allocate_grant_clr);
    return_on_error_log(status, HLD, NOTICE, "Could not read context_allocate_grant_clr table %s", status.message().c_str());
    bool set_clr_xor_bit = bit_utils::get_bit(allocate_grant_set.fields.bitmap_e ^ allocate_grant_clr.fields.bitmap_f,
                                              voq_num % pdvoq_slice_context_allocate_grant_set_memory::fields::BITMAP_E_WIDTH);
    if (set_clr_xor_bit) {
        pdvoq_slice_voq2context_memory voq2context;
...
la_voq_set_impl::update_ics_queue_profile(uint64_t cgm_profile_num, la_slice_id_t slice, uint64_t voq_context)
{
    ...
    ics_slice_queue_profile_memory ics_slice_queue_profile;
    ics_slice_queue_profile.fields.queue_profile_bits = cgm_profile_num;
    la_status status = m_device->m_ll_device->write_memory(
        m_device->m_pacific_tree->slice[slice]->ics->queue_profile, voq_context, ics_slice_queue_profile);
    return_on_error(status);
    return LA_STATUS_SUCCESS;
}
...
la_voq_set_impl::update_pdvoq_voqcgm_profile(uint64_t cgm_profile_num, la_slice_id_t slice, uint64_t voq_context)
{
    ...
    pdvoq_slice_voqcgm_profile_memory pdvoq_slice_voqcgm_profile;
    pdvoq_slice_voqcgm_profile.fields.cgm_profile = cgm_profile_num;
    if (m_device->get_slice_id_manager()->is_fabric_type_slice(slice, fabric_slices_type_e::HW_FABRIC)) {
        voqcgm_profile_mem = (m_device->m_pacific_tree->slice[slice]->fabric_pdvoq->voqcgm_profile);
    } else {
        voqcgm_profile_mem = (m_device->m_pacific_tree->slice[slice]->pdvoq->voqcgm_profile);
    }
                                                        ...
                                                        bool& out_dram_context_valid)
{
    ...
    if (m_device->get_slice_id_manager()->is_fabric_type_slice(slice, fabric_slices_type_e::HW_FABRIC)) {
        fifos_debug_reg = (m_device->m_pacific_tree->slice[slice]->fabric_pdvoq->fifos_debug_reg);
    } else {
        fifos_debug_reg = (m_device->m_pacific_tree->slice[slice]->pdvoq->fifos_debug_reg);
    }
    ...
    size_t cnt = 0;
    pdvoq_slice_fifos_debug_reg_register fdr_reg;
    do { // TBD: Should be wait() function.
         // Due to design team, the value of dram_release_fifo_stat register should be 0 immediately or in a few clock cycles.
    ...
    ics_slice_context_msb_reg_register cmsb_reg;
    cmsb_reg.fields.context_msb = bit_utils::get_bits(voq_context, 11, 6);
    status = m_device->m_ll_device->write_register(m_device->m_pacific_tree->slice[slice]->ics->context_msb_reg, cmsb_reg);
    return_on_error(status);
    ics_slice_queue_in_dram_reg_register qidr_reg;
    status = m_device->m_ll_device->read_register(m_device->m_pacific_tree->slice[slice]->ics->queue_in_dram_reg, qidr_reg);
    return_on_error(status);
    ...
    ics_slice_queue2_dram_mem_memory ics_slice_queue2_dram_mem_mem;
    status = m_device->m_ll_device->read_memory(
        m_device->m_pacific_tree->slice[slice]->ics->queue2_dram_mem, voq_context, ics_slice_queue2_dram_mem_mem);
    return_on_error(status);
...
{
    lld_register_scptr almost_full_conf = nullptr; // PDVOQ_SLICE: almost full configurations.
    if (m_device->get_slice_id_manager()->is_fabric_type_slice(slice, fabric_slices_type_e::HW_FABRIC)) {
        almost_full_conf = (m_device->m_pacific_tree->slice[slice]->fabric_pdvoq->almost_full_conf);
    } else {
        almost_full_conf = (m_device->m_pacific_tree->slice[slice]->pdvoq->almost_full_conf);
    }
    pdvoq_slice_almost_full_conf_register afc_reg;
...
{
    hmc_cgm_context_info_memory val;
    lld_memory_scptr hmc_cgm_context_info = m_device->m_pacific_tree->hmc_cgm->context_info;
    la_status status = m_device->m_ll_device->read_memory(*hmc_cgm_context_info, dram_context, val);
    return_on_error_log(status, HLD, ERROR, "Could not read hmc_cgm.context_info");
    ...
    status = m_device->m_ll_device->write_memory(*hmc_cgm_context_info, dram_context, val);
    return_on_error(status);
    return LA_STATUS_SUCCESS;
}
...
la_voq_set_impl::erase_voq_properties_table(size_t voq_index)
{
    ...
    la_status status = tables[0]->lookup(k, e);
...
la_voq_set_impl::set_state(state_e state)
{
    ...
    return do_set_state(state);
}

la_status
la_voq_set_impl::do_set_state(state_e state)
{
                             ...
                             la_counter_set*& out_counter) const
{
    ...
    voq_counter_set_sptr& voq_counter_set(m_device->m_voq_counter_sets[voq_counter_set_id]);
    if (voq_counter_set == nullptr) {
        return LA_STATUS_ENOTFOUND;
    }

    out_voq_counter_type = voq_counter_set->get_type();
    out_group_size = voq_counter_set->get_group_size();
