...
class ra_unit_test(unittest.TestCase):
    ...
    is_full_chip = False
    inject_from_npu_host = False    # If true - inject from NPUH using inject UP header. If false - inject from network.
    ...
    def init_ra(self, inject_from_npu_host):
        # For RA simulation SDK should provide some files: sdk_cmd_file.mems_initialization.txt, sdk_cmd_file.rest_of_init.txt,packet_extract.py, tm_database.txt, packet_inject.py, sdk_cmd_file.flow.txt.
        # Due to bug in get_device_id the logs must be turned on:

        # sdk.la_set_logging_level(1, sdk.la_logger_component_e_SIM, sdk.la_logger_level_e_DEBUG)
        # sdk.la_set_logging_level(1, sdk.la_logger_component_e_RA, sdk.la_logger_level_e_DEBUG)
        ...
        self.device = hld_sim_utils.create_ra_device('/dev/testdev/rtl',
                                                     ...
                                                     inject_from_npu_host=inject_from_npu_host,
    ...
    def p2p_config(self):
        ...
        # Adding 40 (random number) bytes padding to run the "big packet flow" in NPU injection.
        padding = '\xAB\xCD' * 40
        self.in_packet = \
            Ether(dst=IN_MAC, src=SRC_MAC, type=0x8100) / \
            Dot1Q(vlan=VLAN1) / \
            IP(src=SIP, dst=DIP, ttl=10) / \
            TCP() / Raw(load=padding)
    ...
    def p2p_trap_config(self):
        ...
        self.out_packet = Ether(dst=HOST_MAC_ADDR,
            ...
            Punt(next_header=sdk.la_packet_types.LA_PROTOCOL_TYPE_ETHERNET,
                 ...
                 source_lp=IN_AC_GID | nplapicli.NPL_PUNT_HEADER_L2_SLP_ENCODING,
        ...
        priority = 0
        self.device.clear_trap_configuration(sdk.LA_EVENT_ETHERNET_SA_DA_ERROR)
        self.device.set_trap_configuration(sdk.LA_EVENT_ETHERNET_SA_DA_ERROR, priority, None, self.punt_dest, False, False, True, 0)
    ...
    def p2p_spa_config(self):
        ...
        hw_lb_vec1 = sdk.la_lb_vector_t()
        dst_mac1 = T.mac_addr(self.in_packet1[Ether].dst)
        src_mac1 = T.mac_addr(self.in_packet1[Ether].src)
        hw_lb_vec1.type = sdk.LA_LB_VECTOR_ETHERNET_VLAN_TAG
        hw_lb_vec1.ethernet.vlan_id = VLAN1
        hw_lb_vec1.ethernet.da = dst_mac1.hld_obj
        hw_lb_vec1.ethernet.sa = src_mac1.hld_obj

        soft_lb_vec1 = sdk.la_lb_vector_t()
        soft_lb_vec1.type = sdk.LA_LB_VECTOR_IPV4_TCP_UDP
        soft_lb_vec1.ipv4.sip = T.ipv4_addr(self.in_packet1[IP].src).to_num()
        soft_lb_vec1.ipv4.dip = T.ipv4_addr(self.in_packet1[IP].dst).to_num()
        soft_lb_vec1.ipv4.protocol = self.in_packet1[IP].proto
        soft_lb_vec1.ipv4.src_port = self.in_packet1[TCP].sport
        soft_lb_vec1.ipv4.dest_port = self.in_packet1[TCP].dport

        lb_vec_entry_list = []
        lb_vec_entry_list.append(hw_lb_vec1)
        lb_vec_entry_list.append(soft_lb_vec1)

        out_dest_chain1 = self.device.get_forwarding_load_balance_chain(
            self.out_spa_port.hld_obj, lb_vec_entry_list)
        self.assertEqual(out_dest_chain1[-1].type(), sdk.la_object.object_type_e_SYSTEM_PORT)
        self.out_dsp1 = out_dest_chain1[-1].downcast()

        hw_lb_vec2 = sdk.la_lb_vector_t()
        dst_mac2 = T.mac_addr(self.in_packet2[Ether].dst)
        src_mac2 = T.mac_addr(self.in_packet2[Ether].src)
        hw_lb_vec2.type = sdk.LA_LB_VECTOR_ETHERNET_VLAN_TAG
        hw_lb_vec2.ethernet.vlan_id = VLAN1
        hw_lb_vec2.ethernet.da = dst_mac2.hld_obj
        hw_lb_vec2.ethernet.sa = src_mac2.hld_obj

        soft_lb_vec2 = sdk.la_lb_vector_t()
        soft_lb_vec2.type = sdk.LA_LB_VECTOR_IPV4_TCP_UDP
        soft_lb_vec2.ipv4.sip = T.ipv4_addr(self.in_packet2[IP].src).to_num()
        soft_lb_vec2.ipv4.dip = T.ipv4_addr(self.in_packet2[IP].dst).to_num()
        soft_lb_vec2.ipv4.protocol = self.in_packet2[IP].proto
        soft_lb_vec2.ipv4.src_port = self.in_packet2[TCP].sport
        soft_lb_vec2.ipv4.dest_port = self.in_packet2[TCP].dport

        lb_vec_entry_list = []
        lb_vec_entry_list.append(hw_lb_vec2)
        lb_vec_entry_list.append(soft_lb_vec2)

        out_dest_chain2 = self.device.get_forwarding_load_balance_chain(
            self.out_spa_port.hld_obj, lb_vec_entry_list)
    ...
    def routing_config(self):
        ...
        self.in_packet = \
            ...
            Dot1Q(vlan=VLAN1) / \
            IP(src=SIP, dst=DIP, ttl=10) / \
            TCP() / Raw(load=0x40aba54f6d520000)
    ...
    def check_valid_trap_mem(self):
        for npe_eng in ["rxpp_term", "rxpp_fwd", "txpp"]:
            ...
            for line in range(128):
                npe = self.tree.slice[0].npu.__getattribute__(npe_eng).npe[0]
    ...
    def test_ra_bridging_from_network(self):
        self._test_ra_bridging(inject_from_npu_host=False)

    def test_ra_bridging_from_npuh(self):
        self._test_ra_bridging(inject_from_npu_host=True)

    def _test_ra_bridging(self, inject_from_npu_host):
        self.init_ra(inject_from_npu_host)
    ...
    def test_ra_simple_read_write_with_rtl(self):
        self.init_ra(self.inject_from_npu_host)
