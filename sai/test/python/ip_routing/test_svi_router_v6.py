...
class Test_svi_route_v6(basic_router_common.test_basic_route):
    ...
    def test_rp_to_svi(self):
        ...
        in_pkt = Ether(dst=pytest.tb.router_mac, src=pytest.top.neighbor_mac1) / \
            IPv6(src=pytest.top.neighbor_ip1, dst=pytest.top.svi_dst_neighbor_ip, hlim=1) / \
            UDP(sport=64, dport=2048)
    ...
    def test_svi_to_rp(self):
        ...
        # test: no decrement TTL=1
        in_pkt = Ether(dst=pytest.top.svi_mac, src=pytest.top.svi_dst_neighbor_mac) / \
            IPv6(src=pytest.top.svi_dst_neighbor_ip, dst=pytest.top.neighbor_ip1, hlim=1) / \
            UDP(sport=64, dport=2048)
    ...
    def test_longest_prefix_matching_15x(self):
        ...
        in_pkt = Ether(dst=pytest.top.svi_mac, src=pytest.top.svi_dst_neighbor_mac) / \
            IPv6(src=pytest.top.svi_dst_neighbor_ip, dst=pytest.top.route3_ip, hlim=64) / \
            UDP(sport=64, dport=2048)

        expected_out_pkt = Ether(dst=pytest.top.neighbor_mac1, src=pytest.tb.router_mac) / \
            IPv6(src=pytest.top.svi_dst_neighbor_ip, dst=pytest.top.route3_ip, hlim=63) / \
            UDP(sport=64, dport=2048)

        U.run_and_compare(self, in_pkt, pytest.top.out_port, expected_out_pkt, pytest.top.in_port)

        in_pkt = Ether(dst=pytest.top.svi_mac, src=pytest.top.svi_dst_neighbor_mac) / \
            IPv6(src=pytest.top.svi_dst_neighbor_ip, dst=pytest.top.route4_ip, hlim=64) / \
            UDP(sport=64, dport=2048)

        expected_out_pkt = Ether(dst=pytest.top.neighbor_mac2, src=pytest.tb.router_mac) / \
            IPv6(src=pytest.top.svi_dst_neighbor_ip, dst=pytest.top.route4_ip, hlim=63) / \
            UDP(sport=64, dport=2048)
