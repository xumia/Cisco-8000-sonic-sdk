...
#   1. Ingress ACL test with route meta data configured as UDK field along with other set of acl fields
#      sourced from packet at switch create time.
#


@pytest.mark.usefixtures("basic_route_v4_topology_with_route_user_meta_udk_acl")
@pytest.mark.skipif(not is_asic_env_gibraltar(), reason="The test applicable only on gibraltar and later asics")
class Test_acl_udk_route_user_meta():

    def test_ingress_ipv4_acl_route_user_meta(self):
        aclTestInstance = aclTests()
        aclTestInstance.ingress_ipv4_acl_udk_route_metadata_test()
