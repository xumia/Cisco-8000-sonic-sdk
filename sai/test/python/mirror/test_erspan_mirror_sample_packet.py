...
# Test goal
#   Packet sampling using erspan mirror session objects. Uses sampling rate from packet-sampling object
#
# Test cases covered
# ------------------
#   Ingress-Mirroring
...
#       3. Create/delete PacketSample object of type mirror session. Create Erspan mirror object.
#          Attach erspan mirror object as SAI_PORT_ATTR_INGRESS_SAMPLE_MIRROR_SESSION value on a port
...
#          Erspan mirror object, attach to the same port as a mirror sample object.
#          Check correctness of packet sampling using attributes,destination from erspan mirror
...
#          Attach same erspan mirror object on both ports. Check ingress traffic on
...
#          on port2. Attach same erspan mirror object on both ports. Check ingress traffic on both
...
class HelperUtils():
    def create_erspan_mirror_session(self):
        erspanUtils = ErspanMirrorUtils()
        attrs = erspanUtils.build_mirror_session_attr()
...
class TestErspanMirrorPacketSamplingOnBridgePort():
    '''
        Test packet sampling that uses mirror object/s and applies on bridge port.
        Sampling mirror object used in this case is an ERSPAN mirror object.
    '''

    def __init(self):
        self.utils = HelperUtils()
        self.erspanUtils = ErspanMirrorUtils()
    ...
    def test_packet_sampling_rate_on_one_port(self):
        ...
        Erspan mirror object, attach as sample mirror session object to the
        same port as a packet sample object is attached to. Check correctness
        of packet sampling using attributes,destination from erspan mirror
        instance but sampling rate from packet sample object.
        '''
        self.__init()
        erspanMirrorOid, in_pkt, erspan_pkt = self.erspanUtils.create_erspan_mirror_session_and_bridge_inpkt_erspan_pkt()
        mirrorPacketSampleOid = self.utils.create_mirror_type_samplepacket_session()

        # samplepacket session will bump SDK mirror session gid up by 1
        erspan_pkt[ERSPAN].session_id = erspanMirrorOid + 1

        self.utils.attach_samplepacket_object(pytest.top.in_port, mirrorPacketSampleOid)
        self.utils.attach_sample_mirror_session(pytest.top.in_port, [erspanMirrorOid])

        # Inject traffic and check packet is sampled to erspan mirror object's
    ...
    def test_packet_sampling_rate_on_two_port(self):
        ...
        sample object on second ingress port.  Attach same erspan mirror object
        ...
        self.__init()
        # create Erspan mirror session and pkt to be injected on port1 switched to port2
        erspanMirrorOid, in_pkt1, erspan_pkt1 = self.erspanUtils.create_erspan_mirror_session_and_bridge_inpkt_erspan_pkt()

        # Create packet sampling object
        mirrorPacketSampleOid = self.utils.create_mirror_type_samplepacket_session()
        self.utils.attach_samplepacket_object(pytest.top.in_port, mirrorPacketSampleOid)
        # Attach erspan mirror object as sampling mirror object to use.
        self.utils.attach_sample_mirror_session(pytest.top.in_port, [erspanMirrorOid])

        # each samplepacket session will bump SDK mirror session gid up by 1
        erspan_pkt1[ERSPAN].session_id = erspanMirrorOid + 1

        # Inject traffic and check packet is sampled to erspan mirror object's
        # destination with sampling rate in mirrorPacketSampleOid
        U.run_and_compare_set(
            self, in_pkt1, pytest.top.in_port, {
                pytest.top.out_port: in_pkt1, pytest.top.mirror_dest: erspan_pkt1}, True)

        # Attach packet sampling on second port
        self.utils.attach_samplepacket_object(pytest.top.out_port, mirrorPacketSampleOid)
        self.utils.attach_sample_mirror_session(pytest.top.out_port, [erspanMirrorOid])

        # create pkt to be injected on port2 switched to port1
        attrs = self.erspanUtils.build_mirror_session_attr()
        in_pkt2, erspan_pkt2 = self.erspanUtils.create_bridge_inpkt_and_out_erspan_pkt(attrs, pytest.top.neighbor_mac1)

        # each samplepacket session will bump SDK mirror session gid up by 1
        erspan_pkt2[ERSPAN].session_id = erspanMirrorOid + 2

        U.run_and_compare_set(
            self, in_pkt2, pytest.top.out_port, {
                pytest.top.in_port: in_pkt2, pytest.top.mirror_dest: erspan_pkt2}, True)
    ...
    def test_modify_mirror_object_sampling_rate(self):
        ...
        for k, v in attrs.items():
            pytest.tb.set_object_attr(erspanMirrorOid, k, v, verify=True)
    ...
    def test_modify_mirror_object_encap(self):
        ...
        # Build erspan pkt to reflect change in encap.
        attrs = self.erspanUtils.build_mirror_session_attr()
        attrs[SAI_MIRROR_SESSION_ATTR_SRC_IP_ADDRESS] = "11.11.11.11"
        attrs[SAI_MIRROR_SESSION_ATTR_DST_IP_ADDRESS] = "22.22.22.22"
        _, new_erspan_pkt = self.erspanUtils.create_bridge_inpkt_and_out_erspan_pkt(attrs, pytest.top.neighbor_mac2)

        # each samplepacket session will bump SDK mirror session gid up by 1
        new_erspan_pkt[ERSPAN].session_id = erspanMirrorOid + 1

        # Inject traffic and check packet is sampled to erspan mirror object's
        # destination with modified encap.
        U.run_and_compare_set(
            self, in_pkt, pytest.top.in_port, {
                pytest.top.out_port: in_pkt, pytest.top.mirror_dest: new_erspan_pkt}, True)
...
class TestErspanMirrorPacketSamplingOnRif():
    '''
    ...
    def __route_pkt_from_port1_to_port2(self, in_pkt, expected_out_pkt, erspan_pkt):
        U.run_and_compare_set(
            self, in_pkt, pytest.top.in_port, {
                pytest.top.out_port: expected_out_pkt, pytest.top.mirror_dest: erspan_pkt}, True)

    def __route_pkt_from_port2_to_port1(self, in_pkt, expected_out_pkt, erspan_pkt):
        U.run_and_compare_set(
            self, in_pkt, pytest.top.out_port, {
                pytest.top.in_port: expected_out_pkt, pytest.top.mirror_dest: erspan_pkt}, True)
    ...
    def test_packet_sampling_rate_on_one_port(self):
        ...
        '''
        self.__init()
        erspanMirrorOid, in_pkt, erspan_pkt = self.erspanUtils.create_erspan_mirror_session_and_route_inpkt_erspan_pkt()
        ...
        self.__route_pkt_from_port1_to_port2(in_pkt, expected_out_pkt, erspan_pkt)
    ...
    def test_packet_sampling_rate_on_two_port(self):
        ...
        self.__init()
        # create Erspan mirror session and pkt to be injected on port1 switched to port2
        erspanMirrorOid, in_pkt1, erspan_pkt1 = self.erspanUtils.create_erspan_mirror_session_and_route_inpkt_erspan_pkt()
        ...
        # Inject traffic and check packet is sampled to erspan mirror object's
        # destination with sampling rate in mirrorPacketSampleOid
        self.__route_pkt_from_port1_to_port2(in_pkt1, expected_out_pkt1, erspan_pkt1)
        ...
        # create pkt to be injected on port2 switched to port1
        attrs = self.erspanUtils.build_mirror_session_attr()
        in_pkt2, erspan_pkt2, expected_out_pkt2  = self.erspanUtils.create_routable_inpkt_and_out_erspan_pkt(
            attrs, pytest.top.neighbor_mac2, pytest.top.neighbor_mac1, pytest.top.neighbor_ip2, pytest.top.neighbor_ip1)

        # each samplepacket session will bump SDK mirror session gid up by 1
        erspan_pkt2[ERSPAN].session_id = erspanMirrorOid + 2

        self.__route_pkt_from_port2_to_port1(in_pkt2, expected_out_pkt2, erspan_pkt2)
    ...
    def test_modify_mirror_object_encap(self):
        ...
        attrs[SAI_MIRROR_SESSION_ATTR_SRC_IP_ADDRESS] = "11.11.11.11"
        attrs[SAI_MIRROR_SESSION_ATTR_DST_IP_ADDRESS] = "22.22.22.22"
        _, new_erspan_pkt, _ = self.erspanUtils.create_routable_inpkt_and_out_erspan_pkt(
            attrs, pytest.top.neighbor_mac1, pytest.top.neighbor_mac2, pytest.top.neighbor_ip1, pytest.top.neighbor_ip2)
        ...
        # Inject traffic and check packet is sampled to erspan mirror object's
        # destination with modified encap.
        self.__route_pkt_from_port1_to_port2(in_pkt, expected_out_pkt, new_erspan_pkt)
