...
class SimInjectDownTest : public ::testing::Test
{
public:
...
TEST_F(SimInjectDownTest, basic_route)
{
    /*
    ...
    set_attr_value(SAI_HOSTIF_PACKET_ATTR_EGRESS_PORT_OR_LAG, attr_list[1].value, ports[port_5_for_bridge]);

    sai_status_t status = hostif_api->send_hostif_packet(switch_id, sizeof(egress_packet) / 2, ingress_pkt_buf, 2, attr_list);
    ASSERT_EQ(status, SAI_STATUS_SUCCESS);
