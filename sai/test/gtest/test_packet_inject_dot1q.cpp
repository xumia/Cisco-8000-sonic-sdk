...
class SimInjectL2Test : public ::testing::Test
{
public:
    ...
    sai_object_id_t m_vlan_mem_id3{};
    sai_object_id_t m_vlan_mem_id4{};
    ...
    void deconfigure_topology()
    {
        ...
        ASSERT_EQ(SAI_STATUS_SUCCESS, status);
        status = vlan_api->remove_vlan_member(m_vlan_mem_id3);
        ASSERT_EQ(SAI_STATUS_SUCCESS, status);
        status = vlan_api->remove_vlan_member(m_vlan_mem_id4);
        ASSERT_EQ(SAI_STATUS_SUCCESS, status);
...
TEST_F(SimInjectL2Test, basic_route)
{
    ...
    str_to_uint8(ingress_packet, ingress_pkt_buf, 1024);
    ...
    sai_status_t status = hostif_api->send_hostif_packet(switch_id, sizeof(ingress_packet) / 2, ingress_pkt_buf, 1, attr_list);
    ASSERT_EQ(status, SAI_STATUS_SUCCESS);
    ...
    //==========================================
    const char ingress_packet_v10[] = "00000033555500005d35a5268100000a080045000078000100007f00712652515ffa0c0a0c0a88888888888888"
                                      "888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888"
                                      "888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888"
                                      "888888";
    const char egress_packet_v10[] = "00000033555500005d35a526080045000078000100007f00712652515ffa0c0a0c0a88888888888888"
                                     "888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888"
                                     "888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888"
                                     "888888";
    configure_vlan_member(m_vlan_mem_id4, vlan_10_obj, m_bridge_port_id2, false);
    configure_vlan_member(m_vlan_mem_id3, vlan_10_obj, m_bridge_port_id, true);
    create_mac_entry(vlan_10_obj, svi_acc_host, m_bridge_port_id2);
    std::this_thread::sleep_for(std::chrono::milliseconds{500});

    str_to_uint8(ingress_packet_v10, ingress_pkt_buf, 1024);
    status = hostif_api->send_hostif_packet(switch_id, sizeof(ingress_packet_v10) / 2, ingress_pkt_buf, 1, attr_list);
    ASSERT_EQ(status, SAI_STATUS_SUCCESS);
    std::this_thread::sleep_for(std::chrono::milliseconds{500});
    ...
    ASSERT_EQ(output_packets[0].packet, egress_packet_v10);
    ASSERT_EQ(output_packets[0].slice, expected_output_info.slice);
    ASSERT_EQ(output_packets[0].ifg, expected_output_info.ifg);
    ASSERT_EQ(output_packets[0].pif, expected_output_info.pif);
}
