...
class SimRouteTest : public ::testing::Test
{
public:
                            ...
                            bool vlan_tag = false)
    {
        ...
        attr_ret.id = SAI_BRIDGE_PORT_ATTR_TYPE;
        status = bridge_api->get_bridge_port_attribute(obj_bridge_port_id, 1, &attr_ret);
        ASSERT_EQ(SAI_STATUS_SUCCESS, status);
        ASSERT_EQ(attr_ret.value.s32, port_type);

        if (port_type == SAI_BRIDGE_PORT_TYPE_PORT || port_type == SAI_BRIDGE_PORT_TYPE_SUB_PORT) {
            attr_ret.id = SAI_BRIDGE_PORT_ATTR_PORT_ID;
            status = bridge_api->get_bridge_port_attribute(obj_bridge_port_id, 1, &attr_ret);
            if (status == SAI_STATUS_SUCCESS) {
                ASSERT_EQ(attr_ret.value.oid, obj_port_id);
            }
        }
    }
...
TEST_F(SimRouteTest, route_fwd_nexthop)
{
    ...
    const char bridge_out_packet[] = "000000002526beef5d35a5268100400a080045000078000100007f00712652515ffa0c0a0c0a88888888888888888"
                                     "888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888"
    ...
    size_t test_ecmp1_out_port = port_5_for_bridge;
    size_t test_ecmp2_in_port = port_4_svi_router;
    size_t test_ecmp2_out_port = port_3_for_bridge;
