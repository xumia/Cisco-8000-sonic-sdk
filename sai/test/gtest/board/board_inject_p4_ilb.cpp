...
// This test is same as board_inject_p4.cpp except mac_port is in serdes ILB.
...
class BoardInjectP4ILBTest
{
public:
    static BoardInjectP4ILBTest* m_inst;
    ...
    void configure_port(int first_serdes_id, int num_serdes)
    {
        ...
        set_attr_value(SAI_PORT_ATTR_INTERNAL_LOOPBACK_MODE,
                       attr.value,
                       sai_port_internal_loopback_mode_t::SAI_PORT_INTERNAL_LOOPBACK_MODE_PHY);
        attrs.push_back(attr);
    ...
    static void sai_port_state_change_callback(uint32_t count, const sai_port_oper_status_notification_t* data)
    {
        ...
        for (uint32_t idx = 0; idx < count; idx++) {
            attr.value.u32list.count = 16;
            status = m_inst->port_api->get_port_attribute(data[idx].port_id, 1, &attr);
            ASSERT_EQ(SAI_STATUS_SUCCESS, status);

            slice = attr.value.u32list.list[0] / 32;
            ifg_id = (attr.value.u32list.list[0] / 18) % 2;
            serdes_id = attr.value.u32list.list[0] % 18;
...
BoardInjectP4ILBTest* BoardInjectP4ILBTest::m_inst = nullptr;

BoardInjectP4ILBTest* injectp4mac_test;

void
setup_injectP4mac()
{
    injectp4mac_test = new BoardInjectP4ILBTest;
    injectp4mac_test->SetUp();
}

void
run_injectP4mac()
{
    injectp4mac_test->basic_route();
}
